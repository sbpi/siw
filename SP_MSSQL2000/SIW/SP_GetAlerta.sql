alter procedure SP_GetAlerta
   (@p_cliente   int           = null,
    @p_usuario   int           = null,
    @p_restricao varchar(20)   = null,
    @p_mail      varchar( 1)   = null,
    @p_solic     int           = null
   ) as
begin
   If @p_restricao = 'SOLICGERAL' Begin
      ...
      --open@p_result for
         select a.sq_pessoa as cliente, 
                ...
                coalesce(f.titulo,
                         case when cast(j.sq_siw_solicitacao as varchar) is null then null else f.descricao end,
                         case when cast(l.sq_siw_solicitacao as varchar) is null then null else f.descricao end,
                         case when cast(h.sq_siw_solicitacao as varchar) is null then null else h.assunto end,
                         case c.sigla when 'SR' then coalesce(f.descricao,f.justificativa) else null end
                        ) as titulo,
                coalesce(case when cast(g.sq_siw_solicitacao as varchar) is null then null else f.titulo end,
                         case when cast(i.sq_siw_solicitacao as varchar) is null then null else f.titulo end,
                         case when cast(j.sq_siw_solicitacao as varchar) is null then null else f.descricao end,
                         case when cast(k.sq_siw_solicitacao as varchar) is null then null else f.titulo end,
                         case when cast(l.sq_siw_solicitacao as varchar) is null then null else f.descricao end,
                         case when cast(h.sq_siw_solicitacao as varchar) is null then null else h.assunto end,
                         case c.sigla when 'SR' then coalesce(f.descricao,f.justificativa) else null end
                        ) as titulo1,
                coalesce(case when cast(g.sq_siw_solicitacao as varchar) is null then null else g.aviso_prox_conc end,
                         case when cast(i.sq_siw_solicitacao as varchar) is null then null else i.aviso_prox_conc end,
                         case when cast(j.sq_siw_solicitacao as varchar) is null then null else j.aviso_prox_conc end,
                         case when cast(k.sq_siw_solicitacao as varchar) is null then null else k.aviso_prox_conc end,
                         case when cast(h.sq_siw_solicitacao as varchar) is null then null else h.aviso_prox_conc end
                        ) as aviso_prox_conc,
                coalesce(case when cast(g.sq_siw_solicitacao as varchar) is null then null else (cast(f.fim as datetime)-cast(g.dias_aviso as integer)) end,
                         case when cast(i.sq_siw_solicitacao as varchar) is null then null else (cast(f.fim as datetime)-cast(i.dias_aviso as integer)) end,
                         case when cast(j.sq_siw_solicitacao as varchar) is null then null else (cast(f.fim as datetime)-cast(j.dias_aviso as integer)) end,
                         case when cast(k.sq_siw_solicitacao as varchar) is null then null else (cast(f.fim as datetime)-cast(k.dias_aviso as integer)) end,
                         case when cast(h.sq_siw_solicitacao as varchar) is null then null else (cast(f.fim as datetime)-cast(h.dias_aviso as integer)) end
                        ) as aviso,
                coalesce(case when cast(g.sq_siw_solicitacao as varchar) is null then null else g.inicio_real end,
                         case when cast(i.sq_siw_solicitacao as varchar) is null then null else f.inicio end,
                         case when cast(j.sq_siw_solicitacao as varchar) is null then null else f.inicio end,
                         case when cast(k.sq_siw_solicitacao as varchar) is null then null else k.inicio_real end,
                         case when cast(h.sq_siw_solicitacao as varchar) is null then null else h.inicio_real end
                        ) as inicio_real,
                coalesce(case when cast(g.sq_siw_solicitacao as varchar) is null then null else g.fim_real end,
                         case when cast(i.sq_siw_solicitacao as varchar) is null then null else f.fim end,
                         case when cast(j.sq_siw_solicitacao as varchar) is null then null else j.quitacao end,
                         case when cast(k.sq_siw_solicitacao as varchar) is null then null else k.fim_real end,
                         case when cast(h.sq_siw_solicitacao as varchar) is null then null else h.fim_real end
                        ) as fim_real,
                f2.nome_resumido+' ('+f8.sigla+')' as nm_resp,
                coalesce(case when cast(g.sq_siw_solicitacao as varchar) is null then null else g1.nome end,
                         case when cast(h.sq_siw_solicitacao as varchar) is null then null else h1.nome end,
                         case when cast(i.sq_siw_solicitacao as varchar) is null then null else h1.nome end,
                         f3.nome
                        ) as nm_unid_resp,
          ...
          where ((@p_cliente  is null and a.envia_mail_alerta = coalesce(@p_mail,'S')) or (@p_cliente is not null and a.sq_pessoa = @p_cliente))
            ...
            and ((c.sigla    = 'PR' and g.sq_siw_solicitacao is not null and (f.fim < dbo.trunc(getDate()) or (g.aviso_prox_conc = 'S' and ((cast(f.fim as datetime)-cast(g.dias_aviso as integer))<=dbo.trunc(getDate()))))) or
                 ...
                 (c.sigla    = 'AC' and i.sq_siw_solicitacao is not null and (f.fim < dbo.trunc(getDate()) or (i.aviso_prox_conc = 'S' and ((cast(f.fim as datetime)-cast(i.dias_aviso as integer))<=dbo.trunc(getDate()))))) or
                 (c.sigla    = 'FN' and j.sq_siw_solicitacao is not null and (f.fim < dbo.trunc(getDate()) or (j.aviso_prox_conc = 'S' and ((cast(f.fim as datetime)-cast(j.dias_aviso as integer))<=dbo.trunc(getDate()))))) or
   ...
   End Else If @p_restricao = 'PACOTE' Begin
      ...
      --open@p_result for
         select a.sq_pessoa as cliente, 
          ...
          where ((@p_cliente    is null and a.envia_mail_alerta = coalesce(@p_mail,'S')) or (@p_cliente is not null and a.sq_pessoa = @p_cliente))
            and (@p_usuario     is null or (@p_usuario is not null and a1.sq_pessoa = @p_usuario))
            and (h.fim_previsto < dbo.trunc(getDate()) or (g.aviso_prox_conc_pacote = 'S' and ((f.fim-cast(g.perc_dias_aviso_pacote/100*(cast(h.fim_previsto-h.inicio_previsto as integer)) as integer))<=dbo.trunc(getDate()))))
   ...
   End Else If @p_restricao = 'DOCUMENTOS' Begin
      ...
      --open@p_result for
         select a.sq_pessoa as cliente, 
                ...
                coalesce(case when cast(g.sq_siw_solicitacao as varchar) is null then null else g1.sigla end,
                         case when cast(h.sq_siw_solicitacao as varchar) is null then null else h1.sigla end,
                         case when cast(i.sq_siw_solicitacao as varchar) is null then null else h1.sigla end,
                         f3.sigla
                        ) as sg_unid_resp,
                case when (c.sigla='SR' and e.sigla='AT') then f2.sq_pessoa     else case c.sigla when 'PD' then l1.sq_pessoa     else f4.sq_pessoa     end end as sq_exec,
                case when (c.sigla='SR' and e.sigla='AT') 
