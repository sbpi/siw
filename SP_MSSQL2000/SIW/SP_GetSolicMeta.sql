alter procedure dbo.SP_getSolicMeta
   (@p_cliente    int,
    @p_usuario    int,
    @p_chave      int        = null,
    ...
    @p_uf         varchar(2)  = null,
    @p_cidade     int   	 = null,
    @p_alt_i      datetime   = null,
    @p_alt_f      datetime   = null,
    @p_ref_i      datetime   = null,
    @p_ref_f      datetime   = null,
    @p_restricao  varchar(15)  = null
) as
begin
   If @p_restricao is null or @p_restricao = 'EXISTEMETA' Begin
      -- Recupera as metas ligadas a uma solicitação
         select a.sq_solic_meta as chave_aux, null as sq_plano, a.sq_siw_solicitacao as chave, 
                ...
                dbo.to_char(a.ultima_alteracao, 'dd/mm/yyyy, hh24:mi:ss') as phpdt_alteracao,
                ...
                j.nome_resumido as nm_res@p_meta,
          ...
          where b.cliente            = @p_cliente 
            ...
            and (@p_titulo    is null or (@p_titulo    is not null and dbo.acentos(b.nome)      like '%' + dbo.acentos(@p_titulo) + '%'))
   ...
   End Else If @p_restricao = 'CRONOGRAMA' Begin
