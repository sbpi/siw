alter procedure SP_getIndicador
   (@p_cliente        int,
    @p_usuario        int,
    @p_chave          int         = null,
    ...
    @p_uf             varchar( 2) = null,
    @p_cidade         int         = null,
    @p_afe_i          datetime    = null,
    @p_afe_f          datetime    = null,
    @p_ref_i          datetime    = null,
    @p_ref_f          datetime    = null,
    @p_restricao      varchar(15) = null
   ) as
begin
   If @p_restricao is null or @p_restricao = 'AFERIDOR' or substring(@p_restricao,1,2) = 'VS' or
      @p_restricao = 'META' Begin
      -- Recupera as indicadors de planejamento
         select a.sq_eoindicador as chave, a.cliente, a.nome, a.sigla, a.descricao, a.forma_afericao, 
                ...
                dbo.to_char(b.data,'dd/mm/yyyy, hh24:mi:ss') as phpdt_afericao,
           ...
           from eo_indicador a
                ...
                left  join (select sq_eoindicador, count(sq_pessoa) as qtd 
                             ...
                             where sq_pessoa = @p_usuario 
                               and dbo.trunc(getDate()) between inicio and fim
                            group by sq_eoindicador
   ...
   End Else If @p_restricao = 'TIPOINDIC' Begin
      ...
      -- Verifica se o registro está vinculado a um interessado
         select a.sq_eoindicador as chave, a.cliente, a.nome, a.sigla, a.descricao, a.forma_afericao, 
          ...
          where a.cliente = @p_cliente 
            ...
            and (@p_nome             is null or (@p_nome    is not null and dbo.acentos(a.nome) = dbo.acentos(@p_nome)))
            and (@p_sigla            is null or (@p_sigla   is not null and dbo.acentos(a.sigla) = dbo.acentos(@p_sigla)));
   End Else If @p_restricao = 'AFERICAO' or @p_restricao = 'EDICAO' or @p_restricao = 'EXISTEAF' or @p_restricao = 'INCLUSAO' Begin
      -- Recupera as indicadors de planejamento
         select a.sq_eoindicador, a.cliente, a.nome, a.sigla, a.descricao, a.forma_afericao, 
                ...
                dbo.to_char(b.data_afericao,'dd/mm/yyyy, hh24:mi:ss') as phpdt_afericao,
                dbo.to_char(b.referencia_inicio,'dd/mm/yyyy, hh24:mi:ss') as phpdt_inicio,
                dbo.to_char(b.referencia_fim,'dd/mm/yyyy, hh24:mi:ss') as phpdt_fim,
                dbo.to_char(b.inclusao,'dd/mm/yyyy, hh24:mi:ss') as phpdt_inclusao,
                dbo.to_char(b.ultima_alteracao,'dd/mm/yyyy, hh24:mi:ss') as phpdt_alteracao,
          ...
          where a.cliente    = @p_cliente 
            ...
            and (charindex(@p_restricao,'EDICAO,INCLUSAO')=0 or
   ...
   End Else If @p_restricao = 'VISUALBASE' Begin
