   ...
   If w_interno = 'S' Then
      ...
      open p_result for
         ...
         from siw_menu              v
              ...
              left  join (select sq_menu, count(*) as qtd 
                            from (select c.sq_menu, acesso(sq_siw_solicitacao, p_usuario) as acesso, c.sigla as sg_menu, c.destinatario, d.executor, 
                                         e.sigla as sg_tramite
                                    from siw_tramite                    e
                           ...
                           where ((sg_menu <> 'PJCAD' and (destinatario = 'S' and executor = p_usuario) or (destinatario = 'N' and acesso > 15)) or
                                  (sg_menu =  'PJCAD' and coalesce(sg_tramite,'---') <> 'CI' and acesso >= 8)
                                 )
                          group by sq_menu
                         )          x on (v.sq_menu = x.sq_menu)
              inner join (select sq_menu, count(*) as qtd 
                            from (select c.sq_menu, acesso(sq_siw_solicitacao, p_usuario) as acesso, c.controla_ano, d.ano
                                    from siw_tramite                    e
                                         inner     join siw_solicitacao d on (e.sq_siw_tramite = d.sq_siw_tramite)
                                           inner   join siw_menu        c on (d.sq_menu        = c.sq_menu and
                                                                              c.tramite        = 'S'  and
                           ...
                           where acesso        > 0
                             and (controla_ano = 'N' or (controla_ano = 'S' and ano = p_ano))
