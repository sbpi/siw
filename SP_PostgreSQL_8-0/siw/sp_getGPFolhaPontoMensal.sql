create or replace FUNCTION sp_getGPFolhaPontoMensal
   (p_contrato       numeric,
    p_mes            varchar,
    p_restricao      varchar,
      ...
      from sg_autenticacao                          a
           inner       join eo_unidade_resp         b on (a.sq_pessoa               = b.sq_pessoa and 
             ...
             inner     join gp_contrato_colaborador c on (b.sq_unidade              = c.sq_unidade_exercicio and
                                                          (c.inicio                 between b.inicio and coalesce(b.fim,now()) or
                                                           coalesce(c.fim,b.inicio) between b.inicio and coalesce(b.fim,now())
                                                          )
                                                         )
