CREATE OR REPLACE FUNCTION siw.SP_GetAcordoNota
   (p_cliente           numeric,
    p_chave             numeric,
    p_chave_aux         numeric,
   ...
   If p_restricao is null Then
      open p_result for     
         select a.sq_acordo_nota, a.sq_siw_solicitacao, a.sq_tipo_documento, a.sq_acordo_outra_parte, 
                ...
                c.nome as nm_tipo_documento, c.sigla as sg_tipo_documento,
                f.nome_resumido as nm_outra_parte,
                e.codigo as cd_aditivo, e.sq_cc as cc_aditivo,
                g.sq_cc as cc_acordo,
                ...
                i.vl_cancelamento
           from siw.ac_acordo_nota                     a  
                inner   join siw.ac_acordo             b on (a.sq_siw_solicitacao    = b.sq_siw_solicitacao)
                  inner join siw.siw_solicitacao       g on (b.sq_siw_solicitacao    = g.sq_siw_solicitacao)
                inner   join siw.fn_tipo_documento     c on (a.sq_tipo_documento     = c.sq_tipo_documento)
                left    join siw.ac_acordo_outra_parte d on (a.sq_acordo_outra_parte = d.sq_acordo_outra_parte)
                  left  join siw.co_pessoa             f on (d.outra_parte           = f.sq_pessoa)
                left    join siw.ac_acordo_aditivo     e on (a.sq_acordo_aditivo     = e.sq_acordo_aditivo)
                left    join (select x.sq_acordo_nota,y.quitacao, 
                                 ...
                                 from siw.fn_lancamento_doc            x
                                      inner   join siw.fn_lancamento   y on (x.sq_siw_solicitacao = y.sq_siw_solicitacao and
                                                                         y.sq_acordo_parcela  is not null
                                                                        )
                                        inner join siw.siw_solicitacao z on (y.sq_siw_solicitacao = z.sq_siw_solicitacao)
                                where x.sq_acordo_nota is not null
                               group by x.sq_acordo_nota, y.quitacao
                              )                    h on (a.sq_acordo_nota     = h.sq_acordo_nota)
                left    join (select x.sq_acordo_nota, sum(x.valor) as vl_cancelamento
                                from siw.ac_nota_cancelamento x
   ...
   Elsif p_restricao = 'LANCAMENTO' Then
      open p_result for
         select c.sq_siw_solicitacao
           from siw.ac_acordo_nota                   a
                inner     join siw.ac_parcela_nota   b on (a.sq_acordo_nota    = b.sq_acordo_nota)
                inner     join siw.fn_lancamento     c on (b.sq_acordo_parcela = c.sq_acordo_parcela)
                  inner   join siw.siw_solicitacao   d on (c.sq_siw_solicitacao = d.sq_siw_solicitacao)
                    inner join siw.siw_tramite       e on (d.sq_siw_tramite     = e.sq_siw_tramite and
                                                       'CA'                 <> coalesce(e.sigla,'-')
                                                      )
   ...
   Elsif p_restricao = 'PARCELA' Then
      open p_result for
         select a.sq_acordo_parcela, a.sq_acordo_nota,
                ...
                c.valor_inicial as inicial_parc, c.valor_excedente as excedente_parc, 
                c.valor_reajuste as  reajuste_parc,
                d.valor_inicial as inicial_lanc, d.valor_excedente as excedente_lanc, 
                d.valor_reajuste as reajuste_lanc
           from siw.ac_parcela_nota                a
                inner   join siw.ac_acordo_nota    b on (a.sq_acordo_nota    = b.sq_acordo_nota)
                inner   join siw.ac_acordo_parcela c on (a.sq_acordo_parcela = c.sq_acordo_parcela)
                  left  join siw.fn_lancamento_doc d on (b.sq_acordo_nota    = d.sq_acordo_nota)
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION siw.SP_GetAcordoNota
   (p_cliente           numeric,
    p_chave             numeric,
    p_chave_aux         numeric,
    p_sq_tipo_documento numeric,
