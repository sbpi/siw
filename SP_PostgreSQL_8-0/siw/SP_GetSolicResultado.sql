create or replace FUNCTION SP_GetSolicResultado
   (p_cliente     numeric,
    p_programa    numeric,
    p_projeto     numeric,
    p_unidade     numeric,
    ...
    p_adiantado   varchar,
    p_concluido   varchar,
    p_nini_atraso varchar,
    p_nini_prox   varchar,
    p_nini_normal varchar,
    p_ini_prox    varchar,
    p_ini_normal  varchar,
    p_conc_atraso varchar,
    p_agenda      varchar,
...
DECLARE
   l_item       varchar(18);
   l_tipo       varchar(200) := p_tipo_evento ||',';
   x_tipo       varchar(200) := '';
BEGIN
      ...
      open p_result for 
         select *
          ...
          where (p_atrasado    is null and p_adiantado is null and p_concluido is null and 
             ...
             or (p_nini_atraso is not null and inicio_real is null and fim_previsto < trunc(now()))
             or (p_nini_prox   is not null and inicio_real is null and fim_previsto > trunc(now()) and ((trunc(now())-inicio_previsto)/(fim_previsto-inicio_previsto+1)*100 > (100-perc_dias_aviso_pacote)))
             or (p_nini_normal is not null and inicio_real is null and fim_previsto > trunc(now()) and ((trunc(now())-inicio_previsto)/(fim_previsto-inicio_previsto+1)*100 <= (100-perc_dias_aviso_pacote)))
             or (p_atrasado    is not null and inicio_real is not null and fim_real is null and fim_previsto < trunc(now()))
             or (p_ini_prox    is not null and inicio_real is not null and fim_real is null and fim_previsto > trunc(now()) and ((trunc(now())-inicio_previsto)/(fim_previsto-inicio_previsto+1)*100 > (100-perc_dias_aviso_pacote)))
             or (p_ini_normal  is not null and inicio_real is not null and fim_real is null and fim_previsto > trunc(now()) and ((trunc(now())-inicio_previsto)/(fim_previsto-inicio_previsto+1)*100 <= (100-perc_dias_aviso_pacote)))
