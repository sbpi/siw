CREATE OR REPLACE FUNCTION siw.sp_ajustaDataEtapa(p_projeto numeric,p_todos character varying)
...
begin
  ...
  Elsif coalesce(p_todos,'nulo') = 'TODOS' Then
     ...
     for c_projetos in  select a.sq_siw_solicitacao
        ...
        perform siw.sp_ajustadataEtapa(c_projetos.sq_siw_solicitacao);
     end loop;
  end if;
  update siw.pj_projeto_etapa set inicio_real = null, fim_real = null where pacote_trabalho = 'N' and sq_siw_solicitacao = p_projeto;
  ...
  for  c_pacotes in select a.sq_etapa_pai, a.inicio_real, a.fim_real
     ...
     for c_pais in      select a.sq_projeto_etapa
       from  siw.SP_fGetEtapaList_filtro_etapa(p_chave,0,'DOWN',c_pacotes.sq_etapa_pai)
       loop
        update siw.pj_projeto_etapa set inicio_real = c_pacote.inicio_real where (inicio_real is null or inicio_real > c_pacote.inicio_real) and sq_projeto_etapa = c_pai.sq_projeto_etapa;
     ...
     -- Ajusta a data de término das etapas
      ...
      for c_pais in      select a.sq_projeto_etapa
       from  siw.SP_fGetEtapaList_filtro_etapa(p_chave,0,'DOWN',c_pacotes.sq_etapa_pai)
       loop
        update siw.pj_projeto_etapa  set fim_real = c_pacotes.fim_real 
         where (fim_real is null or fim_real < c_pacote.fim_real) 
         and 0 = (select count(sq_projeto_etapa) from siw.sp_ajustaPesoEtapa_filtra(p_chave,0,'DOWN'))
  ...
  -- Reinicializa as datas das etapas que não são pacote de trabalho
     ...
     if w_inicio is not null then
        update siw.pj_projeto set inicio_real = w_inicio where sq_siw_solicitacao = p_projeto;
     end if;
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION siw.sp_ajustaDataEtapa(p_projeto numeric,p_todos character varying) OWNER TO siw;
