create or replace FUNCTION SP_PutCLSolicItem
   (p_operacao                  varchar,
    p_chave_aux                 numeric,
    p_chave                     numeric,
    ...
    p_valor                     numeric,
    p_cancelado                 varchar,
    p_motivo_cancelamento       varchar 
   ) RETURNS VOID AS $$
DECLARE
   w_chave      numeric(18);
   w_valor      numeric(18,4);
   w_qtd        numeric(18,2) := p_quantidade;
   w_material   numeric(18)   := p_material;
   w_existe     numeric(18);
...
BEGIN
   ...
   Elsif p_operacao = 'E' Then
      ...
      if substr(w_menu.sigla,1,4) = 'CLLC' then
         ...
         DELETE FROM cl_solicitacao_item_vinc where item_pedido = p_chave_aux2;
         ...
         if w_existe <= 1 then
            ...
            DELETE FROM cl_solicitacao_item where sq_solicitacao_item = p_chave_aux;
         end if;
         ...
         -- Remove vínculo entre pedido de compra e item de licitação
         DELETE FROM cl_solicitacao_item_vinc where item_licitacao = p_chave_aux;
         ...
         if w_existe <= 1 then
            ...
            DELETE FROM cl_solicitacao_item where sq_solicitacao_item = p_chave_aux2;
         end if;
