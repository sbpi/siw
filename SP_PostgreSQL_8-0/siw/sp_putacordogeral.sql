create or replace FUNCTION SP_PutAcordoGeral
   (p_operacao             varchar,
    p_cliente              numeric,
    p_chave                numeric,
    p_copia                numeric,
...
DECLARE
   ...
   w_meses_vigencia_renovacao  numeric(4);
   w_valor_original            numeric(18,2);
   w_qtd_parcela_original      numeric(4);
   w_perc_parcela_original     float;
   w_valor                     numeric(18,2);
   w_inicio_vigencia_original  date;
   w_meses_parcela_original    numeric(4);
   w_data                      date;
   w_sigla                     varchar(20);
   w_vincula_projeto           varchar(1) := 'N';
   w_outra_parte               numeric(18);
   w_compra                    numeric(18);
...
BEGIN
   ...
   Elsif p_operacao = 'A' Then -- Alteração
      ...
      If Nvl(p_forma_atual, p_sq_forma_pagamento) <> p_sq_forma_pagamento Then
         ...
         where sq_siw_solicitacao = p_chave;
      End If;
      DELETE FROM pj_etapa_contrato where sq_siw_solicitacao = p_chave;
   ...
   Elsif p_operacao = 'E' Then -- Exclusão
      ...
      Else
         ...
         DELETE FROM ac_acordo_representante where sq_siw_solicitacao = p_chave;
         DELETE FROM ac_acordo_log           where sq_siw_solicitacao = p_chave;
