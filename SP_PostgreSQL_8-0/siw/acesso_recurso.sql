create or replace function acesso_recurso(p_recurso numeric, p_usuario numeric)  RETURNS integer AS $$
DECLARE
/**********************************************************************************
* Nome      : acesso_recurso
...
BEGIN
 ...
 select ount(a.sq_unidade_resp) into w_existe
   from eo_unidade_resp a
  where a.sq_pessoa  = p_usuario
    and a.fim        is null
    and a.sq_unidade in (select sq_unidade from connectby('eo_unidade','sq_unidade','sq_unidade_pai',to_char(w_sq_unidade_gestora),0) as (sq_unidade numeric, sq_unidade_pai numeric, level int));
