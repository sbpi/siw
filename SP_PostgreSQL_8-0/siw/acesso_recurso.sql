create or replace function acesso_recurso(p_recurso in numeric, p_usuario in numeric) returns integer as $$
/**********************************************************************************
* Nome      : acesso_recurso
...
begin
 ...
 select count(a.sq_unidade_resp) into w_existe
  ...
  where a.sq_pessoa  = p_usuario
    and a.fim        is null
    and a.sq_unidade in (select sq_unidade from sp_fGetUorgList(w_sq_unidade_gestora,0,'UP'));
