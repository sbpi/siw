CREATE OR REPLACE FUNCTION siw.SP_GetCV
   (p_cliente   numeric,
    p_chave     numeric,
    p_sigla     varchar,
    p_tipo      varchar)
      ...
      open p_result for 
         select a.sq_pessoa, a.nome, a.nome_resumido,
           ...
           from siw.co_pessoa                          a
                ...
                left     join siw.co_pessoa_fisica b on (a.sq_pessoa          = b.sq_pessoa)
                  left   join siw.co_cidade        e on (b.sq_cidade_nasc     = e.sq_cidade)
                    left join siw.co_pais          h on (e.sq_pais            = h.sq_pais)
                    left join siw.co_uf            i on (e.co_uf              = i.co_uf)
                  left   join siw.co_pais          g on (b.sq_pais_passaporte = g.sq_pais)
                  left   join siw.co_etnia         j on (b.sq_etnia           = j.sq_etnia)
                  left   join siw.co_deficiencia   k on (b.sq_deficiencia     = k.sq_deficiencia)
                  left   join siw.co_formacao      l on (b.sq_formacao        = l.sq_formacao)
                left     join siw.cv_pessoa        c on (a.sq_pessoa          = c.sq_pessoa)
                  left   join siw.cv_pessoa_hist   d on (c.sq_pessoa          = d.sq_pessoa)
                  left   join siw.co_estado_civil  f on (c.sq_estado_civil    = f.sq_estado_civil)
                  left   join siw.siw_arquivo      m on (c.sq_siw_arquivo     = m.sq_siw_arquivo)
