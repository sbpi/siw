create or replace FUNCTION sp_getMatServ
   (p_cliente       numeric,
    ...
    p_catalogo      varchar,
    p_ata_aviso     varchar,
    p_ata_invalida  varchar,
    p_ata_valida    varchar,
    p_aviso         varchar,
    p_invalida      varchar,
    p_valida        varchar,
    p_branco        varchar,
      ...
      open p_result for 
         select a.sq_material as chave, a.cliente, a.sq_tipo_material, a.sq_unidade_medida, 
          ...
          where a.cliente         = p_cliente
            ...
            and (p_catalogo      is null or (p_catalogo      is not null and a.exibe_catalogo   = p_catalogo))
            and (p_ativo         is null or (p_ativo         is not null and a.ativo            = p_ativo))
            and (p_valida        is null or (p_valida        is not null and ((p_valida         = 'S' and g.fim>=now()-1) or
                                                                              (p_valida         = 'N' and g.fim<now())
                                                                             )
                                            )
                )
      ...
      open p_result for 
         select a.sq_material as chave, a.cliente, a.sq_tipo_material, a.sq_unidade_medida, 
          ...
          where a.cliente            = p_cliente
            ...
            and ((p_valida           is null and g.fim >= trunc(i.inclusao) and g.inicio <= coalesce(i.conclusao,trunc(now()))) or 
                 (p_valida           is not null and ((p_valida = 'S' and g.fim>=trunc(now())) or
                                                      (p_valida = 'N' and g.fim<trunc(now()))
                                                     )
                 )
                );
      ...
      open p_result for 
         select /*+ ordered */ a.sq_material as chave, a.cliente, a.sq_tipo_material, a.sq_unidade_medida, 
          ...
          where a.cliente         = p_cliente
            ...
            and ((p_aviso        = 'S' and coalesce(a.pesquisa_validade-f.dias_aviso_pesquisa,now()-1)<now() and a.pesquisa_validade>=now()-1) or
                 (p_invalida     = 'S' and coalesce(a.pesquisa_validade,now()-1)<now()-1) or
                 (p_valida       = 'S' and coalesce(a.pesquisa_validade,now()-1)>=now()-1 and coalesce(a.pesquisa_validade-f.dias_aviso_pesquisa,now()-1)>now()) or
      ...
      open p_result for 
         select a.sq_material as chave, a.cliente, a.sq_tipo_material, a.sq_unidade_medida, 
          ...
          where a.cliente         = p_cliente
            ...
            and ((p_ata_aviso    = 'S' and w2.aviso_prox_conc = 'S' and trunc(now()) between cast(w2.fim as date)-cast(w2.dias_aviso as integer) and cast(w2.fim as date)) or
                 (p_ata_invalida = 'S' and trunc(now()) > cast(w2.fim as date)) or
                 (p_ata_valida   = 'S' and trunc(now()) < case when w2.aviso_prox_conc = 'N' then cast(w2.fim as date) else cast(w2.fim as date)-cast(w2.dias_aviso as integer) end)
                )
            and ((p_aviso        = 'S' and coalesce(a.pesquisa_validade-f.dias_aviso_pesquisa,now()-1)<now() and a.pesquisa_validade>=now()-1) or
                 (p_invalida     = 'S' and coalesce(a.pesquisa_validade,now()-1)<now()-1) or
                 (p_valida       = 'S' and coalesce(a.pesquisa_validade,now()-1)>=now()-1 and coalesce(a.pesquisa_validade-f.dias_aviso_pesquisa,now()-1)>now()) or
