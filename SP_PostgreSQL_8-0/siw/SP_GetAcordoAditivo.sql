...
CREATE OR REPLACE FUNCTION siw.SP_GetAcordoAditivo
   (p_cliente     numeric,
    p_aditivo     numeric,
    p_contrato    numeric,
    p_protocolo   numeric,
...
begin
   ...
   If p_restricao is null Then
      open p_result for
         select a.sq_acordo_aditivo, a.sq_siw_solicitacao, a.protocolo, a.codigo, a.objeto, 
                ...
                case a.prorrogacao when 'S' then 'Sim' else 'Não' end as nm_prorrogacao,
                case a.revisao     when 'S' then 'Sim' else 'Não' end as nm_revisao,
                case a.acrescimo
           ...
           from siw.ac_acordo_aditivo              a
                inner   join siw.ac_acordo         b on (a.sq_siw_solicitacao = b.sq_siw_solicitacao)
                  inner join siw.ac_tipo_acordo    c on (b.sq_tipo_acordo     = c.sq_tipo_acordo)
                left    join (select count(x.sq_acordo_parcela) as qtd_parcela, x.sq_acordo_aditivo
                                from siw.ac_acordo_parcela x
   ...
   Elsif p_restricao = 'EXISTE' Then
      open p_result for
         select count(a.sq_acordo_aditivo) as existe
           from siw.ac_acordo_aditivo            a
   ...
   Elsif substr(p_restricao,1,10) = 'LANCAMENTO' Then
      open p_result for
         select c.sq_siw_solicitacao, d.sq_menu
           from siw.ac_acordo_aditivo                  a
                inner       join siw.ac_acordo_parcela b on (a.sq_acordo_aditivo  = b.sq_acordo_aditivo)
                  inner     join siw.fn_lancamento     c on (b.sq_acordo_parcela  = c.sq_acordo_parcela)
                    inner   join siw.siw_solicitacao   d on (c.sq_siw_solicitacao = d.sq_siw_solicitacao)
                      inner join siw.siw_tramite       e on (d.sq_siw_tramite     = e.sq_siw_tramite)
       ...
       UNION
         select c.sq_siw_solicitacao, d.sq_menu
           from siw.ac_acordo                          a
                inner       join siw.ac_acordo_parcela b on (a.sq_siw_solicitacao = b.sq_siw_solicitacao)
                  inner     join siw.fn_lancamento     c on (b.sq_acordo_parcela  = c.sq_acordo_parcela)
                    inner   join siw.siw_solicitacao   d on (c.sq_siw_solicitacao = d.sq_siw_solicitacao)
                      inner join siw.siw_tramite       e on (d.sq_siw_tramite     = e.sq_siw_tramite)
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION siw.SP_GetAcordoAditivo
   (p_cliente     numeric,
    p_aditivo     numeric,
    p_contrato    numeric,
    p_protocolo   numeric,
