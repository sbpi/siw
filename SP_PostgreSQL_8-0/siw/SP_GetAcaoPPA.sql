CREATE OR REPLACE FUNCTION siw.SP_GetAcaoPPA
   (p_chave               numeric,
    p_cliente             numeric,
    p_programa            numeric,
    p_acao                numeric,
...
begin
   ...
   If p_restricao = 'CADASTRO' Then
      open p_result for
         select a.sq_acao_ppa as chave, a.sq_acao_ppa_pai, a.cliente, a.codigo, a.nome,
                ...
                case a.padrao when 'S' then 'Sim' else 'Não' end as nm_padrao,
                b.nome as nm_acao_pai, b.codigo as cd_pai,
                c.acao as acao, d.outras_acao as outras_acao,
           ...
           from siw.or_acao_ppa                 a
                left outer join siw.or_acao_ppa b on (a.sq_acao_ppa_pai = b.sq_acao_ppa)
                left outer join (select x.sq_acao_ppa,count(*) as acao
                                   from siw.or_acao x
                                 group by x.sq_acao_ppa
                                )           c on (a.sq_acao_ppa     = c.sq_acao_ppa)
                left outer join (select y.sq_acao_ppa,count(*) as outras_acao
                                   from siw.or_acao_financ y
                                 group by y.sq_acao_ppa
                                )           d on (a.sq_acao_ppa     = d.sq_acao_ppa)
                left outer join siw.or_acao     e on (a.sq_acao_ppa     = e.sq_acao_ppa)
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION siw.SP_GetAcaoPPA
   (p_chave               numeric,
    p_cliente             numeric,
    p_programa            numeric,
    p_acao                numeric,
