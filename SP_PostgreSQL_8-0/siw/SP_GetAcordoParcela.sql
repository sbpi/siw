CREATE OR REPLACE FUNCTION siw.SP_GetAcordoParcela
   (p_chave       numeric,
    p_chave_aux   numeric,
    p_restricao   varchar,
    p_outra_parte varchar,
    p_dt_ini      date,
    p_dt_fim      date,
    p_usuario     numeric,
    p_fase        varchar,
   ...
   If p_restricao is null or p_restricao = 'PERIODO' or p_restricao = 'PARCELA' Then
      open p_result for 
         select a.sq_acordo_parcela, a.sq_siw_solicitacao, a.ordem, a.emissao, a.vencimento, a.quitacao,
                ...
                b.sq_siw_solicitacao as sq_lancamento, b.codigo_interno as cd_lancamento, 
           ...
           from siw.ac_acordo_parcela                  a
                left     join (select x.sq_acordo_parcela, x.sq_siw_solicitacao, y.valor,
                                 ...
                                 from siw.fn_lancamento                x
                                      inner   join siw.siw_solicitacao y on (x.sq_siw_solicitacao = y.sq_siw_solicitacao)
                                        inner join siw.siw_tramite     z on (y.sq_siw_tramite     = z.sq_siw_tramite and
                                                                         nvl(z.sigla,'-')     <> 'CA'
                                                                        )
                              )                    b on (a.sq_acordo_parcela = b.sq_acordo_parcela)
                left     join siw.ac_acordo_aditivo    c on (a.sq_acordo_aditivo = c.sq_acordo_aditivo and
                                                         a.sq_siw_solicitacao = c.sq_siw_solicitacao
                                                        )
                inner    join siw.ac_acordo            d on (a.sq_siw_solicitacao = d.sq_siw_solicitacao)
      ...
      open p_result for 
         select a.sq_acordo_parcela, a.sq_siw_solicitacao, a.ordem, a.emissao, a.vencimento, a.quitacao,
           ...
           from siw.ac_acordo_parcela                  a
                ...
                left          join siw.ac_parcela_nota d on (a.sq_acordo_parcela  = d.sq_acordo_parcela)
                  left        join siw.ac_acordo_nota  e on (d.sq_acordo_nota     = e.sq_acordo_nota)
                    left      join (select w.sq_acordo_nota, w.valor_inicial, w.valor_excedente, w.valor_reajuste,
                                       ...
                                       from siw.fn_lancamento_doc              w
                                            inner     join siw.fn_lancamento   x on (w.sq_siw_solicitacao = x.sq_siw_solicitacao and
   ...
   Elsif p_restricao = 'CADASTRO' Then
      open p_result for 
        select a.sq_siw_solicitacao, a.sq_solic_pai, a.solicitante, a.sq_unidade, a.sq_cc, 
               a.codigo_interno as cd_acordo, b.sq_forma_pagamento, b.sq_tipo_pessoa, b.outra_parte,
               b.objeto,
          ...
          from siw.siw_solicitacao                          a
               inner            join siw.ac_acordo          b on (a.sq_siw_solicitacao = b.sq_siw_solicitacao)
                   left outer   join (select x.sq_siw_solicitacao, count(*) as existe
                                        from siw.ac_acordo_nota x
                                      group by x.sq_siw_solicitacao
                                     )                  b1 on (b.sq_siw_solicitacao = b1.sq_siw_solicitacao)
                 inner          join siw.co_pessoa          g  on (b.outra_parte        = g.sq_pessoa)
                   left outer   join siw.co_pessoa_fisica   h  on (g.sq_pessoa          = h.sq_pessoa)
                   left outer   join siw.co_pessoa_juridica i  on (g.sq_pessoa          = i.sq_pessoa)
                 inner          join siw.siw_tramite        j  on (a.sq_siw_tramite     = j.sq_siw_tramite and
                                                               (p_fase              is null or (p_fase is not null and (0 <> InStr(p_fase,Nvl(j.sigla,'-')))))
                                                              )
                 inner          join siw.ac_acordo_parcela  c  on (b.sq_siw_solicitacao = c.sq_siw_solicitacao)
                   left outer   join (select x.sq_acordo_parcela, count(*) as existe
                                        from siw.ac_parcela_nota x
                 ...
                 left outer     join (select w.sq_siw_solicitacao, max(w.sq_acordo_parcela) as sq_acordo_parcela, max(x.sq_siw_solicitacao) as sq_lancamento
                                        from siw.ac_acordo_parcela            w
                                             inner join siw.fn_lancamento     x on (w.sq_acordo_parcela = x.sq_acordo_parcela)
                                             inner   join siw.siw_solicitacao y on (x.sq_siw_solicitacao = y.sq_siw_solicitacao)
                                               inner join siw.siw_tramite     z on (y.sq_siw_tramite     = z.sq_siw_tramite and
                                                                                'CA'                 <> Nvl(z.sigla,'CA')
                                                                               )
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION siw.SP_GetAcordoParcela
   (p_chave       numeric,
    p_chave_aux   numeric,
    p_restricao   varchar,
    p_outra_parte varchar,
    ...
    p_fase        varchar,
    p_menu        numeric,
    p_aditivo     numeric) OWNER TO siw;
