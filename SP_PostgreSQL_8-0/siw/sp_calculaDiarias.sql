create or replace FUNCTION sp_calculaDiarias(p_chave numeric, p_todos varchar, p_tipo varchar ) RETURNS VOID AS $$
DECLARE
  i           numeric(18);
  w_cont      numeric(18) := 0;
  w_desloc    numeric(18) := 0;
  w_existe    numeric(18);
  w_atual     date;
  w_sq_diaria numeric(18);
  w_diaria    varchar(1);
  w_inicio    date;
  w_ini_hora  numeric(4);
  w_fim       date;
  w_ultimo    numeric(18) := 0;
  w_tot_dia   numeric(5,2);
  w_compromisso_saida   varchar(1);
  w_compromisso_retorno varchar(1);
  w_internacional       varchar(1);
  w_fim_semana          varchar(1);

  type diaria is table of numeric(10,1) index by binary_integer;
  
  diarias diaria;
  
  --  que recupera todas as solicitações
   c_solic CURSOR FOR
    select a.sq_siw_solicitacao
           from siw_solicitacao      a
                inner join pd_missao b on (a.sq_siw_solicitacao = b.sq_siw_solicitacao);

   c_deslocamentos CURSOR (w_chave numeric, w_data date) FOR
