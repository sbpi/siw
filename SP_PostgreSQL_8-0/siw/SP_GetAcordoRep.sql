CREATE OR REPLACE FUNCTION siw.SP_GetAcordoRep
   (p_chave       numeric,
    p_cliente     numeric,
    p_sq_pessoa   numeric,
    p_restricao   varchar
      ...
      open p_result for
        select a.sq_pessoa, a.nome as nm_pessoa, a.nome_resumido, a.sq_pessoa_pai,
          ...
          from siw.co_pessoa                                a
               inner      join  siw.ac_acordo_representante e on (a.sq_pessoa          = e.sq_pessoa)
                 inner    join  siw.ac_acordo               g on (e.sq_siw_solicitacao = g.sq_siw_solicitacao)
                   inner  join  siw.co_pessoa               h on (g.outra_parte        = h.sq_pessoa)
               inner      join  siw.co_tipo_pessoa          c on (a.sq_tipo_pessoa     = c.sq_tipo_pessoa)
               inner      join  siw.co_tipo_vinculo         d on (a.sq_tipo_vinculo    = d.sq_tipo_vinculo)
               ...
               inner      join siw.co_pessoa_fisica         j on (a.sq_pessoa      = j.sq_pessoa)
               inner      join siw.sg_autenticacao          n on (a.sq_pessoa      = n.sq_pessoa)
   ...
   Elsif w_sg_modulo = 'PR' Then
      open p_result for
        select a.sq_pessoa, a.nome as nm_pessoa, a.nome_resumido, a.sq_pessoa_pai,
               ...
               Nvl(j.cpf,n.username)as cpf, j.rg_numero, j.rg_emissao, j.rg_emissor, j.sexo
          from siw.co_pessoa                                 a
               inner      join  siw.pj_projeto_representante e on (a.sq_pessoa          = e.sq_pessoa)
                 inner    join  siw.pj_projeto               g on (e.sq_siw_solicitacao = g.sq_siw_solicitacao)
                   inner  join  siw.co_pessoa                h on (g.outra_parte        = h.sq_pessoa)
               inner      join  siw.co_tipo_pessoa           c on (a.sq_tipo_pessoa     = c.sq_tipo_pessoa)
               inner      join  siw.co_tipo_vinculo          d on (a.sq_tipo_vinculo    = d.sq_tipo_vinculo)
               left outer join  (select sq_pessoa, w.sq_pessoa_telefone as sq_pessoa_fax, w.numero as nr_fax
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION siw.SP_GetAcordoRep(p_chave  numeric,p_cliente  numeric, p_sq_pessoa   numeric,p_restricao varchar) OWNER TO siw;
