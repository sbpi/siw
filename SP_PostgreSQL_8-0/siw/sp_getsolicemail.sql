create or replace FUNCTION SP_GetSolicEmail (
   p_cliente       numeric,
   p_result       REFCURSOR
...
BEGIN
   ...
   open p_result for 
     select a.nome, a.sigla, a1.sigla sg_modulo, a2.nome nm_unidade_exec, 
            ...
            case sign(b.fim - now()) when -1 then trunc(now()-b.fim) else -1 end dias_atraso,
            ceil(b.fim-now()) dias_fim,
       ...
       from siw_menu                                       a 
               ...
               inner             join siw_solicitacao      b  on (a.sq_menu                  = b.sq_menu and
                  ...
                  left outer     join siw_solicitacao      r3 on (b.sq_solic_pai             = r3.sq_siw_solicitacao)
                    left outer   join or_acao              r  on (r3.sq_siw_solicitacao      = r.sq_siw_solicitacao)
                      left outer join or_acao_ppa          r1 on (r.sq_acao_ppa              = r1.sq_acao_ppa)
      ...
      where a.sq_pessoa = p_cliente
        ...
        and d.concluida = 'N'
        and ((b.fim < now()) or
             (d.aviso_prox_conc = 'S' and (b.fim-now() < d.dias_aviso))
            )
     UNION
     select a.nome, a.sigla, a1.sigla sg_modulo, a2.nome nm_unidade_exec, 
            a6.nome nm_tit_exec, a5.email em_tit_exec, a8.nome nm_sub_exec, a7.email em_sub_exec, 
