...
select chave, cliente, to_char(data_formatada,'yyyy') as ano, data_formatada, nome, expediente, nm_expediente
  from (select a.sq_data_especial as chave, a.cliente, 
         ...
         from eo_data_especial  a,
              (select to_char(now(),'yyyy')-3 as ano from dual
               UNION
               select to_char(now(),'yyyy')-2 as ano from dual
               UNION
               select to_char(now(),'yyyy')-1 as ano from dual
               UNION
               select to_char(now(),'yyyy')+0 as ano from dual
               UNION
               select to_char(now(),'yyyy')+1 as ano from dual
               UNION
               select to_char(now(),'yyyy')+2 as ano from dual
