create or replace function SP_GetAddressList
   (p_cliente   numeric,
    p_chave     numeric,
    p_restricao varchar,
    p_result    refcursor
   ...
   If p_restricao is null Then
      ...
      open p_result for 
         select a.sq_pessoa_endereco, a.complemento, a.bairro, a.cep, a1.sq_tipo_endereco, a1.nome as tipo_endereco, a1.email, a1.internet, 
                ...
                a.padrao, a.logradouro, a.sq_cidade, b.nome || '-' || c.co_uf as cidade, d.nome, 
         ...
         order by acentos(a.logradouro, null);
   Elsif p_restricao = 'FISICO' Then
      -- Recupera apenas os endereços físicos
      open p_result for 
         select a.sq_pessoa_endereco, a.complemento, a.bairro, a.cep, d.nome as pais,
                a.logradouro||' ('||case c.co_uf when 'EX' then b.nome||'-'||d.nome else b.nome||'-'||c.co_uf end ||')' as endereco,
