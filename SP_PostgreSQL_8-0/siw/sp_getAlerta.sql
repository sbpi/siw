      ...
      open p_result for
         ...
         select a.sq_pessoa as cliente, 
          ...
          where ((p_cliente  is null and a.envia_mail_alerta = coalesce(p_mail,'S')) or (p_cliente is not null and a.sq_pessoa = p_cliente))
            ...
            and ((c.sigla    = 'PR' and g.sq_siw_solicitacao is not null and (f.fim < trunc(now()) or (g.aviso_prox_conc = 'S' and ((cast(f.fim as date)-cast(g.dias_aviso as integer))<=trunc(now()))))) or
                 (c.sigla    = 'PR' and h.sq_siw_solicitacao is not null and (f.fim < trunc(now()) or (h.aviso_prox_conc = 'S' and ((cast(f.fim as date)-cast(h.dias_aviso as integer))<=trunc(now()))))) or
                 (c.sigla    = 'DM' and h.sq_siw_solicitacao is not null and (f.fim < trunc(now()) or (h.aviso_prox_conc = 'S' and ((cast(f.fim as date)-cast(h.dias_aviso as integer))<=trunc(now()))))) or
                 (c.sigla    = 'AC' and i.sq_siw_solicitacao is not null and (f.fim < trunc(now()) or (i.aviso_prox_conc = 'S' and ((cast(f.fim as date)-cast(i.dias_aviso as integer))<=trunc(now()))))) or
                 (c.sigla    = 'FN' and j.sq_siw_solicitacao is not null and (f.fim < trunc(now()) or (j.aviso_prox_conc = 'S' and ((cast(f.fim as date)-cast(j.dias_aviso as integer))<=trunc(now()))))) or
                 (c.sigla    = 'PE' and k.sq_siw_solicitacao is not null and (f.fim < trunc(now()) or (k.aviso_prox_conc = 'S' and ((cast(f.fim as date)-cast(k.dias_aviso as integer))<=trunc(now()))))) or
                 (c.sigla    = 'PD' and l.sq_siw_solicitacao is not null and l.prestou_contas  = 'N' and (cast(f.fim as date)+cast(coalesce(l4.dias_prestacao_contas,w.dias_prestacao_contas,0) as integer))<=trunc(now())) or
   ...
   Elsif p_restricao = 'PACOTE' Then
      ...
      open p_result for
         select a.sq_pessoa as cliente, 
          ...
          where ((p_cliente    is null and a.envia_mail_alerta = coalesce(p_mail,'S')) or (p_cliente is not null and a.sq_pessoa = p_cliente))
            and (p_usuario     is null or (p_usuario is not null and a1.sq_pessoa = p_usuario))
            and (h.fim_previsto < trunc(now()) or (g.aviso_prox_conc_pacote = 'S' and ((f.fim-cast(g.perc_dias_aviso_pacote/100*(h.fim_previsto-h.inicio_previsto) as integer))<=trunc(now()))))
