CREATE OR REPLACE FUNCTION siw.sp_getsolicdata(p_chave numeric, p_restricao character varying, p_result refcursor)
   ...
   Elsif substr(p_restricao,1,2) = 'GD' or p_restricao = 'ORPGERAL' Then
      ...
      open p_result for 
         select a.sq_menu,            a.sq_modulo,                   a.nome,
                ...
                b.palavra_chave,
                b.fim-cast(d.dias_aviso as integer) as aviso,
   ...
   Elsif substr(p_restricao,1,2) in ('PJ','OR') Then
      ...
      open p_result for 
         select a.sq_menu,            a.sq_modulo,                   a.nome,
                ...
                b.inclusao,           b.ultima_alteracao,            b.conclusao,
                b.valor,              b.opiniao,
                b.sq_solic_pai,       b.sq_unidade as sq_unidade_cad,   b.sq_cidade_origem,
                ...
                d.sq_cidade as cidade_evento, d.vincula_contrato,       d.objetivo_superior,
                ...
                e.sq_tipo_unidade,    e.nome as nm_unidade_resp,        e.informal as informal_resp,
                e.vinculada as vinc_resp,e.adm_central as adm_resp,        e.sq_unidade,
                e1.sq_pessoa as titular, e2.sq_pessoa as substituto,
                f.nome_resumido as nm_sol,
                coalesce(f1.ativo,'N') as st_sol,
                g.sq_cc,              g.nome as cc_nome,                g.sigla as cc_sigla,
   ...
   Elsif substr(p_restricao,1,2) = 'GC' Then
      ...
      open p_result for 
         select a.sq_menu,            a.sq_modulo,                   a.nome,
                ...
                case d.vincula_projeto when 'S' then 'Sim' else 'Não' end as nm_vincula_projeto,
                case d.vincula_demanda when 'S' then 'Sim' else 'Não' end as nm_vincula_demanda,
                case d.vincula_viagem  when 'S' then 'Sim' else 'Não' end as nm_vincula_viagem,
                ...
                d3.nome as nm_outra_parte, d3.nome_resumido as nm_outra_parte_resumido,
                ...
                d6.exige_operacao,
                d7.nome as nm_pais,
                d8.nome as nm_lcmodalidade,
                d9.nome as nm_eoindicador,
                d10.nome as nm_lcfonte_recurso,
                d11.nome as nm_espec_despesa,
                d12.aditivo,
                d13.aditivo as aaditivo_excedente,
                d14.aditivo as aditivo_prorrogacao,
                b.fim-cast(d.dias_aviso as integer) as aviso,
                e.sq_tipo_unidade,    e.nome as nm_unidade_resp,        e.informal as informal_resp,
                ...
                nvl(m1.qtd_rubrica,0) as qtd_rubrica
           from siw_menu                                    a 
                inner        join eo_unidade                a2 on (a.sq_unid_executora        = a2.sq_unidade)
                  left       join eo_unidade_resp           a3 on (a2.sq_unidade              = a3.sq_unidade and
                                                                   a3.tipo_respons            = 'T'           and
                ...
                inner             join siw_solicitacao      b  on (a.sq_menu                  = b.sq_menu)
                   ...
                   left           join pj_projeto           m  on (b.sq_solic_pai             = m.sq_siw_solicitacao)
                     left         join (select sq_siw_solicitacao, count(sq_projeto_rubrica) as qtd_rubrica
                                          from pj_rubrica
                                        group by sq_siw_solicitacao
   ...
   Elsif substr(p_restricao,1,2) = 'FN' Then
      ...
      open p_result for 
         select a.sq_menu,            a.sq_modulo,                   a.nome,
                ...
                d.referencia_fim,
                case d.tipo when 1 then 'Dotação incial' when 2 then 'Transferência entre rubricas' when 3 then 'Atualização de aplicação' when 4 then 'Entradas' when 5 then 'Saídas' end as nm_tipo_rubrica,
                ...
                o.nome_resumido as nm_solic, o.nome_resumido||' ('||o2.sigla||')' as nm_resp,
                p.nome_resumido as nm_exec,
                case nvl(m2.sq_siw_solicitacao,0) when 0 then q2.titulo             else m5.titulo end as nm_projeto,
                case nvl(m2.sq_siw_solicitacao,0) when 0 then q.sq_siw_solicitacao else m2.sq_siw_solicitacao end as sq_projeto,
                case nvl(m3.sq_siw_solicitacao,0) when 0 then q1.qtd_rubrica       else m3.qtd_rubrica        end as qtd_rubrica
           from siw_menu                                    a 
                inner        join eo_unidade                a2 on (a.sq_unid_executora        = a2.sq_unidade)
                  left       join eo_unidade_resp           a3 on (a2.sq_unidade              = a3.sq_unidade and
                                                                   a3.tipo_respons            = 'T'           and
   ...
   Elsif substr(p_restricao,1,2) = 'PD' Then
      ...
      open p_result for 
         select a.sq_menu,            a.sq_modulo,                   a.nome,
                ...
                a.emite_os,           a.consulta_opiniao,            a.envia_email,
                a.exibe_relatorio,    a.vinculacao,                  a.data_hora,
                a.envia_dia_util,     a.descricao,                   a.justificativa as justif_solic,
                a1.nome as nm_modulo,    a1.sigla as sg_modulo,            a1.objetivo_geral,
                a2.sq_tipo_unidade as tp_exec, a2.nome as nm_unidade_exec, a2.informal as informal_exec,
                a2.vinculada as vinc_exec,a2.adm_central as adm_exec,
                ...
                case d1.tipo when 'I' then 'Inicial' when 'P' then 'Prorrogação' when 'C' then 'Complementação' end as  nm_tipo_missao,
                ...
                coalesce(d21.ativo,'N')as st_prop,
                d3.sq_tipo_vinculo,   d3.nome as nm_tipo_vinculo,
                d4.sexo,              d4.cpf,
                d6.sq_agencia,        d6.codigo as cd_agencia,          d6.nome as nm_agencia,
                d7.sq_banco,          d7.codigo as cd_banco,            d7.nome as nm_banco,
                d7.exige_operacao,
                ...
                e.sq_tipo_unidade,    e.nome as nm_unidade_resp,        e.informal as informal_resp,
                e.vinculada as vinc_resp,e.adm_central as adm_resp,        e.sigla as sg_unidade_resp,
                e1.sq_pessoa as titular, e2.sq_pessoa as substituto,       e12.nome as nm_titular,
                f.sq_pais,            f.sq_regiao,                   f.co_uf,
                g.sq_cc,              g.nome as nm_cc,                  g.sigla as sg_cc,
                o.nome_resumido as nm_solic, o.nome_resumido||' ('||o2.sigla||')' as  nm_resp,
                coalesce(o1.ativo,'N') as st_sol,
                p.nome_resumido as nm_exec
           from siw_menu                                               a
   ...
   Elsif substr(p_restricao,1,2) = 'SR' Then
      ...
      open p_result for 
         select a.sq_menu,            a.sq_modulo,                   a.nome,
                ...
                b1.sq_siw_tramite,    b1.nome as nm_tramite,            b1.ordem as or_tramite,
                b1.sigla as sg_tramite,  b1.ativo,                      b1.envia_mail,
                b2.nome as  nm_opiniao,
                b3.sq_veiculo,        b3.qtd_pessoas,                b3.carga,
                b3.hodometro_saida,   b3.hodometro_chegada,          b3.destino,
                b3.parcial,           b3.procedimento,
                to_char(b3.horario_saida,'dd/mm/yyyy, hh24:mi:ss')   as phpdt_horario_saida,
                to_char(b3.horario_chegada,'dd/mm/yyyy, hh24:mi:ss') as phpdt_horario_chegada,
                ...
                c.sq_tipo_unidade,    c.nome as nm_unidade_exec,        c.informal,
                c.vinculada,          c.adm_central,
                e.sq_tipo_unidade,    e.nome as nm_unidade_solic,        e.informal as informal_solic,
                e.vinculada as vinc_solic,e.adm_central as adm_solic,       e.sigla as sg_unidade_solic,
                e1.sq_pessoa as titular, e2.sq_pessoa as substituto,
                f.nome_resumido as nm_sol,
                coalesce(f1.ativo,'N') as st_sol,
                g.sq_cc,              g.nome as cc_nome,                g.sigla as cc_sigla,
                h.sq_pais,            h.sq_regiao,                   h.co_uf,
                h.nome as nm_cidade,
                i.nome_resumido as nm_exec,
                j.nome_resumido as nm_recebedor,
   ...
   Elsif substr(p_restricao,1,4) = 'PEPR' Then
      ...
      open p_result for 
         select a.sq_menu,            a.sq_modulo,                   a.nome,
                ...
                b2.ativo as st_plano,
                b4.sq_unidade as sq_unidade_adm, b4.nome as nm_unidade_adm, b4.sigla as sg_unidade_adm,
                b5.sq_pessoa as tit_adm, b6.sq_pessoa as subst_adm,
                c.sq_tipo_unidade,    c.nome as nm_unidade_exec,        c.informal,
                c.vinculada,          c.adm_central,
                d.sq_siw_solicitacao as sq_programa,
                d.sq_pehorizonte,     d.sq_penatureza,               b.codigo_interno as cd_programa,
   ...
   Elsif substr(p_restricao,1,3) = 'PAD' Then
      ...
      open p_result for 
         select a.sq_menu,            a.sq_modulo,                   a.nome,
                ...
                b.fim-cast(k.dias_aviso as integer) as aviso,
                e.sq_unidade as sq_unidade_resp,
                e.sq_tipo_unidade,    e.nome as nm_unidade_resp,        e.informal as informal_resp,
                e.vinculada as vinc_resp,e.adm_central as adm_resp,        e.sigla as sg_unidade_resp,
                e1.sq_pessoa as titular, e2.sq_pessoa as substituto,
