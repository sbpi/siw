create or replace FUNCTION GeraCodigoInterno
...
DECLARE
   Result       varchar(60);
   w_ano        numeric(4) := coalesce(p_ano,to_char(now(),'yyyy'));
...
BEGIN
  ...
  If w_ano >= w_reg.ano_corrente Then
     ...
     If to_char(now(),'yyyy') > w_reg.ano_corrente Then
        -- Configura o ano do acordo para o ano informado corrente
        w_ano        := to_char(now(),'yyyy');
        w_sequencial := 1;
     Else
