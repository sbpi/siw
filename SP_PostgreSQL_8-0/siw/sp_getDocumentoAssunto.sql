CREATE OR REPLACE FUNCTION siw.sp_getDocumentoAssunto
   (p_chave     numeric,
    p_chave_aux numeric,
    p_principal varchar,
    p_restricao varchar)
   ...
   If p_restricao is null Then
      open p_result for
         select a.sq_siw_solicitacao as chave, a.sq_assunto as chave_aux, a.principal,
           ...
           from siw.pa_documento_assunto        a
                inner       join siw.pa_assunto     b on (a.sq_assunto           = b.sq_assunto)
                  inner     join siw.pa_tipo_guarda c on (b.fase_corrente_guarda = c.sq_tipo_guarda)
                  inner     join siw.pa_tipo_guarda d on (b.fase_intermed_guarda = d.sq_tipo_guarda)
                  inner     join siw.pa_tipo_guarda e on (b.fase_final_guarda    = e.sq_tipo_guarda)
                  inner     join siw.pa_tipo_guarda f on (b.destinacao_final     = f.sq_tipo_guarda)
                  left      join siw.pa_assunto     g on (b.sq_assunto_pai       = g.sq_assunto)
                    left    join siw.pa_assunto     h on (g.sq_assunto_pai       = h.sq_assunto)
                      left  join siw.pa_assunto     i on (h.sq_assunto_pai       = i.sq_assunto)
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION siw.sp_getDocumentoAssunto
   (p_chave     numeric,
    p_chave_aux numeric,
    p_principal varchar,
