create or replace trigger TG_SIW_SOLIC_UP
  ...
  for each row
declare
  w_tramite_new varchar(20);
  w_tramite_old varchar(20);
  w_sigla_menu  varchar(20);
  w_tipo        numeric(1);
  
  cursor c_lancamento_rubrica (l_solicitacao numeric) is
    ...
    select c.sq_rubrica_origem, c.sq_rubrica_destino, c.valor
      from fn_lancamento                      a
           inner join fn_lancamento_doc       b on (a.sq_siw_solicitacao = b.sq_siw_solicitacao)
             inner join fn_lancamento_rubrica c on (b.sq_lancamento_doc  = c.sq_lancamento_doc)
     where a.sq_siw_solicitacao = l_solicitacao;
     
  cursor c_lancamento_item (l_solicitacao numeric) is
    -- Retorna os dados das rubricas ligadas aos documentos da solicitação
    select c.sq_projeto_rubrica, c.valor_total
      from fn_lancamento                      a
           inner join fn_lancamento_doc       b on (a.sq_siw_solicitacao = b.sq_siw_solicitacao)
             inner join fn_documento_item     c on (b.sq_lancamento_doc  = c.sq_lancamento_doc)
     where a.sq_siw_solicitacao = l_solicitacao;
        ...
        If w_tipo is not null Then
           ...
           If w_tramite_new = 'AT' Then
              If w_tipo < 5 Then
                 ...
                 for crec c_lancamento_rubrica(:new.sq_siw_solicitacao) loop
                    If w_tipo = 1 Then
              ...
              Else
                 ...
                 for crec c_lancamento_item(:new.sq_siw_solicitacao) loop
