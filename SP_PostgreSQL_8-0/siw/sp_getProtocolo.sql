create or replace FUNCTION sp_getProtocolo
   (p_menu         numeric,
    p_pessoa       numeric,
    p_restricao    varchar,
    p_chave        numeric DEFAULT NULL,
    p_chave_aux    numeric DEFAULT NULL,
    p_prefixo      numeric DEFAULT NULL,
    p_numero       numeric DEFAULT NULL,
    p_ano          numeric DEFAULT NULL,
    p_unid_autua   numeric DEFAULT NULL,
    p_unid_posse   numeric DEFAULT NULL,
    p_nu_guia      numeric DEFAULT NULL,
    p_ano_guia     numeric DEFAULT NULL,
    p_ini          date    DEFAULT NULL,
    p_fim          date    DEFAULT NULL,
    p_tipo         numeric DEFAULT NULL,
    p_despacho     numeric DEFAULT NULL,
    p_empenho      varchar DEFAULT NULL,
    p_solicitante  numeric DEFAULT NULL,
    p_unidade      numeric DEFAULT NULL,
    p_proponente   varchar DEFAULT NULL,
    p_cd_assunto   varchar DEFAULT NULL,
    p_assunto      varchar DEFAULT NULL,
    p_processo     varchar DEFAULT NULL,
    p_result       REFCURSOR DEFAULT NULL) RETURNS REFCURSOR AS $$
DECLARE
...
BEGIN
   ...
   If p_restricao = 'RELPATRAM' or p_restricao = 'RELPAETIQ' Then
      ...
      select b.inicio, b.fim, b.sq_siw_solicitacao, b.descricao,
             c.numero_original,
             c.prefixo||'.'||substr(cast(1000000+c.numero_documento as varchar),2,6)||'/'||to_char(to_char(c.ano))||'-'||substr(cast(100+to_number(c.digito) as varchar),2,2) as protocolo,
             c1.sigla sg_unidade,
             c2.nome as nm_especie,
             case when c5.sq_siw_solicitacao is not null
                  then c5.prefixo||'.'||substr(cast(1000000+c5.numero_documento as varchar),2,6)||'/'||to_char(c5.ano)||'-'||substr(cast(100+to_number(c5.digito) as varchar),2,2) 
                  else null
   ...
   Elsif instr('PADAUTUA,PADANEXA,PADJUNTA,PACLASSIF, PADTRANSF,PADELIM,PADEMPREST,PAENVCEN,PADDESM,PADALTREG', p_restricao) > 0 Then
      ...
      select b.inicio, b.fim, b.sq_siw_solicitacao, b.sq_solic_pai, b.descricao,
       ...
       where a.sq_menu     = p_menu
         ...
         and (p_tipo       = 1 or (p_tipo      = 2 and acesso(c.sq_siw_solicitacao, p_pessoa) > 0))
   ...
   Elsif p_restricao = 'PADTRAM' Then
      ...
      select b.sq_siw_solicitacao, b.inicio, b.fim, b.sq_siw_tramite, b.sq_solic_pai, b.descricao,
       ...
       where a.sq_menu      = p_menu
         ...
         and (p_tipo       = 1 or
              (p_tipo      = 2 and (w_filtro = 'true' or 
                                    (c.unidade_int_posse = w.sq_unidade or 0 < (select count(*) from eo_unidade_resp where sq_pessoa = p_pessoa and sq_unidade = c.unidade_int_posse and fim is null)) or
                                    acesso(b.sq_siw_solicitacao, p_pessoa) > 0
                                   )
              )
             )
   ...
   Elsif p_restricao = 'EXISTE' Then
      ...
      select a.sq_siw_solicitacao, a.processo,
             a.prefixo||'.'||substr(cast(1000000+a.numero_documento as varchar),2,6)||'/'||to_char(a.ano)||'-'||substr(cast(100+to_number(a.digito) as varchar),2,2) as protocolo
