CREATE OR REPLACE FUNCTION siw.SP_GetEtapaComentario
   (p_chave     numeric,
    p_chave_aux numeric,
    p_ativo     varchar,
    p_restricao varchar)
   ...
   If p_restricao is null Then
      ...
      open p_result for 
         select a.sq_etapa_comentario, a.sq_projeto_etapa, a.sq_pessoa_inclusao, a.comentario, 
           ...
           from siw.pj_etapa_comentario a
                inner     join siw.pj_projeto_etapa  b on (a.sq_projeto_etapa    = b.sq_projeto_etapa)
                inner     join siw.co_pessoa         c on (a.sq_pessoa_inclusao  = c.sq_pessoa)
                  inner   join siw.sg_autenticacao   d on (c.sq_pessoa           = d.sq_pessoa)
                    inner join siw.eo_unidade        e on (d.sq_unidade          = e.sq_unidade)
                left      join siw.pj_comentario_arq f on (a.sq_etapa_comentario = f.sq_etapa_comentario)
                  left    join siw.siw_arquivo       g on (f.sq_siw_arquivo      = g.sq_siw_arquivo)
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION siw.SP_GetEtapaComentario
   (p_chave     numeric,
    p_chave_aux numeric,
    p_ativo     varchar,
