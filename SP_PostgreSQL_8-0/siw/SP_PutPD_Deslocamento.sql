create or replace FUNCTION SP_PutPD_Deslocamento
   (p_operacao             varchar,
    p_chave               numeric,
    p_chave_aux           numeric,
    p_origem              numeric,
    p_data_saida          date,
    p_hora_saida          varchar,
    p_destino             numeric,
    p_data_chegada        date,
    p_hora_chegada        varchar,
    p_sq_cia_transporte   numeric,
    p_codigo_voo          varchar,
    p_passagem            varchar,
    p_meio_transp         numeric,
    p_valor_trecho        numeric,
    p_compromisso         varchar,
    p_aero_orig           varchar,
    p_aero_dest           varchar,
    p_tipo                varchar  
   ) RETURNS VOID AS $$
...
BEGIN
   If p_operacao in ('A','E') Then
      ...
      DELETE FROM pd_diaria where sq_siw_solicitacao = p_chave and tipo = (select tipo from pd_deslocamento where sq_deslocamento = p_chave_aux);
   End If;
   ...
   Elsif p_operacao = 'E' Then -- Exclusão
      ...
      DELETE FROM pd_deslocamento where sq_deslocamento = p_chave_aux;
   End If;
