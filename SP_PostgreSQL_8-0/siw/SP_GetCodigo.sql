create or replace FUNCTION SP_GetCodigo
  (p_cliente        numeric, 
   p_restricao      varchar, 
...
BEGIN
  ...
  Elsif p_restricao = 'LOCALIZACAO' Then
     open p_result for select sq_localizacao codigo_interno, codigo_externo from EO_LOCALIZACAO where sq_localizacao = p_chave_interna and sq_unidade (select sq_unidade from eo_unidade where sq_pessoa = p_cliente);
