CREATE OR REPLACE FUNCTION siw.SP_GetCTEspecificacao
   (p_cliente      numeric,
    p_chave        numeric,
    p_chave_aux    numeric,
    p_ano          varchar,
...
begin
   If p_restricao is null Then
      ...
      open p_result for 
         select a.sq_especificacao_despesa as chave, 
                case when b.nome is null then a.nome else b.nome||' - '||a.nome end as nm_tipo,
                a.sq_cc, a.especificacao_pai as chave_pai, a.ano, a.codigo, a.nome, a.valor, a.ultimo_nivel,
                a.ativo,
                c.nome as nm_ct_cc
           from siw.ct_especificacao_despesa               a
                   left      join siw.ct_especificacao_despesa b on (a.especificacao_pai = b.sq_especificacao_despesa)
                     left    join siw.ct_especificacao_despesa d on (b.especificacao_pai = d.sq_especificacao_despesa)
                       left  join siw.ct_especificacao_despesa e on (d.especificacao_pai = e.sq_especificacao_despesa)
                   inner     join siw.ct_cc                    c on (a.sq_cc             = c.sq_cc)
   ...
   Elsif p_restricao = 'SUBORDINACAO' Then
      ...
      open p_result for 
         select a.sq_especificacao_despesa as chave, a.nome
           from siw.ct_especificacao_despesa a
          where a.cliente            = p_cliente
   ...
   Elsif p_restricao = 'PAI' Then
      ...
      open p_result for
         select a.sq_especificacao_despesa as chave, a.cliente, a.sq_cc, a.especificacao_pai as chave_pai, a.ano, a.codigo, 
                a.nome, valor, a.ultimo_nivel, a.ativo, Nvl(b.Filho,0) as Filho,
                c.nome as nm_ct_cc
         from siw.ct_especificacao_despesa a
                 left outer join (select especificacao_pai,count(*) as Filho 
                                    from siw.ct_especificacao_despesa x 
  ...
  COST 100;
ALTER FUNCTION siw.SP_GetCTEspecificacao
   (p_cliente      numeric,
    p_chave        numeric,
    p_chave_aux    numeric,
    p_ano          varchar,
