   ...
   open p_result for 
     select distinct a.sq_pessoa, a.nome as nm_pessoa, a.nome_resumido, a.nome_indice, a.nome_resumido_ind,
       ...
       from siw.co_pessoa                           a
            left join  siw.co_tipo_pessoa           c on (a.sq_tipo_pessoa     = c.sq_tipo_pessoa)
            left join  siw.co_tipo_vinculo          d on (a.sq_tipo_vinculo    = d.sq_tipo_vinculo)
            left join  (select w.sq_pessoa_conta, sq_pessoa, x.sq_banco, w.sq_agencia, 
                               w.operacao, w.numero, x.codigo as cd_agencia, x.nome as nm_agencia,
                               y.codigo as cd_banco, y.nome as nm_banco
                          from siw.co_pessoa_conta         w
                               inner   join siw.co_agencia x on (w.sq_agencia = x.sq_agencia)
                                 inner join siw.co_banco   y on (x.sq_banco   = y.sq_banco)
                         where w.ativo              = 'S'
                           and w.padrao             = 'S'
                           and x.ativo              = 'S'
                       )                        e on (a.sq_pessoa          = e.sq_pessoa)
            left join  (select sq_pessoa, w.sq_pessoa_telefone ,w.sq_pessoa_fax, w.numero, w.nr_fax
            ...
            left join  (select sq_pessoa, w.sq_pessoa_telefone, w.ddd, w.numero, w.nr_telefone
                          from siw.co_pessoa_telefone          w
                               inner join siw.co_tipo_telefone x on (w.sq_tipo_telefone   = x.sq_tipo_telefone)
                               inner join siw.co_pessoa        z on (w.sq_pessoa          = z.sq_pessoa)
                         where x.sq_tipo_pessoa     = z.sq_tipo_pessoa
            ...
            left join  (select sq_pessoa, w.sq_pessoa_telefone, w.sq_pessoa_celular, w.numero, w.nr_celular
                          from siw.co_pessoa_telefone          w
                               inner join siw.co_tipo_telefone x on (w.sq_tipo_telefone   = x.sq_tipo_telefone)
                               inner join siw.co_pessoa        z on (w.sq_pessoa          = z.sq_pessoa)
                         where x.sq_tipo_pessoa     = z.sq_tipo_pessoa
            ...
            left join siw.co_pessoa_fisica          j on (a.sq_pessoa          = j.sq_pessoa)
              left join siw.co_pais                 o on (o.sq_pais            = j.sq_pais_passaporte)
            left join siw.co_pessoa_juridica        k on (a.sq_pessoa          = k.sq_pessoa)
            left join siw.sg_autenticacao           n on (a.sq_pessoa          = n.sq_pessoa)
              left join siw.eo_unidade              p on (n.sq_unidade         = p.sq_unidade)
                left join siw.eo_unidade            q on (p.sq_unidade_pai     = q.sq_unidade)
