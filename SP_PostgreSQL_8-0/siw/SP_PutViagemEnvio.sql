create or replace FUNCTION SP_PutViagemEnvio
   (p_menu                numeric,
    p_chave               numeric,
    p_pessoa              numeric,
    p_tramite             numeric,
...
DECLARE
   ...
   w_especial      numeric(18);
   w_pendencia     numeric(18) := 0;
   
    c_missao CURSOR FOR
      select * from pd_missao where sq_siw_solicitacao = p_chave;
      
    c_financeiro_pendente CURSOR (l_cliente numeric, l_pessoa numeric) FOR
    ...
    c_reembolso CURSOR FOR
      select x.codigo_interno as cd_interno, w.sq_pessoa as cliente, w.sq_menu, w.sq_unid_executora, 
             ...
             soma_dias(w_cliente,trunc(now()),2,'U') as vencimento, 
    ...
    c_ressarcimento_geral CURSOR FOR
      select x.codigo_interno as cd_interno, w.sq_pessoa as cliente, w.sq_menu, w.sq_unid_executora, 
    ...
    c_ressarcimento_item CURSOR FOR
...
BEGIN
   ...
   If p_devolucao = 'N' Then
      ...
      select count(*) into w_pendencia
       ...
       where 0           > soma_dias(e.cliente,trunc(b.fim),f.dias_prestacao_contas + 1,'U') - trunc(now())
      ...
      If w_sg_tramite ('CI','PC') Then
         -- Calcula as quantidades de diárias
         PERFORM SP_CalculaDiarias(p_chave,null);
      ...
      If w_sg_tramite = 'PC' or (w_sg_tramite = 'EE' and (w_reembolso = 'S' or w_ressarcimento = 'S' or w_complemento > 0)) Then
         If w_reembolso = 'S' or w_complemento > 0 or w_sg_tramite = 'PC' Then
            ...
            for crec in c_reembolso loop
                ...
                DELETE FROM fn_documento_item where sq_lancamento_doc = w_sq_doc;
