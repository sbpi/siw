            ...
            select x.sq_unidade, y.nivel
              from pd_unidade x,
                   (select sq_unidade, nome, level as nivel
                      from eo_unidade a
                    where a.sq_unidade in (select sq_unidade from connectby('eo_unidade','sq_unidade_pai','sq_unidade',to_char(p_chave),0) as (sq_unidade numeric, sq_unidade_pai numeric, level int))
                   ) y
                        ...
                        (p_chave is not null and ((p_restricao <> 'RELATSUB' and a.sq_unidade        = p_chave) or
                                                  (p_restricao = 'RELATSUB' and
                                                   a.sq_unidade in (select sq_unidade
                                                                      from eo_unidade
                                                                     where sq_unidade in (select sq_unidade from connectby('eo_unidade','sq_unidade','sq_unidade_pai',to_char(p_chave),0) as (sq_unidade numeric, sq_unidade_pai numeric, level int))
                                                                   )
                                                  )
                                                 )
                        )
