create or replace procedure sp_PutXMLProduto_SIG
...
begin
   If p_ativo = 'N' Then
      -- Desativa o registro
      update is_sig_produto set ativo='N' where cd_produto = p_chave;
   Else
      select count(*) into w_cont from is_sig_produto a where a.cd_produto = p_chave;
      If w_cont = 0 
         Then w_operacao := 'I';
         Else w_operacao := 'A';
      End If;
