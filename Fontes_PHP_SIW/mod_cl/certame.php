...
//exibevariaveis();
  ...
  if ($w_Troca>'') {
    ...
    BodyOpen('onLoad=\'document.Form.p_empenho.focus()\';');
  } elseif ($P1==2) {
    BodyOpen(null);
  } else {
  ...
  if ($O=='L') {
    ...
    if ($w_tipo!='WORD') {
      ...
      //ShowHTML('          <td><b>'.LinkOrdena('Data limite','fim').'</td>');
      if ($P1!=1) {
      ...
      if ($_SESSION['INTERNO']=='S') ShowHTML ('          <td><b>Vinculação</td>');
    ...
    if (count($RS)<=0) {
      ...
      foreach($RS1 as $row) {
        ...
        if ($P1!=1) {
          if ($_SESSION['INTERNO']=='S') ShowHTML('        <td align="right" width="1%" nowrap>&nbsp;'.formatNumber(f($row,'valor')).'&nbsp;</td>');
...
function Itens() {
  ...
  if ($w_troca>'' && $O <> 'E') {
    ...
    $RS = db_getCLSolicItem::getInstanceOf($dbms,null,null,null,null,null,null,null,null,null,null,null,null,'LCITEM');
    ...
    $RS = db_getCLSolicItem::getInstanceOf($dbms,null,$w_chave,null,null,null,null,null,null,null,null,null,null,'LICITACAO');
    $RS = SortArray($RS,'ordem','asc','nm_tipo_material_pai','asc','nm_tipo_material','asc','nome','asc','dados_pai','asc'); 
  } elseif (strpos('AE',$O)!==false) {
    $RS = db_getCLSolicItem::getInstanceOf($dbms,$w_chave_aux,null,null,null,null,null,null,null,null,null,null,null,'LICITACAO');
    foreach ($RS as $row) {$RS = $row; break;}
...
function ItensContrato() {
  ...
  if ($P1!=1) {
    ...
    $RS_Cont = db_getSolicData::getInstanceOf($dbms,$w_chave,$SG);
...
function PesquisaPreco() {
  ...
  if ($w_troca>'') {
    ...
    if ($w_pesquisa=='S') {
      ...
      $RS = db_getCLSolicItem::getInstanceOf($dbms,null,$w_chave,null,null,null,null,null,null,null,null,null,null,'PROPOSTA');
    }
    if (nvl($p_ordena,'')>'') {
      $lista = explode(',',str_replace(' ',',',$p_ordena));
  ...
  if (strpos('IAP',$O)!==false) {
    ...
    if ($O=='P') {
      ...
      CompValor('["w_valor[]"][ind]','Valor da pesquisa de preço','>','0','zero');
      if ($w_pesquisa=='S') {
  ...
  if ($O=='L') {
    ...
    if($w_pesquisa=='S') {
      $RS = db_getCLSolicItem::getInstanceOf($dbms,null,$w_chave,$w_sq_pessoa,null,null,null,null,null,null,null,null,null,'FORNECEDORC');
...
function DadosPrevios() {
  ...
  $RS_Solic = db_getSolicCL::getInstanceOf($dbms,null,$_SESSION['SQ_PESSOA'],$SG,3,
        null,null,null,null,null,null,null,null,null,null,
        $w_chave,null,null,null,null,null,null,
        null,null,null,null,null,null,null,null,null,null,null);
  foreach($RS_Solic as $row){$RS_Solic=$row; break;}
  ...
  if (nvl($w_sq_lcmodalidade,'')!='') {
    $RS = db_getLCModalidade::getInstanceOf($dbms, $w_sq_lcmodalidade, $w_cliente, null, null, null, null);
    foreach ($RS as $row) { $RS = $row; break; }
...
function DadosAnalise() {
  ...
  if (nvl($w_sq_lcmodalidade,'')!='') {
    $RS_Modal = db_getLCModalidade::getInstanceOf($dbms, $w_sq_lcmodalidade, $w_cliente, null, null, null, null);
  ...
  Validate('w_prioridade','Prioridade','SELECT',1,1,1,'','0123456789');
  Validate('w_fim','Previsão de conclusão','DATA',1,10,10,'','0123456789/');
  ...
  ShowHTML('      <tr><td colspan=3 align="center" height="1" bgcolor="#000000"></td></tr>');
  $RS = db_getCLSolicItem::getInstanceOf($dbms,null,$w_chave,null,null,null,null,null,null,null,null,null,null,'COMPRA');
  $RS = SortArray($RS,'ordem','asc','nm_tipo_material_pai','asc','nm_tipo_material','asc','nome','asc'); 
  ...
  foreach($RS as $row) { 
    ...
    ShowHTML('        <td align="center" nowrap><INPUT class="sti" type="text" name="w_ordem[]" size="5" maxlength="5" value="'.nvl(nvl($w_ordem[$i],f($row,'ordem')),$i).'" title="Ordem do item na licitação."></td>');
    ShowHTML('        <td align="center" nowrap><INPUT class="sti" type="text" name="w_dias_item[]" size="4" maxlength="10" value="'.nvl($w_dias_item[$i],f($row,'dias_validade_item')).'" title="Mínimo de dias de validade das propostas de preço para este item."></td>');
    ShowHTML('        <td>'.f($row,'nm_tipo_material_pai').'</td>');
    ShowHTML('        <td>'.f($row,'codigo_interno').'</td>');
    ShowHTML('        <td>'.ExibeMaterial($w_dir_volta,$w_cliente,f($row,'nome'),f($row,'sq_material'),$TP,null).'</td>');
    $w_atual = f($row,'sq_material');
...
function Informar() {
  ...
  ShowHTML('            <input class="stb" type="submit" name="Botao" value="Gravar">');
  ShowHTML('            <input class="stb" type="button" onClick="location.href=\''.montaURL_JS($w_dir,$R.'&O=L&SG=CLLCCAD&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.MontaFiltro('GET')).'\';" name="Botao" value="Cancelar">');
  ShowHTML('          </td>');
  ShowHTML('      </tr>');
  ShowHTML('    </table>');
  ShowHTML('    </TD>');
...
function Concluir() {
  ...
  if (substr(Nvl($w_erro,'nulo'),0,1)!='0') {
    ...
    if ($w_indica_usuario=='S') {
      ...
      ShowHTML('      <li><b>SERÁ GERADO '.strtoUpper(f($RS_Fin,'nome')).', NO TRÂMITE DE '.strtoUpper(f($RS_Tramite,'nome')).', DISPONÍVEL PARA O RESPONSÁVEL PELO RECEBIMENTO INDICADO ABAIXO.</b>');
    }
    ShowHTML('      </b></font></td>');
    ShowHTML('    <tr valign="top">');
        ...
        if (count($RS1)>0) {
          ...
          foreach($RS1 as $row) {
            if (nvl(f($row,'sq_cia_transporte'),'')>'') {
              if ($i==0) {
                $w_html .= $crlf.'        <tr><td colspan="2" align="center" bgcolor="'.$w_TrBgColor.'"><b>Bilhete de passagem</td>';
...
function Grava() {
  ...
  case 'GCZITEM':
    ...
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
      if ($O=='I' || $O=='A') {
        ...
        // Testa a existência do código
        $RS = db_getCLSolicItem::getInstanceOf($dbms,null,$_REQUEST['w_chave'],null,null,null,null,$_REQUEST['w_codigo'],null,null,null,null,null,'ITEMARP');
    ...
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
      if ($O=='I' || $O=='A') {
        if ($_REQUEST['w_tipo_pessoa']==1) {
          ...
          $RS = db_getBenef::getInstanceOf($dbms,$w_cliente,$_REQUEST['w_sq_pessoa'],null,nvl($_REQUEST['w_cpf'],'0'),null,null,$_REQUEST['w_tipo_pessoa'],null,null,null,null,null,null,null,'EXISTE');
          if (count($RS)>0) {
            ScriptOpen('JavaScript');
          ...
          $RS = db_getBenef::getInstanceOf($dbms,$w_cliente,$_REQUEST['w_sq_pessoa'],null,null,null,nvl($_REQUEST['w_nome'],'0'),$_REQUEST['w_tipo_pessoa'],null,null,null,null,null,null,null,'EXISTE');
          if (count($RS)>0) {
            foreach ($RS as $row) {
          ...
          $RS = db_getBenef::getInstanceOf($dbms,$w_cliente,$_REQUEST['w_sq_pessoa'],null,null,nvl($_REQUEST['w_cnpj'],'0'),null,$_REQUEST['w_tipo_pessoa'],null,null,null,null,null,null,null,'EXISTE');
          if (count($RS)>0) {
            ScriptOpen('JavaScript');
  ...
  case 'CLLCDADOS':
    // Verifica se a Assinatura Eletrônica é válida
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
      dml_putCLDados::getInstanceOf($dbms,'DADOS',$_REQUEST['w_chave'],$_REQUEST['w_sq_lcmodalidade'],
        ...
        $_REQUEST['w_dias'],null,$_REQUEST['w_protocolo'],$_REQUEST['w_fim'],$_REQUEST['w_prioridade']);
      
      // Atualiza a ordem dos itens da solicitação
      for ($i=0; $i<=count($_POST['w_chave_aux'])-1; $i=$i+1) {
        if (Nvl($_REQUEST['w_chave_aux'][$i],'')>'') {
          dml_putCLDados::getInstanceOf($dbms,'ORDENACAO',$_REQUEST['w_chave_aux'][$i],null,null,null,null,null,null,
              null,null,null,null,null,null,null,null,null,null,null,$_REQUEST['w_ordem'][$i],null,
              $_REQUEST['w_dias_item'][$i],null,null,null);
        } 
      }
      ...
      $w_cont = 1;
      foreach($RS as $row) {
        dml_putCLDados::getInstanceOf($dbms,'ORDENACAO',f($row,'chave'),null,null,null,
          null,null,null,null,null,null,null,null,null,null,null,null,null,null,$w_cont,null,null,null,null,null);
        $w_cont+=1;
      }
      ScriptOpen('JavaScript');
  ...
  case 'CLLCPROT':
    // Verifica se a Assinatura Eletrônica é válida
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
      dml_putCLDados::getInstanceOf($dbms,'PROT',$_REQUEST['w_chave'],$_REQUEST['w_sq_lcmodalidade'],
  ...
  case 'CLLCSITUACAO':
    // Verifica se a Assinatura Eletrônica é válida
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
      dml_putCLDados::getInstanceOf($dbms,'SITUACAO',$_REQUEST['w_chave'],null,null,$_REQUEST['w_abertura'],
        null,null,null,null,null,null,null,null,null,$_REQUEST['w_sq_lcsituacao'],null,null,null,null,null,
        null,null,null,$_REQUEST['w_fim'],$_REQUEST['w_prioridade']);
      ScriptOpen('JavaScript');
    ...
    case 'CLLCCONC':
      ...
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
        ...
        if (f($RS,'sq_siw_tramite')!=$_REQUEST['w_tramite']) {
          ...
          dml_putCLDados::getInstanceOf($dbms,'CONCLUSAO',$_REQUEST['w_chave'],null,null,null,null,null,null,null,null,
              null,null,null,null,null,null,$_REQUEST['w_homologacao'],$_REQUEST['w_data_diario'],
              $_REQUEST['w_pagina_diario'],null,null,null,null,null,null);

          // Registra o vencedor de cada item
          for ($i=0; $i<=count($_POST['w_vencedor'])-1; $i=$i+1) {
            if (Nvl($_REQUEST['w_vencedor'][$i],'')>'') {
              dml_putCLDados::getInstanceOf($dbms,'VENCEDOR',$_REQUEST['w_vencedor'][$i],null,null,null,
