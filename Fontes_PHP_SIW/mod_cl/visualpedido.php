...
function VisualPedido($v_chave,$l_O,$w_usuario,$l_P1,$l_P4) {
  ...
  if ($l_O=='L' || $l_O=='V') {
    ...
    $w_html.=chr(13).'          <td bgColor="#f0f0f0" colspan=2><b>Preço</td>';
    $w_html.=chr(13).'        </tr>';
    $w_html.=chr(13).'        <tr align="center">';
    ...
    if (count($RS1)==0) {
      ...
      foreach ($RS1 as $row1) {
            ...
            if (count($RS2)>0) {
              ...
              foreach($RS2 as $row2) {
                if ($j==0) {
                  ...
                  $w_html .= chr(13).ExibePessoa($w_dir_volta,$w_cliente,f($row2,'sq_pessoa'),$TP,f($row2,'nome_resumido'));
                  $j = 1;
                } else {
        ...
        if (Nvl(f($row1,'caminho'),'')>'') {
          $w_html.=chr(13).'        <td>'.CRLF2BR(Nvl(f($row1,'observacao'),'---').'<br>'.LinkArquivo('HL',$w_cliente,f($row1,'sq_siw_arquivo'),'_blank','Clique para exibir o anexo em outra janela.','Anexo - '.f($row1,'tipo').' - '.round(f($row1,'tamanho')/1024,1).' KB',null)).'</td>';
        } else {
          $w_html.=chr(13).'        <td>'.CRLF2BR(Nvl(f($row1,'observacao'),'---')).'</td>';
        }         
        $w_html.=chr(13).'        <td nowrap>'.ExibePessoa('../',$w_cliente,f($row1,'sq_pessoa'),$TP,f($row1,'responsavel')).'</td>';
