...
// -------------------------------------------------------------------------
function ExibePdf($l_chave,$operacao,$l_usuario,$l_tipo) {
  extract($GLOBALS);  
  ...
  if (Nvl(f($RS,'sq_acordo'),'')>'') {
    if (substr(f($RS,'sg_acordo'),0,3)=='GCC') {
      ...
      $programa = f($RS,'cd_programa').' - '.f($RS,'nm_programa');
  } else {
	...
	//Header
	$programa = f($RS,'cd_programa').' - '.f($RS,'nm_programa');
	$projeto = f($RS,'titulo').' ('.nvl(f($RS,'codigo_interno'),f($RS,'sq_siw_solicitacao')).')';	
	// Identificação do projeto
	$titulo = $l_nome_menu['GERAL'];
	$localexec = f($RS,'nm_cidade').' ('.f($RS,'co_uf').')';
	$responsavel = f($RS,'nm_sol');
	$unresponsavel = f($RS,'nm_unidade_resp').' ('.f($RS,'sq_unidade').')';
	$propext = nvl(f($RS,'proponente'),'---');
	$orcamento = formatNumber(f($RS,'valor'));
	$palavrachave = nvl(f($RS,'palavra_chave'),'---');
	$dtinicio = FormataDataEdicao(f($RS,'inicio'));
	$dttermino = FormataDataEdicao(f($RS,'fim'));
	$prioridade = RetornaPrioridade(f($RS,'prioridade'));  	
	$header = array("programa" => $programa, "projeto" => $projeto, "localexec" => $localexec, "titulo" => $titulo, "localexec" => $localexec, "responsavel" => $responsavel, "unresponsavel" => $unresponsavel, "propext" => $propext, "orcamento" => $orcamento, "palavrachave" => $palavrachave, "dtinicio" => $dtinicio, "dttermino" => $dttermino, "prioridade" => $prioridade);
	//Fim da Header
