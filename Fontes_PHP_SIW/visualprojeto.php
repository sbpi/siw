...
function VisualProjeto($l_chave,$l_O,$l_usuario,$l_tipo=null) {
  ...
  if ($l_O=='L' || $l_O=='V' || $l_O=='T') {
    ...
    if ($w_tipo_visao==0) { 
      ...
      $l_html .= chr(13).'      <td>'.((nvl(f($RS,'sb_moeda'),'')!='') ? ' '.f($RS,'sb_moeda').' ' : ' ').formatNumber(f($RS,'valor')).' </td></tr>';
    ...
    if (f($RS,'concluida')=='S' && Nvl(f($RS,'data_conclusao'),'') > '') {
      ...
      if ($w_tipo_visao==0) { 
        ...
        $l_html .= chr(13).'      <td>'.((nvl(f($RS,'sb_moeda'),'')!='') ? ' '.f($RS,'sb_moeda').' ' : ' ').formatNumber(f($RS,'custo_real')).' </td></tr>';
      }
      if ($w_tipo_visao==0) { 
  ...
  if ($l_nome_menu['RUBRICA']!='' && $w_tipo_visao!=2 && ($l_O=='T' || nvl(f($RS_Cliente,'sg_segmento'),'-')=='OI')) {
    $sql = new db_getSolicRubrica; $RSQuery = $sql->getInstanceOf($dbms,$l_chave,null,'S',null,null,null,null,null,'VISUAL');
    ...
    if (count($RSQuery)>0 && $w_financeiro=='S' && $w_cliente!='10135' && nvl(f($RS_Cliente,'sg_segmento'),'-')!='OI') {
      ...
      $l_html .= chr(13).'            <td rowspan="2" bgColor="#f0f0f0"><b>Valor Inicial'.((nvl(f($RS,'sb_moeda'),'')!='') ? ' ('.f($RS,'sb_moeda').')' : '').'</td>';
      $l_html .= chr(13).'            <td colspan="3" bgcolor="'.$conTrBgColorLightBlue1.'" align="center"><b>Entrada'.((nvl(f($RS,'sb_moeda'),'')!='') ? ' ('.f($RS,'sb_moeda').')' : '').'</td>';
      $l_html .= chr(13).'            <td colspan="3" bgcolor="'.$conTrBgColorLightRed1.'" align="center"><b>Saída'.((nvl(f($RS,'sb_moeda'),'')!='') ? ' ('.f($RS,'sb_moeda').')' : '').'</td>';
      $l_html .= chr(13).'          </tr>';
      ...
      $l_html .= chr(13).'            <td rowspan="2" bgColor="#f0f0f0" align="center"><b>Orçamento'.((nvl(f($RS,'sb_moeda'),'')!='') ? ' ('.f($RS,'sb_moeda').')' : '').'</td>';
      $l_html .= chr(13).'          </tr>';
      $l_html .= chr(13).'          <tr align="center" >';
      if ($w_unid_med) {
      ...
      foreach ($RSQuery as $row) {
        ...
        $l_html .= chr(13).'      </tr>';
        if (f($row,'ultimo_nivel')=='S') $w_total_previsto += f($row,'total_previsto');
      } 
      $l_html .= chr(13).'          <tr>';
      $l_html .= chr(13).'            <td align="right" colspan="'.$cs.'" bgColor="#f0f0f0"><b>Total&nbsp;</td>';
      ...
      $t_html .= chr(13).'      <tr><td colspan="2"><b>Cronograma desembolso</b></td></tr>';
      $t_html .= chr(13).'      <tr><td align="center" colspan="2">';
      $t_html .= chr(13).'          <table width=100%  border="1" bordercolor="#00000">';
      $t_html .= chr(13).'          <tr align="center">';
      $t_html .= chr(13).'            <td rowspan=2 bgColor="#f0f0f0" width="1%" nowrap><b>Código</td>';
      $t_html .= chr(13).'            <td rowspan=2 bgColor="#f0f0f0"><b>Nome</td>';
      if ($w_unid_med) $t_html .= chr(13).'            <td colspan="2" bgColor="#f0f0f0"><b>Quantitativo</td>';
      $t_html .= chr(13).'            <td rowspan=2 bgColor="#f0f0f0"><b>Período</td>';
      $t_html .= chr(13).'            <td rowspan=2 bgColor="#f0f0f0"><b>Orçamento'.((nvl(f($RS,'sb_moeda'),'')!='') ? ' ('.f($RS,'sb_moeda').')' : '').'</td>';
      $t_html .= chr(13).'          </tr>';      
      $t_html .= chr(13).'          <tr align="center">';
      if ($w_unid_med) {
        $t_html .= chr(13).'            <td bgColor="#f0f0f0"><b>U.M.</td>';
        $t_html .= chr(13).'            <td bgColor="#f0f0f0"><b>Quantidade</td>';
      }
      $t_html .= chr(13).'          </tr>';      
      $w_cor=$conTrBgColor;
      foreach ($RSQuery as $row) {
        if (f($row,'ultimo_nivel')=='N') continue;
        $sql = new db_getCronograma; $RSQuery_Cronograma = $sql->getInstanceOf($dbms,f($row,'sq_projeto_rubrica'),null,null,null,null,null);
        ...
        $t_html .= chr(13).'      <tr valign="top">';
        if($l_tipo!='WORD') $t_html .= chr(13).'        <td '.$w_rowspan.'><A class="hl" HREF="javascript:this.status.value;" onClick="window.open(\''.montaURL_JS(null,$conRootSIW.'projeto.php?par=Cronograma&w_edita=N&O=L&w_chave='.f($row,'sq_projeto_rubrica').'&w_chave_pai='.$l_chave.'&w_tipo=&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.' - Extrato Rubrica'.'&SG=PJCRONOGRAMA'.MontaFiltro('GET')).'\',\'Ficha3\',\'toolbar=no,width=780,height=530,top=30,left=10,scrollbars=yes\');" title="Exibe as informações desta rubrica.">'.f($row,'codigo').'</A>&nbsp;';
        else       $t_html .= chr(13).'        <td '.$w_rowspan.'>'.f($row,'codigo').'&nbsp;';
        $t_html .= chr(13).'        <td '.$w_rowspan.'>'.f($row,'nome').' </td>';
        if ($w_unid_med) {
          $t_html .= chr(13).'        <td align="center" title="'.f($row,'nm_unidade').'" '.$w_rowspan.'>'.nvl(f($row,'sg_unidade'),'&nbsp').'</td>';
          $t_html .= chr(13).'        <td align="center" '.$w_rowspan.'>'.((nvl(f($row,'quantidade'),'')=='' || nvl(f($row,'sq_unidade_medida'),'')=='') ? '&nbsp;' : formatNumber(f($row,'quantidade'),0)).'</td>';
        }
        if ($w_cont<2) $w_cont = 0;
        if (count($RSQuery_Cronograma)>0) {
          $w_rubrica_previsto = 0;
          foreach ($RSQuery_Cronograma as $row1) {
            $w_cont++;
            $w_cor = ($w_cor==$conTrBgColor || $w_cor=='') ? $w_cor=$conTrAlternateBgColor : $w_cor=$conTrBgColor;
            $t_html .= chr(13).'        <td align="center" bgcolor="'.$w_cor.'">'.FormataDataEdicao(f($row1,'inicio'),5).' a '.FormataDataEdicao(f($row1,'fim'),5).'</td>';
            $t_html .= chr(13).'        <td align="right" bgcolor="'.$w_cor.'">'.formatNumber(f($row1,'valor_previsto')).'</td>';
            $t_html .= chr(13).'      </tr>';
            $w_rubrica_previsto += f($row1,'valor_previsto');
          } 
          $t_html .= chr(13).'      <tr>';
          $t_html .= chr(13).'          <td align="right"><b>Total da rubrica&nbsp;</td>';
          $t_html .= chr(13).'          <td align="right"><b>'.formatNumber($w_rubrica_previsto).' </b></td>';
          $t_html .= chr(13).'      </tr>';
        } else {
          $t_html .= chr(13).'        <td colspan=4>*** Cronograma desembolso da rubrica não informado';
        }
      } 
      $t_html .= chr(13).'      <tr>';
      $t_html .= chr(13).'          <td align="right" colspan="'.(($w_unid_med) ? '5' : '3').'" bgColor="#f0f0f0"><b>Total&nbsp;</td>';
      $t_html .= chr(13).'          <td align="right" bgColor="#f0f0f0"><b>'.formatNumber($w_total_previsto).' </b></td>';
      $t_html .= chr(13).'      </tr>';
      $t_html .= chr(13).'         </table></td></tr>';
    }
    if ($w_cont>1) $l_html .= $t_html;
  } 
  ...
  if ($l_nome_menu['APOIO']!='' && $w_tipo_visao!=2 && ($l_O=='T' || nvl(f($RS_Cliente,'sg_segmento'),'-')=='OI')) {
    ...
    if (count($RSQuery)>0) {
      $l_html.=chr(13).'        <tr><td colspan=2><br><font size="2"><b>'.$l_nome_menu['APOIO'].' ('.count($RSQuery).')<hr NOSHADE color=#000000 SIZE=1></b></font></td></tr>';
      $l_html .= chr(13).'      <tr><td align="center" colspan="2">';
      $l_html .= chr(13).'          <table width=100%  border="1" bordercolor="#00000">';
      $l_html .= chr(13).'          <tr align="center">';
      $l_html .= chr(13).'            <td colspan="2" bgColor="#f0f0f0"><b>'.colapsar($RSQuery[0]['sq_siw_solicitacao']-1).'Entidade</td>';
      $l_html .= chr(13).'            <td bgColor="#f0f0f0"><b>Tipo</td>';
      $l_html .= chr(13).'            <td bgColor="#f0f0f0"><b>Descrição</td>';
      $l_html .= chr(13).'            <td bgColor="#f0f0f0"><b>Valor'.((nvl(f($RS,'sb_moeda'),'')!='') ? ' ('.f($RS,'sb_moeda').')' : '').'</td>';
      $l_html .= chr(13).'            <td bgColor="#f0f0f0"><b>%</td>';
      $l_html .= chr(13).'          </tr>';
      $w_soma = 0;
      foreach ($RSQuery as $row) {
        $w_soma += f($row,'valor');
        $l_html .= chr(13).'      <tr valign="top" id="tr-' . (f($row,'sq_siw_solicitacao')-1) . '_' . f($row,'sq_solic_apoio') . '" class="arvore">';
        $l_html .= chr(13).'        <td colspan="2">' . montaArvore((f($row,'sq_siw_solicitacao')-1) . '_' . f($row,'sq_solic_apoio')) . ' ' . f($row,'entidade').'</td>';
        $l_html .= chr(13).'        <td>'.f($row,'nm_tipo_apoio').'</td>';
        $l_html .= chr(13).'        <td>'.nvl(crlf2br(f($row,'descricao')),'&nbsp;').'</td>';
        $l_html .= chr(13).'        <td align="right">'.formatNumber(f($row,'valor')).'</td>';
        $l_html .= chr(13).'        <td align="center">'.(($w_total_previsto>0) ? formatNumber((f($row,'valor')/$w_total_previsto*100),1) : '&nbsp;').'</td>';
        $l_html .= chr(13).'      </tr>';
        // Recupera o cronograma da fonte, se existir.
        $sql = new db_getSolicApoioList; $RSQuery_Cronograma = $sql->getInstanceOf($dbms,$l_chave,f($row,'sq_solic_apoio'),'CRONOGRAMA');
        if (count($RSQuery_Cronograma)>0) {
          $RSQuery = SortArray($RSQuery,'codigo','asc','inicio','asc');
          // Monta array com os valores de rowspan para cada rubrica
          unset($rowspan);
          foreach ($RSQuery_Cronograma as $row1) {
            if (isset($rowspan[f($row1,'sq_projeto_rubrica')])) {
              $rowspan[f($row1,'sq_projeto_rubrica')]++;
            } else {
              $rowspan[f($row1,'sq_projeto_rubrica')] = 1;
            }
          }
          $l_html .= chr(13).'      <tr id="tr-' . (f($row,'sq_siw_solicitacao')-1) . '_' . f($row,'sq_solic_apoio') . '-' . f($row,'sq_projeto_rubrica'). '" class="arvore" style="display:none"><td width="5%"><td align="center" colspan="4">';
          $l_html .= chr(13).'        <table width=100%  border="1" bordercolor="#00000">';
          $l_html .= chr(13).'          <tr align="center">';
          ...
          $l_html .= chr(13).'            <td colspan=2 bgColor="#f0f0f0"><b>Orçamento'.((nvl(f($RS,'sb_moeda'),'')!='') ? ' ('.f($RS,'sb_moeda').')' : '').'</td>';
          ...
          foreach ($RSQuery_Cronograma as $row1) {
            $l_html .= chr(13).'      <tr valign="top">';
            if ($w_atual <> f($row1,'sq_projeto_rubrica')) {
              if ($l_tipo!='WORD') $l_html .= chr(13).'        <td rowspan="'.$rowspan[f($row1,'sq_projeto_rubrica')].'"><A class="hl" HREF="javascript:this.status.value;" onClick="window.open(\''.montaURL_JS(null,$conRootSIW.'projeto.php?par=Cronograma&w_edita=N&O=L&w_chave='.f($row1,'sq_projeto_rubrica').'&w_chave_pai='.$l_chave.'&w_tipo=&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.' - Extrato Rubrica'.'&SG=PJCRONOGRAMA'.MontaFiltro('GET')).'\',\'Ficha3\',\'toolbar=no,width=780,height=530,top=30,left=10,scrollbars=yes\');" title="Exibe as informações desta rubrica.">'.f($row1,'codigo').'</A>&nbsp;';
              else                 $l_html .= chr(13).'        <td rowspan="'.$rowspan[f($row1,'sq_projeto_rubrica')].'">'.f($row1,'codigo').'&nbsp;';
              $l_html .= chr(13).'        <td rowspan="'.$rowspan[f($row1,'sq_projeto_rubrica')].'">'.f($row1,'nome').' </td>';
            
              $w_atual = f($row1,'sq_projeto_rubrica');
            }
            $w_cor = ($w_cor==$conTrBgColor || $w_cor=='') ? $w_cor=$conTrAlternateBgColor : $w_cor=$conTrBgColor;
            $l_html .= chr(13).'        <td align="center" bgcolor="'.$w_cor.'">'.FormataDataEdicao(f($row1,'inicio'),5).' a '.FormataDataEdicao(f($row1,'fim'),5).'</td>';
            $l_html .= chr(13).'        <td align="right" bgcolor="'.$w_cor.'">'.formatNumber(f($row1,'vl_fonte_prev')).'</td>';
            $l_html .= chr(13).'        <td align="right" bgcolor="'.$w_cor.'">'.formatNumber(f($row1,'vl_fonte_real')).'</td>';
            $l_html .= chr(13).'        <td align="center" bgcolor="'.$w_cor.'">'.formatNumber(((f($row1,'vl_fonte_prev')>0) ? f($row1,'vl_fonte_real')/f($row1,'vl_fonte_prev')*100 : '&nbsp;')).'</td>';
          }
          $l_html .= chr(13).'        </table>';
        }
      }
      if (count($RSQuery)) {
        $l_html .= chr(13).'      <tr bgColor="#f0f0f0" valign="top">';
        $l_html .= chr(13).'        <td colspan="4" align="right"><b>Total</b></td>';
        $l_html .= chr(13).'        <td align="right"><b>'.formatNumber($w_soma).'</b></td>';
        $l_html .= chr(13).'        <td align="center"><b>'.(($w_total_previsto>0) ? formatNumber(($w_soma/$w_total_previsto*100),1) : '&nbsp;').'</b></td>';
        $l_html .= chr(13).'      </tr>';
      }
      $l_html .= chr(13).'         </table></td></tr>';
    }
  ...
  if($l_nome_menu['ETAPA']!='') {
    ...
    if (count($RSQuery)>0) {
      ...
      $l_html .= chr(13).'            <td rowspan=2 bgColor="#f0f0f0" align="center"><b>Orc.'.((nvl(f($RS,'sb_moeda'),'')!='') ? ' ('.f($RS,'sb_moeda').')' : '').'</b></td>';
  ...
  if ($l_O!='V') {
    // Prestacao de contas
    $sql = new db_getContasCronograma; $RSQuery = $sql->getInstanceOf($dbms,null,$l_chave,null,null,null,null,null,null);
    $RSQuery = SortArray($RSQuery,'tipo','desc','fim','desc');
    if (count($RSQuery)>0) {
      $l_html.=chr(13).'      <tr><td colspan="2"><br><font size="2"><b>CRONOGRAMA DE PRESTAÇÃO DE CONTAS ('.count($RSQuery).')<hr NOSHADE color=#000000 SIZE=1></b></font></td></tr>';
      $l_html .= chr(13).'      <tr><td align="center" colspan="2">';
      $l_html.=chr(13).'          <table width=100%  border="1" bordercolor="#00000">';
      $w_cor=$conTrBgColor;
      foreach ($RSQuery as $row) {
        $l_html .= chr(13).'          <tr><td bgColor="#f0f0f0" width="10%" nowrap align="center"><b>Início</b></td>';
        $l_html .= chr(13).'              <td bgColor="#f0f0f0" width="10%" align="center"><b>Fim</b></td>';
        $l_html .= chr(13).'              <td bgColor="#f0f0f0" align="center"><b>Tipo</b></td>';
        $l_html .= chr(13).'              <td bgColor="#f0f0f0" align="center"><b>Prestação contas</b></td>';
        $l_html .= chr(13).'              <td bgColor="#f0f0f0" align="center"><b>Limite</b></td>';
        $l_html .= chr(13).'              <td bgColor="#f0f0f0" align="center"><b>Última atualização</b></td>';
        $l_html .= chr(13).'          </tr>';
        $l_html .= chr(13).'      <tr>';
        ...
        $l_html .= chr(13).'      </tr>';
        $sql = new db_getPrestacaoContas; $RSQuery_Prest = $sql->getInstanceOf($dbms,$w_cliente,f($row,'sq_prestacao_contas'),f($row,'chave'),null,null,null,'CRONOGRAMA');
        if(count($RSQuery_Prest)>0) {
          $l_html .= chr(13).'      <tr><td colspan=6>';
          $l_html .= chr(13).'<table border="0" cellpadding="0" cellspacing="0" width="100%">';
          $l_html .= chr(13).'      <tr valign="top"><td><b>Item</b><td><b>Situação</b><td><b>Observação</b>';
          $l_html .= chr(13).'      <tr><td colspan="3"><hr NOSHADE color=#000000 size=1></td></tr>';
          foreach($RSQuery_Prest as $row_prest) {
            if (f($row_prest,'Filho')>0) {
              $l_html .= chr(13).'      <tr valign="top"><td colspan=3><b><img src="images/Folder/FolderClose.gif" border=0 align="center"> '.f($row_prest,'nome').'</b>';
              $sql = new db_getPrestacaoContas; $RSQuery1 = $sql->getInstanceOf($dbms,$w_cliente,null,null,null,null,null,f($row_prest,'chave'));
              foreach($RSQuery1 as $row1) {
                if (f($row1,'Filho')>0) {
                  $l_html .= chr(13).'      <tr valign="top"><td colspan=3><b>&nbsp;&nbsp;<img src="images/Folder/FolderClose.gif" border=0 align="center"> '.f($row1,'nome').'</b>';
                  $sql = new db_getPrestacaoContas; $RSQuery2 = $sql->getInstanceOf($dbms,$w_cliente,null,null,null,null,null,f($row1,'chave'));
                  foreach($RSQuery2 as $row2) {
                    if (f($row2,'Filho')>0) {
                      $l_html .= chr(13).'      <tr valign="top"><td colspan=3><b>&nbsp;&nbsp;&nbsp;&nbsp;<img src="images/Folder/FolderClose.gif" border=0 align="center"> '.f($row2,'nome').'</b>';
                      $sql = new db_getPrestacaoContas; $RSQuery3 = $sql->getInstanceOf($dbms,$w_cliente,null,null,null,null,null,f($row2,'chave'));
                      foreach($RSQuery3 as $row3) {
                        $l_html .= chr(13).'      <tr valign="top"><td><b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src="images/Folder/SheetLittle.gif" border=0 align="center"> '.f($row3,'nome').'</b>';                  
                        $sql = new db_getContasRegistro; $RSQuery_Cron = $sql->getInstanceOf($dbms,null,f($row3,'chave'),f($row,'chave'),null);
                        foreach($RSQuery_Cron as $row_cron){$RSQuery_Cron=$row_cron; break;}
                        $l_html .= chr(13).'      <td nowrap>'.nvl(f($RSQuery_Cron,'nm_pendencia'),'Não informado');
                        $l_html .= chr(13).'      <td>'.nvl(f($RSQuery_Cron,'observacao'),'Não informado');
                        $l_html .= chr(13).'    <BR>';
                      } 
                    } else {
                      $l_html .= chr(13).'      <tr valign="top"><td><b>&nbsp;&nbsp;&nbsp;&nbsp;<img src="images/Folder/SheetLittle.gif" border=0 align="center"> '.f($row2,'nome').'</b>';
                      $sql = new db_getContasRegistro; $RSQuery_Cron = $sql->getInstanceOf($dbms,null,f($row2,'chave'),f($row,'chave'),null);
                      foreach($RSQuery_Cron as $row_cron){$RSQuery_Cron=$row_cron; break;}
                      $l_html .= chr(13).'      <td nowrap>'.nvl(f($RSQuery_Cron,'nm_pendencia'),'Não informado');
                      $l_html .= chr(13).'      <td>'.nvl(f($RSQuery_Cron,'observacao'),'Não informado');
                      $l_html .= chr(13).'    <BR>';
                  ...
                  $l_html .= chr(13).'      <tr valign="top"><td><b>&nbsp;&nbsp;<img src="images/Folder/SheetLittle.gif" border=0 align="center"> '.f($row1,'nome').'</b>';
                  $sql = new db_getContasRegistro; $RSQuery_Cron = $sql->getInstanceOf($dbms,null,f($row1,'chave'),f($row,'chave'),null);
                  foreach($RSQuery_Cron as $row_cron){$RSQuery_Cron=$row_cron; break;}
                  $l_html .= chr(13).'      <td nowrap>'.nvl(f($RSQuery_Cron,'nm_pendencia'),'Não informado');
                  $l_html .= chr(13).'      <td>'.nvl(f($RSQuery_Cron,'observacao'),'Não informado');
                  $l_html .= chr(13).'    <BR>';
              ...
              $l_html .= chr(13).'      <tr valign="top"><td><b><img src="images/Folder/SheetLittle.gif" border=0 align="center"> '.f($row,'nome').'</b>';
              $l_html .= chr(13).'    <BR>';
              $sql = new db_getContasRegistro; $RSQuery_Cron = $sql->getInstanceOf($dbms,null,f($row_prest,'chave'),f($row,'chave'),null);
              foreach($RSQuery_Cron as $row_cron){$RSQuery_Cron=$row_cron; break;}
              $l_html .= chr(13).'      <td nowrap>'.nvl(f($RSQuery_Cron,'nm_pendencia'),'Não informado');
              $l_html .= chr(13).'      <td>'.nvl(f($RSQuery_Cron,'observacao'),'Não informado');
              $l_html .= chr(13).'    <BR>';
