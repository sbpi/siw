...
function Inicial() {
  ...
  $p_co_ini = $_REQUEST['p_co_ini'];
  $p_co_fim = $_REQUEST['p_co_fim'];
  ...
  if ($w_tipo_rel == 'WORD') {
    ...
    if ($O == 'P') {
      ...
      Validate('p_co_ini', 'Vencimento inicial', 'DATA', '', '10', '10', '', '0123456789/');
      Validate('p_co_fim', 'Vencimento final', 'DATA', '', '10', '10', '', '0123456789/');
      CompData('p_co_ini', 'Vencimento inicial', '<=', 'p_co_fim', 'Vencimento final');
  ...
  if ($O == 'L') {
    ...
    $w_filtro = '';
    if ($p_dt_ini > '')
      $w_filtro = $w_filtro . '<tr valign="top"><td align="right">Vencimento de <td><b>' . $p_dt_ini . '</b> até <b>' . $p_dt_fim . '</b>';
    if ($p_pg_ini > '')
    ...
    if (Nvl($p_ordena, '') > '') {
      ...
      switch (strtoupper($lista[0])) {
        case 'VENCIMENTO': $w_filtro = $w_filtro . 'Vencimento';
          break;
        case 'NM_PESSOA_RESUMIDO':
          ...
          break;
        case 'NM_TRAMITE': $w_filtro = $w_filtro . 'Situação';
          break;
      }
      $w_filtro = $w_filtro . '</b>';
    }
    ...
    if ($w_filtro > '')
      ShowHTML('<table border=0><tr valign="top"><td><b>Filtro:</b><td nowrap><ul>' . $w_filtro . '</ul></tr></table>');
    ...
    if (substr($SG, 2, 1) == 'R') {
      ...
      ShowHTML('       <td><b>' . linkOrdena('Fornecedor', 'nm_pessoa_resumido') . '</td>');
    }
    ShowHTML('          <td><b>' . linkOrdena('Histórico', 'descricao') . '</td>');
    ShowHTML('          <td><b>' . linkOrdena('Prazo', 'prazo') . '</td>');
    ...
    if (count($RS) <= 0) {
      ...
      foreach ($RS as $row) {
        ...
        $w_linha += 1;
        switch (strtoupper($lista[0])) {
          case 'VENCIMENTO': $w_atual = f($row, 'vencimento');
            break;
          case 'NM_PESSOA_RESUMIDO': $w_atual = f($row, 'sq_pessoa');
            break;
          case 'NM_TRAMITE': $w_atual = f($row, 'nm_tramite');
            break;
        }
      }
      ShowHTML('      <tr bgcolor="' . $conTrAlternateBgColor . '" valign="top">');
      switch (strtoupper($lista[0])) {
        ...
        case 'NM_TRAMITE':
          ...
          break;
        default:
          ShowHTML('        <td colspan=5 align="right" height=18><b>Total</b></td>');
          break;
      }
      ShowHTML('        <td align="right" nowrap><b>' . number_format($w_valor, 2, ',', '.') . '</b></td>');
      ShowHTML('      </tr>');
    ...
    ShowHTML('      <tr><td valign="top"><br><font size="1"><b><u>P</u>agamento entre:</b><br><input ' . $w_Disabled . ' accesskey="V" type="text" name="p_pg_ini" class="sti" SIZE="10" MAXLENGTH="10" VALUE="' . Nvl($p_pg_ini, FormataDataEdicao(First_Day(time()))) . '" onKeyDown="FormataData(this,event);" onKeyUp="SaltaCampo(this.form.name,this,10,event);">' . ExibeCalendario('Form', 'p_pg_ini') . ' e <input ' . $w_Disabled . ' type="text" name="p_pg_fim" class="sti" SIZE="10" MAXLENGTH="10" VALUE="' . Nvl($p_pg_fim, FormataDataEdicao(Last_Day(time()))) . '" onKeyDown="FormataData(this,event);" onKeyUp="SaltaCampo(this.form.name,this,10,event);">' . ExibeCalendario('Form', 'p_pg_fim') . '</td>');
    ShowHTML('      <tr><td valign="top"><br><font size="1"><b><u>L</u>ançamento financeiro concluído entre:</b><br><input ' . $w_Disabled . ' accesskey="V" type="text" name="p_co_ini" class="sti" SIZE="10" MAXLENGTH="10" VALUE="' . Nvl($p_co_ini, FormataDataEdicao(First_Day(time()))) . '" onKeyDown="FormataData(this,event);" onKeyUp="SaltaCampo(this.form.name,this,10,event);">' . ExibeCalendario('Form', 'p_co_ini') . ' e <input ' . $w_Disabled . ' type="text" name="p_co_fim" class="sti" SIZE="10" MAXLENGTH="10" VALUE="' . Nvl($p_co_fim, FormataDataEdicao(Last_Day(time()))) . '" onKeyDown="FormataData(this,event);" onKeyUp="SaltaCampo(this.form.name,this,10,event);">' . ExibeCalendario('Form', 'p_co_fim') . '</td>');
