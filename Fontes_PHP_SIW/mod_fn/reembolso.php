...
function Inicial() {
  ...
  if ($O=='L') {
    ...
    $colspan++; ShowHTML('          <td><b>'.(($w_tipo=='WORD') ? 'Código' : LinkOrdena('Código','ord_codigo_interno')).'</td>');
...
function SolicMail($p_solic,$p_tipo) {
  ...
  if(f($RS,'envia_mail_tramite')=='S' && (f($RS_Menu,'envia_email')=='S') && (f($RSM,'envia_mail')=='S')) {
    ...
    if (count($RSM1) > 0) {
      ...
      if (Nvl(f($RSM,'sq_tipo_pessoa'),0)==1 || Nvl(f($RSM,'sq_tipo_pessoa'),0)==3) {
        ...
        if (f($RS, 'sq_siw_tramite') != $_REQUEST['w_tramite']) {
          ...
          if (Nvl($_REQUEST['w_valor'],0)>0) {
            ...
            $SQL = new dml_putFinanceiroGeral; $SQL->getInstanceOf($dbms,'A',$w_cliente,$_REQUEST['w_chave'],f($RS,'sq_menu'),
                f($RS,'sq_unidade'),f($RS,'solicitante'),f($RS,'cadastrador'),f($RS,'sq_cc'),f($RS,'descricao'),FormataDataEdicao(f($RS,'fim')),
      ...
      if (f($RS, 'sq_siw_tramite') != $_REQUEST['w_tramite']) {
        ...
        if (f($RS, 'sq_siw_tramite') != $_REQUEST['w_tramite']) {
          ...
          if (Nvl($_REQUEST['w_valor'],0)>0) {
            ...
            $w_chave_doc = f($RSC,'sq_lancamento_doc');
        ...
        if (Nvl($_POST['w_chave'][$i], '') > '') {
          ...
          if ($_POST['w_envio']=='N') {
            ...
            if ($w_sg_tramite=='EE') {
              ...
              $w_erro = ValidaReembolso($w_cliente,$w_chave,$_POST['p_agrega'],null,null,null,$w_tramite);
              if (substr(Nvl($w_erro,'nulo'),0,1)=='0') {
      ...
      if (f($RS,'sq_siw_tramite')!=$_REQUEST['w_tramite']) {
        ...
        if (UPLOAD_ERR_OK==0) {
          ...
          $SQL = new dml_putFinanceiroConc; $SQL->getInstanceOf($dbms,$w_menu,$_REQUEST['w_chave'],$w_usuario,$_REQUEST['w_tramite'],
                  ...
                  $_REQUEST['w_sq_tipo_lancamento'],$_REQUEST['w_sq_projeto_rubrica'],
