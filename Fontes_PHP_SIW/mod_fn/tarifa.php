...
include_once('visualtarifa.php');
include_once('validatarifa.php');
// =========================================================================
//  /tarifa.php
// ------------------------------------------------------------------------
...
function Visual() {
  ...
  ShowHTML(VisualTarifa($w_chave,'L',$w_usuario,$P1,$w_embed));
  if ($w_embed!='WORD') ShowHTML('<center><B><font size=1>Clique <span class="lk"><a class="hl" href="javascript:history.back(1);">aqui</a> para voltar à tela anterior</span></font></b></center>');
  ScriptOpen('JavaScript');
...
function Excluir() {
  ...
  ShowHTML('<table border="0" cellpadding="0" cellspacing="0" width="100%">');
  // Chama a rotina de visualização dos dados do lançamento, na opção 'Listagem'
  ShowHTML(VisualTarifa($w_chave,'V',$w_usuario,$P1,$P4));
  ShowHTML('<HR>');
...
function Encaminhamento() {
  ...
  if ($O=='V') $w_erro = ValidaTarifa($w_cliente,$w_chave,$SG,null,null,null,$w_tramite);
  Cabecalho();
  head();
  Estrutura_CSS($w_cliente);
  ShowHTML('<meta http-equiv="Refresh" content="'.$conRefreshSec.'; URL=../'.MontaURL('MESA').'">');
...
function Concluir() {
  ...
  // Se for envio, executa verificações nos dados da solicitação
  $w_erro = ValidaTarifa($w_cliente,$w_chave,$SG,null,null,null,$w_tramite);
  Cabecalho();
  head();
  Estrutura_CSS($w_cliente);
  ShowHTML('<meta http-equiv="Refresh" content="'.$conRefreshSec.'; URL=../'.MontaURL('MESA').'">');
...
function Grava() {
  ...
  if ($SG=='FNDTARIFA') {
    ...
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {
      ...
      if ($O=='A') {
        $w_html = VisualTarifa($_REQUEST['w_chave'],'L',$w_usuario,1,'1');
      ...
      if ((false!==(strpos(upper($_SERVER['HTTP_CONTENT_TYPE']),'MULTIPART/FORM-DATA'))) || (false!==(strpos(upper($_SERVER['CONTENT_TYPE']),'MULTIPART/FORM-DATA')))) {
        ...
        if (UPLOAD_ERR_OK==0) {
          ...
          if($_REQUEST['w_tramite']!=$_REQUEST['w_novo_tramite']) {
            ...
            $w_sg_tramite = f($RS,'sigla');
            if($w_sg_tramite=='CI') {
              $w_html = VisualTarifa($_REQUEST['w_chave'],'L',$w_usuario,$P1,'1');
              CriaBaseLine($_REQUEST['w_chave'],$w_html,f($RS_Menu,'nome'),$_REQUEST['w_tramite']);
            }
          }
        } else {
    ...
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {
      ...
      if($_REQUEST['w_tramite']!=$_REQUEST['w_novo_tramite']) {
        ...
        $w_sg_tramite = f($RS,'sigla');
        if($w_sg_tramite=='CI') {
          ShowHTML(VisualTarifa($_REQUEST['w_chave'],'V',$w_usuario,$P1,$P4));
          CriaBaseLine($_REQUEST['w_chave'],$w_html,f($RS_Menu,'nome'),$_REQUEST['w_tramite']);
        }
      }  
