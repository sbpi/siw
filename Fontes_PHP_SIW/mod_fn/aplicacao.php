...
function Inicial() {
  ...
  if ($O=='L') {
    ...
    if ($w_tipo!='WORD') {    
      ...
      ShowHTML('          <td><b>'.LinkOrdena('Conta','conta_debito').'</td>');
...
function FichaRubrica() {
  ...
  if (count($RS)<=0) {
    ...
    foreach($RS as $row) {
      ...
      ShowHTML('      <td><A class="hl" HREF="mod_fn/aplicacao.php?par=Visual&O=L&w_chave='.f($row,'sq_lancamento').'&w_tipo=Volta&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$l_P4.'&TP='.$TP.'&SG='.f($row,'sg_lancamento_menu').MontaFiltro('GET').'" title="Exibe as informações do lançamento"> '.f($row,'cd_lancamento').' - '.f($row,'nm_lancamento').'</a>');
      ShowHTML('    </tr>');
      if(f($row,'tipo_rubrica')==5)
...
function Grava() {
  ...
  if (strpos($SG,'FNAAPLICA')!==false) {
    ...
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],upper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
      $SQL = new dml_putFinanceiroGeral; $SQL->getInstanceOf($dbms1,$O,$w_cliente,$_REQUEST['w_chave'],$_REQUEST['w_menu'],
