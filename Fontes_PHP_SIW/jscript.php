...
function Validate($VariableName,$DisplayName,$DataType,$ValueRequired,$MinimumLength,$MaximumLength,$AllowLetters,$AllowDigits) {
  ...
  if ($ValueRequired>"") {
    ...
    if (upper($DataType)=="RADIO") { 
      ...
      print "  if (".$Form.$VariableName."[0]==undefined) {"."\r\n"; 
  ...
  if ($AllowLetters>"" || $AllowDigits>"") {
    ...
    print "  if (!allValid)"."\r\n";
    print "  {"."\r\n";
    if     ($AllowLetters>"" && $AllowDigits>"")  { print "    alert('VocÍ digitou caracteres inv·lidos no campo ".$DisplayName." (PosiÁ„o '+(i+1)+': ['+ch.charCodeAt(0)+']).\\n\\nRetire ASPAS, TRAVESS’ES e outros caracteres especiais.\\nIsso tambÈm pode ocorrer se vocÍ copiou o conte˙do de outro arquivo, por exemplo o MS-Word; nesse caso, tente antes copiar para o bloco de notas!');"."\r\n"; }
