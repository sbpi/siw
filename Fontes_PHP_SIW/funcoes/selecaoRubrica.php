...
function selecaoRubrica($label,$accesskey,$hint,$chave,$chaveAux,$sq_rubrica_destino,$campo,$restricao,$atributo,$colspan=1) {
  extract($GLOBALS);
  if ($restricao=='RUBRICAS')                  { $sql = new db_getSolicRubrica; $RS = $sql->getInstanceOf($dbms,$chaveAux,null,'S',$sq_rubrica_destino,null,'N',null,null,'FOLHA'); }
  elseif ($restricao=='ARVORE')                { $sql = new db_getSolicRubrica; $RS = $sql->getInstanceOf($dbms,$chaveAux,null,'S',$sq_rubrica_destino,null,'N',null,null,'SUBORDINACAO'); }
  ...
  foreach ($RS as $row) {
    ShowHTML('          <option value="'.f($row,'sq_projeto_rubrica').'"'.(($restricao=='RUBRICAS' && f($row,'ultimo_nivel')=='N') ? ' DISABLED': '').((nvl(f($row,'sq_projeto_rubrica'),0)==nvl($chave,0)) ? ' SELECTED' : '').'>'.f($row,'codigo').' - '.f($row,'nome'));
  } 
  ShowHTML('          </select>');
} 
?>