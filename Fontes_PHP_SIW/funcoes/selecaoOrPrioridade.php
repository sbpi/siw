...
function SelecaoOrPrioridade($label,$accesskey,$hint,$chave,$chaveAux,$campo,$restricao,$atributo,$colspan=1) {
  ...
  if (!isset($hint)) {
    ShowHTML('          <td colspan="'.$colspan.'" valign=\'top\'><font size=\'1\'><b>'.$label.'</b><br><SELECT ACCESSKEY=\''.$accesskey.'\' CLASS=\'STS\' NAME=\''.$campo.'\' '.$w_Disabled.' '.$atributo.'>');
  } else {
    ShowHTML('          <td colspan="'.$colspan.'" valign=\'top\' title=\''.$hint.'\'><font size=\'1\'><b>'.$label.'</b><br><SELECT ACCESSKEY=\''.$accesskey.'\' CLASS=\'STS\' NAME=\''.$campo.'\' '.$w_Disabled.' '.$atributo.'>');
  } 
