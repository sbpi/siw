...
function selecaoTipoEndereco($label,$accesskey,$hint,$chave,$chaveAux,$campo,$restricao,$atributo) {
  ...
  $RS = db_getAdressTypeList::getInstanceOf($dbms);
  ...
  foreach($RS as $row) {
    if (nvl(f($row,'sq_tipo_endereco'),0)==nvl($chave,0)) {
      ShowHTML('          <option value="'.f($row,'sq_tipo_endereco').'" SELECTED>'.f($row,'nome'));
    } else {
      ShowHTML('          <option value="'.f($row,'sq_tipo_endereco').'">'.f($row,'nome'));
    }
  }
  ShowHTML('          </select>');
  return $function_ret;
