...
function montaRadioMes($label,$Chave,$Campo) {
  ...
  for ($l_i=1; $l_i<=6; $l_i=$l_i+1) {
    ...
    ShowHTML('              <tr><td valing="top"><font size="1"><input '.$w_Disabled.' type="radio" name="'.$campo.'" value="'.substr(100+$l_i,1,2).'" '.$l_texto.'> '.$l_mes[$l_i]);
    if ($chave==$l_i+6) $l_texto='checked';
    else                $l_texto='';
    ShowHTML('                  <td valing="top"><font size="1"><input '.$w_Disabled.' type="radio" name="'.$campo.'" value="'.substr(100+$l_i+6,1,2).'" '.$l_texto.'> '.$l_mes[$l_i+6]);
  } 
  ShowHTML('</table>');
} 
?>