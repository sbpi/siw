...
function SelecaoTelefoneTT($label,$accesskey,$hint,$chave,$chaveAux,$campo,$O,$restricao) {
  ...
  $sql = new db_getFoneList; $RS = $sql->getInstanceOf($dbms, $w_cliente, null, $restricao, null);
  ...
  if ($O=='A') {
    ...
    $sql = new db_getFoneList; $RS = $sql->getInstanceOf($dbms, $w_cliente, null, 'TRONCO', null);
    foreach ($RS as $row) {
      if (nvl(f($row,'sq_pessoa_telefone'),0)!=nvl($chave,0)) {
