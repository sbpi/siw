...
function selecaoCalculo($label,$accesskey,$hint,$chave,$chaveAux,$cliente,$campo,$restricao,$atributo) {
  ...
  if ($chaveAux=='Nominal')  ShowHTML(' <option value=0 SELECTED>Nominal');  else ShowHTML(' <option value=0>Nominal');
  if ($chaveAux=='Retenção') ShowHTML(' <option value=1 SELECTED>Retencao'); else ShowHTML(' <option value=1>Retencao');
ShowHTML('          </select>');
}  
?>