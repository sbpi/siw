...
// -------------------------------------------------------------------------
function selecaoPessoaTT($label,$accesskey,$hint,$chave,$chaveAux,$campo,$O,$restricao) {
  extract($GLOBALS);
  ...
  if ($O=='A') {
    ...
    $RS = db_getPersonList::getInstanceOf($dbms, $w_cliente, $ChaveAux, 'TTUSUCENTRAL', null, null, null, null);
    foreach ($RS as $row) {
      if (nvl(f($row,'sq_pessoa'),0)!=nvl($chave,0)) {
