...
function exibeLog($l_chave,$l_O,$l_usuario,$l_tramite_ativo,$l_formato) {
  ...
  // Anotações
  if ($l_formato=='EXIBEANOTACAO') {
      $l_html.=chr(13).'      <tr><td colspan="2"><br><span id="colxanot"></span><font size="2"><b>'.$TP.'</b></font><hr NOSHADE color=#000000 SIZE=1 /></td></tr>';
  }
  ...
  if (count($RS_Log)==0) {
    if ($l_formato=='EXIBEANOTACAO') {
      $l_html.=chr(13).'      <tr><td colspan="2" align="center"><b><font color="#FF0000">Nenhuma anotação registrada!</font></b></td></tr>';
    }
  } else {
    if ($l_formato!='EXIBEANOTACAO') {
    ...
    foreach($RS_Log as $row) {
      ...
      if (Nvl(f($row,'caminho'),'')>'' && ($l_formato=='HTML' || $l_formato=='EXIBEANOTACAO')) {
