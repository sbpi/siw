...
function selecaoUnidade_IS($label,$accesskey,$hint,$chave,$chaveAux,$campo,$atributo,$tipo) {
  ...
  if (count($RS)>100) {
    ...
    ShowHTML('              <a class="SS" href="#" onClick="window.open(\''.$w_dir_volta.'EO.php?par=BuscaUnidade&TP='.$TP.'&w_cliente='.$w_cliente.'&ChaveAux='.$ChaveAux.'&restricao='.$restricao.'&campo='.$campo.'\',\'Unidade\',\'top=70 left=100 width=600 height=400 toolbar=yes status=yes resizable=yes scrollbars=yes\'); return false;" title="Clique aqui para selecionar a unidade."><img src=images/Folder/Explorer.gif border=0></a>');
