...
function Inicial() {
  ...
  if ($O == 'L') {
    ...
    if ((strpos(upper($R),'GR_'))===false && $P1!=6 && $w_tipo!='WORD') {
      if ($w_copia > '') {
        // Se for cópia
        if (strpos(str_replace('p_ordena','w_ordena',MontaFiltro('GET')),'p_')) ShowHTML('                         <a accesskey="F" class="SS" href="'.$w_dir.$w_pagina.$par.'&R='.$w_pagina.$par.'&O=C&P1='.$P1.'&P2='.$P2.'&P3=1&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'"><u><font color="#BC5100">F</u>iltrar (Ativo)</font></a>');
...
function Informar() {
  ...
  $sql = new db_getSolicInter; $RS1 = $sql->getInstanceOf($dbms,$w_chave,$w_usuario,'LISTA');
  if (count($RS1)>0) {
    $w_exibe3 = true;;
  }
  ...
  if (strpos('IAEV',$O)!==false) {
    ...
    ShowHTML('      <tr><td align="center">');
    ShowHTML('            <input class="STB" type="submit" name="Botao" value="Gravar">');
    $sql = new db_getLinkData; $RS1 = $sql->getInstanceOf($dbms,$_SESSION['P_CLIENTE'],'PJCAD');
...
function EtapaLinha($l_chave,$l_chave_aux,$l_titulo,$l_resp,$l_setor,$l_inicio,$l_fim,$l_inicio_real,$l_fim_real,$l_perc,$l_ativ,$l_destaque,$l_oper,$l_tipo,$l_sq_resp,$l_sq_setor,$l_vincula_contrato,$l_contr, $l_valor=null,$l_nivel=0,$l_restricao='N',$l_peso='1',$l_arquivo=0, $l_desafio='', $l_pacote='N') {
  ...
  if (nvl($l_chave_aux,'')!='') {
    ...
    $sql = new db_getSolicAreas; $RSPacote = $sql->getInstanceOf($dbms,$l_chave,$l_chave_aux,'PACOTE');
...
function Grava() {
  ...
  if($SG=='PJGERAL' || $SG=='PJBGERAL') {
    ...
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {   
      // Se for operação de exclusão, verifica se é necessário excluir os arquivos físicos
      //$SQL = new dml_putProjetoDescritivo; $SQL->getInstanceOf($dbms,$_REQUEST['w_chave'],$_REQUEST['w_instancia_articulacao'], $_REQUEST['w_composicao_instancia'], $_REQUEST['w_estudos'], $_REQUEST['w_objetivo_superior'],$_REQUEST['w_descricao'],
    ...
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {   
      // Se for operação de exclusão, verifica se é necessário excluir os arquivos físicos
      $SQL = new dml_putProjetoAnalise; $SQL->getInstanceOf($dbms,$_REQUEST['w_chave'],$_REQUEST['w_analise1'], $_REQUEST['w_analise2'], $_REQUEST['w_analise3'], $_REQUEST['w_analise4']);
      // Aqui deve ser usada a variável de sessão para evitar erro na recuperação do link
      $sql = new db_getLinkData; $RS1 = $sql->getInstanceOf($dbms,$_SESSION['P_CLIENTE'],'PJCAD');
      ScriptOpen('JavaScript');
