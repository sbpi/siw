...
function Plano() {
  ...
  if ($O!='L') {
    ...
    if ($O!='P' && $O!='H' && $O!='M') {
      if ($O=='I' || $O=='A') {
        ...
        Validate('w_missao','Objetivo','1','1','5','2000','1','1');
  ...
  if ($O=='L') {
    ...
    ShowHTML('<INPUT type="hidden" name="w_valores" value="Não se aplica.">');
    ShowHTML('<INPUT type="hidden" name="w_visao_presente" value="Não se aplica.">');
    ShowHTML('<INPUT type="hidden" name="w_visao_futuro" value="Não se aplica.">');
    ...
    ShowHTML('      <tr><td><b><U>O</U>bjetivo:<br><TEXTAREA ACCESSKEY="O" class="sti" name="w_missao" rows=5 cols=80 title="Informe o objetivo deste plano estratégico." '.$w_Disabled.'>'.$w_missao.'</textarea></td>');
...
*/
  ...
  if (count($RS)>0) {
    ...
    ShowHTML('         [<A class="HL" HREF="'.$conRootSIW.'mod_pe/graficos.php?par=hier&w_chave='.$w_sq_plano.'&w_pitce=1" TARGET="PLANO" TITLE="Exibe diagrama hierárquico da estrutura do plano.">DIAGRAMA HIERÁRQUICO</A>]');
    ShowHTML('         [<A class="HL" HREF="'.$conRootSIW.'mod_pe/graficos.php?par=gantt&w_chave='.$w_sq_plano.'&w_pitce=1" TARGET="PLANO" TITLE="Exibe gráfico de Gantt da estrutura do plano.">GRÁFICO DE GANTT</A>]');
      ...
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {
        if (UPLOAD_ERR_OK===0) {
          ...
          $SQL = new dml_putSolicArquivo; $SQL->getInstanceOf($dbms,$O,
            $w_cliente,$_REQUEST['w_chave'],$_REQUEST['w_chave_aux'],$_REQUEST['w_nome'],$_REQUEST['w_descricao'],$w_file,$w_tamanho,$w_tipo,$w_nome, $w_chave_arquivo);
        } else {
          ScriptOpen('JavaScript');
