...
$p_agrega        = strtoupper(nvl($_REQUEST['p_agrega'],'GREVRESP'));
...
function Gerencial() {
  ...
  if ($O=='L' || $O=='V' || $p_tipo=='WORD' || $p_tipo=='PDF') {
    ...
    switch ($p_agrega) {
      ...
      case 'GREVCC':      $w_filtro.='<tr valign="top"><td align="right">Agregação <td>[<b>Classificação</b>]';     break;
    ...
    $RS1 = db_getSolicEV::getInstanceOf($dbms, $w_cliente,$P2,$w_usuario,$p_agrega,5,
  ...
  if ($O=='L' || $w_embed == 'WORD') {
    ...
    if (count($RS1)<=0) {
      ...
      if ($O=='L' && $w_embed != 'WORD') {
        ...
        switch ($p_agrega) {
          ...
          case 'GREVCC':        ShowHTML('      document.Form.p_sqcc.value=filtro;');           break;
        ...
        switch ($p_agrega) {
          ...
          case 'GREVCC':        ShowHTML('    else document.Form.p_sqcc.value=\''.$_REQUEST['p_sqcc'].'\';');               break;
        ...
        switch ($p_agrega) {
          ...
          case 'GREVCC':        if ($_REQUEST['p_sqcc']=='')        ShowHTML('<input type="Hidden" name="p_sqcc" value="">');         break;
      ...
      foreach($RS1 as $row) {
        ...
        if ($w_embed == 'WORD' && $w_linha>$w_linha_pag) {
          ...
          elseif ($p_agrega=='GREVCC')      ShowHTML('      <tr bgcolor="'.$w_cor.'" valign="top"><td><b>'.f($row,'sg_cc'));
    ...
    if (f($RS_Menu,'solicita_cc')=='S') {
      if ($p_agrega=='GREVCC') ShowHTML('          <option value="GREVCC" selected>Classificação'); else ShowHTML('          <option value="GREVCC">Classificação');
    }
    foreach ($RS as $row) {
...
function ImprimeCabecalho() {
  ...
  switch ($p_agrega) {
    ...
    case 'GREVCC':      ShowHTML('          <td><b>Classificação</td>');        break;
