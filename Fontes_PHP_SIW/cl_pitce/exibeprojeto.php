...
function ExibeProjeto($l_chave,$operacao,$l_usuario,$l_tipo) {
  ...
  $l_html.=chr(13).'   <tr><td colspan="2"><table width=100%  border="1" bordercolor="#00000">';
      ...
      foreach($RS2 as $row) {
        if (f($row,'sg_tipo_interessado')!='PDPCES'&&f($row,'sg_tipo_interessado')!='PDPCEGS'&&f($row,'sg_tipo_interessado')!='PDPCECGS') {
          if (f($row,'sg_tipo_interessado')=='PDPCEGT') {
            $l_html.=chr(13).'       <tr valign="top" bgColor="#f0f0f0">';
  ...
  if(nvl($_REQUEST['p_qualit'],'')!='') {
    ...
    if(nvl($_REQUEST['p_pr'],'')!='') $l_html .= chr(13).'    <tr valign="top"><td colspan="2"><b>Estratégias:</b><td colspan="2">'.Nvl(CRLF2BR(f($RS,'restricoes')),'---').' </td></tr>';
    if(nvl($_REQUEST['p_os'],'')!='') $l_html.=chr(13).'      <tr valign="top"><td colspan="2"><b>Objetivo superior:</b><td colspan="2">'.Nvl(CRLF2BR(f($RS,'objetivo_superior')),'---').' </td></tr>';
    if(nvl($_REQUEST['p_oe'],'')!='') $l_html.=chr(13).'      <tr valign="top"><td colspan="2"><b>Objetivo estratégicos:</b><td colspan="2">'.Nvl(CRLF2BR(f($RS,'descricao')),'---').' </td></tr>';
    if(nvl($_REQUEST['p_ee'],'')!='') $l_html.=chr(13).'      <tr valign="top"><td colspan="2"><b>Desafios:</b><td colspan="2">'.Nvl(CRLF2BR(f($RS,'exclusoes')),'---').' </td></tr>';
    if(nvl($_REQUEST['p_re'],'')!='') $l_html.=chr(13).'      <tr valign="top" bgcolor="#FECC90"><td colspan="2"><b>Prioridades:</b><td colspan="2">'.Nvl(CRLF2BR(f($RS,'premissas')),'---').' </td></tr>';
  }
  ...
  if(nvl($_REQUEST['p_etapa'],'')!='') {
    ...
    if(count($RS)>0 && $l_nome_menu['ETAPA']!='') {
      ...
      $l_html .= chr(13).'          <tr><td rowspan=2 bgColor="#f0f0f0"><div align="center"><b>Item</b></td>';
      $l_html .= chr(13).'            <td rowspan=2 bgColor="#f0f0f0"><div align="center"><b>'.colapsar($l_chave).'Agenda de ação</b></td>';
      $l_html .= chr(13).'            <td rowspan=2 bgColor="#f0f0f0"><div align="center"><b>Entidade executora</b></td>';
      $l_html .= chr(13).'            <td rowspan=2 bgColor="#f0f0f0"><div align="center"><b>Responsável</b></td>';
      $l_html .= chr(13).'            <td colspan=2 bgColor="#f0f0f0"><div align="center"><b>Execução</b></td>';
	  ...
	  $l_html .= chr(13).'            <td rowspan=2 bgColor="#f0f0f0"><div align="center"><b>Desafios</b></td>';
      ...
      */
      $l_html .= chr(13).'          </tr>';
      $l_html .= chr(13).'          <tr>';
      $l_html .= chr(13).'            <td bgColor="#f0f0f0"><div align="center"><b>Início</b></td>';
      $l_html .= chr(13).'            <td bgColor="#f0f0f0"><div align="center"><b>Fim</b></td>';
      $l_html .= chr(13).'          </tr>';
      //Se for visualização normal, irá visualizar somente as etapas
      //print_r(array($l_chave,f($row,'sq_projeto_etapa'),f($row,'titulo'),f($row,'nm_resp'),f($row,'sg_setor'),f($row,'inicio_previsto'),f($row,'fim_previsto'),f($row,'inicio_real'),f($row,'fim_real'),f($row,'perc_conclusao'),f($row,'qt_ativ'),'',null,'PROJETO',f($row,'sq_pessoa'),f($row,'sq_unidade'),f($row,'pj_vincula_contrato'),f($row,'qt_contr'),f($row,'orcamento'),(f($row,'level')-1),f($row,'restricao'),f($row,'peso'),f($row,'qt_anexo')));
      if(nvl($_REQUEST['p_tr'],'')=='') {
        foreach($RS as $row) {
          $l_html .= chr(13).EtapaLinha($l_chave,f($row,'sq_projeto_etapa'),f($row,'titulo'),f($row,'nm_resp'),f($row,'sg_setor'),f($row,'inicio_previsto'),f($row,'fim_previsto'),f($row,'inicio_real'),f($row,'fim_real'),f($row,'perc_conclusao'),f($row,'qt_ativ'),((nvl(f($row,'sq_etapa_pai'),'')=='') ? '<b>' : ''),null,'PROJETO',f($row,'sq_pessoa'),f($row,'sq_unidade'),f($row,'pj_vincula_contrato'),f($row,'qt_contr'),f($row,'orcamento'),(f($row,'level')-1),f($row,'restricao'),f($row,'peso'),f($row,'qt_anexo'),f($row,'unidade_medida'));
        } 
      } else {
        //Se for visualização total, ira visualizar as etapas e as tarefas correspondentes
        foreach($RS as $row) {
          $l_html .= chr(13).EtapaLinhaAtiv($l_chave,f($row,'sq_projeto_etapa'),f($row,'titulo'),f($row,'nm_resp'),f($row,'sg_setor'),f($row,'inicio_previsto'),f($row,'fim_previsto'),f($row,'inicio_real'),f($row,'fim_real'),f($row,'perc_conclusao'),f($row,'qt_ativ'),((nvl(f($row,'sq_etapa_pai'),'')=='') ? '<b>' : ''),null,'PROJETO','RESUMIDO',f($row,'sq_pessoa'),f($row,'sq_unidade'),f($row,'pj_vincula_contrato'),f($row,'qt_contr'),f($row,'orcamento'),(f($row,'level')-1),f($row,'restricao'),f($row,'peso'),f($row,'qt_anexo'),f($row,'unidade_medida'));
        } 
      } 
      $l_html .= chr(13).'      </form>';
      ...
      if ($_REQUEST['p_legenda']) {
        ...
        $l_html .= chr(13).'  <tr valign="top"><td colspan=3><b>Legenda dos sinalizadores:</b>'.ExibeImagemSolic('ETAPA',null,null,null,null,null,null,null, null,true);
        $l_html .= chr(13).'  </table>';
      }
    }
  }
  ...
  if (nvl($_REQUEST['p_risco'],'')!='') {
    ...
    if (count($RS)>0 && $l_nome_menu['RESTSOLIC']!='') {
      ...
      foreach ($RS as $row) {
        $l_row = 1;
        if (nvl($_REQUEST['p_cf'],'')!='') {
          $RS = db_getSolicEtapa::getInstanceOf($dbms,f($row,'chave_aux'),null,'PACOTES',null);
          if(count($RS)>0) {
            $l_row += count($RS);
            $l_row += 2;
          }
        ...
        if (nvl($_REQUEST['p_tf'],'')!='') {
          $RS = db_getSolicRestricao::getInstanceOf($dbms,f($row,'chave_aux'), null, null, null, null, null, 'TAREFA');
          if(count($RS)>0) {
            $l_row += count($RS);
            $l_row += 2;  
          }
      ...
      foreach ($RS as $row) {
        ...
        if (nvl($_REQUEST['p_ca'],'')!='') {     
          ...
          if ($w_cont > 0) {
            ...
            $l_html .= chr(13).'            <td rowspan=2 bgColor="#f0f0f0"><div align="center"><b>Item</b></div></td>';
            $l_html .= chr(13).'            <td rowspan=2 bgColor="#f0f0f0"><div align="center"><b>Agenda de ação</b></div></td>';
            $l_html .= chr(13).'            <td rowspan=2 bgColor="#f0f0f0"><div align="center"><b>Entidade executora</b></div></td>';
            $l_html .= chr(13).'            <td rowspan=2 bgColor="#f0f0f0"><div align="center"><b>Responsável pela atualização</b></div></td>';
            ...
            //Se for visualização normal, irá visualizar somente as etapas
            foreach($RS_Etapa as $row1) {
              if (f($row1,'vinculado_inter')>0) $l_html .= chr(13).EtapaLinha($l_chave,f($row1,'sq_projeto_etapa'),f($row1,'titulo'),f($row1,'nm_resp'),f($row1,'sg_setor'),f($row1,'inicio_previsto'),f($row1,'fim_previsto'),f($row1,'inicio_real'),f($row1,'fim_real'),f($row1,'perc_conclusao'),f($row1,'qt_ativ'),((nvl(f($row,'sq_etapa_pai'),'')=='') ? '<b>' : ''),'N','PROJETO',f($row1,'sq_pessoa'),f($row1,'sq_unidade'),f($row1,'pj_vincula_contrato'),f($row1,'qt_contr'),f($row1,'orcamento'),0,f($row1,'restricao'));
            }
          }
        }
      } 
...
//  if ($p_tipo!='WORD') $l_html .= '<A class="hl" HREF="javascript:this.status.value;" onClick="window.open(\''.montaURL_JS(null,$conRootSIW.'mod_pr/restricao.php?par=ComentarioEtapa&w_solic='.$l_chave.'&w_chave='.$l_chave_aux.'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP=Comentários&SG=PJETACOM').'\',\'Etapa\',\'width=780,height=550,top=10,left=10,toolbar=no,scrollbars=yes,resizable=yes,status=no\'); return false;" title="Clique para exibir ou registrar comentários sobre este item."><img src="'.$conImgSheet.'" border=0>&nbsp;</A>';
  ...
  if($l_arquivo>0) {
    if ($w_embed != 'WORD') {
        $l_html .= chr(13).'        <td width="1%" nowrap align="center" >'.ExibeEtapa('V',$l_chave,$l_chave_aux,'Volta',10,$l_arquivo,$TP,$SG).'</td>';
    }else{
        $l_html .= chr(13).'        <td width="1%" nowrap align="center" >'.ExibeEtapa('V',$l_chave,$l_chave_aux,'PDF',10,$l_arquivo,$TP,$SG).'</td>';
    }
  }  else{
