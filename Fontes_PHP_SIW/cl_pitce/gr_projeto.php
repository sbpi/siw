...
function Gerencial() {
  ...
  if ($O=='L' || $w_embed == 'WORD') {
    ...
    ShowHTML(geraGraficoGoogle($w_titulo.' - Resumo',$SG,'bar',
                               array($t_totsolic,$t_totconc,$t_tottram,$t_totcad,$t_totatraso,$t_totaviso,$t_totacima),
                               $w_legenda
                              )
            );
    //ShowHTML('<tr><td align="center"><IMG SRC="'.$conRootSIW.'geragrafico.php?p_genero=M&p_objeto='.f($RS_Menu,'nome').'&p_graf='.$SG.'&p_grafico=Pizza&p_tot='.$t_totsolic.'&p_cad='.$t_totcad.'&p_tram='.$t_tottram.'&p_conc='.$t_totconc.'&p_atraso='.$t_totatraso.'&p_aviso='.$t_totaviso.'&p_acima='.$t_totacima.'">');
    ShowHTML('<tr><td align="center"><br>');
    ShowHTML(geraGraficoGoogle($w_titulo.' em andamento',$SG,'pie',
                               array($t_totaviso,$t_totatraso,($t_tottram+$t_totcad-$t_totatraso-$t_totaviso)),
                               array('Normal','Aviso','Atrasadas')
                              )
            );
  } elseif ($O=='P') {
