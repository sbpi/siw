...
// Descricao: Retorna projetos de programa estratégico através de interface externa
        ...
        foreach ($RS as $row) {
          if (nvl(f($row,'ln_programa'),'')==$w_username && nvl($w_codigo,f($row,'codigo_interno'))==f($row,'codigo_interno')) {
            ...
            $sql = new db_getSolicList; $RS1 = $sql->getInstanceOf($dbms,f($RS1,'sq_menu'),$w_usuario,f($RS1,'sigla'),5,
                ...
                null, null, $p_empenho, $p_processo);
            $RS1 = SortArray($RS1,'ac_titulo', 'asc', 'codigo_interno','asc');
            foreach ($RS1 as $row1) {
              ...
              $response.=$crlf .
                      f($row, 'codigo_interno') . ';' . // código do programa
                      f($row1, 'codigo_interno') . ';' . // código do projeto
                      f($row1, 'titulo') . ';' . // título do projeto
                      f($row1, 'nm_solic') . ';' . // responsável pelo projeto
                      str_replace($crlf, '&lt;br>', f($row1, 'objetivo_superior')) . ';' . // objetivo do projeto
                      formatNumber(f($row1, 'ide')) . ';' . // índice relativo de desenvolvimento de escopo
                      formatNumber(f($row1, 'ige')) . ';' . // índice geral de desenvolvimento de escopo
                      formatNumber(f($row1, 'idc')) . ';' . // índice relativo de desenvolvimento de custo
                      formatNumber(f($row1, 'igc'));         // índice geral de desenvolvimento de custo
            }
