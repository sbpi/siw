...
//    unidade   = unidade responsável pelo programa
...
if (count($_POST) > 0) {
  ...
  if ($auth>'') {
    ...
    if ($w_result>'') { $w_erro.=$crlf.'Código do unidade: '.$w_result; } 
    ...
    if ($w_erro>'') {
      ...
      $RS = SortArray($RS,'cd_unidade_adm','asc','codigo_interno','asc');
          
      $w_cont = 0;
      foreach ($RS as $row) {
        if (nvl(f($row,'ln_programa'),'')==$w_username && nvl($w_unidade,f($row,'cd_unidade_adm'))==f($row,'cd_unidade_adm') && nvl($w_codigo,f($row,'codigo_interno'))==f($row,'codigo_interno')) {
          $w_cont++;
        }
      }
      ...
      if ($w_cont==0) {
        if (nvl($w_codigo,'')=='') {
          $response = '501'.$crlf.'Nenhum programa vinculado ao ['.$w_username.']'.((nvl($w_unidade,'')!='') ? ' na unidade ['.$w_unidade.']' : '').' foi encontrado na base de dados';
        } else {
          $response = '501'.$crlf.'Não existe programa ['.$w_codigo.']'.((nvl($w_unidade,'')!='') ? ' na unidade ['.$w_unidade.']' : '').' criado pelo ['.$w_username.']';
        }
      } else {
        $response = '200';
        $w_cont = 0;
        ...
        foreach ($RS as $row) {
          if (nvl(f($row,'ln_programa'),'')==$w_username && nvl($w_unidade,f($row,'cd_unidade_adm'))==f($row,'cd_unidade_adm') && nvl($w_codigo,f($row,'codigo_interno'))==f($row,'codigo_interno')) {
            if ($w_atual!=f($row,'cd_unidade_adm')) {
              if ($w_atual!='') {
