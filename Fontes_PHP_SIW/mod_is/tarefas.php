      ...
      foreach($RS1 as $row) {
        ...
        if ($P1!=3 && $P1!=5) {
          ...
          if ($w_copia>'') {
            ...
            if ($w_submenu>'') ShowHTML('          <A class="HL" HREF="menu.php?par=ExibeDocs&O=A&w_chave='.f($row,'sq_siw_solicitacao').'&R='.$w_pagina.$par.'&SG='.$SG.'&TP='.$TP.'&w_documento=Nr. '.f($row,'sq_siw_solicitacao').MontaFiltro('GET').'" title="Altera as informações cadastrais da tarefa" TARGET="menu">Alterar</a>&nbsp;');
            else               ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.$par.'&R='.$w_pagina.$par.'&O=A&w_chave='.f($row,'sq_siw_solicitacao').'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'" title="Altera as informações cadastrais da tarefa">Alterar</A>&nbsp');
            ...
            ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.'Envio&R='.$w_pagina.$par.'&O=V&w_chave='.f($row,'sq_siw_solicitacao').'&w_tipo=Volta&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'" title="Encaminhamento da tarefa.">Enviar</A>&nbsp');
          } elseif ($P1==2) {
            // Se for execução
...
function SolicMail($p_solic,$p_tipo) {
  ...
  if ($p_tipo==1)       $w_html.='      <tr valign="top"><td align="center"><font size=2><b>INCLUSÃO DE TAREFA</b></font><br><br><td></tr>'.chr(13);
  elseif ($p_tipo==2)   $w_html.='      <tr valign="top"><td align="center"><font size=2><b>TRAMITAÇÃO DE TAREFA</b></font><br><br><td></tr>'.chr(13);
  elseif ($p_tipo==3)   $w_html.='      <tr valign="top"><td align="center"><font size=2><b>CONCLUSÃO DE TAREFA</b></font><br><br><td></tr>'.chr(13);
  ...
  $w_html.=chr(13).'      <tr><td valign="top"><font size="1">Tarefa: <b>'.f($RSM,'titulo').'</b></td>';
  $w_html.=chr(13).'      <tr><td valign="top"><font size="1">Ação: <b>'.f($RSM,'nm_projeto').'</b></td>';
  $w_html.=chr(13).'      <tr><td><font size=1>Detalhamento: <b>'.CRLF2BR(f($RSM,'assunto')).'</b></font></td></tr>';
  // Identificação da tarefa
  $w_html.=chr(13).'      <tr><td valign="top" colspan="2" align="center" bgcolor="#D0D0D0" style="border: 2px solid rgb(0,0,0);"><font size="1"><b>EXTRATO DA TAREFA</td>';
  ...
  $w_html.=chr(13).'          <td><font size="1">Área planejamento:<br><b>'.f($RSM,'nm_unidade_resp').'</b></td>';
  $w_html.=chr(13).'          <tr valign="top">';
  ...
  $w_html.=chr(13).'          <td><font size="1">Prioridade:<br><b>'.RetornaPrioridade(f($RSM,'prioridade')).' </b></td>';
  $w_html.=chr(13).'          </table>';
  // Informações adicionais
  if (Nvl(f($RSM,'descricao'),'')>'') $w_html.=chr(13).'      <tr><td valign="top"><font size="1">Resultados da tarefa:<br><b>'.CRLF2BR(f($RSM,'descricao')).' </b></td>';
  $w_html.=chr(13).'    </table>';
  $w_html.=chr(13).'</tr>';
...
function Grava() {
  ...
  switch ($SG) {
    case 'ISTAGERAL':
      ...
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
        ...
        if ($O=='I') {
          ...
          ShowHTML('  location.href=\''.$R.'&O=L&R='.$R.'&SG=ISTCAD&w_menu='.$w_menu.'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&p_proponente='.$_REQUEST['p_proponente'].MontaFiltro('GET').'\';');
        } else {
    ...
    case 'VLRTGERAL':
      // Verifica se a Assinatura Eletrônica é válida
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
        dml_putTarefaLimite::getInstanceOf($dbms,$_REQUEST['w_chave'],$w_usuario,$_REQUEST['w_sq_tramite'],$_REQUEST['w_valor']);
        ScriptOpen('JavaScript');
        // Recupera a sigla do serviço pai, para fazer a chamada ao menu
        $RS = db_getLinkData::getInstanceOF($dbms,$w_cliente,$SG);
        ShowHTML('  location.href=\''.str_replace($w_dir,'',f($RS,'link')).'&O=P&w_chave='.$_REQUEST['w_chave'].'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'\';');
        ScriptClose();
      } else {
        ScriptOpen('JavaScript');
        ShowHTML('  alert(\'Assinatura Eletrônica inválida!\');');
    ...
    case 'GDCONC':
      // Verifica se a Assinatura Eletrônica é válida
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
        $RS = db_getSolicData_IS::getInstanceof($dbms,$_REQUEST['w_chave'],'ISTAGERAL');
        foreach($RS as $row){$RS=$row; break;}
