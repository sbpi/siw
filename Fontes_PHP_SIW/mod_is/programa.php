...
function Inicial() {
  ...
  if ($O=='L') {
    if (!(strpos(upper($R),'GR_')===false)) {
      $w_filtro='';
      if ($p_programa>'') {
        $sql = new db_getSolicData_IS; $RS = $sql->getInstanceOf($dbms,$p_programa,'ISPRGERAL');
    ...
    $sql = new db_getLinkData; $RS = $sql->getInstanceOf($dbms,$w_cliente,'ISPCAD');
    if ($w_copia>'') {
      ...
      foreach($RS1 as $row) {
        ...
        if ($P1!=3 && $P1!=5) {
          ...
          if ($w_copia>'') {
            ...
            // Se for cadastramento
            if ($w_submenu>'') ShowHTML('          <A class="HL" HREF="menu.php?par=ExibeDocs&O=A&w_chave='.f($row,'sq_siw_solicitacao').'&R='.$w_pagina.$par.'&SG='.$SG.'&TP='.$TP.'&w_documento=Nr. '.f($row,'sq_siw_solicitacao').MontaFiltro('GET').'" title="Altera as informações cadastrais do programa" TARGET="menu">AL</a>&nbsp;');
...
function Indicadores() {
  ...
  if ($w_troca>'') {
    ...
    // Recupera todos os registros para a listagem
    $sql = new db_getSolicIndic_IS; $RS = $sql->getInstanceOf($dbms,$w_chave,null,'LISTA',null,null);
    $RS = SortArray($RS,'ordem','asc');
  } elseif (!(strpos('AEV',$O)===false) && $w_troca=='') {
    // Recupera os dados do endereço informado
    $sql = new db_getSolicIndic_IS; $RS = $sql->getInstanceOf($dbms,$w_chave,$w_chave_aux,'REGISTRO',null,null);
    foreach($RS as $row){$RS=$row; break;}
    ...
    if (count($RS)<=0) {
      ...
      foreach($RS as $row) {
        ...
        if ($w_acesso==1) {
          ...
          else                            ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.$par.'&R='.$w_pagina.$par.'&O=A&w_chave='.$w_chave.'&w_chave_aux='.f($row,'sq_indicador').'&w_cd_programa='.$w_cd_programa.'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'">AL</A>&nbsp');
          if ((Nvl(f($row,'cd_indicador'),'')=='') && ($P1!=2 && $P1!=3 && $P1!=5)) ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.'GRAVA&R='.$w_pagina.$par.'&O=E&w_chave='.$w_chave.'&w_chave_aux='.f($row,'sq_indicador').'&w_cd_programa='.$w_cd_programa.'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'" onClick="return confirm(\'Confirma a exclusão do registro?\');">EX</A>&nbsp');
        } else {
...
function AtualizaIndicador() {
  ...
  $sql = new db_getSolicData_IS; $RS = $sql->getInstanceOf($dbms,$w_chave,'ISPRGERAL');
  foreach($RS as $row){$RS=$row; break;}
...
function Interessados() {
  ...
  if ($O=='L') {
    ...
    if ($P1!=4) {
      ...
      $sql = new db_getSolicData_IS; $RS1 = $sql->getInstanceOf($dbms,$w_chave,'ISPRGERAL');
      foreach($RS1 as $row){$RS1=$row; break;}
...
function Iniciativas() {
  ...
  $sql = new db_getOrPrioridadeList; $RS = $sql->getInstanceOf($dbms,$w_chave,$w_cliente,$w_sq_isprojeto);
...
function Financiamento() {
  ...
  if ($w_troca>'') {
    ...
    // Recupera todos os registros para a listagem
    $sql = new db_getFinancAcaoPPA; $RS = $sql->getInstanceOf($dbms,$w_chave,$w_cliente,null);
  } elseif (!(strpos('AEV',$O)===false) && $w_troca=='') {
    // Recupera os dados do financiamento
    $sql = new db_getFinancAcaoPPA; $RS = $sql->getInstanceOf($dbms,$w_chave,$w_cliente,$_REQUEST['w_sq_acao_ppa']);
    foreach($RS as $row){$RS=$row; break;}
...
function SolicMail($p_solic,$p_tipo) {
  ...
  if(f($RS,'envia_mail_tramite')=='S' && (f($RS_Menu,'envia_email')=='S')) {
    ...
    // Recupera os dados da ação
    $sql = new db_getSolicData_IS; $RSM = $sql->getInstanceOf($dbms,$p_solic,'ISPRGERAL');
    ...
    $sql = new db_getCustomerData; $RS = $sql->getInstanceOf($dbms,$_REQUEST['p_cliente'.'_session']);
...
function RestricaoMail($l_solic,$l_descricao,$l_tp_restricao,$l_providencia,$l_tipo) {
  ...
  $sql = new db_getSolicData_IS; $RSM = $sql->getInstanceOf($dbms,$l_solic,'ISPRGERAL');
  foreach($RSM as $row){$RSM=$row; break;}
  $w_html.=$crlf.'<tr bgcolor="'.$conTrBgColor.'"><td align="center">';
  $w_html.=$crlf.'    <table width="99%" border="0">';
  $w_html.=$crlf.'      <tr><td><font size=2>Programa: <b>'.f($RSM,'titulo').'</b></font></td>';
  ...
  $sql = new db_getTPRestricao_IS; $RSM = $sql->getInstanceOf($dbms,$l_tp_restricao,null);
  foreach($RSM as $row){$RSM=$row; break;}
  $w_html.=$crlf.'          <tr valign="top">';
  ...
  $sql = new db_getPersonData; $RS = $sql->getInstanceOf($dbms,$w_cliente,$_REQUEST['sq_pessoa'.'_session'],null,null);
  if ((strpos($w_destinatarios,f($RS,'email').'; ')===false)) $w_destinatarios=$w_destinatarios.f($RS,'email').'; ';
  // Recupera o e-mail dos interessados
  $sql = new db_getSolicInter; $RSM = $sql->getInstanceOf($dbms,$l_solic,null,'LISTA');
  if (count($RS)) {
   foreach ($RSM as $row) {
    $sql = new db_getPersonData; $RS = $sql->getInstanceOf($dbms,$w_cliente,f($RSM,'sq_pessoa'),null,null);
    if ((strpos($w_destinatarios,f($RS,'email').'; ')===false)) $w_destinatarios=$w_destinatarios.f($RS,'email').'; ';
   } 
  } 
  // Prepara os dados necessários ao envio
  ...
  if ($SG=='ISPRGERAL' || $SG=='VLRPGERAL') {
    ...
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {
      if ($O=='E' && f($RS_Menu,'cancela_sem_tramite')=='N') {
        ...
        if ($O=='I') {
          $sql = new db_getPrograma_IS; $RS = $sql->getInstanceOf($dbms,$_REQUEST['w_cd_programa'],$w_ano,$w_cliente,null);
          foreach ($RS as $row){$RS=$row; break;}
          if (f($RS,'Existe')>0) {
            ScriptOpen('JavaScript');
    ...
    // Verifica se a Assinatura Eletrônica é válida
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {
      $SQL = new dml_putRespPrograma_IS; $SQL->getInstanceOf($dbms,$_REQUEST['w_chave'],
    ...
    // Verifica se a Assinatura Eletrônica é válida
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {
      $SQL = new dml_putIndicador_IS; $SQL->getInstanceOf($dbms,$O,
    ...
    // Verifica se a Assinatura Eletrônica é válida
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {
      $SQL = new dml_putRestricao_IS; $SQL->getInstanceOf($dbms,$O,$SG,$_REQUEST['w_chave'],$_REQUEST['w_chave_aux'],null,null,
    ...
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {
      ...
      if (UPLOAD_ERR_OK==0) {
        ...
        $SQL = new dml_putSolicArquivo; $SQL->getInstanceOf($dbms,$O,
          ...
          $w_file,$w_tamanho,$w_tipo,$w_nome, $w_chave_arquivo);
      } else {
        ScriptOpen('JavaScript');
    ...
    // Verifica se a Assinatura Eletrônica é válida
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {
      $sql = new db_getSolicData_IS; $RS = $sql->getInstanceOf($dbms,$_REQUEST['w_chave'],'ISPRGERAL');
      foreach($RS as $row){$RS=$row; break;}
