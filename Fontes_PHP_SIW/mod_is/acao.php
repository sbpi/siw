  ...
  if ($O=='L') {
    if (!(strpos(upper($R),'GR_')===false)) {
      ...
      if ($p_acao>'' && $p_sq_acao_ppa=='') {
        $sql = new db_getSolicData_IS; $RS = $sql->getInstanceOf($dbms,$p_acao,'ISACGERAL');
        foreach ($RS as $row){$RS=$row; break;}
        $w_filtro=$w_filtro.'<tr valign="top"><td align="right">Ação <td>[<b>'.f($RS,'titulo').'</b>]';
      } 
      if ($p_sq_acao_ppa>'') {
        $sql = new db_getAcaoPPA_IS; $RS = $sql->getInstanceOf($dbms,$w_cliente,$w_ano,substr($p_sq_acao_ppa,0,4),substr($p_sq_acao_ppa,4,4),null,substr($p_sq_acao_ppa,12,17),null,null,null,null,null);
        foreach ($RS as $row){$RS=$row; break;}
      ...
      if ($p_sq_isprojeto>'') {
        $sql = new db_getProjeto_IS; $RS = $sql->getInstanceOf($dbms,$p_sq_isprojeto,$w_cliente,null,null,null,null,null,null,null,null,null,null,null,null);
        foreach ($RS as $row){$RS=$row; break;}
...
function Geral() {
  ...
  if ($w_troca>'') {
    ...
    if ($w_sq_acao_ppa>'') {
      $sql = new db_getAcaoPPA_IS; $RS = $sql->getInstanceOf($dbms,$w_cliente,$w_ano,substr($w_sq_acao_ppa,0,4),substr($w_sq_acao_ppa,4,4),null,substr($w_sq_acao_ppa,12,17),null,null,null,null,null);
      foreach($RS as $row){$RS=$row; break;}
      $w_titulo = substr(f($RS,'descricao_acao'),0,69).' - '.substr(f($RS,'ds_unidade'),0,28);
    } elseif ($w_sq_isprojeto>'') {
      $sql = new db_getProjeto_IS; $RS = $sql->getInstanceOf($dbms,$w_sq_isprojeto,$w_cliente,null,null,null,null,null,null,null,null,null,null,null,null);
      foreach($RS as $row){$RS=$row; break;}
      $w_titulo = f($RS,'nome');
    } 
  } else {
...
function RecursoProgramado() {
  ...
  if (!(strpos('A',$O)===false)) {
    ...
    if (Nvl(f($RS,'cd_acao'),'')>'') {
      $sql = new db_getAcaoPPA_IS; $RS1 = $sql->getInstanceOf($dbms,$w_cliente,$w_ano,f($RS,'cd_ppa_pai'),f($RS,'cd_acao'),null,f($RS,'cd_unidade'),null,null,null,null,null);
      ShowHTML('     <tr valign="top">');
    ...
    if (Nvl(f($RS,'cd_acao'),'')>'') {
      ...
      if (f($RS,'cd_tipo_acao')!=3) {
        $sql = new db_getPPADadoFinanc_IS; $RS1 = $sql->getInstanceOf($dbms,f($RS,'cd_acao'),f($RS,'cd_unidade'),$w_ano,$w_cliente,'VALORFONTEACAO');
        if (count($RS1)<=0) {
          ...
          $sql = new db_getPPADadoFinanc_IS; $RS1 = $sql->getInstanceOf($dbms,f($RS,'cd_acao'),f($RS,'cd_unidade'),$w_ano,$w_cliente,'VALORTOTALACAO');
      ...
      $sql = new db_getFinancAcaoPPA_IS; $RS1 = $sql->getInstanceOf($dbms,$w_chave,$w_cliente,$w_ano,null,null,null);
    ...
    ShowHTML('      <tr><td><b>'.$_SESSION['LABEL_CAMPO'].':<BR> <INPUT ACCESSKEY="A" class="sti" type="PASSWORD" name="w_assinatura" size="30" maxlength="30" value=""></td></tr>');
...
function Responsaveis() {
  ...
  if ($O=='L') {
    ...
    if ($w_programa>'' && $w_acao>'' && $w_unidade>'') {
      $w_tipo=1;
      $sql = new db_getAcaoPPA_IS; $RS = $sql->getInstanceOf($dbms,$w_cliente,$w_ano,$w_programa,$w_acao,null,$w_unidade,null,null,null,null,null);
      foreach ($RS as $row) {$RS=$row; break;}
    } elseif ($w_sq_isprojeto>'') {
      $w_tipo=2;
      $sql = new db_getProjeto_IS; $RS = $sql->getInstanceOf($dbms,$w_sq_isprojeto,$w_cliente,null,null,null,null,null,null,null,null,null,null,null,null);
...
function Metas() {
  ...
  if ($w_troca>'') {
    ...
    // Recupera todos os registros para a listagem
    $sql = new db_getSolicMeta_IS; $RS = $sql->getInstanceOf($dbms,$w_cliente,$w_chave,null,'LISTA',null,null,null,null,null,null,null,null,null);
    $RS = SortArray($RS,'ordem','asc');
  } elseif (!(strpos('AEV',$O)===false) && $w_troca=='') {
    // Recupera os dados do endereço informado
    $sql = new db_getSolicMeta_IS; $RS = $sql->getInstanceOf($dbms,$w_cliente,$w_chave,$w_chave_aux,'REGISTRO',null,null,null,null,null,null,null,null,null);
    foreach($RS as $row) {$RS=$row; break;}
    $w_titulo                   =f($RS,'titulo');
    $w_ordem                    =f($RS,'ordem');
    $w_descricao                =f($RS,'descricao');
  ...
  if ($O=='L') {
    ...
    if ($w_cd_subacao>'') $w_Disabled='DISABLED';
    ShowHTML('     <tr><td valign="top" colspan="2"><table border=0 width="100%" cellspacing=0><tr valign="top">');
    ShowHTML('         <tr><td align="left"><b><u>Q</u>uantitativo:<br><input accesskey="Q" type="text" name="w_quantidade" class="STI" SIZE="18" MAXLENGTH="18" VALUE="'.$w_quantidade.'" '.$w_Disabled.' style="text-align:right;" onKeyDown="FormataValor(this,18,2,event);" title="Indicar a quantidade da meta da ação programada para determinado período de tempo."></td>');
...
function AtualizaMeta() {
  ...
  $sql = new db_getSolicData_IS; $RS = $sql->getInstanceOf($dbms,$w_chave,'ISACGERAL');
  foreach($RS as $row) {$RS=$row; break;}
  ...
  if ($w_troca>'') {
    ...
    //$RS = SortArray($RS,'ordem','asc');
    // Recupera as metas
    $sql = new db_getSolicMeta_IS; $RS = $sql->getInstanceOf($dbms,$w_cliente,$w_chave,null,'LSTNULL',null,null,null,null,null,null,null,null,null);
    $RS = SortArray($RS,'ordem','asc');
  } elseif (!(strpos('AEV',$O)===false) && $w_troca=='') {
    // Recupera os dados do endereço informado
    $sql = new db_getSolicMeta_IS; $RS = $sql->getInstanceOf($dbms,$w_cliente,$w_chave,$w_chave_aux,'REGISTRO',null,null,null,null,null,null,null,null,null);
    ...
    $sql = new db_getMetaMensal_IS; $RS = $sql->getInstanceOf($dbms,$w_chave_aux);
    if (count($RS)>0) {
      foreach($RS as $row) {
  ...
  if ($O=='L') {
    ...
    ShowHTML('    <td align="right"><b>Registros: '.count($RS).'</td></tr>');
    ShowHTML('  <tr><td align="center" colspan="3">');
    ShowHTML('      <TABLE class="tudo" WIDTH="100%" bgcolor="'.$conTableBgColor.'" BORDER="'.$conTableBorder.'" CELLSPACING="'.$conTableCellSpacing.'" CELLPADDING="'.$conTableCellPadding.'" BorderColorDark="'.$conTableBorderColorDark.'" BorderColorLight="'.$conTableBorderColorLight.'">');
    ShowHTML('        <tr bgcolor="'.$conTrBgColor.'" align="center">');
    ShowHTML('          <td><b>Produtos</td>');
    ...
    $sql = new db_getPersonData; $RS = $sql->getInstanceOf($dbms,$w_cliente,Nvl($w_sq_pessoa_atualizacao,0),null,null);
...
function Financiamento() {
  ...
  if ($w_troca>'') {
    ...
    $sql = new db_getFinancAcaoPPA_IS; $RS = $sql->getInstanceOf($dbms,$w_chave,$w_cliente,$w_ano,substr($w_chave_aux,0,4),substr($w_chave_aux,4,4),substr($w_chave_aux,8,4));
    foreach ($RS as $row) {$RS=$row; break;}
  ...
  if ($O=='L') {
    ...
    if (Nvl(f($RS,'cd_acao'),'')>'') {
      if (f($RS,'cd_tipo_acao')!=3) {
        ...
        // Recupera todos os registros para a listagem
        $sql = new db_getFinancAcaoPPA_IS; $RS = $sql->getInstanceOf($dbms,$w_chave,$w_cliente,$w_ano,null,null,null);
        // Exibe a quantidade de registros apresentados na listagem e o cabeçalho da tabela de listagem
        ShowHTML('<tr><td><font size="2"><a accesskey="I" class="SS" href="'.$w_dir.$w_pagina.$par.'&R='.$w_pagina.$par.'&O=I&w_chave='.$w_chave.'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'"><u>I</u>ncluir</a>&nbsp;');
        ShowHTML('    <td align="right"><b>Registros: '.count($RS));
        ShowHTML('<tr><td align="center" colspan=3>');
...
function Restricoes() {
  ...
  if ($w_troca>'') {
    ...
    // Recupera todos os registros para a listagem
    $sql = new db_getRestricao_IS; $RS = $sql->getInstanceOf($dbms,$SG,$w_chave,null);
    $RS = SortArray($RS,'inclusao','desc');
  } elseif (!(strpos('AEV',$O)===false) && $w_troca=='') {
    $sql = new db_getRestricao_IS; $RS = $sql->getInstanceOf($dbms,$SG,$w_chave,$w_chave_aux);
    foreach ($RS as $row) {$RS=$row; break; }
    $w_cd_subacao           = f($RS,'cd_subacao');
  ...
  if ($O=='L') {
    ...
    $sql = new db_getPPALocalizador_IS; $RS = $sql->getInstanceOf($dbms,$w_cliente,$w_ano,$w_cd_programa,$w_cd_acao,$w_cd_unidade,null);
    if (count($RS)>1) {
      $sql = new db_getPPALocalizador_IS; $RS = $sql->getInstanceOf($dbms,$w_cliente,$w_ano,$w_cd_programa,$w_cd_acao,$w_cd_unidade,$w_cd_subacao);
...
function Interessados() {
  ...
  if ($O=='L') {
    ...
    if ($P1!=4) {
      ...
      $sql = new db_getSolicData_IS; $RS1 = $sql->getInstanceOf($dbms,$w_chave,'ISACVISUAL');
      foreach ($RS1 as $row1){$RS1=$row1; break;}
...
function Concluir() {
  ...
  ShowHTML('              <td valign="top"><b><u>R</u>ecurso executado:</b><br><input '.$w_Disabled.' accesskey="O" type="text" name="w_custo_real" class="STI" SIZE="18" MAXLENGTH="18" VALUE="'.$w_custo_real.'" style="text-align:right;" onKeyDown="FormataValor(this,18,2,event);" title="Informe o valor que foi efetivamente gasto com a execução da ação."></td>');
  ShowHTML('          </table>');
...
function SolicMail($p_solic,$p_tipo) {
  ...
  if(f($RS,'envia_mail_tramite')=='S' && (f($RS_Menu,'envia_email')=='S')) {
    ...
    // Recupera os dados da ação
    $sql = new db_getSolicData_IS; $RSM = $sql->getInstanceOf($dbms,$p_solic,'ISACGERAL');
    ...
    if ($p_tipo==2) {
      ...
      $sql = new db_getPersonData; $RS1 = $sql->getInstanceOf($dbms,$w_cliente,f($RS,'sq_pessoa_destinatario'),null,null);
...
function RestricaoMail($l_solic,$l_descricao,$l_tl_restricao,$l_providencia,$l_tipo) {
  ...
  $sql = new db_getSolicData_IS; $RSM = $sql->getInstanceOf($dbms,$l_solic,'ISACGERAL');
  foreach($RSM as $row){$RSM=$row; break;}
  $w_html .= $crlf.'<tr bgcolor="'.$conTrBgColor.'"><td align="center">';
  $w_html .= $crlf.'    <table width="99%" border="0">';
  ...
  $sql = new db_getTPRestricao_IS; $RSM = $sql->getInstanceOf($dbms,$l_tl_restricao,null);
  foreach($RSM as $row){$RSM=$row; break;}
  $w_html .= $crlf.'          <tr valign="top">';
  ...
  $sql = new db_getSolicInter; $RSM = $sql->getInstanceOf($dbms,$l_solic,null,'LISTA');
  if (count($RSM)>0) {
    foreach ($RSM as $row) {
      $sql = new db_getPersonData; $RS = $sql->getInstanceOf($dbms,$w_cliente,f($row,'sq_pessoa'),null,null);
      if ((strpos($w_destinatarios,f($RS,'email').'; ')===false)) $w_destinatarios=$w_destinatarios.f($RS,'email').'; ';
    } 
  } 
  // Prepara os dados necessários ao envio
...
function BuscaAcao() {
  ...
  if ($restricao=='FINANCIAMENTO') {
    $sql = new db_getAcaoPPA_IS; $RS = $sql->getIntanceOf($dbms,$w_cliente,$w_ano,$w_programa,$ChaveAux,null,$w_unidade,$restricao,$w_chave,$w_nome,null,null);
    $RS = SortArray($RS,'descricao_acao','asc');
  } elseif ($restricao=='IDENTIFICACAO') {
    $sql = new db_getAcaoPPA_IS; $RS = $sql->getInstanceOf($dbms,$w_cliente,$w_ano,null,$ChaveAux,null,null,$restricao,null,$w_nome,null,null);
    $RS = SortArray($RS,'descricao_acao','asc');
  } else {
    $sql = new db_getAcaoPPA_IS; $RS = $sql->getInstanceOf($dbms,$w_cliente,$w_ano,$w_programa,$ChaveAux,null,$w_unidade,null,null,$w_nome,null,null);
    $RS = SortArray($RS,'descricao_acao','asc');
  } 
  Cabecalho();
  ...
  if ($w_nome>'' || $ChaveAux>'') {
    ...
    if (count($RS)<=0) {
      ...
      foreach($RS as $row) {
        ...
        ShowHTML('            <td>'.f($row,'descricao_acao').'</td>');
        ShowHTML('            <td><a class="ss" HREF="javascript:this.status.value;" onClick="javascript:volta(\''.f($row,'chave').'\');">Selecionar</a>');
      } 
      ShowHTML('        </table></tr>');
      ShowHTML('      </center>');
      ShowHTML('    </table>');
  ...
  if ($SG=='ISACGERAL' || $SG=='VLRAGERAL') {
    ...
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {
      if ($O=='I' && $_REQUEST['w_sq_acao_ppa']>'') {
        $sql = new db_getAcao_IS; $RS = $sql->getInstanceOf($dbms,substr($_REQUEST['w_sq_acao_ppa'],0,4),substr($_REQUEST['w_sq_acao_ppa'],4,4),substr($_REQUEST['w_sq_acao_ppa'],12,5),$w_ano,$w_cliente,null,null);
        foreach ($RS as $row) {$RS = $row; break;}
        if (f($RS,'Existe')>0) {
          ScriptOpen('JavaScript');
      ...
      $SQL = new dml_putAcaoGeral_IS; $SQL->getInstanceOf($dbms,$O,
    ...
    // Verifica se a Assinatura Eletrônica é válida
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {
      $SQL = new dml_putProgQualitativa_IS; $SQL->getInstanceOf($dbms,
    ...
    // Verifica se a Assinatura Eletrônica é válida
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {
      $SQL = new dml_putAcaoMeta_IS; $SQL->getInstanceOf($dbms,$O,$_REQUEST['w_chave'],$_REQUEST['w_chave_aux'],
      ...
      If (($O!='E') && $_REQUEST['w_programada']=='S'){
        $SQL = new dml_putMetaMensalIni_IS; $SQL->getInstanceOf($dbms,'W',$_REQUEST['w_chave_aux'],$w_cliente,
    ...
    // Verifica se a Assinatura Eletrônica é válida
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {
      $SQL = new dml_putFinancAcaoPPA_IS; $SQL->getInstanceOf($dbms,$O,$_REQUEST['w_chave'],substr($_REQUEST['w_chave_aux'],0,4),
    ...
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {
      ...
      $SQL = new dml_putAtualizaMeta_IS; $SQL->getInstanceOf($dbms,$_REQUEST['w_chave'],$_REQUEST['w_chave_aux'],Nvl($w_perc_conclusao,0),$_REQUEST['w_situacao_atual'],
      ...
      $SQL = new dml_putMetaMensal_IS; $SQL->getInstanceOf($dbms,'E',$_REQUEST['w_chave_aux'],$_REQUEST['w_realizado_'.$i],$_REQUEST['w_revisado_'.$i],$_REQUEST['w_referencia_'.$i],$w_cliente);
      for ($i=1; $i<13; $i=$i+1) {
        $SQL = new dml_putMetaMensal_IS; $SQL->getInstanceOf($dbms,'Z',$_REQUEST['w_chave_aux'],Nvl($_REQUEST['w_realizado_'.$i],0),Nvl($_REQUEST['w_revisado_'.$i],0),$_REQUEST['w_referencia_'.$i],$w_cliente);
        if (Nvl($_REQUEST['w_realizado_'.$i],0)>0 || Nvl($_REQUEST['w_revisado_'.$i],0)>0) {
          $SQL = new dml_putMetaMensal_IS; $SQL->getInstanceOf($dbms,'I',$_REQUEST['w_chave_aux'],$_REQUEST['w_realizado_'.$i.''],$_REQUEST['w_revisado_'.$i],$_REQUEST['w_referencia_'.$i],$w_cliente);
        } 
      } 
      ScriptOpen('JavaScript');
      // Recupera a sigla do serviço pai, para fazer a chamada ao menu
    ...
    // Verifica se a Assinatura Eletrônica é válida
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {
      $SQL = new dml_putRestricao_IS; $SQL->getInstanceOf($dbms,$O,$SG,$_REQUEST['w_chave'],$_REQUEST['w_chave_aux'],$_REQUEST['w_cd_subacao'],$_REQUEST['w_sq_isprojeto'],
    ...
    // Verifica se a Assinatura Eletrônica é válida
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {
      $sql = new db_getSolicData_IS; $RS = $sql->getInstanceOf($dbms,$_REQUEST['w_chave'],'ISACGERAL');
      foreach($RS as $row){$RS=$row; break;}
