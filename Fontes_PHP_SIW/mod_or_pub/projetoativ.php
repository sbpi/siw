...
function Inicial() {
  ...
  if ($O=='L') {
    ...
    $sql = new db_getLinkData; $RS = $sql->getInstanceOf($dbms,$w_cliente,'ORPCAD');
    if ($w_copia>'') {
      ...
      $sql = new db_getSolicList; $RS = $sql->getInstanceOf($dbms,f($RS,'sq_menu'),$w_usuario,$SG,$P1,
              $p_ini_i,$p_ini_f,$p_fim_i,$p_fim_f,$p_atraso,$p_solicitante,
              $p_unidade,$p_prioridade,$p_ativo,$p_proponente,
              $p_chave,$p_assunto,$p_pais,$p_regiao,$p_uf,$p_cidade,$p_usu_resp,
              $p_uorg_resp,$p_palavra,$p_prazo,$p_fase,$p_sqcc,$p_projeto,$p_atividade,null,null);
  ...
  if ($O=='L') {
      ...
      foreach($RS1 as $row) {
        ...
        if ($P1!=3) {
          ...
          if ($w_copia>'') {
            ...
            $sql = new db_getLinkSubMenu; $RS2 = $sql->getInstanceOf($dbms,$w_cliente,$_REQUEST['SG']);
            ...
            if ($w_usuario==f($row,'executor')) {
              ...
              ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.'envio&R='.$w_pagina.$par.'&O=V&w_chave='.f($row,'sq_siw_solicitacao').'&w_tramite='.f($row,'sq_siw_tramite').'&w_tipo=Volta&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'" title="Envia a tarefa para outro responsável.">EN</A>&nbsp');
    ...
    ShowHTML('      <tr>');
    $sql = new db_getLinkData; $RS = $sql->getInstanceOf($dbms,$w_cliente,'ORCAD');
    SelecaoProjeto('Açã<u>o</u>:','O','Selecione a ação da tarefa na relação.',$p_projeto,$w_usuario,f($RS,'sq_menu'),null,null,null,'p_projeto','PJLIST','onChange="document.Form.action=\''.$w_dir.$w_pagina.$par.'\'; document.Form.O.value=\''.$O.'\'; document.Form.w_troca.value=\'p_atividade\'; document.Form.submit();"');
    ShowHTML('      </tr>');
    ShowHTML('      <tr>');
...
function Geral() {
  ...
  if (!(strpos('IAEV',$O)===false)) {
    ...
    ShowHTML('      <tr>');
    $sql = new db_getLinkData; $RS = $sql->getInstanceOf($dbms,$w_cliente,'ORCAD');
    SelecaoProjeto('Açã<u>o</u>:','O','Selecione a ação a qual a tarefa está vinculda.',$w_projeto,$w_usuario,f($RS,'sq_menu'),null,null,null,'w_projeto','PJLISTCAD',null);
    ShowHTML('      </tr>');
    //ShowHTML '      <tr>'
    ...
    switch (f($RS_Menu,'data_hora')) {
      case 1: ShowHTML('              <td valign="top"><b>Fim previs<u>t</u>o:</b><br><input '.$w_Disabled.' accesskey="T" type="text" name="w_fim" class="STI" SIZE="10" MAXLENGTH="10" VALUE="'.$w_fim.'" onKeyDown="FormataData(this,event);" onKeyUp="SaltaCampo(this.form.name,this,10,event);"></td>');      break;
      case 2: ShowHTML('              <td valign="top"><b>Fim previs<u>t</u>o:</b><br><input '.$w_Disabled.' accesskey="T" type="text" name="w_fim" class="STI" SIZE="17" MAXLENGTH="17" VALUE="'.$w_fim.'" onKeyDown="FormataDataHora(this,event);" onKeyUp="SaltaCampo(this.form.name,this,17,event);"></td>');  break;
      case 3: ShowHTML('              <td valign="top"><b>Iní<u>c</u>io previsto:</b><br><input '.$w_Disabled.' accesskey="C" type="text" name="w_inicio" class="STI" SIZE="10" MAXLENGTH="10" VALUE="'.Nvl($w_inicio,FormataDataEdicao(time())).'" onKeyDown="FormataData(this,event);" onKeyUp="SaltaCampo(this.form.name,this,10,event);"></td>');
              ShowHTML('              <td valign="top"><b>Fim previs<u>t</u>o:</b><br><input '.$w_Disabled.' accesskey="T" type="text" name="w_fim" class="STI" SIZE="10" MAXLENGTH="10" VALUE="'.$w_fim.'" onKeyDown="FormataData(this,event);" onKeyUp="SaltaCampo(this.form.name,this,10,event);"></td>');
        break;
      case 4: ShowHTML('              <td valign="top"><b>Iní<u>c</u>io previsto:</b><br><input '.$w_Disabled.' accesskey="C" type="text" name="w_inicio" class="STI" SIZE="17" MAXLENGTH="17" VALUE="'.$w_inicio.'" onKeyDown="FormataDataHora(this,event);" onKeyUp="SaltaCampo(this.form.name,this,17,event);"></td>');
              ShowHTML('              <td valign="top"><b>Fim previs<u>t</u>o:</b><br><input '.$w_Disabled.' accesskey="T" type="text" name="w_fim" class="STI" SIZE="17" MAXLENGTH="17" VALUE="'.$w_fim.'" onKeyDown="FormataDataHora(this,event);" onKeyUp="SaltaCampo(this.form.name,this,17,event);"></td>');
        break;
    } 
    ShowHTML('              <td valign="top"><b><u>R</u>ecurso programado:</b><br><input '.$w_Disabled.' accesskey="O" type="text" name="w_valor" class="STI" SIZE="18" MAXLENGTH="18" VALUE="'.$w_valor.'" style="text-align:right;" onKeyDown="FormataValor(this,18,2,event);" title="Informe o recurso programado para execução da tarefa, ou zero se não for o caso."></td>');
...
function Excluir() {
  ...
  $sql = new db_getSolicData; $RS = $sql->getInstanceOf($dbms,$w_chave,'ORPGERAL');
...
function Concluir(){
  ...
  switch (f($RS_Menu,'data_hora')) {
    ...
    case 4:    ShowHTML('              <td valign="top"><b>Iní<u>c</u>io da execução:</b><br><input '.$w_Disabled.' accesskey="C" type="text" name="w_inicio_real" class="STI" SIZE="17" MAXLENGTH="17" VALUE="'.$w_inicio_real.'" onKeyDown="FormataDataHora(this,event);" onKeyUp="SaltaCampo(this.form.name,this,17,event);" title="Informe a data/hora de início da execução da tarefa."></td>');
               ShowHTML('              <td valign="top"><b><u>T</u>érmino da execução:</b><br><input '.$w_Disabled.' accesskey="T" type="text" name="w_fim_real" class="STI" SIZE="17" MAXLENGTH="17" VALUE="'.$w_fim_real.'" onKeyDown="FormataDataHora(this,event);" onKeyUp="SaltaCampo(this.form.name,this,17,event);" title="Informe a data de término da execução da tarefa."></td>');
      break;
  } 
  ShowHTML('              <td valign="top"><b><u>R</u>ecurso executado:</b><br><input '.$w_Disabled.' accesskey="O" type="text" name="w_custo_real" class="STI" SIZE="18" MAXLENGTH="18" VALUE="'.$w_custo_real.'" style="text-align:right;" onKeyDown="FormataValor(this,18,2,event);" title="Informe o recurso utilizado para execução da tarefa, ou zero se não for o caso."></td>');
  ShowHTML('          </table>');
...
function SolicMail($p_solic,$p_tipo) {
  ...
  if(f($RS,'envia_mail_tramite')=='S' && (f($RS_Menu,'envia_email')=='S')) {
    ...
    if ($p_tipo==2) {
      ...
      $RS = SortArray($RS,'phpdt_data','desc','sq_siw_solic_log','desc','depacho','desc');
    ...
    case 'ORPGERAL':
      ...
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {   
        ...
        if ($O=='E' && f($RS_Menu,'cancela_sem_tramite')=='N') {
          ...
          $sql = new db_get10PercentDays; $RS = $sql->getInstanceOf($dbms,$_REQUEST['w_inicio'],$_REQUEST['w_fim']);
          $w_dias = f($RS,'dias');
    ...
    case 'ORPENVIO':
      ...
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {   
        ...
        if ((false!==(strpos(upper($_SERVER['HTTP_CONTENT_TYPE']),'MULTIPART/FORM-DATA'))) || (false!==(strpos(upper($_SERVER['CONTENT_TYPE']),'MULTIPART/FORM-DATA')))) {
          ...
          $RS = $sql->getInstanceOf($dbms, $_REQUEST['w_chave'], 'GDGERAL');
      ...
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {  
        ...
        if (f($RS,'sq_siw_tramite')!=$_REQUEST['w_tramite']) { 
          ...
          if (UPLOAD_ERR_OK==0) {
            ...
            $SQL = new dml_putDemandaConc; $SQL->getInstanceOf($dbms,$w_menu,$_REQUEST['w_chave'],$w_usuario,$_REQUEST['w_tramite'],
