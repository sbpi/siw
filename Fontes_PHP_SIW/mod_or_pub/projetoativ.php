  ...
  if ($O=='L') {
    if ((!(strpos(strtoupper($R),'GR_')===false)) || (!(strpos(strtoupper($R),'PROJETO')===false))) {
      ...
      if ($p_projeto>'') {
        ...
        $w_filtro = $w_filtro.'<tr valign="top"><td align="right">Ação <td>[<b><A class="HL" HREF="'.$w_dir.'projeto.php?par=Visual&O=L&w_chave='.$p_projeto.'&w_tipo=Volta&P1=2&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'" title="Exibe as informações do projeto.">'.f($RS,'titulo').'</a></b>]';
      } 
      if ($p_atividade>''){
        $RS = db_getSolicEtapa::getInstanceOf($dbms,$p_projeto,$p_atividade,'REGISTRO',null);
        foreach ($RS as $row){$RS=$row; break;}
        $w_filtro=$w_filtro.'<tr valign="top"><td align="right">Etapa <td>[<b>'.MontaOrdemEtapa(f($RS,'sq_projeto_etapa')).' - '.f($RS,'titulo').'</b>]';
      } 
      if ($p_sqcc>'') {
      ...
      if ($p_chave>'') $w_filtro = $w_filtro.'<tr valign="top"><td align="right">Atividade nº <td>[<b>'.$p_chave.'</b>]';
    ...
    if ($w_copia>'') {
      ...
      switch ($_REQUEST['p_agrega']) {
        case 'GRDMRESPATU': $RS->Filter='executor <> null';  break;
      } 
    } 
    if (Nvl($p_ordena,'')>'') {
  ...
  if ($O=='L') {
    ...
    if (count($RS)<=0) {
      ...
      foreach($RS1 as $row) {
        ...
        ShowHTML('        <td><A class="HL" HREF="'.$w_dir.'projeto.php?par=Visual&O=L&w_chave='.f($row,'sq_solic_pai').'&w_tipo=Volta&P1=2&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'" title="Exibe as informações do projeto.">'.f($row,'nm_projeto').'</a></td>');
        ...
        if ($_REQUEST['p_tamanho']=='N') {
          ...
          if (f($row,'sg_tramite')=='CA') {
            ...
            ShowHTML('        <td><strike>'.Nvl(f($row,'assunto'),'-').'</strike></td>');
          } else {
            //, '"'), '''', '"'),VbCrLf,'\n') & '''>' & w_titulo & '</td>'
            ShowHTML('        <td>'.Nvl(f($row,'assunto'),'-').'</td>');
          } 
        ...
        if ($P1!=3) {
          ...
          if ($w_copia>'') {
            ...
            if ($w_submenu>'') {
              ShowHTML('          <A class="HL" HREF="menu.php?par=ExibeDocs&O=A&w_chave='.f($row,'sq_siw_solicitacao').'&R='.$w_pagina.$par.'&SG='.$SG.'&TP='.$TP.'&w_documento=Nr. '.f($row,'sq_siw_solicitacao').MontaFiltro('GET').'" title="Altera as informações cadastrais da tarefa" TARGET="menu">Alterar</a>&nbsp;');
            }  else {
    ...
    SelecaoProjeto('Açã<u>o</u>:','O','Selecione a ação da tarefa na relação.',$p_projeto,$w_usuario,f($RS,'sq_menu'),'p_projeto','PJLIST','onChange="document.Form.action=\''.$w_dir.$w_pagina.$par.'\'; document.Form.O.value=\''.$O.'\'; document.Form.w_troca.value=\'p_atividade\'; document.Form.submit();"');
    ShowHTML('      </tr>');
    ShowHTML('      <tr>');
    ...
    if ($P1!=1 || $O=='C') {
      ...
      ShowHTML('          <td valign="top"><b>Parcerias exter<u>n</u>as:<br><INPUT ACCESSKEY="N" '.$w_Disabled.' class="STI" type="text" name="p_proponente" size="25" maxlength="90" value="'.$p_proponente.'"></td>');
      ShowHTML('      <tr>');
    ...
    if ($p_ordena=='ASSUNTO')          ShowHTML('          <option value="assunto" SELECTED>Detalhamento<option value="inicio">Data de recebimento<option value="">Data limite para conclusão<option value="nm_tramite">Fase atual<option value="prioridade">Prioridade<option value="proponente">Parcerias externas');
    elseif ($p_ordena=='INICIO')       ShowHTML('          <option value="assunto">Detalhamento<option value="inicio" SELECTED>Data de recebimento<option value="">Data limite para conclusão<option value="nm_tramite">Fase atual<option value="prioridade">Prioridade<option value="proponente">Parcerias externas');
    elseif ($p_ordena=='NM_TRAMITE')   ShowHTML('          <option value="assunto">Detalhamento<option value="inicio">Data de recebimento<option value="">Data limite para conclusão<option value="nm_tramite" SELECTED>Fase atual<option value="prioridade">Prioridade<option value="proponente">Parcerias externas');
    elseif ($p_ordena=='PRIORIDADE')   ShowHTML('          <option value="assunto">Detalhamento<option value="inicio">Data de recebimento<option value="">Data limite para conclusão<option value="nm_tramite">Fase atual<option value="prioridade" SELECTED>Prioridade<option value="proponente">Parcerias externas');
    elseif ($p_ordena=='PROPONENTE')   ShowHTML('          <option value="assunto">Detalhamento<option value="inicio">Data de recebimento<option value="">Data limite para conclusão<option value="nm_tramite">Fase atual<option value="prioridade">Prioridade<option value="proponente" SELECTED>Parcerias externas');
    else                               ShowHTML('          <option value="assunto">Detalhamento<option value="inicio">Data de recebimento<option value="" SELECTED>Data limite para conclusão<option value="nm_tramite">Fase atual<option value="prioridade">Prioridade<option value="proponente">Parcerias externas');
    ShowHTML('          </select></td>');
...
function Geral() {
  ...
  if (!(strpos('IAEV',$O)===false)) {
    ...
    ShowHTML('      <tr><td align="center" height="1" bgcolor="#000000"></td></tr>');
    ShowHTML('      <tr><td>Os dados deste bloco serão utilizados para identificação da tarefa, bem como para o controle de sua execução.</td></tr>');
    ShowHTML('      <tr><td align="center" height="1" bgcolor="#000000"></td></tr>');
    // Recupera dados da opção Projetos
    ShowHTML('      <tr>');
    $RS = db_getLinkData::getInstanceOf($dbms,$w_cliente,'ORCAD');
    SelecaoProjeto('Açã<u>o</u>:','O','Selecione a ação a qual a tarefa está vinculda.',$w_projeto,$w_usuario,f($RS,'sq_menu'),'w_projeto','PJLISTCAD',null);
    ShowHTML('      </tr>');
    //ShowHTML '      <tr>'
    ...
    //ShowHTML '      </tr>'
    ShowHTML('      <tr><td valign="top"><b>Detalh<u>a</u>mento:</b><br><textarea '.$w_Disabled.' accesskey="A" name="w_assunto" class="STI" ROWS=5 cols=75 title="Escreva um texto de detalhamento para esta tarefa.">'.$w_assunto.'</TEXTAREA></td>');
    if (f($RS_Menu,'solicita_cc')=='S') {
      ShowHTML('          <tr>');
    ...
    ShowHTML('          </table>');
    ShowHTML('      <tr><td valign="top" colspan="2"><table border=0 width="100%" cellspacing=0>');
    ShowHTML('              <td align="left"><b><u>O</u>rdem:<br><INPUT ACCESSKEY="O" TYPE="TEXT" CLASS="STI" NAME="w_ordem" SIZE=3 MAXLENGTH=3 VALUE="'.$w_ordem.'" '.$w_Disabled.'></td>');
    switch (f($RS_Menu,'data_hora')) {
      case 1: ShowHTML('              <td valign="top"><b>Fim previs<u>t</u>o:</b><br><input '.$w_Disabled.' accesskey="T" type="text" name="w_fim" class="STI" SIZE="10" MAXLENGTH="10" VALUE="'.$w_fim.'" onKeyDown="FormataData(this,event);"></td>');      break;
      case 2: ShowHTML('              <td valign="top"><b>Fim previs<u>t</u>o:</b><br><input '.$w_Disabled.' accesskey="T" type="text" name="w_fim" class="STI" SIZE="17" MAXLENGTH="17" VALUE="'.$w_fim.'" onKeyDown="FormataDataHora(this,event);"></td>');  break;
      case 3: ShowHTML('              <td valign="top"><b>Iní<u>c</u>io previsto:</b><br><input '.$w_Disabled.' accesskey="C" type="text" name="w_inicio" class="STI" SIZE="10" MAXLENGTH="10" VALUE="'.Nvl($w_inicio,FormataDataEdicao(time())).'" onKeyDown="FormataData(this,event);"></td>');
              ShowHTML('              <td valign="top"><b>Fim previs<u>t</u>o:</b><br><input '.$w_Disabled.' accesskey="T" type="text" name="w_fim" class="STI" SIZE="10" MAXLENGTH="10" VALUE="'.$w_fim.'" onKeyDown="FormataData(this,event);"></td>');
        break;
      case 4: ShowHTML('              <td valign="top"><b>Iní<u>c</u>io previsto:</b><br><input '.$w_Disabled.' accesskey="C" type="text" name="w_inicio" class="STI" SIZE="17" MAXLENGTH="17" VALUE="'.$w_inicio.'" onKeyDown="FormataDataHora(this,event);"></td>');
              ShowHTML('              <td valign="top"><b>Fim previs<u>t</u>o:</b><br><input '.$w_Disabled.' accesskey="T" type="text" name="w_fim" class="STI" SIZE="17" MAXLENGTH="17" VALUE="'.$w_fim.'" onKeyDown="FormataDataHora(this,event);"></td>');
        break;
    } 
    ShowHTML('              <td valign="top"><b><u>R</u>ecurso programado:</b><br><input '.$w_Disabled.' accesskey="O" type="text" name="w_valor" class="STI" SIZE="18" MAXLENGTH="18" VALUE="'.$w_valor.'" onKeyDown="FormataValor(this,18,2,event);" title="Informe o recurso programado para execução da tarefa, ou zero se não for o caso."></td>');
    SelecaoPrioridade('<u>P</u>rioridade:','P','Informe a prioridade desta tarefa.',$w_prioridade,null,'w_prioridade',null,null);
    ShowHTML('          </table>');
    ShowHTML('      <tr><td valign="top"><b>Parc<u>e</u>rias externas:<br><INPUT ACCESSKEY="E" '.$w_Disabled.' class="STI" type="text" name="w_proponente" size="90" maxlength="90" value="'.$w_proponente.'" title="Parcerias externas necessárias ao cumprimento da tarefa. Preencha apenas se houver."></td>');
    ShowHTML('      <tr><td valign="top"><b>Responsáve<u>l</u>:<br><INPUT ACCESSKEY="L" '.$w_Disabled.' class="STI" type="text" name="w_palavra_chave" size="90" maxlength="90" value="'.$w_palavra_chave.'" title="Informe o responsável pela tarefa"></td>');
    //ShowHTML '      <tr><td align=''center'' height=''2'' bgcolor=''#000000''></td></tr>'
    //ShowHTML '      <tr><td align=''center'' height=''1'' bgcolor=''#000000''></td></tr>'
    ...
    if (f($RS_Menu,'descricao')=='S' || f($RS_Menu,'justificativa')=='S') {
      ...
      ShowHTML('      <tr><td align="center" height="1" bgcolor="#000000"></td></tr>');
      ShowHTML('      <tr><td>Os dados deste bloco visam orientar os executores da tarefa.</td></tr>');
      ShowHTML('      <tr><td align="center" height="1" bgcolor="#000000"></td></tr>');
      if (f($RS_Menu,'descricao')=='S') {
        ShowHTML('      <tr><td valign="top"><b>Res<u>u</u>ltados esperados:</b><br><textarea '.$w_Disabled.' accesskey="U" name="w_descricao" class="STI" ROWS=5 cols=75 title="Descreva os resultados físicos esperados com a execução da tarefa.">'.$w_descricao.'</TEXTAREA></td>');
      } 
      if (f($RS_Menu,'justificativa')=='S') {
        ShowHTML('      <tr><td valign="top"><b>Obse<u>r</u>vações:</b><br><textarea '.$w_Disabled.' accesskey="R" name="w_justificativa" class="STI" ROWS=5 cols=75 >'.$w_justificativa.'</TEXTAREA></td>');
      } 
    } 
    //ShowHTML '      <tr><td align=''center'' height=''2'' bgcolor=''#000000''></td></tr>'
    //ShowHTML '      <tr><td align=''center'' height=''1'' bgcolor=''#000000''></td></tr>'
    ...
    //ShowHTML '      <tr><td align=''center'' height=''1'' bgcolor=''#000000''></td></tr>'
    //ShowHTML '      <tr><td>Os dados abaixo indicam como deve ser tratada a proximidade da data limite para conclusão da tarefa.</td></tr>'
    //ShowHTML '      <tr><td align=''center'' height=''1'' bgcolor=''#000000''></td></tr>'
    //ShowHTML '      <tr><td><table border=''0'' width=''100'''>'
    //ShowHTML '          <tr>'
    //MontaRadioNS '<b>Emite alerta?</b>', w_aviso, 'w_aviso'
    //ShowHTML '              <td valign=''top''><b>Quantos <U>d</U>ias antes da data limite?<br><INPUT ACCESSKEY=''D'' ' & w_Disabled & ' class=''STI'' type=''text'' name=''w_dias'' size=''2'' maxlength=''2'' value=''' & w_dias & ''' title=''Número de dias para emissão do alerta de proximidade da data limite para conclusão da tarefa.''></td>'
    //ShowHTML '          </table>'
  ...
  if ($O=='L') {
    ...
    if (count($RS)<=0) {
      ...
      foreach($RS as $row) {
        ...
        ShowHTML('        <td>'.RetornaTipoVisao(f($RS,'tipo_visao')).'</td>');
        ShowHTML('        <td align="center">'.str_replace('N','Não',str_replace('S','Sim',f($RS,'envia_email'))).'</td>');
        ShowHTML('        <td align="top" nowrap>');
        ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.$par.'&R='.$w_pagina.$par.'&O=A&w_chave='.$w_chave.'&w_chave_aux='.f($RS,'sq_pessoa').'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'">Alterar</A>&nbsp');
        ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.'GRAVA&R='.$w_pagina.$par.'&O=E&w_chave='.$w_chave.'&w_chave_aux='.f($RS,'sq_pessoa').'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'" onClick="return confirm(\'Confirma a exclusão do registro?\');">Excluir</A>&nbsp');
        ShowHTML('        </td>');
        ShowHTML('      </tr>');
        $RS->MoveNext;
      } 
  ...
  if ($O=='L') {
    ...
    if (count($RS)<=0) {
      ...
      foreach($RS as $row) {
        ...
        ShowHTML('        <td align="top" nowrap>');
        ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.$par.'&R='.$w_pagina.$par.'&O=A&w_chave='.$w_chave.'&w_chave_aux='.f($row,'sq_unidade').'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'">Alterar</A>&nbsp');
        ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.'GRAVA&R='.$w_pagina.$par.'&O=E&w_chave='.$w_chave.'&w_chave_aux='.f($row,'sq_unidade').'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'" onClick="return confirm(\'Confirma a exclusão do registro?\');">Excluir</A>&nbsp');
        ShowHTML('        </td>');
        ShowHTML('      </tr>');
      } 
    } 
    ...
    ShowHTML('      <tr><td valign="top"><b><u>P</u>apel desempenhado:</b><br><textarea '.$w_Disabled.' accesskey="P" name="w_papel" class="STI" ROWS=5 cols=75 title="Descreva o papel desempenhado pela área ou instituição na execução da tarefa.">'.$w_papel.'</TEXTAREA></td>');
    ShowHTML('          </table>');
    ShowHTML('      <tr><td align="center" colspan=4><hr>');
    if ($O=='E') {
      ShowHTML('   <input class="STB" type="submit" name="Botao" value="Excluir">');
...
function Visual(){
  ...
  if ($w_tipo=='WORD') header('Content-type: '.'application/msword');  else cabecalho(); 
  ShowHTML('<HEAD>');
...
function Excluir() {
  ...
  $RS = db_getSolicData::getInstanceOf($dbms,$w_chave,'ORPGERAL');
...
function Encaminhamento() {
  ...
  ShowHTML('    <tr><td valign="top" colspan=2><b>D<u>e</u>spacho:</b><br><textarea '.$w_Disabled.' accesskey="E" name="w_despacho" class="STI" ROWS=5 cols=75 title="Descreva o papel desempenhado pela área ou instituição na execução da tarefa.">'.$w_despacho.'</TEXTAREA></td>');
  ShowHTML('      </table>');
  ShowHTML('      <tr><td align="LEFT" colspan=4><b><U>A</U>ssinatura Eletrônica:<BR> <INPUT ACCESSKEY="A" class="STI" type="PASSWORD" name="w_assinatura" size="30" maxlength="30" value=""></td></tr>');
  ShowHTML('    <tr><td align="center" colspan=4><hr>');
...
function Concluir(){
  ...
  ShowHTML('          <tr>');
  switch (f($RS_Menu,'data_hora')) {
    case 1:    ShowHTML('              <td valign="top"><b><u>T</u>érmino da execução:</b><br><input '.$w_Disabled.' accesskey="T" type="text" name="w_fim_real" class="STI" SIZE="10" MAXLENGTH="10" VALUE="'.$w_fim_real.'" onKeyDown="FormataData(this,event);" title="Informe a data de término da execução da tarefa."></td>');             break;
    case 2:    ShowHTML('              <td valign="top"><b><u>T</u>érmino da execução:</b><br><input '.$w_Disabled.' accesskey="T" type="text" name="w_fim_real" class="STI" SIZE="17" MAXLENGTH="17" VALUE="'.$w_fim_real.'" onKeyDown="FormataDataHora(this,event);" title="Informe a data/hora de término da execução da tarefa."></td>');    break;
    case 3:    ShowHTML('              <td valign="top"><b>Iní<u>c</u>io da execução:</b><br><input '.$w_Disabled.' accesskey="C" type="text" name="w_inicio_real" class="STI" SIZE="10" MAXLENGTH="10" VALUE="'.$w_inicio_real.'" onKeyDown="FormataData(this,event);" title="Informe a data/hora de início da execução da tarefa."></td>');
               ShowHTML('              <td valign="top"><b><u>T</u>érmino da execução:</b><br><input '.$w_Disabled.' accesskey="T" type="text" name="w_fim_real" class="STI" SIZE="10" MAXLENGTH="10" VALUE="'.$w_fim_real.'" onKeyDown="FormataData(this,event);" title="Informe a data de término da execução da tarefa."></td>');
      break;
    case 4:    ShowHTML('              <td valign="top"><b>Iní<u>c</u>io da execução:</b><br><input '.$w_Disabled.' accesskey="C" type="text" name="w_inicio_real" class="STI" SIZE="17" MAXLENGTH="17" VALUE="'.$w_inicio_real.'" onKeyDown="FormataDataHora(this,event);" title="Informe a data/hora de início da execução da tarefa."></td>');
               ShowHTML('              <td valign="top"><b><u>T</u>érmino da execução:</b><br><input '.$w_Disabled.' accesskey="T" type="text" name="w_fim_real" class="STI" SIZE="17" MAXLENGTH="17" VALUE="'.$w_fim_real.'" onKeyDown="FormataDataHora(this,event);" title="Informe a data de término da execução da tarefa."></td>');
      break;
  } 
  ShowHTML('              <td valign="top"><b><u>R</u>ecurso executado:</b><br><input '.$w_Disabled.' accesskey="O" type="text" name="w_custo_real" class="STI" SIZE="18" MAXLENGTH="18" VALUE="'.$w_custo_real.'" onKeyDown="FormataValor(this,18,2,event);" title="Informe o recurso utilizado para execução da tarefa, ou zero se não for o caso."></td>');
  ShowHTML('          </table>');
...
function SolicMail($p_solic,$p_tipo) {
  ...
  if ($p_tipo==1) {
    $w_html=$w_html.'      <tr valign="top"><td align="center"><font size=2><b>INCLUSÃO DE TAREFA</b></font><br><br><td></tr>'.chr(13);
  } elseif ($p_tipo==2) {
    $w_html=$w_html.'      <tr valign="top"><td align="center"><font size=2><b>TRAMITAÇÃO DE TAREFA</b></font><br><br><td></tr>'.chr(13);
  } elseif ($p_tipo==3) {
    $w_html=$w_html.'      <tr valign="top"><td align="center"><font size=2><b>CONCLUSÃO DE TAREFA</b></font><br><br><td></tr>'.chr(13);
  } 
  ...
  // Recupera os dados da tarefa
  $RS = db_getSolicData::getInstanceOf($dbms,$p_solic,'GDGERAL');
  ...
  $w_html = $w_html.chr(13).'      <tr><td valign="top">Ação: <b>'.f($RSM,'nm_projeto').'</b></td>';
  ...
  $w_html=$w_html.chr(13).'      <tr><td>Detalhamento: <b>'.CRLF2BR(f($RSM,'assunto')).'</b></td></tr>';
  // Identificação da tarefa
  $w_html=$w_html.chr(13).'      <tr><td valign="top" colspan="2" align="center" bgcolor="#D0D0D0" style="border: 2px solid rgb(0,0,0);"><b>EXTRATO DA TAREFA</td>';
  ...
  $w_html = $w_html.chr(13).'          <td>Responsável pelo monitoramento:<br><b>'.f($RSM,'nm_sol').'</b></td>';
  $w_html = $w_html.chr(13).'          <td>Unidade responsável pelo monitoramento:<br><b>'.f($RSM,'nm_unidade_resp').'</b></td>';
  $w_html = $w_html.chr(13).'          <tr valign="top">';
  ...
  if (Nvl(f($RSM,'descricao'),'')>'') {
    if (Nvl(f($RSM,'descricao'),'')>'') {
      $w_html = $w_html.chr(13).'      <tr><td valign="top">Resultados da tarefa:<br><b>'.CRLF2BR(f($RSM,'descricao')).' </b></td>';
    }
    $w_html = $w_html.chr(13).'    </table>';
    ...
    if ($p_tipo==2) {
      ...
      $w_html = $w_html.chr(13).'      <tr><td valign="top" colspan="2"><table border=0 width="100%" cellspacing=0>';
      $w_html = $w_html.chr(13).'          <tr valign="top">';
      $w_html = $w_html.chr(13).'          <td>De:<br><b>'.f($RS,'responsavel').'</b></td>';
      $w_html = $w_html.chr(13).'          <td>Para:<br><b>'.f($RS,'destinatario').'</b></td>';
      $w_html = $w_html.chr(13).'          <tr valign="top"><td colspan=2>Despacho:<br><b>'.CRLF2BR(Nvl(f($RS,'despacho'),'---')).' </b></td>';
      $w_html = $w_html.chr(13).'          </table>';
    ...
    $_REQUEST['p_cliente'.'_session']=$w_html=$w_html.'      <tr valign="top"><td><font size=2>'.chr(13);
    ...
    $w_html = $w_html.'         <li>Data do servidor: <b>'.FormataDataEdicao(time(),3).'</b></li>'.chr(13);
    ...
    case 'ORPGERAL':
      ...
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {   
        ...
        dml_putDemandaGeral::getInstanceOf($dbms,$O,
          ...
          $_SESSION['SQ_PESSOA'],null,$_REQUEST['w_sqcc'],$_REQUEST['w_descricao'],$_REQUEST['w_justificativa'],$_REQUEST['w_ordem'],$_REQUEST['w_inicio'],$_REQUEST['w_fim'],$_REQUEST['w_valor'],
        ...
        if ($O=='I') {
          ...
          ShowHTML('  location.href=\''.$R.'&O=L&R='.$R.'&SG=ORPCAD&w_menu='.$w_menu.'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&p_proponente='.$_REQUEST['p_proponente'].MontaFiltro('GET').'\';');
        } else {
    ...
    case 'ORPAREAS':
      ...
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') { 
        if ($O=='I') { 
          ...
          foreach ($RS as $row) {
            if ($_REQUEST['w_chave_aux']==f($row,'sq_unidade')) {
              ScriptOpen('JavaScript');
              ShowHTML('  alert(\'Área já cadastrada!\');');
              ShowHTML('  history.back(1);');
              ScriptClose();
              exit();
            }
      ...
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {  
        ...
        if (f($RS,'sq_siw_tramite')!=$_REQUEST['w_tramite']) { 
          ...
          if (UPLOAD_ERR_OK==0) {
            ...
            dml_putDemandaConc::getInstanceOf($dbms,$w_menu,$_REQUEST['w_chave'],$w_usuario,$_REQUEST['w_tramite'],
                $_REQUEST['w_inicio_real'],$_REQUEST['w_fim_real'],$_REQUEST['w_nota_conclusao'],$_REQUEST['w_custo_real'],
                $w_file,$w_tamanho,$w_tipo,$w_nome);              
          } else {
            ScriptOpen('JavaScript');
            ShowHTML('  alert(\'ATENÇÃO: ocorreu um erro na transferência do arquivo. Tente novamente!\');');
...
function Main() {
  ...
  if ((strlen($LOTACAO_session.'')==0 || strlen($LOCALIZACAO_session.'')==0) && $_REQUEST['LogOn'.'_session']=='Sim') {
    ScriptOpen('JavaScript');
