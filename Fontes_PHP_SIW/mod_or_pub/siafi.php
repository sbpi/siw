...
function Inicial() {
  ...
  if ($w_troca  >'') {
    ...
    $RS= SortArray($RS,'data_arquivo','desc');
    ...
    $w_data_arquivo     = f($RS,'data_arquivo');
    $w_arquivo_recebido = f($RS,'arquivo_recebido');
    $w_arquivo_registro = f($RS,'arquivo_registro');
    $w_registros        = f($RS,'registros');
    $w_importados       = f($RS,'importados');
    $w_rejeitados       = f($RS,'rejeitados');
  ...
  elseif (!(strpos('IA',$O) ===false))  BodyOpen('onLoad=\'document.Form.w_data_arquivo.focus()\';');
  ...
  if ($O=='L') {
    ...
    if (count($RS)<=0) {
      ...
      ShowHTML('          '.LinkArquivo('HL',$w_cliente,f($row,'chave_result'),'_blank','Exibe o registro de importação.','Registro',null).'&nbsp');
      ShowHTML('        </td>');
      ShowHTML('      </tr>');
      }   
    } 
  ...
  if ($O=='E') ShowHTML('          <input class="STB" type="submit" name="Botao" value="Excluir">');
    else         ShowHTML('          <input class="STB" type="submit" name="Botao" value="Incluir">');  
...
/*function Grava() {
  ...
  case 'ORIMPSIAFI':
    ...
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
      ...
      if (UPLOAD_ERR_OK==0) {
        ...
        foreach ($_FILES as $Chv => $Field) {
          if ($Field['size'] > 0) {
            ...
            $w_caminho_recebido = str_replace('.tmp',substr($Field['name'],(strpos($Field['name'],'.') ? strpos($Field['name'],'.')+1 : 0)-1,30));
         ...
         // $F1->WriteLine'(Linha '.$w_cont.') '.$w_linha;
          ...
          if ($w_erro==0) {
            ...
            if (count($RS)<=0) {
              ...
              dml_putAcaoPPA::getInstanceOf($dbms,'U',
                null,$w_cliente,null,null,null,null,null,null,null,null,$w_dotacao,null,
                $w_empenhado,$w_liquidado,null,null,null,null,$w_programa,$acao);
            } 
          } 
          $w_registros = $w_registros+1;
          if ($w_erro==0) $w_importados = $w_importados+1;
          else            $w_rejeitados = $w_rejeitados+1;         
        } 
      ...
      ScriptOpen('JavaScript');
      ShowHTML('  location.href=\''.$R.'&w_chave='.$_REQUEST['w_chave'].'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('UL').'\';');
      ScriptClose();
    } else {
      ScriptOpen('JavaScript');
