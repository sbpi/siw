...
if ($RS_menu('ultimo_nivel')='S')  {
  ...
  $RS = db_getMenuData::getInstanceOf($dbms,f($RS_menu'sq_menu_pai'));
} 
...
function PPA() {
  ...
  if ($w_troca>'') {
    ...
    // Recupera os dados do endereço informado
    $RS = db_getAcaoPPA::getInstanceOf($dbms,$w_chave,$w_cliente,null,null,null,null,null,null,null,null);
    $w_codigo=f($RS,'codigo');
    $w_nome=f($RS,'nome');
    $w_responsavel=f($RS,'responsavel');
    $w_telefone=f($RS,'telefone');
    ...
    $w_ativo=f($RS,'ativo');
    $w_padrao=f($RS,'padrao');
    $w_aprovado=$FormatNumber[f($RS,'aprovado')][2];
    $w_saldo=$FormatNumber[f($RS,'saldo')][2];
    $w_empenhado=$FormatNumber[f($RS,'empenhado')][2];
    $w_liquidado=$FormatNumber[f($RS,'liquidado')][2];
    $w_liquidar=$FormatNumber[f($RS,'liquidar')][2];
  ...
  if ($O=='L') {
    ...
    ShowHTML('      <tr bgcolor="'.$conTrBgColor.'"><td colspan=4 align="center"><font size="1"><b>Não foram encontrados registros.</b></td></tr>');
      ...
      foreach ($RS as $rwow) {
        $w_cor = ($w_cor==$conTrBgColor || $w_cor=='') ? $w_cor=$conTrAlternateBgColor : $w_cor=$conTrBgColor;
        ShowHTML('      <tr bgcolor="'.$w_cor.'" valign="top">');
        if (Nvl(f($RS,'sq_acao_ppa_pai'),'')=='') {
          ShowHTML('        <td><font size="1"><b>'.f($RS,'codigo').'</td>');
        } else {
          ShowHTML('        <td><font size="1">&nbsp;&nbsp;'.f($RS,'codigo').'</td>');
        } 
        ShowHTML('        <td><font size="1">'.f($RS,'nome').'</td>');
        ShowHTML('        <td align="center"><font size="1">'.f($RS,'nm_ativo').'</td>');
        ShowHTML('        <td align="center"><font size="1">'.f($RS,'nm_padrao').'</td>');
        ShowHTML('        <td align="top" nowrap><font size="1">');
        ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.$par.'&R='.$w_pagina.$par.'&O=A&w_chave='.f($RS,'chave').'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'">Alterar</A>&nbsp');
...
function Rel_PPA() {
  ...
  if ($O=='L') {
    ...
    // Recupera todos os registros para a listagem
    $RS = db_getAcaoPPA::getInstanceOf($dbms,null,$w_cliente,$p_sq_acao_ppa_pai,$p_sq_acao_ppa,$p_responsavel,$p_selecionada_mpog,$p_selecionada_relevante,null,null,null);
    $RS = SortArray($RS,'ordena','asc');
  } 
  if ($w_tipo_rel=='WORD') {
    HeaderWord(null);
  ...
  if ($O=='L') {
    ...
    // Exibe a quantidade de registros apresentados na listagem e o cabeçalho da tabela de listagem
    $w_filtro='';
    if ($p_responsavel>'') $w_filtro=$w_filtro.'<tr valign="top"><td align="right"><font size=1>Responsável<td><font size=1>[<b>'.$p_responsavel.'</b>]';
    ...
    if ($p_selecionada_mpog>'') $w_filtro=$w_filtro.'<tr valign="top"><td align="right"><font size=1>Selecionada MP<td><font size=1>[<b>'.$p_selecionada_mpog.'</b>]';
    if ($p_selecionada_relevante>'') $w_filtro=$w_filtro.'<tr valign="top"><td align="right"><font size=1>Selecionada Relevante<td><font size=1>[<b>'.$p_selecionada_relevante.'</b>]';
    if ($p_tarefas_atraso>'') $w_filtro=$w_filtro.'<td><font size=1>Ações com tarefas em atraso&nbsp;<font size=1>[<b>'.$p_tarefas_atraso.'</b>]&nbsp;';
    ...
    if (count($RS)<=0) {
        ...
        if (Nvl(f($RS,'sq_acao_ppa_pai'),'')=='') {
          if ($w_atual>'') {
            if (!(strpos($p_campos,'aprovado')===false)) || (!(strpos($p_campos,'saldo')===false)) || (!(strpos($p_campos,'empenhado')===false)) || (!(strpos($p_campos,'liquidado')===false)) || (!(strpos($p_campos,'liquidar')===false)) {
              ...
              if (!(strpos($p_campos,'aprovado')===false)) {
                ShowHTML('        <td align="right"><font size="1">'.$FormatNumber[$w_acao_aprovado][2].'</td>');        
              } if (!(strpos($p_campos,'empenhado')===false)) {
                ShowHTML('        <td align="right"><font size="1">'.$FormatNumber[$w_acao_empenhado][2].'</td>');        
              } if (!(strpos($p_campos,'saldo')===false)) {
                ShowHTML('        <td align="right"><font size="1">'.$FormatNumber[$w_acao_saldo][2].'</td>');       
              } if (!(strpos($p_campos,'liquidado')===false)) {
                ShowHTML('        <td align="right"><font size="1">'.$FormatNumber[$w_acao_liquidado][2].'</td>');         
              } if (!(strpos($p_campos,'liquidar')===false)) {
                ShowHTML('        <td align="right"><font size="1">'.$FormatNumber[$w_acao_liquidar][2].'</td>');        
              }
              ShowHTML('      </tr>');
          ...
          ShowHTML('        <td><font size="1"><b>'.f($RS,'codigo').'</td>');
          ShowHTML('        <td><font size="1"><b>'.f($RS,'nome').'</td>');
          if ((strpos($p_campos,'responsavel')===false)) {
            ShowHTML('        <td><font size="1">'.Nvl(f($RS,'responsavel'),'---').'</td>');    
          } if ((strpos($p_campos,'email')===false)) {
            ShowHTML('        <td><font size="1">'.Nvl(f($RS,'email'),'---').'</td>');     
          } if ((strpos($p_campos,'telefone')===false)) {
            ShowHTML('        <td><font size="1">'.Nvl(f($RS,'telefone'),'---').'</td>');     
          } if ((strpos($p_campos,'aprovado') ===false)) || (!(strpos($p_campos,'saldo')===false)) || (!(strpos($p_campos,'empenhado')===false)) || (!(strpos($p_campos,'liquidado')===false)) || (!(strpos($p_campos,'liquidar')===false)) {
          ...
          if (!(strpos($p_campos,'responsavel')===false))  {
            ...
            ShowHTML('        <td align="right"><font size="1">'.$FormatNumber[f($RS,'aprovado')][2].'</td>');    
          } if (!(strpos($p_campos,'empenhado')===false)) {
            ShowHTML('        <td align="right"><font size="1">'.$FormatNumber[f($RS,'empenhado')][2].'</td>');    
          } if (!(strpos($p_campos,'saldo')===false)) {
            ShowHTML('        <td align="right"><font size="1">'.$FormatNumber[f($RS,'aprovado')-f($RS,'empenhado')][2].'</td>');    
          } if (!(strpos($p_campos,'liquidado'===false)) {
            ShowHTML('        <td align="right"><font size="1">'.$FormatNumber[f($RS,'liquidado')][2].'</td>');    
          } if (!(strpos($p_campos,'liquidar'===false)) {
            ShowHTML('        <td align="right"><font size="1">'.$FormatNumber[f($RS,'empenhado')-f($RS,'liquidado')][2].'</td>');    
          }
          $w_linha+=1;
          ShowHTML('</tr>');
          if ($p_metas>'') {
            ...
            $RS2 = SortArray ($RS2,'fim','asc','prioridade','asc');
            if (count($RS2)<=0) {
              ...
              if (count($RS3)<=0){
                ...
                $w_linha+=1;
                foreach ($RS3 as $row) {
                  ShowHtml(EtapaLinha(f($RS2,'sq_siw_solicitacao'),f($RS3,'sq_projeto_etapa'),f($RS3,'titulo'),$w_tipo_rel,f($RS3,'programada'),f($RS3,'unidade_medida'),f($RS3,'quantidade'),f($RS3,'fim_previsto'),f($RS3,'perc_conclusao'),'S','PROJETO'));
                  $w_linha=$w_linha+1;
                } 
              } 
            } 
            ...
            $RS2 = db_getSolicList::getInstanceOf($dbms,f($RS1,'sq_menu'),$w_usuario,f($RS1,'sigla'),5,
            null,null,null,null,null,null,null,null,null,null);
            $RS2 = SortArray=('fim','asc','prioridade','asc');
            if (count($RS2)<=0) {
              ...
              if (count($RS3)<=0) {
                ...
                foreach ($RS3 as $row) {
                  ...
                  if (f($RS3,'concluida')=='N') {
                    ...
                    if (f($RS3,'fim')<Nvl(f($RS3,'fim_real'),f($RS3,'fim'))) {
                  ...
                  ShowHTML('        <A class="HL" HREF="'.$w_dir.'ProjetoAtiv.php?par=Visual&R='.$w_pagina.$par.'&O=L&w_chave='.f($RS3,'sq_siw_solicitacao').'&w_tipo=&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'" TARGET="VisualTarefa" title="Exibe as informações desta tarefa.">'.f($RS3,'sq_siw_solicitacao').'&nbsp;</a>');
                  if (strlen(Nvl(f($RS3,'assunto'),'-'))>50) {
                    $w_titulo=substr(Nvl(f($RS3,'assunto'),'-'),0,50).'...';
                  } else {
                    $w_titulo=Nvl(f($RS3,'assunto'),'-');
                  }
                } 
                ShowHTML('        <td><font size="1">'.$w_titulo.'</td>');
                ShowHTML('        <td><font size="1">'.f($RS3,'nm_solic').'</td>');
                ShowHTML('        <td><font size="1">'.Nvl(f($RS3,'proponente'),'---').'</td>');
                ShowHTML('        <td align="center"><font size="1">&nbsp;'.Nvl($FormatDateTime[f($RS3,'fim')][2],'-').'</td>');
                ShowHTML('        <td align="right"><font size="1">'.$FormatNumber[f($RS3,'valor')][2].'&nbsp;</td>');
                ShowHTML('        <td align="right"><font size="1">'.$FormatNumber[f($RS3,'custo_real')][2].'&nbsp;</td>');
                ShowHTML('        <td nowrap><font size="1">'.f($RS3,'nm_tramite').'</td>');
                if ($p_prioridade=='') {
                  ShowHTML('<td nowrap><font size="1">'.RetornaPrioridade(f($RS3,'prioridade')).'</td>');         
                }
                ShowHTML('        </td>');
                ShowHTML('      </tr>');
                $w_linha+=1;
          ...
          if (count($RS2)<=0) {
            ...
            foreach ($RS2 as $row) {
              ShowHTML('        <tr bgcolor="'.$conTrBgColor.'" align="center">');
              ShowHTML('          <td align="left"><font size="1"><b>'.f($row,'nm_unidade_resp').'</font></td>');
              ShowHTML('        </tr>');
              $w_linha+=1;
            } 
          } 
      ...
      ShowHTML('      </tr>');
      $w_acao_aprovado  = $w_acao_aprovado+f($RS,'aprovado');
      $w_acao_saldo     = $w_acao_saldo+f($RS,'aprovado')-f($RS,'empenhado');
      $w_acao_empenhado = $w_acao_empenhado+f($RS,'empenhado');
      $w_acao_liquidado = $w_acao_liquidado+f($RS,'liquidado');
      $w_acao_liquidar  = $w_acao_liquidar+f($RS,'empenhado')-f($RS,'liquidado');  
      $w_tot_aprovado   = $w_tot_aprovado+f($RS,'aprovado');
      $w_tot_saldo      = $w_tot_saldo+f($RS,'aprovado')-f($RS,'empenhado');
      $w_tot_empenhado  = $w_tot_empenhado+f($RS,'empenhado');
      $w_tot_liquidado  = $w_tot_liquidado+f($RS,'liquidado');
      $w_tot_liquidar   = $w_tot_liquidar+f($RS,'empenhado')-f($RS,'liquidado');
    } if (!(strpos($p_campos,'aprovado')===false) || (!(strpos($p_campos,'saldo')===false)) || (!(strpos($p_campos,'empenhado')===false)) || (!(strpos($p_campos,'liquidado')===false)) || (!(strpos($p_campos,'liquidar')===false))) {
      if (!$p_sq_acao_ppa>' ') {
        ShowHTML('      <tr bgcolor="'.$conTrBgColor.'" valign="top">');
        ShowHTML('        <td colspan='.$w_col.' align="right"><font size="1"><b>Totais do programa <b>'.$w_atual.'</b></td>');
        if ((strpos($p_campos,'aprovado') ? strpos($p_campos,'aprovado')+1 : 0)) {
          ShowHTML('        <td align="right"><font size="1">'.$FormatNumber[$w_acao_aprovado][2].'</td>');  
        } if ((strpos($p_campos,'empenhado') ? strpos($p_campos,'empenhado')+1 : 0)) {
          ShowHTML('        <td align="right"><font size="1">'.$FormatNumber[$w_acao_empenhado][2].'</td>');   
        } if ((strpos($p_campos,'saldo') ? strpos($p_campos,'saldo')+1 : 0)) {
          ShowHTML('        <td align="right"><font size="1">'.$FormatNumber[$w_acao_saldo][2].'</td>');  
        } if ((strpos($p_campos,'liquidado') ? strpos($p_campos,'liquidado')+1 : 0)) {
          ShowHTML('        <td align="right"><font size="1">'.$FormatNumber[$w_acao_liquidado][2].'</td>');    
        } if ((strpos($p_campos,'liquidar') ? strpos($p_campos,'liquidar')+1 : 0)) {
          ShowHTML('        <td align="right"><font size="1">'.$FormatNumber[$w_acao_liquidar][2].'</td>');  
        }
        ShowHTML('      </tr>');
        $w_linha+=1;
      ...
      if (!(strpos($p_campos,'aprovado')===false)){
          ShowHTML('        <td align="right"><font size="1">'.$FormatNumber[$w_tot_aprovado][2].'</td>');
      } if (!(strpos($p_campos,'empenhado')===false)) {
        ShowHTML('        <td align="right"><font size="1">'.$FormatNumber[$w_tot_empenhado][2].'</td>');
      } if (!(strpos($p_campos,'saldo')===false)) {
        ShowHTML('        <td align="right"><font size="1">'.$FormatNumber[$w_tot_saldo][2].'</td>'); 
      } if (!(strpos($p_campos,'liquidado')===false)) {
        ShowHTML('        <td align="right"><font size="1">'.$FormatNumber[$w_tot_liquidado][2].'</td>'); 
      } if (!(strpos($p_campos,'liquidar')===false)) {
        ShowHTML('        <td align="right"><font size="1">'.$FormatNumber[$w_tot_liquidar][2].'</td>');
      }
      ShowHTML('      </tr>');
      $w_linha+=1;
    } 
...
if ($p_responsavel>'')
{
$w_filtro=$w_filtro.'<tr valign="top"><td align="right"><font size=1>Responsável<td><font size=1>[<b>'.RetornaSimNao($p_responsavel).'</b>]';
}
;
} 
if ($p_prioridade>'')
...
{
$w_filtro=$w_filtro.'<tr valign="top"><td align="right"><font size=1>Selecionada MP<td><font size=1>[<b>'.RetornaSimNao($p_selecionada_mpog).'</b>]';
}
;
} 
if ($p_selecionada_relevante>'')
{
$w_filtro=$w_filtro.'<tr valign="top"><td align="right"><font size=1>Selecionada Relevante<td><font size=1>[<b>'.RetornaSimNao($p_selecionada_relevante).'</b>]';
}
;
} 
if ($p_tarefas_atraso>'')
{
$w_filtro=$w_filtro.'<tr valign="top"><td align="right"><font size=1>Tarefas em atraso&nbsp;<font size=1>[<b>'.RetornaSimNao($p_tarefas_atraso).'</b>]&nbsp;';
}
;
} 
...
('<table border=0><tr valign="top"><td><font size=1><b>Filtro:</b><td nowrap><font size=1><ul>'.$w_filtro.'</ul></td></tr></table></td></tr>');} 

ShowHTML('<tr><td align="center" colspan=3>');
ShowHTML('    <TABLE WIDTH="100%" bgcolor="'.$conTableBgColor.'" BORDER="'.$conTableBorder.'" CELLSPACING="'.$conTableCellSpacing.'" CELLPADDING="'.$conTableCellPadding.'" BorderColorDark="'.$conTableBorderColorDark.'" BorderColorLight="'.$conTableBorderColorLight.'">');
ShowHTML('      <tr bgcolor="'.$conTrBgColor.'" align="center">');
ShowHTML('        <td><font size="1"><b>Nome</font></td>');
if ((strpos($p_campos,'responsavel') ? strpos($p_campos,'responsavel')+1 : 0))
{
...
if ((strpos($p_campos,'email') ? strpos($p_campos,'email')+1 : 0))
{
...
if ((strpos($p_campos,'telefone') ? strpos($p_campos,'telefone')+1 : 0))
{
...
if ($w_linha>20 && $w_tipo_rel=='WORD')
{
  ...
  CabecalhoWordRel($w_logo,$w_pag,$w_linha,$p_responsavel,$p_prioridade,
  $p_selecionada_mpog,$p_selecionada_relevante,$p_tarefas_atraso,$w_filtro,
  $p_campos);
  $w_linha=$w_linha+1;
} 
...
if ($w_atual!=f($RS,'chave'))
{

if (((strpos($p_campos,'aprovado') ? strpos($p_campos,'aprovado')+1 : 0) || (strpos($p_campos,'saldo') ? strpos($p_campos,'saldo')+1 : 0) || (strpos($p_campos,'empenhado') ? strpos($p_campos,'empenhado')+1 : 0) || (strpos($p_campos,'liquidado') ? strpos($p_campos,'liquidado')+1 : 0) || (strpos($p_campos,'liquidar') ? strpos($p_campos,'liquidar')+1 : 0)) && $p_sq_orprioridade=='' && $w_sq_siw_solicitacao>'')
{
...
('        <td align="right"><font size="1">'.$FormatNumber[$w_acao_aprovado][2].'</td>');  } 

  if ((strpos($p_campos,'empenhado') ? strpos($p_campos,'empenhado')+1 : 0))
  {
    ShowHTML;
  }
('        <td align="right"><font size="1">'.$FormatNumber[$w_acao_empenhado][2].'</td>');  } 

  if ((strpos($p_campos,'saldo') ? strpos($p_campos,'saldo')+1 : 0))
  {
    ShowHTML;
  }
('        <td align="right"><font size="1">'.$FormatNumber[$w_acao_saldo][2].'</td>');  } 

  if ((strpos($p_campos,'liquidado') ? strpos($p_campos,'liquidado')+1 : 0))
  {
    ShowHTML;
  }
('        <td align="right"><font size="1">'.$FormatNumber[$w_acao_liquidado][2].'</td>');  } 

  if ((strpos($p_campos,'liquidar') ? strpos($p_campos,'liquidar')+1 : 0))
  {
    ShowHTML;
  }
('        <td align="right"><font size="1">'.$FormatNumber[$w_acao_liquidar][2].'</td>');  } 
...
$w_acao_liquidar=0.00;
ShowHTML('      <tr bgcolor="'.$conTrAlternateBgColor.'" valign="top">');
ShowHTML('        <td><font size="1"><b>Iniciativa: '.f($RS,'nome').'</td>');
if ((strpos($p_campos,'responsavel') ? strpos($p_campos,'responsavel')+1 : 0))
{
  ShowHTML;
}
('        <td><font size="1">'.Nvl(f($RS,'responsavel'),'---').'</td>');} 

if ((strpos($p_campos,'email') ? strpos($p_campos,'email')+1 : 0))
{
  ShowHTML;
}
('        <td><font size="1">'.Nvl(f($RS,'email'),'---').'</td>');} 

if ((strpos($p_campos,'telefone') ? strpos($p_campos,'telefone')+1 : 0))
{
  ShowHTML;
}
('        <td><font size="1">'.Nvl(f($RS,'telefone'),'---').'</td>');} 

if ((strpos($p_campos,'aprovado') ? strpos($p_campos,'aprovado')+1 : 0) || (strpos($p_campos,'saldo') ? strpos($p_campos,'saldo')+1 : 0) || (strpos($p_campos,'empenhado') ? strpos($p_campos,'empenhado')+1 : 0) || (strpos($p_campos,'liquidado') ? strpos($p_campos,'liquidado')+1 : 0) || (strpos($p_campos,'liquidar') ? strpos($p_campos,'liquidar')+1 : 0))
{
...
$w_sq_siw_solicitacao=Nvl(f($RS,'sq_siw_solicitacao'),'');
$w_atual=f($RS,'chave');
...
if ($w_tipo_rel=='WORD')
{
  ...
  ShowHTML('        <td colspan='.$w_col.'><font size="1"><b>Ação:</b>'.f($RS,'titulo').'</td>');
}
  else
{

  ShowHTML('        <td colspan='.$w_col.'><font size="1"><b>Ação:</b> <A class="HL" HREF="'.$w_dir.'Projeto.php?par=Visual&O=L&w_chave='.f($RS,'sq_siw_solicitacao').'&w_tipo=&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'" TARGET="VisualAcao" title="Exibe as informações da ação.">'.f($RS,'titulo').'</a></td>');
} 
...
('        <td align="right"><font size="1">'.$FormatNumber[Nvl(f($RS,'aprovado'),0)][2].'</td>');} 

if ((strpos($p_campos,'empenhado') ? strpos($p_campos,'empenhado')+1 : 0))
{
  ShowHTML;
}
('        <td align="right"><font size="1">'.$FormatNumber[Nvl(f($RS,'empenhado'),0)][2].'</td>');} 

if ((strpos($p_campos,'saldo') ? strpos($p_campos,'saldo')+1 : 0))
{
  ShowHTML;
}
('        <td align="right"><font size="1">'.$FormatNumber[Nvl(f($RS,'aprovado'),0)-Nvl(f($RS,'empenhado'),0)][2].'</td>');} 

if ((strpos($p_campos,'liquidado') ? strpos($p_campos,'liquidado')+1 : 0))
{
  ShowHTML;
}
('        <td align="right"><font size="1">'.$FormatNumber[Nvl(f($RS,'liquidado'),0)][2].'</td>');} 

if ((strpos($p_campos,'liquidar') ? strpos($p_campos,'liquidar')+1 : 0))
{
  ShowHTML;
}
('        <td align="right"><font size="1">'.$FormatNumber[Nvl(f($RS,'empenhado'),0)-Nvl(f($RS,'liquidado'),0)][2].'</td>');} 
...
('<td><font size="1"><b>Prioridade</font></td>');      } 
...
//If w_cor = conTrBgColor or w_cor = '' Then w_cor = conTrAlternateBgColor Else w_cor = conTrBgColor End If
        ...
        if (f($RS3,'concluida')=='N')
        {
          ...
          if (f($RS3,'aviso_prox_conc')=='S' && (f($RS3,'aviso')<=time()()))
          {
        ...
        if ($w_tipo_rel=='WORD')
        {
          ...
          ShowHTML(''.f($RS3,'sq_siw_solicitacao').'&nbsp;');
        }
          else
        {
        ...
        ShowHTML('   </font></td>');
        ShowHTML('   <td><font size="1">'.Nvl(f($RS3,'assunto'),'-').'</td>');
        ShowHTML('   <td><font size="1">'.f($RS3,'nm_solic').'</td>');
        ShowHTML('   <td><font size="1">'.Nvl(f($RS3,'proponente'),'---').'</td>');
        ShowHTML('   <td align="center"><font size="1">&nbsp;'.Nvl($FormatDateTime[f($RS3,'fim')][2],'-').'</td>');
        ShowHTML('   <td align="right"><font size="1">'.$FormatNumber[f($RS3,'valor')][2].'&nbsp;</td>');
...
('<td nowrap><font size="1">'.RetornaPrioridade(f($RS3,'prioridade')).'</td>');        } 
...
$RS2->sort='fim, prioridade';
    ...
    else
  {
    ...
    while(!$RS2->EOF)
    {
      ...
      ShowHTML('   <tr bgcolor="'.$conTrBgColor.'" align="center">');
      ShowHTML('     <td align="left"><font size="1"><b>'.f($RS2,'nm_unidade_resp').'</font></td>');
      ShowHTML('   </tr>');
      $w_linha=$w_linha+1;
$RS2->MoveNext;
    } 
...
ShowHTML('           </tr>');
$w_acao_aprovado=$w_acao_aprovado+Nvl(f($RS,'aprovado'),0.00)];
$w_acao_saldo=$w_acao_saldo+Nvl(f($RS,'aprovado'),0.00)]-Nvl(f($RS,'empenhado'),0.00);
$w_acao_empenhado=$w_acao_empenhado+Nvl(f($RS,'empenhado'),0.00);
$w_acao_liquidado=$w_acao_liquidado+Nvl(f($RS,'liquidado'),0.00);
$w_acao_liquidar=$w_acao_liquidar+Nvl(f($RS,'empenhado'),0.00)-Nvl(f($RS,'liquidado'),0.00);

$w_tot_aprovado=$w_tot_aprovado+Nvl(f($RS,'aprovado'),0.00);
$w_tot_saldo=$w_tot_saldo+Nvl(f($RS,'aprovado'),0.00)-Nvl(f($RS,'empenhado'),0.00);
$w_tot_empenhado=$w_tot_empenhado+Nvl(f($RS,'empenhado'),0.00);
$w_tot_liquidado=$w_tot_liquidado+Nvl(f($RS,'liquidado'),0.00);
$w_tot_liquidar=$w_tot_liquidar+Nvl(f($RS,'empenhado'),0.00)-Nvl(f($RS,'liquidado'),0.00);
$RS->MoveNext;
} 
if ((strpos($p_campos,'aprovado') ? strpos($p_campos,'aprovado')+1 : 0) || (strpos($p_campos,'saldo') ? strpos($p_campos,'saldo')+1 : 0) || (strpos($p_campos,'empenhado') ? strpos($p_campos,'empenhado')+1 : 0) || (strpos($p_campos,'liquidado') ? strpos($p_campos,'liquidado')+1 : 0) || (strpos($p_campos,'liquidar') ? strpos($p_campos,'liquidar')+1 : 0))
{
...
('        <td align="right"><font size="1">'.$FormatNumber[$w_tot_aprovado][2].'</td>');} 

if ((strpos($p_campos,'empenhado') ? strpos($p_campos,'empenhado')+1 : 0))
{
ShowHTML;
}
('        <td align="right"><font size="1">'.$FormatNumber[$w_tot_empenhado][2].'</td>');} 

if ((strpos($p_campos,'saldo') ? strpos($p_campos,'saldo')+1 : 0))
{
ShowHTML;
}
('        <td align="right"><font size="1">'.$FormatNumber[$w_tot_saldo][2].'</td>');} 

if ((strpos($p_campos,'liquidado') ? strpos($p_campos,'liquidado')+1 : 0))
{
ShowHTML;
}
('        <td align="right"><font size="1">'.$FormatNumber[$w_tot_liquidado][2].'</td>');} 

if ((strpos($p_campos,'liquidar') ? strpos($p_campos,'liquidar')+1 : 0))
{
ShowHTML;
}
('        <td align="right"><font size="1">'.$FormatNumber[$w_tot_liquidar][2].'</td>');} 
...
$w_acao_aprovado=null;

$w_acao_saldo=null;

$w_acao_empenhado=null;

$w_acao_liquidado=null;

$w_acao_liquidar=null;

$w_tot_aprovado=null;

$w_tot_saldo=null;

$w_tot_empenhado=null;

$w_tot_liquidado=null;
...
$p_selecionada_relevante=null;

$p_responsavel=null;

$p_sq_unidade_resp=null;
...
if ($p_responsavel>'')
{
$w_filtro=$w_filtro.'<td><font size=1>Responsável&nbsp;<font size=1>[<b>'.$p_responsavel.'</b>]&nbsp;';
}
;
} 
if ($p_prioridade>'')
{
$w_filtro=$w_filtro.'<td><font size=1>Prioridade&nbsp;<font size=1>[<b>'.RetornaPrioridade($p_prioridade).'</b>]&nbsp;';
}
;
} 
if ($p_selecionada_mpog>'')
{
$w_filtro=$w_filtro.'<td><font size=1>Selecionada MP&nbsp;<font size=1>[<b>'.$p_selecionada_mpog.'</b>]&nbsp;';
}
;
} 
if ($p_selecionada_relevante>'')
{
$w_filtro=$w_filtro.'<td><font size=1>Selecionada Relevante&nbsp;<font size=1>[<b>'.$p_selecionada_relevante.'</b>]&nbsp;';
}
;
} 
if ($p_programada>'')
{
$w_filtro=$w_filtro.'<td><font size=1>Meta LOA&nbsp;<font size=1>[<b>'.$p_programada.'</b>]&nbsp;';
}
;
} 
if ($p_exequivel>'')
{
$w_filtro=$w_filtro.'<td><font size=1>Meta será cumprida&nbsp;<font size=1>[<b>'.$p_exequivel.'</b>]&nbsp;';
}
;
} 
if ($p_fim_previsto>'')
{
$w_filtro=$w_filtro.'<td><font size=1>Metas em atraso&nbsp;<font size=1>[<b>'.$p_fim_previsto.'</b>]&nbsp;';
}
;
} 
if ($p_atraso>'')
{
$w_filtro=$w_filtro.'<td><font size=1>Ações em atraso&nbsp;<font size=1>[<b>'.$p_atraso.'</b>]&nbsp;';
}
;
} 
if ($p_tarefas_atraso>'')
{
$w_filtro=$w_filtro.'<td><font size=1>Ações com tarefas em atraso&nbsp;<font size=1>[<b>'.$p_tarefas_atraso.'</b>]&nbsp;';
}
;
} 
...
('<table border=0 width="100%"><tr><td width="25%"><font size=1><b>Filtro:</b><td><font size=1><ul>'.$w_filtro.'</ul></tr></table>');} 
...
$RS1 = db_getLinkData::getInstanceOf($dbms,$w_cliente,'ORCAD');
$RS2 = db_getSolicList::getInstanceOf($dbms,f($RS1,'sq_menu'),$w_usuario,f($RS1,'sigla'),5,
null,null,null,null,$p_atraso,null,null,null,null,null);
$RS2->sort='fim, prioridade';
...
$w_cont=$w_cont+1;
if ($w_atual)!=f($RS,'chave') || $p_programada>'' || $p_exequivel>'' || $p_fim_previsto>'' || $p_atraso>'')
{
...
if ($w_sq_siw_solicitacao>'')
{
  ...
  if ($w_tipo_rel=='WORD')
  {
    ...
    ShowHTML('        <td><font size="1"><b>'.f($RS,'titulo').'</td>');
  }
    else
  {

    ShowHTML('        <td><font size="1"><b><A class="HL" HREF="'.$w_dir.'Projeto.php?par=Visual&O=L&w_chave='.f($RS,'sq_siw_solicitacao').'&w_tipo=&P1=1&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'" TARGET="VisualAcao" title="Exibe as informações da ação.">'.f($RS,'titulo').'</a></td>');
  } 
...
// Se não foram selecionados registros, exibe mensagem
      ...
      else
    {
      ...
      if ($w_tipo_rel=='WORD')
      {
        ...
        ShowHTML('      <td><font size="1">'.f($RS3,'titulo').'</td>');
      }
        else
      {

        ShowHTML('      <td><font size="1"><A class="HL" HREF="#" onClick="window.open('Projeto.php?par=AtualizaEtapa&O=V&w_chave='.f($RS2,'sq_siw_solicitacao').'&w_chave_aux='.f($RS3,'sq_projeto_etapa').'&w_tipo=Volta&P1=10&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'','Meta','width=600, height=350, top=50, left=50, toolbar=no, scrollbars=yes, resizable=yes, status=no'); return false;" title="Clique para exibir os dados!">'.f($RS3,'titulo').'</A></td>');
      } 

      ShowHTML('      <td nowrap align="center"><font size="1">'.Nvl(f($RS3,'unidade_medida'),'---').'</td>');
      ShowHTML('      <td nowrap align="right" ><font size="1">'.$FormatNumber[f($RS3,'quantidade')][0].'</td>');
...
$RS4->Sort='referencia desc';
      if (!$RS4->EOF)
      {
        ...
        if (f($RS3,'cumulativa')=='S')
        {
          ...
          ShowHTML('      <td nowrap align="right" ><font size="1">'.$FormatNumber[Nvl(f($RS4,'execucao_fisica'),0)][0].'</td>');
        }
          else
        {
          ...
          while(!$RS4->EOF)
          {
            ...
            $w_quantitativo_total=$w_quantitativo_total+Nvl(f($RS4,'execucao_fisica'),0);
$RS4->MoveNext;
          } 
      ...
      ShowHTML('      <td nowrap align="right" ><font size="1">'.f($RS3,'perc_conclusao').'</td>');
$RS3->MoveNext;
      if (!$RS3->EOF)
      {
...
if ($p_programada=='' && $p_exequivel=='' && $p_fim_previsto=='' && $p_atraso=='')
{
  ...
  if ($w_teste_acoes==1)
  {
    ...
    ShowHTML('        <td colspan="1"><font size="1"><b>'.f($RS,'titulo').'</b></td>');
...
('<table border=0><tr valign="top"><td><font size=1><b>Filtro:</b><td nowrap><font size=1><ul>'.$w_filtro.'</ul></tr></table>');} 
...
ShowHTML('          <td rowspan="1" colspan="5"><font size="1"><b>Dados SIAFI&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Atualização: '.Nvl(FormataDataEdicao(f($RS1,'data_arquivo')),'-').'</font></td>');
$RS1->Close;
...
$RS3->Filter='programada = ''.$p_programada.'' and exequivel = ''.$p_exequivel.'' and fim_previsto < ''.time()().''';
  }
    else
  if ($p_programada>'' && $p_exequivel>'')
  {
...
$RS3->Filter='programada = ''.$p_programada.'' and fim_previsto < ''.time()().''';
  }
    else
  if ($p_fim_previsto>'' && $p_exequivel>'')
  {

$RS3->Filter='exequivel = ''.$p_exequivel.'' and fim_previsto < ''.time()().''';
  }
    else
  if ($p_programada>'')
  {
...
if (Nvl(f($RS,'sq_acao_ppa_pai'),'')=='' || $p_programada>'' || $p_exequivel>'' || $p_fim_previsto>'' || $p_atraso>'')
{
...
$w_linha=$w_linha+1;
ShowHTML('      <td><font size="1"><b>'.f($RS,'codigo').'</td>');
if ($w_tipo_rel=='WORD' || f($RS,'acao')==0)
{
...
ShowHTML('   <td><font size="1"><b><A class="HL" HREF="'.$w_dir.'Projeto.php?par=Visual&O=L&w_chave='.f($RS,'sq_siw_solicitacao').'&w_tipo=&P1=2&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'" TARGET="VisualAcao" title="Exibe as informações da ação.">'.f($RS,'nome').'</a></td>');
} 

ShowHTML('      <td align="right"><font size="1">'.$FormatNumber[f($RS,'aprovado')][2].'</td>');
ShowHTML('      <td align="right"><font size="1">'.$FormatNumber[f($RS,'empenhado')][2].'</td>');
ShowHTML('      <td align="right"><font size="1">'.$FormatNumber[Nvl(f($RS,'aprovado'),0.00)]-Nvl(f($RS,'empenhado'),0.00)][2].'</td>');
ShowHTML('      <td align="right"><font size="1">'.$FormatNumber[f($RS,'liquidado')][2].'</td>');
ShowHTML('      <td align="right"><font size="1">'.$FormatNumber[(Nvl(f($RS,'empenhado'),0.00)]-Nvl(f($RS,'liquidado'),0.00)])[2].'</td>');
if ($RS2->EOF)
{
  ...
  else
{
  ...
  ShowHTML('      <td nowrap align="center"><font size="1">'.Nvl(f($RS3,'unidade_medida'),'---').'</td>');
  ShowHTML('      <td nowrap align="right" ><font size="1">'.f($RS3,'quantidade').'</td>');
  $RS4 = db_getEtapaMensal::getInstanceOf($dbms,f($RS3,'sq_projeto_etapa'));
$RS4->Sort='referencia desc';
  if (!$RS4->EOF)
  {
    ...
    if (f($RS3,'cumulativa')=='S')
    {
      ...
      ShowHTML('      <td nowrap align="right" ><font size="1">'.Nvl(f($RS4,'execucao_fisica'),0).'</td>');
    }
      else
    {
      ...
      while(!$RS4->EOF)
      {
        ...
        $w_quantitativo_total=$w_quantitativo_total+$cDbl[Nvl(f($RS4,'execucao_fisica'),0)];
$RS4->MoveNext;
      } 
  ...
  ShowHTML('<td nowrap align="right" ><font size="1">'.f($RS3,'perc_conclusao').'</td>');
  ShowHTML('<td nowrap align="right" ><font size="1">'.f($RS3,'nm_programada').'</td>');
$RS3->MoveNext;
  if (!$RS3->EOF)
  {
...
if ($w_teste_acoes==1)
{
  ...
  ShowHTML('        <td colspan="1"><font size="1"><b>'.f($RS,'codigo').'</b></td>');
  ShowHTML('        <td colspan="1"><font size="1"><b>'.f($RS,'nome').'</b></td>');
  if ($w_teste_metas==3)
  {
...
$_REQUEST['w_empenhado'],$_REQUEST['w_liquidado']);
...
function EtapaLinha($p_chave,$p_chave_aux,$p_titulo,$p_word,$p_programada,$p_unidade_medida,$p_quantidade,$p_fim,$p_perc,$p_oper,$p_tipo)
{
  extract($GLOBALS);
...
$l_html=$l_html.'\r\n'.'<A class="HL" HREF="#" onClick="window.open('projeto.php?par=AtualizaEtapa&O=V&w_chave='.$p_chave.'&w_chave_aux='.$p_chave_aux.'&w_tipo=Volta&P1=10&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'','Meta','width=600, height=350, top=50, left=50, toolbar=no, scrollbars=yes, resizable=yes, status=no'); return false;" title="Clique para exibir os dados!">'.$p_titulo.'</A>';
}
  else
{
...
$l_html=$l_html.'\r\n'.'        <td align="center"><font size="1">'.RetornaSimNao($p_programada).'</b>';
$l_html=$l_html.'\r\n'.'        <td align="center" '.$l_row.'><font size="1">'.FormataDataEdicao($p_fim).'</td>';
$l_html=$l_html.'\r\n'.'        <td nowrap '.$l_row.'><font size="1">'.Nvl($p_unidade_medida,'---').' </td>';
$l_html=$l_html.'\r\n'.'        <td nowrap align="right" '.$l_row.'><font size="1">'.$p_quantidade.'</td>';
