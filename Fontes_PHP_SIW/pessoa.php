...
function Benef() {
  ...
  if (!(strpos('IAETDV',$O)===false)) {
    ...
    if ($P1!=0 && ($w_cpf=='' || (isset($_REQUEST['Botao']) && ((!(strpos($_REQUEST['Botao'],'Troca')===false)) || (!(strpos($_REQUEST['Botao'],'Procurar')===false)))))) {
      ...
      ShowHTML('      </table>');
      if ($_REQUEST['w_nome']>"") {
        $SQL = new db_getPersonList; $RS = $SQL->getInstanceOf($dbms,$w_cliente,null,"NOVOUSO",$_REQUEST['w_nome'],null,null,null);
...
function BuscaPessoa() {
  ...
  ShowHTML('<tr bgcolor="'.$conTrBgColor.'"><td>');
  ShowHTML('    <table width="100%" border="0">');
  ShowHTML('      <tr><td colspan=2><b>Parte do <U>n</U>ome da pessoa:<br><INPUT ACCESSKEY="N" '.$w_Disabled.' class="sti" type="text" name="p_nome" size="50" maxlength="80" value="'.$p_nome.'">');
  ShowHTML('      <tr valign="top">');
