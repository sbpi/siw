...
function VisualDemanda($w_chave,$operacao,$w_usuario) {
  ...
  if ($operacao=='L' || $operacao=='V') {
    ...
    if (nvl(f($RS,'nm_projeto'),'')>'') {
      $w_html.=chr(13).'      <tr><td valign="top"><font size="1">Projeto: <b>'.f($RS,'nm_projeto').'  ('.f($RS,'sq_solic_pai').')</b></td>';
    } 

    if (nvl(f($RS,'nm_etapa'),'')>'') {
      $w_html.=chr(13).'      <tr><td valign="top"><font size="1">Etapa: <b>'.MontaOrdemEtapa(f($RS,'sq_projeto_etapa')).'. '.f($RS,'nm_etapa').' </b></td>';
    } 

    $w_html.=chr(13).'      <tr><td><font size=1>Detalhamento: <b>'.$w_chave.'<br>'.CRLF2BR(f($RS,'assunto')).'</b></font></td></tr>';
    ...
    if (nvl(f($RS,'sq_cc'),'')>'') {
      $w_html.=chr(13).'      <tr><td valign="top"><font size="1">Classificação:<br><b>'.f($RS,'cc_nome').' </b></td>';
    } 
    ...
    if (Nvl(f($RS,'proponente'),'')>'') {
      $w_html.=chr(13).'          <td colspan=2><font size="1">Proponente externo:<br><b>'.f($RS,'proponente').' </b></td>';
    } else {
      $w_html.=chr(13).'          <td colspan=2><font size="1">Proponente externo:<br><b> --- </b></td>';
    } 

    $w_html.=chr(13).'          <tr valign="top">';
    $w_html.=chr(13).'          <td><font size="1">Responsável:<br><b>'.ExibePessoa(null,$w_cliente,f($RS,'solicitante'),$TP,f($RS,'nm_sol')).'</b></td>';
    $w_html.=chr(13).'          <td><font size="1">Unidade responsável:<br><b>'.ExibeUnidade(null,$w_cliente,f($RS,'nm_unidade_resp'),f($RS,'sq_unidade_resp'),$TP).'</b></td>';

    if ($w_tipo_visao==0) {
      // Se for visão completa
      $w_html.=chr(13).'          <td valign="top"><font size="1">Orçamento disponível:<br><b>'.number_format(f($RS,'valor'),2,',','.').' </b></td>';
    } 

    $w_html.=chr(13).'          <tr valign="top">';
    $w_html.=chr(13).'          <td><font size="1">Data de recebimento:<br><b>'.FormataDataEdicao(f($RS,'inicio')).' </b></td>';
    $w_html.=chr(13).'          <td><font size="1">Limite para conclusão:<br><b>'.FormataDataEdicao(f($RS,'fim')).' </b></td>';
    $w_html.=chr(13).'          <td><font size="1">Prioridade:<br><b>'.RetornaPrioridade(f($RS,'prioridade')).' </b></td>';
    $w_html.=chr(13).'          <tr>';
    $w_html.=chr(13).'          <td colspan=3><font size="1">Palavras-chave:<br><b>'.f($RS,'palavra_chave').' </b></td>';
    $w_html.=chr(13).'          </table>';
    ...
    if ($w_tipo_visao==0 || $w_tipo_visao==1) {
      ...
      if (Nvl(f($RS,'descricao'),'')>'' || Nvl(f($RS,'justificativa'),'')>'') {
        ...
        if (Nvl(f($RS,'descricao'),'')>'') $w_html.=chr(13).'      <tr><td valign="top"><font size="1">Resultados da demanda:<br><b>'.CRLF2BR(f($RS,'descricao')).' </b></td>';
        if ($w_tipo_visao==0 && Nvl(f($RS,'justificativa'),'')>'') {
          // Se for visão completa
          $w_html.=chr(13).'      <tr><td valign="top"><font size="1">Recomendações superiores:<br><b>'.CRLF2BR(f($RS,'justificativa')).' </b></td>';
        } 
      } 
    } 
    ...
    if (f($RS,'concluida')=='S' && Nvl(f($RS,'data_conclusao'),'')>'') {
      ...
      $w_html.=chr(13).'      <tr><td valign="top" colspan="2"><table border=0 width="100%" cellspacing=0>';
      $w_html.=chr(13).'          <tr valign="top">';
      $w_html.=chr(13).'          <td><font size="1">Início da execução:<br><b>'.FormataDataEdicao(f($RS,'inicio_real')).' </b></td>';
      $w_html.=chr(13).'          <td><font size="1">Término da execução:<br><b>'.FormataDataEdicao(f($RS,'fim_real')).' </b></td>';
      if ($w_tipo_visao==0) {
        $w_html.=chr(13).'          <td><font size="1">Custo real:<br><b>'.number_format(f($RS,'custo_real'),2,',','.').' </b></td>';
      } 
      $w_html.=chr(13).'          </table>';
      if ($w_tipo_visao==0) {
        $w_html.=chr(13).'      <tr><td valign="top"><font size="1">Nota de conclusão:<br><b>'.CRLF2BR(f($RS,'nota_conclusao')).' </b></td>';
      } 
    } 
  } 
  ...
  if ($operacao=='L' && $w_tipo_visao!=2) {
    ...
    if (count($RS)>0) {
      $w_html.=chr(13).'      <tr><td valign="top" colspan="2" align="center" bgcolor="#D0D0D0" style="border: 2px solid rgb(0,0,0);"><font size="1"><b>Interessados na execução</td>';
      ...
      $w_html.=chr(13).'            <td><font size="1"><b>Tipo de visão</font></td>';
      $w_html.=chr(13).'            <td><font size="1"><b>Envia e-mail</font></td>';
      $w_html.=chr(13).'          </tr>';
      $w_cor=$conTrBgColor;
      foreach($RS as $row) {
        $w_cor = ($w_cor==$conTrBgColor || $w_cor=='') ? $w_cor=$conTrAlternateBgColor : $w_cor=$conTrBgColor;
        $w_html.=chr(13).'      <tr valign="top" bgcolor="'.$w_cor.'">';
        $w_html.=chr(13).'        <td><font size="1">'.ExibePessoa(null,$w_cliente,f($row,'sq_pessoa'),$TP,f($row,'nome').' ('.f($row,'lotacao').')').'</td>';
        $w_html.=chr(13).'        <td><font size="1">'.RetornaTipoVisao(f($row,'tipo_visao')).'</td>';
        $w_html.=chr(13).'        <td align="center"><font size="1">'.str_replace('N','Não',str_replace('S','Sim',f($row,'envia_email'))).'</td>';
        $w_html.=chr(13).'      </tr>';
      } 
    ...
    $RS = SortArray($RS,'nome','asc');
    if (count($RS)>0) {
      $w_html.=chr(13).'      <tr><td valign="top" colspan="2" align="center" bgcolor="#D0D0D0" style="border: 2px solid rgb(0,0,0);"><font size="1"><b>Áreas/Instituições envolvidas</td>';
      $w_html.=chr(13).'      <tr><td align="center" colspan="2">';
      $w_html.=chr(13).'        <TABLE WIDTH="100%" bgcolor="'.$conTableBgColor.'" BORDER="'.$conTableBorder.'" CELLSPACING="'.$conTableCellSpacing.'" CELLPADDING="'.$conTableCellPadding.'" BorderColorDark="'.$conTableBorderColorDark.'" BorderColorLight="'.$conTableBorderColorLight.'">';
      $w_html.=chr(13).'          <tr bgcolor="'.$conTrBgColor.'" align="center">';
      $w_html.=chr(13).'            <td><font size="1"><b>Nome</font></td>';
      $w_html.=chr(13).'            <td><font size="1"><b>Papel</font></td>';
      $w_html.=chr(13).'          </tr>';
      $w_cor=$conTrBgColor;
      foreach($RS as $row) {
        $w_cor = ($w_cor==$conTrBgColor || $w_cor=='') ? $w_cor=$conTrAlternateBgColor : $w_cor=$conTrBgColor;
        ...
        $w_html.=chr(13).'        <td><font size="1">'.f($row,'papel').'</td>';
        $w_html.=chr(13).'      </tr>';
      } 
      $w_html.=chr(13).'         </table></td></tr>';
    } 
