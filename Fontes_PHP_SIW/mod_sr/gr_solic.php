...
// Descricao: Consultas gerenciais à base de solicitações de recursos logísticos
// Mail     : alex@sbpi.com.br
// Criação  : 21/11/2006, 18:56
// Versao   : 1.0.0.0
// Local    : Brasília - DF
// -------------------------------------------------------------------------
// 
  ...
  if ($O=='L' || $O=='W') {
    ...
    if (count($RS1)>0 && $p_tipo=='N') {
      ...
      if (($t_totcad+$t_tottram)>0) {
        ShowHTML('<tr><td align="center"><IMG SRC="'.$conPHP4.$w_dir.'geragrafico.php?p_genero=F&p_objeto='.f($RS_Menu,'nome').'&p_tipo='.$SG.'&p_grafico=Pizza&p_tot='.$t_totsolic.'&p_cad='.$t_totcad.'&p_tram='.$t_tottram.'&p_conc='.$t_totconc.'&p_atraso='.$t_totatraso.'&p_aviso='.$t_totaviso.'&p_acima='.$t_totacima.'">');
      } 
    } 
  } elseif ($O='P') {
