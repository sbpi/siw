...
// -------------------------------------------------------------------------
function celular_termoDevol($l_chave,$l_sg,$l_formato=0) {
  extract($GLOBALS);
  $l_html='';
  ...
  if ($l_formato!=1) {
    ...
    $l_html.=chr(13).'   <tr><td width="20%">Período inicialmente solicitado:</td>';
    $l_html.=chr(13).'       <td><font size="2"><b><b>'.FormataDataEdicao(f($RS1,'inicio'),5).' a '.FormataDataEdicao(f($RS1,'fim'),5).'</b></font></td></tr>';
    $l_html.=chr(13).'   <tr><td width="20%">Período do empréstimo:</td>';
  ...
  if (f($RS1,'pendencia')=='S') {
    $l_html.=chr(13).'<p align="justify"><b><font color="#FF0000">Atenção: Devolução com os seguintes itens pendentes:<br><b>'.f($RS1,'acessorios_pendentes').'</b></font></b></p>';
  }
  if ($l_formato!=1) {
