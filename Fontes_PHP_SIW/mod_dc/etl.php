  ...
  if ($O=='L') {
    ...
    if (count($RS)<=0) {
      ...
      foreach($RS1 as $row) {
        ...
        ShowHTML('        <td>'.strtolower(f($row,'nm_usuario').'.'.f($row,'nome')).'</td>');
    ...
    if (count($RS)<=0) {
      ...
      foreach($RS as $row) {
        ...
        if($w_formato=='T')  {
          ...
          if (strtoupper(f($row,'nm_coluna_tipo'))=='DATE')  ShowHTML('        <td nowrap><input '.$w_Disabled.' type="text" name="w_mascara_data[]" class="sti" SIZE="20" MAXLENGTH="50" VALUE="'.$w_mascara_data.'"></td>');
...
function Importacao() {
  ...
  if ($O=='L') {
    ...
    ShowHTML('          <input class="stb" type="button" onClick="location.href=\''.$R.'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.'TIMPORT'.'&O=L\';" name="Botao" value="Cancelar">');
    ShowHTML('          </td>');
    ShowHTML('      </tr>');
    ShowHTML('    </table>');
    ShowHTML('    </TD>');
...
function Registro() {
  ...
  if ($w_troca>'') {
    ...
    // Recupera todos os ws_url para a listagem
    $RS = db_getEsquemaInsert::getInstanceOf($dbms,null,null,$w_sq_esquema_tabela,null,null);
    $RS = SortArray($RS,'registro','asc','or_coluna','asc');
  } elseif (!(strpos('IA',$O)===false)) {
    $RS = db_getEsquemaAtributo::getInstanceOf($dbms,null,$w_sq_esquema_tabela,null,null);
    $RS = SortArray($RS,'ordem','asc','nm_coluna','asc');
    //$RS = db_getColuna::getInstanceOf($dbms,$w_cliente,null,$w_sq_tabela,null,null,null,null,$w_sq_esquema_tabela);
    //$RS = SortArray($RS,'ordem','asc','nm_coluna','asc');
  } 
  ...
  if ($O=='L') {
    ...
    if (count($RS)<=0) {
      ...
      foreach($RS as $row) {
        ...
        ShowHTML('      <tr bgcolor="'.$w_cor.'" valign="top">');        
        if($O=='A') {
          $RS1 = db_getEsquemaInsert::getInstanceOf($dbms,null,null,$w_sq_esquema_tabela, f($row,'sq_coluna'),$w_registro);
          foreach($RS1 as $row1){$RS1=$row1; break;}
          if (count($RS1)>0) {
            $w_valor = f($RS1,'valor');
    ...
    ShowHTML('<tr><td align="center" colspan="3">');
    ShowHTML('            <input class="stb" type="submit" name="Botao" value="Gravar">');
    ShowHTML('            <input class="stb" type="button" onClick="location.href=\''.$w_pagina.'Registro&w_sq_esquema_tabela='.$w_sq_esquema_tabela.'&w_sq_tabela='.$w_sq_tabela.'&w_sq_esquema='.$w_sq_esquema.'&R='.$R.'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'&w_menu='.$w_menu.'&O=L\';" name="Botao" value="Cancelar">');
    ShowHTML('  </td>');
    ShowHTML('</FORM>');
  } else {
    ScriptOpen('JavaScript');
...
function Script() {
  ...
  if ($w_troca>'') {
    ...
    $RS = db_getEsquemaScript::getInstanceOf($dbms,$w_sq_esquema,null,$w_cliente);
    $RS = SortArray($RS,'ordem','asc');
  } elseif (!(strpos('AEV',$O)===false) && $w_troca=='') {
    // Recupera os dados do endereço informado 
    $RS = db_getEsquemaScript::getInstanceOf($dbms,$w_sq_esquema,$w_sq_arquivo,$w_cliente);
    foreach($RS as $row) {$RS=$row; break;}
    $w_nome         = f($RS,'nome');
    $w_descricao    = f($RS,'descricao');
  ...
  if ($O=='L') {
    ...
    ShowHTML('            <input class="STB" type="button" onClick="location.href=\''.$w_pagina.$par.'&w_sq_esquema='.$w_sq_esquema.'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'&O=L\';" name="Botao" value="Cancelar">');
    ShowHTML('          </td>');
    ShowHTML('      </tr>');
    ShowHTML('    </table>');
    ShowHTML('    </TD>');
  ...
  switch ($SG) {
    ...
    case 'ISSIGMAP':
      ...
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
        ...
        if ($O=='I') {
          for ($i=0; $i<=count($_POST['w_sq_coluna'])-1; $i=$i+1) {
            if ($_REQUEST['w_sq_coluna'][$i]>'') {
              if (strtoupper($_REQUEST['w_tipo_coluna'][$i])=='DATE') $w_valor_mascara = $_REQUEST['w_mascara_data'][$i]; else $w_valor_mascara = '';
        ...
        ScriptOpen('JavaScript');
        ShowHTML('  location.href=\''.$w_pagina.'Tabela'.'&O=L&w_sq_esquema='.$_REQUEST['w_sq_esquema'].'&w_sq_esquema_tabela='.$_REQUEST['w_sq_esquema_tabela'].'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.'ISSIGTAB'.MontaFiltro('GET').'\';');
        ScriptClose();
      } else {
        ScriptOpen('JavaScript');
        ShowHTML('  alert(\'Assinatura Eletrônica inválida!\');');
    ...
    case 'TIREGISTRO':
      ...
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
        ...
        ScriptOpen('JavaScript');
        ShowHTML('  location.href=\''.$R.'&O=L&w_sq_esquema='.$_REQUEST['w_sq_esquema'].'&w_sq_esquema_tabela='.$_REQUEST['w_sq_esquema_tabela'].'&w_sq_tabela='.$_REQUEST['w_sq_tabela'].'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'&w_menu='.$w_menu.MontaFiltro('GET').'\';');
        ScriptClose();
      } else {
        ScriptOpen('JavaScript');
        ShowHTML('  alert(\'Assinatura Eletrônica inválida!\');');
   ...
   case 'TISCRIPT':
    ...
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
      if (UPLOAD_ERR_OK==0) {
        ...
        foreach ($_FILES as $Chv => $Field) {
          ...
          if ($Field['size'] >= 0) {
            ...
            if ($_REQUEST['w_atual']>'') {
              $RS = db_getEsquemaScript::getInstanceOf($dbms,$_REQUEST['w_sq_esquema'],$_REQUEST['w_sq_arquivo'],$w_cliente);
              foreach ($RS as $row) {
     ...
     // Recupera a sigla do serviço pai, para fazer a chamada ao menu 
      ...
      ShowHTML('  location.href=\''.$R.'&O=L&w_sq_esquema='.$_REQUEST['w_sq_esquema'].'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'\';');
      ScriptClose();
    } else {
      ScriptOpen('JavaScript');
      ShowHTML('  alert(\'Assinatura Eletrônica inválida!\');');
   ...
   case 'IMPTXT':
     ...
     if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
      ...
      // Recupera a sigla do serviço pai, para fazer a chamada ao menu 
       ...
       ShowHTML('  location.href=\''.str_replace($w_dir,'',f($RS1,'link')).'&O=L&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.f($RS1,'sigla').MontaFiltro('GET').'\';');
       ScriptClose();
     } else {
       ScriptOpen('JavaScript');
       ShowHTML('  alert(\'Assinatura Eletrônica inválida!\');');
   ...
   case 'GERAARQ':
      ...
      $RS_Esquema = db_getEsquema::getInstanceOf($dbms,$w_cliente,null,$w_esquema,null,null,null,null,null,null,null,null);
      ...
      foreach($RS_Esquema as $row_esquema) {
        if (f($row_esquema,'ativo')=='S') {
          ...
          $i = 0;
          // Recupera informações dos esquemas ativos
          $RS_Tabela = db_getEsquemaTabela::getInstanceOf($dbms,null,f($row_esquema,'sq_esquema'),null);
          ...
          foreach($RS_Tabela as $row_tabela) {
            ...
            if ($w_atual=='' || $w_atual <> f($row_tabela,'sq_esquema_tabela')) {
              if ($w_atual=='') {
                ...
                $l_arquivo .= '                                   )'.$crlf;
                $RS_Insert = db_getEsquemaInsert::getInstanceOf($dbms,null,null,$w_atual,null,null);
          ...
          $i = 0;
          $RS_Script = db_getEsquemaScript::getInstanceOf($dbms,f($row_esquema,'sq_esquema'),null,$w_cliente);
          ...
          $w_arq_conf  = $w_raiz_conf.strtolower(f($row_esquema,'nome')).'.php';

          // Se o diretório de dados do esquema não existir, tenta criá-lo.
          if (!is_dir($w_raiz_arq.strtolower(f($row_esquema,'nome'))))  mkdir($w_raiz_arq.strtolower(f($row_esquema,'nome')));
