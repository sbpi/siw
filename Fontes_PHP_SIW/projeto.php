  ...
  if ($O == 'L') {
    ...
    if (count($RS)<=0) {
      ...
      foreach ($RS1 as $row) {
        ...
        ShowHTML('        <td nowrap>');
        if ($w_tipo!='EXCEL') ShowHTML(ExibeImagemSolic(f($row,'sigla'),f($row,'inicio'),f($row,'fim'),f($row,'inicio_real'),f($row,'fim_real'),f($row,'aviso_prox_conc'),f($row,'aviso'),f($row,'sg_tramite'), null));
        if ($w_embed!='WORD') {
        ...
        if ($P1!=1 && $P1!=2) {
          if ($_SESSION['INTERNO']=='S') {
            if (f($row,'sg_tramite')=='AT') $w_valor = f($row,'custo_real');
            else $w_valor = f($row,'valor');
            ShowHTML('        <td align="right">'.((nvl(f($row,'sb_moeda'),'')!='') ? f($row,'sb_moeda').' ' : '').formatNumber($w_valor).'&nbsp;</td>');
      ...
      if ($P1 != 1 && $P1 != 2 && $_SESSION['INTERNO'] == 'S') {
        ...
        if (ceil(count($RS)/$P4)>1) {
          ...
          ShowHTML('          <td colspan=7 align="right"><b>Tota'.((count($w_parcial)==1) ? 'l' : 'is').' desta página&nbsp;</td>');
        ...
        if ($P3==ceil(count($RS)/$P4)) {
          ...
          ShowHTML('        <tr bgcolor="'.$conTrBgColor.'" valign="top">');
          ShowHTML('          <td colspan=7 align="right"><b>Tota'.((count($w_total)==1) ? 'l' : 'is').' da listagem&nbsp;</td>');
          ShowHTML('          <td align="right" nowrap><b>');
          $i = 0;
          ksort($w_total);
  ...
  if ($O=='L') {
    ...
    if (count($RS)<=0) {
      ...
      foreach ($RS as $row) {
        ...
        ShowHTML('        <td align="top" class="remover" nowrap>');
...
function PrestacaoContas() {
  ...
  if ($O=='P') {
    ...
    ShowHTML('<tr><td colspan="2"><hr NOSHADE color=#000000 size=4></td></tr>');
    ShowHTML('</table>');
    $sql = new db_getPrestacaoContas; $RS = $sql->getInstanceOf($dbms,$w_cliente,f($RS_Cron,'sq_prestacao_contas'),null,null,null,null,'IS NULL');    
    $w_contOut = 0;
    $w_cont    = 1;
    ...
    if ($w_contOut==0) {
      ...
      ShowHTML('      <tr><td><br><b>'.$_SESSION['LABEL_CAMPO'].':<br><INPUT ACCESSKEY="A" class="sti" type="PASSWORD" name="w_assinatura" size="30" maxlength="30" value=""></td>');
