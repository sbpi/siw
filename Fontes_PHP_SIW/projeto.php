...
function AnexosEtapas() {
  ...
  if ($O=='L') {
    ...
    ShowHTML('        <a accesskey="V" class="SS" href="'.$R.'&R='.$R.'&O=L&w_chave='.$w_chave.'&SG=PJETAPA&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.RemoveTP($TP).MontaFiltro('GET').'"><u>V</u>oltar</a>&nbsp;');
    ShowHTML('    <td align="right"><b>Registros existentes: '.count($RS));
    ShowHTML('<tr><td align="center" colspan=3>');
    ShowHTML('    <TABLE WIDTH="100%" bgcolor="'.$conTableBgColor.'" BORDER="'.$conTableBorder.'" CELLSPACING="'.$conTableCellSpacing.'" CELLPADDING="'.$conTableCellPadding.'" BorderColorDark="'.$conTableBorderColorDark.'" BorderColorLight="'.$conTableBorderColorLight.'">');
    ShowHTML('        <tr bgcolor="'.$conTrBgColor.'" align="center">');
  ...
  if ($O=='L') {
    ...
    if (count($RS)<=0) {
      ...
      foreach ($RS1 as $row) {
        ...
        if($w_financeiro=='N' || $w_cliente=='10135') {
          ShowHTML('          <A class="HL" HREF="'.$w_pagina.'Cronograma&R='.$w_pagina.'Cronograma&O=L&w_chave='.f($row,'sq_projeto_rubrica').'&w_chave_pai='.$w_chave.'&w_tipo=Volta&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG=PJCRONOGRAMA'.MontaFiltro('GET').'" title="Cadastrar o cronograma desembolso deste registro." target="Cronograma desembolso">CD</A>&nbsp');
        }
        ShowHTML('        </td>');
        ShowHTML('      </tr>');
      } 
  ...
  if ($O=='L') {
    ...
    if (count($RS)<=0) {
      ...
      foreach ($RS1 as $row) {
        ...
        ShowHTML('        <td align="right">'.formatNumber(f($row,'total_real')).'</td>');
        ShowHTML('        <td>');
        ShowHTML('          <A class="HL" HREF="'.$w_pagina.$par.'&R='.$w_pagina.$par.'&O=V&w_chave_rub='.f($row,'sq_projeto_rubrica').'&w_chave='.$w_chave.'&w_tipo=Volta&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.MontaFiltro('GET').'" title="Atualizar o orçamento realizado no cronograma desembolso." target="Cronograma desembolso">Atualizar</A>&nbsp');
        ShowHTML('        </td>');
        ShowHTML('      </tr>');
      } 
        ShowHTML('      <tr>');
...
function Etapas() {
  ...
  if ($O=='L') {
    ...
    if (count($RS)<=0) {
      ...
      ShowHTML(EtapaLinha($w_chave,null,null,null,null,$w_previsto_menor,$w_previsto_maior,$w_real_menor,$w_real_maior,null,$w_total_tarefa,'','S','PROJETO',null,null,'N',null,$w_total_orcamento,0,null,$w_total_peso));
      ShowHTML('</FORM>');
    } 
    ShowHTML('      </center>');
    ShowHTML('    </table>');
    ...
    ShowHTML('      <tr valign="top">');
    ShowHTML('        <td><b>Orça<u>m</u>ento previsto:</b><br><input '.$w_Disabled.' accesskey="M" type="text" name="w_orcamento" class="STI" SIZE="18" MAXLENGTH="18" VALUE="'.$w_orcamento.'" onKeyDown="FormataValor(this,18,2,event);" title="Orçamento previsto para execução desta etapa."></td>');
...
function AtualizaEtapa() {
  ...
  if ($O=='L') {
    ...
    ShowHTML('              <td>Orçamento previsto:<b><br>'.formatNumber($w_orcamento).'</td>');
    ShowHTML('          <tr valign="top">');
    ...
    if ($O=='V') {
      ShowHTML('      <tr valign="top">');
      ShowHTML('        <td>Percentual de conclusão:<br><b>'.nvl($w_perc_conclusao,0).'%</b></td>');
