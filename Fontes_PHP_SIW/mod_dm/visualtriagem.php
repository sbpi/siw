...
function VisualTriagem($l_chave,$operacao,$w_usuario,$l_tipo=null) {
  ...
  if ($operacao=='L' || $operacao=='V') {
    ...
    if (f($RS,'reuniao')=='N') {
      ...
      $l_html.=chr(13).'      <tr><td><b>Data da reunião:</b></td>';
    }
      ...
      if (Nvl(f($RS,'descricao'),'')>'' || Nvl(f($RS,'justificativa'),'')>'') {
        ...
        if (Nvl(f($RS,'descricao'),'')>''){ 
          if (f($RS,'reuniao')=='N') {
            ...
            $l_html.=chr(13).'      <tr valign="top"><td><b>Pauta da reunião:</b></td>';
          }
  ...
  if ($operacao=='L' || $operacao=='V') {
    ...
    if ($w_tipo_visao!=2 && $w_erro>'') {
      ...
      if (substr($w_erro,0,1)=='0') {
        ...
        $l_html.=chr(13).'  <font color="#BC3131"><b>ATENÇÃO:</b></font> Foram identificadas as pendências listadas abaixo. Seu encaminhamento para fases posteriores à atual só pode ser feito por um gestor do sistema ou do módulo de demandas.';
      } else {
