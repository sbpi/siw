...
function ExibePrograma($l_chave,$operacao,$l_usuario,$l_tipo) {
  ...
  if (nvl(f($RS,'sq_plano'),'')!='') {
    ...
    else                 $l_html.=chr(13).'      <tr><td colspan="2"  bgcolor="#f0f0f0"><font size="2">PLANO ESTRATÉGICO: '.ExibePlano('../',$w_cliente,f($RS,'sq_plano'),$TP,f($RS,'nm_plano')).'</font></td></tr>';
  }
  ...
  if(nvl($_REQUEST['p_projetos'],'')!='') {
    ...
    if (count($RS1)>0) {
      ...
      $l_html .=chr(13).'      <tr><td colspan="2"><ul><li>A listagem exibe apenas documentos nos quais você tenha alguma permissão.</li>';
