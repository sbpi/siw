...
function Inventario() {
  ...
  if ($p_tipo == 'WORD') {
    ...
    CabecalhoWord($w_cliente,'Inventário de Estoque',$w_pag);
    $w_embed = 'WORD';
    if ($w_filtro>'') ShowHTML($w_filtro);
  } elseif($p_tipo == 'PDF'){
    $w_linha_pag = ((nvl($_REQUEST['orientacao'],'PORTRAIT')=='PORTRAIT') ? 60: 35);
    $w_embed = 'WORD';
    HeaderPdf('Inventário de Estoque',$w_pag);
    if ($w_filtro>'') ShowHTML($w_filtro);
  } else {
    $w_embed = 'HTML';
    Cabecalho();
    ...
    if ($O=='L') {
      CabecalhoRelatorio($w_cliente,'Inventário de Estoque',4);
      if ($w_filtro>'') ShowHTML($w_filtro);
    } else {
  ...
  if ($O=='L' || $w_embed == 'WORD') {
    ...
    ShowHTML('          <td><b>Unid. Medida</b></td>');
    ShowHTML('          <td><b>Quantidade</b></td>');
    ...
    ShowHTML('      <tr>');
    SelecaoAlmoxarifado('Al<u>m</u>oxarifado:','M', 'Selecione o almoxarifado onde o material será armazenado.', $p_chave,'p_chave',null,null,2);
    ShowHTML('      <tr>');
    selecaoTipoMatServSubord('<u>T</u>ipo de material/serviço:','S','Selecione o grupo/subgrupo de material/serviço desejado.',null,$p_pais,'p_pais','SUBTODOS',null,2);
    ShowHTML('      </tr>');
    ...
    SelecaoClasseCheck('Recuperar classes:','S',null,$p_fase,$P2,'p_fase[]',null,null);
...
function Entrada() {
  ...
  if ($p_tipo == 'WORD') {
    HeaderWord($_REQUEST['orientacao']);
    $w_linha_pag = ((nvl($_REQUEST['orientacao'],'PORTRAIT')=='PORTRAIT') ? 45: 30);
    CabecalhoWord($w_cliente,'Entradas',$w_pag);
    $w_embed = 'WORD';
    if ($w_filtro>'') ShowHTML($w_filtro);
  } elseif($p_tipo == 'PDF'){
    $w_linha_pag = ((nvl($_REQUEST['orientacao'],'PORTRAIT')=='PORTRAIT') ? 60: 35);
    $w_embed = 'WORD';
    HeaderPdf('Entradas',$w_pag);
    if ($w_filtro>'') ShowHTML($w_filtro);
  } else {
    $w_embed = 'HTML';
    Cabecalho();
    ...
    if ($O=='L') {
      CabecalhoRelatorio($w_cliente,'Entradas',4);
      if ($w_filtro>'') ShowHTML($w_filtro);
    } else {
      ShowHTML('<B><FONT COLOR="#000000">'.$w_TP.'</font></B>');
      ShowHTML('<HR>');
  ...
  if ($O=='L' || $w_embed == 'WORD') {
    ...
    foreach ($RS1 as $row) {
      ...
      ShowHTML('          <td>'.f($row,'sg_tip_doc').' '.f($row,'nr_doc').'</td>');
    ...
    ShowHTML('<INPUT type="hidden" name="w_troca" value="">');
    ShowHTML('      <tr>');
    SelecaoAlmoxarifado('Al<u>m</u>oxarifado:','M', 'Selecione o almoxarifado onde o material será armazenado.', $p_chave,'p_chave',null,null,1);
