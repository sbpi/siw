...
function Classif() {
  ...
  if ($p_inicio) {
    ...
    if (Nvl($p_ordena,'') > '') {
      ...
      $RS = SortArray($RS,'data_tombamento','asc','numero_rgp','asc'); 
    }
  }
  ...
  if ($O=='L') {
    ...
    if (!count($RS)) {
      ...
      foreach($RS as $row) {
        ...
        ShowHTML('        <td align="center">&nbsp;'.Nvl(FormataDataEdicao(f($row,'data_tombamento'),5),'-').'</td>');
