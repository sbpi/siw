...
function VisualAcordo($l_chave,$l_O,$l_usuario,$l_P1,$l_P4) {
      ...
      foreach($RSQuery as $row) {
        ...
        if ($l_P1==4) {
          ...
          if (substr($SG,0,3)=='GCR') {
            ...
            $w_html.=chr(13).'      <tr><td colspan="2"><font size="1">Forma de pagamento:<b><br>'.f($RS,'nm_forma_pagamento').'</td>';
          } else {
            ...
            $w_html.=chr(13).'      <tr><td colspan="2"><font size="1">Forma de pagamento/recebimento:<b><br>'.f($RS,'nm_forma_pagamento').'</td>';
          } 
          if (substr($SG,0,3)!='GCR') {
            $w_html.=chr(13).'      <tr><td colspan="2"><table border=0 width="100%" cellspacing=0>';
  ...
  $w_erro = ValidaAcordo($w_cliente,$l_chave,substr($SG,0,3).'GERAL',null,null,null,Nvl($w_tramite,0));
  if ($w_erro>'') {
