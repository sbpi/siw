...
function VisualAcordo($l_chave,$l_O,$l_usuario,$l_P1,$l_tipo) {
  ...
  if ($l_O=='L' || $l_O=='V') {
    ...
    if (substr($w_sigla,0,3)=='GCB'){ 
      ...
      $l_html.=chr(13).'        <td><b><font size=1>Plano de trabalho: </b></td>';
      $l_html.=chr(13).'        <td>'.CRLF2BR(f($RS,'objeto')).'</td></tr>';
    } else {                        
      $l_html.=chr(13).'      <tr valign="top">';
      $l_html.=chr(13).'        <td><b><font size=1>Objeto: </b></td>';
      $l_html.=chr(13).'        <td>'.CRLF2BR(f($RS,'objeto')).'</td></tr>';
    }
    $l_html.=chr(13).'      <tr><td valign="top"><b>Tipo:</b></td>';
    ...
    if(substr($w_sigla,0,3)=='GCR' || substr($w_sigla,0,3)=='GCD' || substr($w_sigla,0,3)=='GCZ') {
      ...
      if ($w_segmento=='Público' && substr($w_sigla,0,3)!='GCB') { 
        ...
        $l_html.=chr(13).'          <tr><td><b>Página D.O.:</b></td>';
        $l_html.=chr(13).'              <td>'.nvl(CRLF2BR(f($RS,'numero_certame')),'---').'</td></tr>';
      }        
    } else {
      $l_html.=chr(13).'          <tr><td><b>Vigência:</b></td>';
      $l_html.=chr(13).'              <td>'.FormataDataEdicao(f($RS,'inicio')).' a '.FormataDataEdicao(f($RS,'fim')).'</td></tr>';
    }
    ...
    if ($w_tipo_visao==0 || $w_tipo_visao==1) {
      ...
      if(($w_segmento=='Público' || $w_segmento=='Agência') && (substr($w_sigla,0,3)=='GCD' || substr($w_sigla,0,3)=='GCZ')) {
        ...
        if ($w_segmento=='Público' && substr($w_sigla,0,3)!='GCZ') {
          ...
          $l_html.=chr(13).'        <td>'.nvl(CRLF2BR(f($RS,'nm_espec_despesa')),'---').'</td></tr>';
        }
        $l_html.=chr(13).'      <tr valign="top">';
        $l_html.=chr(13).'          <td><b>Modalidade:</b></td>';
        $l_html.=chr(13).'          <td>'.Nvl(f($RS,'nm_lcmodalidade'),'---').'</td></tr>';
        ...
        if (substr($w_sigla,0,3)!='GCZ') {
          ...
          if(nvl(f($RS,'tipo_reajuste'),'')==1) {
            ...
            $l_html.=chr(13).'        <td>'.nvl(f($RS,'nm_eoindicador'),'---').' de '.nvl(f($RS,'indice_base'),'---');
            if (nvl(f($RS,'vl_indice_base'),'')!='') $l_html.=' ('.formatNumber(f($RS,'vl_indice_base'),4).')';
            else $l_html.=' (não informado)';
          }
          $l_html.=chr(13).'      <tr valign="top">';
          $l_html.=chr(13).'        <td><b>Alteração contratual:</b></td>';
    ...
    if ($l_P1==4 && substr($w_sigla,0,3)!='GCZ') {
      ...
      $l_html.=chr(13).'      <tr valign="top">';
      $l_html.=chr(13).'        <td ><b>Atividades a serem desenvolvidas:</b></td>';
      $l_html.=chr(13).'        <td>'.nvl(CRLF2BR(f($RS,'atividades')),'---').'</td></tr>';
      $l_html.=chr(13).'      <tr valign="top">';
      ...
      if (substr($w_sigla,0,3)=='GCB'){
        $l_html.=chr(13).'      <tr valign="top">';
        $l_html.=chr(13).'      <td><b>Código para o bolsista:</b></td>';
        $l_html.=chr(13).'      <td>'.Nvl(f($RS,'codigo_externo'),'---').'</td></tr>';
      } else {
        $l_html.=chr(13).'      <tr valign="top">';
        $l_html.=chr(13).'        <td><b>Código para a outra parte:</b></td>';
        $l_html.=chr(13).'        <td>'.Nvl(f($RS,'codigo_externo'),'---').'</td></tr>';
      }
      if ($w_tipo_visao!=2 && Nvl(f($RS,'cd_modalidade'),'')=='F') {
        $l_html.=chr(13).'      <tr><td><b>Pemite vinculação de projetos?</b></td>';
        $l_html.=chr(13).'        <td>'.f($RS,'nm_vincula_projeto').'</td></tr>';
        $l_html.=chr(13).'      <tr><td><b>Pemite vinculação de demandas?</b></td>';
        $l_html.=chr(13).'        <td>'.f($RS,'nm_vincula_demanda').'</td></tr>';
        $l_html.=chr(13).'       <tr><td><b>Pemite vinculação de viagens?</b></td>';
        $l_html.=chr(13).'        <td>'.f($RS,'nm_vincula_viagem').'</td></tr>';
      }
    } 
    // Outra parte
      ...
      foreach($RSQuery as $row) { 
        ...
        if ($l_P1==4) {
          ...
          if (f($RSQuery1,'sq_tipo_pessoa')==1) {
            $l_html.=chr(13).'      <tr><td colspan="2">';
            $l_html.=chr(13).'          <tr><td><b>Sexo:</b></td>'; 
            $l_html.=chr(13).'              <td>'.f($RSQuery1,'nm_sexo').'</td></tr>';
            $l_html.=chr(13).'          <tr><td><b>Data de nascimento:</b></td>'; 
            $l_html.=chr(13).'              <td>'.FormataDataEdicao(Nvl(f($RSQuery1,'nascimento'),'---')).'</td></tr>';
            $l_html.=chr(13).'          <tr><td><b>Identidade:</b></td>'; 
            $l_html.=chr(13).'              <td>'.f($RSQuery1,'rg_numero').'</td></tr>';
            $l_html.=chr(13).'          <tr><td><b>Data de emissão:</b></td>'; 
            $l_html.=chr(13).'              <td>'.FormataDataEdicao(Nvl(f($RSQuery1,'rg_emissao'),'---')).'</td>';
            $l_html.=chr(13).'          <tr><td><b>Órgão emissor:</b></td>'; 
            $l_html.=chr(13).'              <td>'.f($RSQuery1,'rg_emissor').'</td></tr>';
            $l_html.=chr(13).'          <tr><td><b>Passaporte:</b></td>'; 
            $l_html.=chr(13).'              <td>'.Nvl(f($RSQuery1,'passaporte_numero'),'---').'</td></tr>';
            $l_html.=chr(13).'          <tr><td><b>País emissor:</b></td>'; 
            $l_html.=chr(13).'              <td>'.Nvl(f($RSQuery1,'nm_pais_passaporte'),'---').'</td></tr>';
          } else {
            $l_html.=chr(13).'      <tr><td><b>Inscrição estadual:</b></td>'; 
            $l_html.=chr(13).'          <td>'.Nvl(f($RSQuery1,'inscricao_estadual'),'---').'</td></tr>';
          } 
          if (f($RSQuery1,'sq_tipo_pessoa')==1) {
          ...
          $l_html.=chr(13).'      <tr><td colspan="2">';
          $l_html.=chr(13).'          <tr valign="top">';
          $l_html.=chr(13).'            <td><b>Telefone:</b></td>'; 
          if (nvl(f($RSQuery1,'ddd'),'nulo')!='nulo') {
            $l_html.=chr(13).'            <td>('.f($RSQuery1,'ddd').') '.f($RSQuery1,'nr_telefone').'</td></tr>';
          } else {
            $l_html.=chr(13).'            <td>---</td></tr>';
          }
          $l_html.=chr(13).'          <tr><td><b>Fax:</b></td>'; 
          $l_html.=chr(13).'            <td>'.Nvl(f($RSQuery1,'nr_fax'),'---').'</td></tr>';
          $l_html.=chr(13).'          <tr><td><b>Celular:</b></td>'; 
          $l_html.=chr(13).'            <td>'.Nvl(f($RSQuery1,'nr_celular'),'---').'</td></tr>';
          $l_html.=chr(13).'          <tr valign="top">';
          $l_html.=chr(13).'             <td><b>Endereço:</b></td>'; 
          $l_html.=chr(13).'            <td>'.f($RSQuery1,'logradouro').'</td></tr>';
          $l_html.=chr(13).'          <tr><td><b>Complemento:</b></td>'; 
          $l_html.=chr(13).'            <td>'.Nvl(f($RSQuery1,'complemento'),'---').'</td></tr>';
          $l_html.=chr(13).'          <tr><td><b>Bairro:</b></td>'; 
          $l_html.=chr(13).'            <td>'.Nvl(f($RSQuery1,'bairro'),'---').'</td></tr>';
          $l_html.=chr(13).'          <tr valign="top">';
          if (f($RSQuery1,'pd_pais')=='S') {
            $l_html.=chr(13).'          <td><b>Cidade:</b></td>'; 
            $l_html.=chr(13).'          <td>'.f($RSQuery1,'nm_cidade').'-'.f($RSQuery1,'co_uf').'</td></tr>';
          } else {
            $l_html.=chr(13).'          <td><b>Cidade:</b></td>'; 
            $l_html.=chr(13).'          <td>'.f($RSQuery1,'nm_cidade').'-'.f($RSQuery1,'nm_pais').'</td></tr>';
          } 
          ...
          if (Nvl(f($RSQuery1,'email'),'nulo')!='nulo') {
            if ($l_tipo!='WORD') {
              ...
              $l_html.=chr(13).'              <tr><td><b>e-Mail:</b></td>';
              $l_html.=chr(13).'                <td>'.f($RSQuery1,'email').'</td></tr>';
            } 
          } else {
            $l_html.=chr(13).'              <tr><td><b>e-Mail:</b></td>';
            $l_html.=chr(13).'                <td>---</td></tr>';
          ...
          if (substr($w_sigla,0,3)!='GCR' && substr($w_sigla,0,3)!='GCZ') {
            if (!(strpos('CREDITO,DEPOSITO',f($RS,'sg_forma_pagamento'))===false)) {
              ...
              $l_html.=chr(13).'          <tr valign="top">';
              if (Nvl(f($RS,'cd_banco'),'')>'') {
                $l_html.=chr(13).'          <td><b>Banco:<b><br>'.f($RS,'cd_banco').' - '.f($RS,'nm_banco').'</td>';
                $l_html.=chr(13).'          <td><b>Agência:<b><br>'.f($RS,'cd_agencia').' - '.f($RS,'nm_agencia').'</td>';
              } else {
                $l_html.=chr(13).'          <td><b>Banco:<b><br>---</td>';
                $l_html.=chr(13).'          <td><b>Agência:<b><br>---</td>';
              } 
        ...
        if (Nvl(f($RS,'sq_tipo_pessoa'),0)==2 && $l_P1==4) {
          if ($w_tipo_visao!=2) {
            ...
            if (count($RSQuery1)==0) {
              ...
              foreach($RSQuery1 as $row1) {
                ...
                $l_html.=chr(13).'      <tr>';
                $l_html.=chr(13).'        <td >'.f($row1,'nm_pessoa').'</td>';
                $l_html.=chr(13).'        <td align="center">'.f($row1,'cpf').'</td>';
                ...
                $l_html.=chr(13).'        <td>'.Nvl(f($row1,'cargo'),'---').'</td>';
              }
          ...
          if (count($RSQuery)==0) {
            ...
            foreach($RSQuery as $row2) {
              ...
              $l_html.=chr(13).'        <td>'.Nvl(f($row2,'cargo'),'---').'</td>';
            }
            $l_html.=chr(13).'        </table></td></tr>';      
          } 
        }
