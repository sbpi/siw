...
$RS = new db_getSiwCliModLis; $RS = $RS->getInstanceOf($dbms,$w_cliente,null,'CO');
...
function BuscaCompra() {
  ...
  $RS = new db_getLinkData; $RS = $RS->getInstanceOf($dbms,$w_cliente,'CLLCCAD');
...
function Geral() {
  ...
  if ($O=='I' || $O=='A') {
    ...
    if($w_sq_menu_relac!='CLASSIF') { $RS_Relac = new db_getMenuData; $RS_Relac = $RS_Relac->getInstanceOf($dbms,$w_sq_menu_relac); }
    if(f($RS_Relac,'sg_modulo')=='PR') {
...
*       $RS1 = new db_getLinkData; $RS1 = $RS1->getInstanceOf($dbms,$w_cliente,substr($SG,0,3).'CAD');
...
function OutraParte() {
  ...
  if ($O=='L') {
    ...
    if (($w_cpf=='' && $w_cnpj=='') || strpos($_REQUEST['Botao'],'Alterar')!==false || strpos($_REQUEST['Botao'],'Procurar')!==false) {
      ...
      ShowHTML('      </table>');
      if ($w_nome>'') {
        $sql = new db_getBenef; $RS = $sql->getInstanceOf($dbms,$w_cliente,null,null,null,null,$w_nome,$w_sq_tipo_pessoa,null,null,null,null,null,null,null);
        ShowHTML('<tr><td colspan=3>');
        ShowHTML('    <TABLE WIDTH="100%" bgcolor="'.$conTableBgColor.'" BORDER="'.$conTableBorder.'" CELLSPACING="'.$conTableCellSpacing.'" CELLPADDING="'.$conTableCellPadding.'" BorderColorDark="'.$conTableBorderColorDark.'" BorderColorLight="'.$conTableBorderColorLight.'">');
        ShowHTML('        <tr bgcolor="'.$conTrBgColor.'" align="center">');
...
function Representante() {
  ...
  if ($w_troca>'') {
    ...
    if ($O=='L') {
      ...
      // Recupera os dados do beneficiário em co_pessoa
      $sql = new db_getBenef; $RS = $sql->getInstanceOf($dbms,$w_cliente,$w_sq_pessoa,null,$w_cpf,null,null,null,null,null,null,null,null,null,null);
      if (!count($RS)<=0) {
        foreach($RS as $row) {
          $w_sq_pessoa            = f($row,'sq_pessoa');
          $w_nome                 = f($row,'nm_pessoa');
  ...
  if ($O=='L') { 
    ...
    if ($w_cpf=='' || strpos($_REQUEST['Botao'],'Alterar')!==false || strpos($_REQUEST['Botao'],'Procurar')!==false) {
      ...
      ShowHTML('      </table>');
      if ($w_nome>'') {
        $sql = new db_getBenef; $RS = $sql->getInstanceOf($dbms,$w_cliente,null,null,null,null,$w_nome,1,null,null,null,null,null,null,null);// Recupera apenas pessoas físicas
...
function SolicMail($p_solic,$p_tipo) {
  ...
  if(f($RS,'envia_mail_tramite')=='S' && (f($RS_Menu,'envia_email')=='S') && (f($RSM,'envia_mail')=='S')) {
    ...
    // Outra parte
    $sql = new db_getBenef; $RSM1 = $sql->getInstanceOf($dbms,$w_cliente,Nvl(f($RSM,'outra_parte'),0),null,null,null,null,Nvl(f($RSM,'sq_tipo_pessoa'),0),null,null,null,null,null,null,null);
    if (count($RSM1)>0) {
      foreach($RSM1 as $row) {
...
function Grava() {
  ...
  if (substr($SG,3,3)=='CAD') {
    ...
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],upper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {    
      ...
      $SQL = new dml_putAcordoNota; $SQL->getInstanceOf($dbms,'EXCLUIPARCELA',
          $_REQUEST['w_chave_aux'],null,null,null,null,null,null,null,null,null,null,null,null,null,null,null);
      $SQL = new dml_putAcordoNota; $SQL->getInstanceOf($dbms, $O,
      ...
      for ($i=0; $i<=count($_POST['w_sq_acordo_parcela'])-1; $i=$i+1) {
        if (Nvl($_REQUEST['w_sq_acordo_parcela'][$i],'')>'') {
          $SQL = new dml_putAcordoNota; $SQL->getInstanceOf($dbms,'PARCELA',
