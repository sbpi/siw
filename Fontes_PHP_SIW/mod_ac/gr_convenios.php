...
function Gerencial() {
  ...
  if ($O=='L' || $O=='V' || $O=='W') {
    ...
    if ($p_proponente>'')   $w_filtro=$w_filtro.'<tr valign="top"><td align="right"><font size=1>Outra parte <td><font size=1>[<b>'.$p_proponente.'</b>]';
    if ($p_objeto>'')       $w_filtro=$w_filtro.'<tr valign="top"><td align="right"><font size=1>Objeto <td><font size=1>[<b>'.$p_objeto.'</b>]';
  ...
  if ($O=='L' || $O=='W') {
    ...
    if ($p_agrega==substr(f($RS_Menu,'sigla'),0,3).'ETAPA')             ShowHTML('          <option value="'.substr(f($RS_Menu,'sigla'),0,3).'ETAPA" selected>Alínea');             else ShowHTML('          <option value="'.substr(f($RS_Menu,'sigla'),0,3).'ETAPA">Alínea');
    ...
    SelecaoProjeto('Pro<u>j</u>eto:','J','Selecione o projeto do contrato na relação.',$p_projeto,$w_usuario,f($RS,'sq_menu'),'p_projeto','PJLIST',$w_atributo);
    ShowHTML('      </tr>');
    ShowHTML('          </table>');
    ...
    if($w_segmento=='Público') {
      ShowHTML('          <td><font size="1"><b><U>N</U>úmero do empenho:<br><INPUT ACCESSKEY="D" '.$w_Disabled.' class="sti" type="text" name="p_empenho" size="18" maxlength="18" value="'.$p_empenho.'"></td>');
...
function ImprimeCabecalho() {
  ...
  switch ($p_agrega) {
    case substr(f($RS_Menu,'sigla'),0,3).'ETAPA':       ShowHTML('          <td><font size="1"><b>Alínea</font></td>');             break;
