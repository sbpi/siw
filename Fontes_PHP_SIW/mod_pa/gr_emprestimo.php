...
$w_pagina       = 'gr_emprestimo.php?par=';
$w_Disabled     = 'ENABLED';
$w_dir          = 'mod_pa/';
...
$p_chave_pai     = strtoupper($_REQUEST['p_chave_pai']);
$p_empenho       = strtolower($_REQUEST['p_empenho']);
...
function Gerencial() {
  ...
  if ($O=='L' || $O=='V' || $p_tipo=='WORD' || $p_tipo=='PDF') {
    ...
    $RS1 = db_getSolicPA::getInstanceOf($dbms,$P2,$w_usuario,$p_agrega,5,
  ...
  if ($p_tipo == 'WORD') {
    ...
    if ($O=='P') {
      ...
      Validate('p_ini_i','Solicitação inicial','DATA','','10','10','','0123456789/');
      Validate('p_ini_f','Solicitação final','DATA','','10','10','','0123456789/');
      ShowHTML('  if ((theForm.p_ini_i.value != \'\' && theForm.p_ini_f.value == \'\') || (theForm.p_ini_i.value == \'\' && theForm.p_ini_f.value != \'\')) {');
      ShowHTML('     alert (\'Informe ambas as datas de SOLICITAÇÃO ou nenhuma delas!\');');
      ...
      ShowHTML('    return false;');
      ShowHTML('  }');      
      CompData('p_ini_i','Solicitação inicial','<=','p_ini_f','Solicitação final');
      Validate('p_fim_i','Devolução inicial','DATA','','10','10','','0123456789/');
      Validate('p_fim_f','Devolução final','DATA','','10','10','','0123456789/');
      ShowHTML('  if ((theForm.p_fim_i.value != \'\' && theForm.p_fim_f.value == \'\') || (theForm.p_fim_i.value == \'\' && theForm.p_fim_f.value != \'\')) {');
      ShowHTML('     alert (\'Informe ambas as datas da Devolução ou nenhuma delas!\');');
      ShowHTML('     theForm.p_fim_i.focus();');
      ShowHTML('     return false;');
      ShowHTML('  }');
      CompData('p_fim_i','Devolução inicial','<=','p_fim_f','Devolução final');
      ...
      if ($SG=='PROJETO') {
        ShowHTML('  if (theForm.p_agrega[theForm.p_agrega.selectedIndex].value==\'GREMETAPA\' && theForm.p_chave_pai.selectedIndex==0) {');
  ...
  if ($O=='L' || $w_embed == 'WORD') {
    ...
    if (count($RS1)<=0) {
      ...
      if ($O=='L' && $w_embed != 'WORD') {
        ...
        switch ($p_agrega) {
          case 'GREMETAPA':     ShowHTML('      document.Form.p_atividade.value=filtro;');      break;
          case 'GREMPROJ':      ShowHTML('      document.Form.p_chave_pai.value=filtro;');      break;
          case 'GREMPROP':      ShowHTML('      document.Form.p_proponente.value=filtro;');     break;
          case 'GREMRESP':      ShowHTML('      document.Form.p_solicitante.value=filtro;');    break;
        ...
        ShowHTML('    }');
        switch ($p_agrega) {
          case 'GREMETAPA':     ShowHTML('    else document.Form.p_atividade.value=\''.$_REQUEST['p_atividade'].'\';');       break;
          case 'GREMPROJ':      ShowHTML('    else document.Form.p_chave_pai.value=\''.$_REQUEST['p_chave_pai'].'\';');       break;
          case 'GREMPROP':      ShowHTML('    else document.Form.p_proponente.value=\''.$_REQUEST['p_proponente'].'\';');     break;
          case 'GREMRESP':      ShowHTML('    else document.Form.p_solicitante.value=\''.$_REQUEST['p_solicitante'].'\';');   break;
        ...
        switch ($p_agrega) {
          ...
          case 'GREMPROP':      if ($_REQUEST['p_proponente']=='')  ShowHTML('<input type="Hidden" name="p_proponente" value="">');   break;
          case 'GREMRESP':      if ($_REQUEST['p_solicitante']=='') ShowHTML('<input type="Hidden" name="p_solicitante" value="">');  break;
      ...
      foreach($RS1 as $row) {
        ...
        if ($w_embed == 'WORD' && $w_linha>$w_linha_pag) {
          ...
          ImprimeCabecalho();
          switch ($p_agrega) {
            case 'GREMETAPA':       ShowHTML('      <tr bgcolor="'.$w_cor.'" valign="top"><td><b>'.f($row,'nm_etapa'));          break;
            case 'GREMPROJ':        ShowHTML('      <tr bgcolor="'.$w_cor.'" valign="top"><td><b>'.f($row,'nm_projeto'));        break;
            case 'GREMPROP':        ShowHTML('      <tr bgcolor="'.$w_cor.'" valign="top"><td><b>'.f($row,'nm_pessoa_origem'));  break;
            case 'GREMRESP':        ShowHTML('      <tr bgcolor="'.$w_cor.'" valign="top"><td><b>'.f($row,'nm_solic'));          break;
            case 'GREMRESPATU':     ShowHTML('      <tr bgcolor="'.$w_cor.'" valign="top"><td><b>'.f($row,'nm_exec'));           break;
            case 'GREMCC':          ShowHTML('      <tr bgcolor="'.$w_cor.'" valign="top"><td><b>'.f($row,'sg_cc'));             break;
            case 'GREMSETOR':       ShowHTML('      <tr bgcolor="'.$w_cor.'" valign="top"><td><b>'.f($row,'nm_unidade_resp'));   break;
            case 'GREMPRIO':        ShowHTML('      <tr bgcolor="'.$w_cor.'" valign="top"><td><b>'.f($row,'nm_tipo_despacho'));  break;
            case 'GREMLOCAL':       ShowHTML('      <tr bgcolor="'.$w_cor.'" valign="top"><td><b>'.f($row,'co_uf'));             break;
            case 'GREMTIPDEM':      ShowHTML('      <tr bgcolor="'.$w_cor.'" valign="top"><td><b>'.f($row,'co_empenho'));        break;
          } 
          $w_linha = $w_linha + 1;
        } 
    ...
    //  if ($p_agrega=='GREMCC')      ShowHTML('          <option value="GREMCC" selected>Classificação');              else ShowHTML('          <option value="GREMCC">Classificação');
    //} 
    //if ($SG=='PROJETO') {
    //  if ($p_agrega=='GREMETAPA')   ShowHTML('          <option value="GREMETAPA" selected>Etapa de projeto');        else ShowHTML('          <option value="GREMETAPA">Etapa de projeto');
    //} 
    //if ($p_agrega=='GREMPRIO')      ShowHTML('          <option value="GREMPRIO" selected>Último despacho');                 else ShowHTML('          <option value="GREMPRIO">Último despacho');
    //if ($p_agrega=='GREMRESPATU')   ShowHTML('          <option value="GREMRESPATU" selected>Executor');              else ShowHTML('          <option value="GREMRESPATU">Executor');
    //if ($p_agrega=='GREMPROP')      ShowHTML('          <option value="GREMPROP" selected>Procedência externa');              else ShowHTML('          <option value="GREMPROP">Procedência externa');
    //if ($SG=='PROJETO') {
    //  if ($p_agrega=='GREMPROJ')    ShowHTML('          <option value="GREMPROJ" selected>Projeto');                  else ShowHTML('          <option value="GREMPROJ">Projeto');
    //} 
    if ($p_agrega=='GREMRESP')                       ShowHTML('          <option value="GREMRESP" selected>Solicitante');                else ShowHTML('          <option value="GREMRESP">Solicitante');
    if (Nvl($p_agrega,'GREMSETOR')=='GREMSETOR')     ShowHTML('          <option value="GREMSETOR" selected>Unidade solicitante');       else ShowHTML('          <option value="GREMSETOR">Unidade solicitante');
    //if (substr(f($RS_Menu,'sigla'),0,3)=='GDT') {
    //  if ($p_agrega=='GREMTIPDEM')  ShowHTML('          <option value="GREMTIPDEM" selected>Tipo da demanda');        else ShowHTML('          <option value="GREMTIPDEM">Tipo da demanda');
    //}
    //if ($p_agrega=='GREMLOCAL')     ShowHTML('          <option value="GREMLOCAL" selected>UF');                      else ShowHTML('          <option value="GREMLOCAL">UF');
    ShowHTML('          </select></td>');
...
function ImprimeCabecalho() {
  ...
  switch ($p_agrega) {
    case 'GREMETAPA':   ShowHTML('          <td><b>Etapa</td>');                break;
    case 'GREMPROJ':    ShowHTML('          <td><b>Projeto</td>');              break;
    case 'GREMPROP':    ShowHTML('          <td><b>Origem externa</td>');       break;
    case 'GREMRESP':    ShowHTML('          <td><b>Responsável</td>');          break;
    case 'GREMRESPATU': ShowHTML('          <td><b>Executor</td>');             break;
    case 'GREMCC':      ShowHTML('          <td><b>Classificação</td>');        break;
    case 'GREMSETOR':   ShowHTML('          <td><b>Unidade solicitante</td>');  break;
    case 'GREMPRIO':    ShowHTML('          <td><b>Último despacho</td>');      break;
    case 'GREMLOCAL':   ShowHTML('          <td><b>UF</td>');                   break;
    case 'GREMTIPDEM':  ShowHTML('          <td><b>Tipo de demanda</td>');      break;
  } 
  ShowHTML('          <td><b>Total</td>');
  ShowHTML('          <td><b>Cadastramento</td>');
  ShowHTML('          <td><b>Execução</td>');
  ShowHTML('          <td><b>Concluído</td>');
