...
function Geral() {
  ...
  if (strpos('IAEV',$O)!==false) {
    ...
    ShowHTML('      <tr><td colspan=3><b><u>O</u>bservações a respeito da eliminação:</b><br><textarea '.$w_Disabled.' accesskey="O" name="w_observacao" class="STI" ROWS=5 cols=75 title="É obrigatório registrar alguma observação a respeito da eliminação, permitindo a tomada de decisão pelos gestores.">'.$w_observacao.'</TEXTAREA></td>');
...
function Concluir() {
  ...
  Validate('w_eliminacao','Data da eliminação','DATA','1',10,10,'','0123456789/');
  CompData('w_eliminacao','Data da eliminação','<=',FormataDataEdicao(time()),'data atual');
  Validate('w_assinatura','Assinatura Eletrônica','1','1','6','30','1','1');
  ShowHTML('  theForm.Botao[0].disabled=true;');
  ShowHTML('  theForm.Botao[1].disabled=true;');
  ValidateClose();
    ...
    case 'PAELICONC':
      ...
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],upper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
        ...
        if (f($RS,'sq_siw_tramite')!=$_REQUEST['w_tramite']) {
          ...
          $concluir = true;
          foreach($RS as $row) {
            dml_putPAElimItem::getInstanceOf($dbms,$O,f($row,'chave'),$_REQUEST['w_chave'],$_REQUEST['w_eliminacao']);
          }
