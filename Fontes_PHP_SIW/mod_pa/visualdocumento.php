  ...
  if ($l_ocorrencia=='S') {
    ...
    if (count($RS)>0 && $l_ocorrencia=='S') {
      ...
      foreach($RS as $row) {
        ...
        $l_html.=chr(13).'        <td>'.ExibePessoa('../',$w_cliente,f($row,'sq_pessoa_resp'),$TP,f($row,'nm_pessoa_resp')).'</td>';
        $l_html.=chr(13).'        <td>'.ExibeUnidade('../',$w_cliente,f($row,'nm_origem'),f($row,'sq_origem'),$TP).'</td>';

        if (nvl(f($row,'sq_destinatario'),'')!='') {
          if (f($row,'tipo_destinatario')=='PESSOA') {
            $l_html.=chr(13).'        <td>'.ExibePessoa('../',$w_cliente,f($row,'sq_destinatario'),$TP,f($row,'nm_destinatario'));
            if (f($row,'interno')=='N') {
              $l_html.='<br>Pessoa: '.nvl(f($row,'pessoa_externa'),'---');
              $l_html.='<br>Unidade: '.nvl(f($row,'unidade_externa'),'---');
            }
          } else {
            $l_html.=chr(13).'        <td>'.ExibeUnidade('../',$w_cliente,f($row,'nm_destinatario'),f($row,'sq_destinatario'),$TP);
          }
        } else {
