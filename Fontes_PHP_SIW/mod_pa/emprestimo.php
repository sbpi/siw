          ...
          if ($P1!=3 && $P1!=5 && $P1!=6) {
            ...
            if ($w_copia>'') {
              ...
              if (f($row,'sg_tramite')=='EE') {
                ...
                //ShowHTML('          <A target="GF" HREF="'.montaURL_JS($w_dir,'relatorio.php?par=EmitirGF'.'&R='.$w_pagina.'IMPRIMIR'.'&O=L&w_chave='.f($row,'sq_siw_solicitacao').'&w_formato=WORD&orientacao=PORTRAIT&&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG).'" class="HL" title="Imprime a guia fora.">GF</A>&nbsp');
...
function Geral() {
  ...
  if ($O=='I' || $O=='A') {
    ...
    Validate('w_fim','Data de devolução','DATA','1',10,10,'1','0123456789/');
    CompData('w_fim','Data de devolução','>=',FormataDataEdicao(time()),'data atual');
  } 
  ValidateClose();
  ScriptClose();
  ShowHTML('</HEAD>');
  ...
  if (strpos('IAEV',$O)!==false) {
    ...
    ShowHTML('      <tr><td colspan=3><b><u>J</u>ustificativa da necessidade:</b><br><textarea '.$w_Disabled.' accesskey="J" name="w_justificativa" class="STI" ROWS=5 cols=75 title="É obrigatório justificar.">'.$w_justificativa.'</TEXTAREA></td>');
    ShowHTML('      <tr><td valign="top"><b><u>D</u>ata prevista para devolução:</b><br><input '.$w_Disabled.' accesskey="D" type="text" name="w_fim" class="STI" SIZE="10" MAXLENGTH="10" VALUE="'.$w_fim.'" onKeyDown="FormataData(this,event);" onKeyUp="SaltaCampo(this.form.name,this,10,event);" title="Data prevista para devolução dos itens.">'.ExibeCalendario('Form','w_fim').'</td>');
...
function Itens() {
  ...
  if ($w_troca>'' && $O <> 'E') {
    ...
    $sql = new db_getSolicList; $RS = $sql->getInstanceOf($dbms,f($RS,'sq_menu'),$w_usuario,f($RS,'sigla'),7,
          $p_ini_i,$p_ini_f,$p_fim_i,$p_fim_f,$p_atraso,$p_solicitante,
          $p_unidade,$p_prioridade,$p_ativo,$p_proponente,
          $p_chave, $p_assunto, $p_pais, $p_regiao, $p_uf, $p_cidade, $p_usu_resp,
    ...
    $sql = new db_getPAEmpItem; $RS = $sql->getInstanceOf($dbms,null,$w_chave,null,null,null,null);
    $RS = SortArray($RS,'protocolo','asc'); 
  } elseif (strpos('AEV',$O)!==false) {
    $sql = new db_getPAEmpItem; $RS = $sql->getInstanceOf($dbms,$w_chave_aux,null,null,null,null,null);
    foreach ($RS as $row) {$RS = $row; break;}
  ...
  ShowHTML('            <td>Data prevista de devolução:<b><br>'.formataDataEdicao(f($RS_Solic,'fim')).'</td>');
  ShowHTML('          <tr valign="top">');
  ...
  if ($O=='L') {
    ...
    ShowHTML('                <a accesskey="F" class="ss" HREF="javascript:this.status.value;" onClick="javascript:window.close(); opener.location.reload(); opener.focus();"><u>F</u>echar</a>&nbsp;');
...
function Concluir() {
  ...
  ShowHTML('<tr><td colspan=4><b>Informe para cada item a data de devolução. A solicitação será concluída quando todos os itens tiverem data de devolução informada.</b>');
  ...
  switch ($SG) {
    case 'PAEMP':
      ...
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {
        $SQL = new dml_putPAEmpGeral; $SQL->getInstanceOf($dbms,$O,$_REQUEST['w_chave'],$_REQUEST['w_copia'],$_REQUEST['w_menu'],$_SESSION['LOTACAO'],
