...
function Tramitacao() {
  ...
  if ($p_tipo_despacho == f($RS_Parametro, 'despacho_desmembrar'))
    $w_desmembrar = 'S'; else
    $w_desmembrar = 'N';
  ...
  if ($O == 'L') {
    if ($p_tipo_despacho == f($RS_Parametro, 'despacho_arqcentral')) {
      ...
      $sql = new db_getCaixa;
      $RS = $sql->getInstanceOf($dbms, $p_chave, $w_cliente, $w_usuario, nvl($w_sq_unidade, $_SESSION['LOTACAO']), null, null, null, null, null, null, null, 'TRAMITE');
      if (Nvl($p_ordena, '') > '') {
        $lista = explode(',', str_replace(' ', ',', $p_ordena));
  ...
  if ($O == 'P') {
    ...
    if ($w_envia_protocolo == 'N') {
      ...
      ShowHTML('     theForm.w_dias.value = \'\';');
      ShowHTML('  }');
      if ($w_envia_arquivo == 'N' && $w_envia_protocolo == 'N' && $p_tipo_despacho != f($RS_Parametro, 'despacho_arqsetorial') && $w_gestor)
        Validate('w_interno', 'Tipo da unidade/pessoa', 'SELECT', 1, 1, 1, 'SN', '');
      if ($w_interno == 'N') {
        Validate('w_pessoa_destino', 'Pessoa de destino', 'HIDDEN', 1, 1, 18, '', '0123456789');
        Validate('w_unidade_externa', 'Unidade externa', '', '', 2, 60, '1', '1');
  ...
  if ($O == 'L') {
    ...
    if ($w_outra_unidade)
      ShowHTML('      <tr><td colspan="11"><b>ATENÇÃO: Linha na cor vermelha indica que o protocolo está de posse de unidade diferente da sua!');
    ShowHTML('    </table>');
    ...
    if (count($RS) <= 0) {
      ...
      foreach ($RS as $row) {
        ...
        if (strlen(Nvl(f($row, 'observacao_setorial'), '-')) > 50)
          $w_titulo = substr(Nvl(f($row, 'observacao_setorial'), '-'), 0, 50) . '...'; else
...
//    SelecaoUnidade('<U>U</U>nidade emissora da guia:', 'U', 'Selecione a unidade emissora da guia.', $w_unid_autua, null, 'w_unid_autua', 'MOD_PA_PROT', null);
    SelecaoUnidade('<U>Ú</U>ltima procedência:', 'U', 'Selecione a unidade de posse.', $p_unid_autua, null, 'w_unid_autua', 'MOD_PA', null);
    ShowHTML('      <tr valign="top">');
...
function BuscaProtocolo() {
  ...
  if ($restricao == 'JUNTADA')
    $l_uf = 'S';

  $sql = new db_getSolicList;
  $RS = $sql->getInstanceOf($dbms, f($RS_Menu, 'sq_menu'), $w_usuario, f($RS_Menu, 'sigla'), 5,
  ...
  if ($SG == 'PADGERAL') {
    ...
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'], upper($_REQUEST['w_assinatura'])) || $w_assinatura == '') {
      ...
      if (nvl($_REQUEST['w_pessoa_destino'], '') != '') {
        $sql = new db_getBenef;
        $RS_Destino = $sql->getInstanceOf($dbms, $w_cliente, $_REQUEST['w_pessoa_destino'], null, null, null, null, null, null, null, null, null, null, null, null);
...
function Main() {
  ...
  switch ($par) {
    ...
    case 'CONCLUIR': Concluir();
      break;
    case 'BUSCAASSUNTO': BuscaAssunto();
      break;
    case 'TRAMIT': Tramitacao();
      break;
    case 'TRAMITCENTRAL': TramitCentral();
      break;
    case 'CLASSIF': Classificacao();
      break;
    case 'RECEB': Recebimento();
      break;
    case 'BUSCAPROTOCOLO': BuscaProtocolo();
      break;
    case 'GRAVA': Grava();
      break;
    default:
