...
if (strpos('PADOCANEXO,PAINTERESS,PADOCASS',$SG)!==false) {
  ...
  // Se for acompanhamento, entra na filtragem
    if ($P1==3 || $SG=='PADTRAM' || $SG=='PADRECEB' || $SG=='PACLASSIF') $O='P'; else $O='L';
} 
switch ($O) {
...
$sql = new db_getUnidade_PA; $RS_PAUnidade = $sql->getInstanceOf($dbms,$w_cliente,$_SESSION['LOTACAO'],null,null);
    ...
    if (!(strpos(upper($R),'GR_')===false)) {
      ...
      if ($p_proponente>'') {
        if (is_numeric($p_proponente)) {
          $sql = new db_getPersonData; $RS = $sql->getInstanceOf($dbms,$w_cliente,$p_proponente,null,null);
      ...
      if ($p_ini_i>'')            $w_filtro .= '<tr valign="top"><td align="right">Data criação/recebimento entre <td>[<b>'.$p_ini_i.'-'.$p_ini_f.'</b>]';
      if ($p_fim_i>'')            $w_filtro .= '<tr valign="top"><td align="right">Limite da tramitação entre <td>[<b>'.$p_fim_i.'-'.$p_fim_f.'</b>]';
  ...
  if ($O=='L') {
    ...
    if (count($RS)<=0) {
      ...
      foreach($RS1 as $row) {
        ...
        if ($w_embed!='WORD') ShowHTML('        <A class="HL" HREF="'.$w_dir.$w_pagina.'Visual&R='.$w_pagina.$par.'&O=L&w_chave='.f($row,'sq_siw_solicitacao').'&w_tipo=Volta&P1=2&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'" title="Exibe as informações deste registro.">'.f($row,'protocolo').'&nbsp;</a>');
...
function Geral() {
  ...
  $sql = new db_getUorgList; $RS = $sql->getInstanceOf($dbms,$w_cliente,$_SESSION['LOTACAO'],'PAUNID',null,null,$w_ano);
  if (count($RS)==0) {
    ScriptOpen('JavaScript');
  ...
  if (nvl($w_especie_documento,'')!='') {
    ...
    if ($O=='I' ||(nvl($w_assunto,'')=='' && nvl(f($RS,'sq_assunto'),'')!='')) $w_assunto = f($RS,'sq_assunto');
  }
  ...
  if ($O=='I' || $O=='A') {
    ...
    if ($w_gestor) Validate('w_interno','Procedência','SELECT',1,1,1,'SN','');
    if ($w_interno=='N') {
...
function Interessados() {
  ...
  if ($w_troca>'') {
    ...
    $sql = new db_getDocumentoInter; $RS = $sql->getInstanceOf($dbms,$w_chave,null,'N',null);
    $RS = SortArray($RS,'principal','desc','nome','asc');
  } elseif (!(strpos('AEV',$O)===false) && $w_troca=='') {
    // Recupera os dados do registro informado
    $sql = new db_getDocumentoInter; $RS = $sql->getInstanceOf($dbms,$w_chave,$w_chave_aux,'N',null);
    foreach($RS as $row){$RS=$row; break;}
...
function Assuntos() {
  ...
  if ($w_troca>'') {
    ...
    // Recupera todos os registros para a listagem
    $sql = new db_getDocumentoAssunto; $RS = $sql->getInstanceOf($dbms,$w_chave,null,'N',null);
    $RS = SortArray($RS,'descricao','asc');
  } elseif (!(strpos('AEV',$O)===false) && $w_troca=='') {
    // Recupera os dados do assunto informado
    $sql = new db_getDocumentoAssunto; $RS = $sql->getInstanceOf($dbms,$w_chave,$w_chave_aux,'N',null);
    foreach($RS as $row){$RS=$row; break;}
    $w_principal            = f($RS,'principal');
  } 
  Cabecalho();
  ...
  if ($O=='L') {
    ...
    if (nvl($w_chave_aux,'')!='') {
      $sql = new db_getAssunto_PA; $RS = $sql->getInstanceOf($dbms,$w_cliente,$w_chave_aux,null,null,null,null,null,null,null,null,'REGISTROS');
...
function Encaminhamento() {
  ...
  if (strpos('V',$O)!==false) {
    ...
    ShowHTML('  }');
    if ($w_gestor) Validate('w_interno','Tipo da unidade/pessoa','SELECT',1,1,1,'SN','');
    if ($w_interno=='N') {
...
function BuscaAssunto() {
  ...
  $sql = new db_getAssunto_PA; $RS = $sql->getInstanceOf($dbms,$w_cliente,$chave,null,$w_codigo,$w_nome,null,null,null,null,'S','BUSCA');
...
function Tramitacao() {
  ...
  if ($p_tipo_despacho>'') {
    $sql = new db_getTipoDespacho_PA; $RS = $sql->getInstanceOf($dbms,$p_tipo_despacho,$w_cliente,null,null,null,null);
    foreach ($RS as $row) {$RS = $row; break;}
  ...
  if ($O=='L') {
    if ($p_tipo_despacho==f($RS_Parametro,'despacho_arqcentral')) {
      ...
      $sql = new db_getCaixa; $RS = $sql->getInstanceOf($dbms, $p_chave, $w_cliente, $w_usuario,nvl($w_sq_unidade,$_SESSION['LOTACAO']), null, null, null, null,null, null, null, 'TRAMITE');
      if (Nvl($p_ordena,'')>'') {
        $lista = explode(',',str_replace(' ',',',$p_ordena));
  ...
  if ($O=='P') {
    ...
    if ($w_envia_protocolo=='N') {
      ...
      ShowHTML('     theForm.w_dias.value = \'\';');
      ShowHTML('  }');
      if ($w_envia_arquivo=='N' && $w_envia_protocolo=='N' && $p_tipo_despacho!=f($RS_Parametro,'despacho_arqsetorial') && $w_gestor) Validate('w_interno','Tipo da unidade/pessoa','SELECT',1,1,1,'SN','');
      if ($w_interno=='N') {
        Validate('w_pessoa_destino','Pessoa de destino','HIDDEN',1,1,18,'','0123456789');
        Validate('w_unidade_externa','Unidade externa','','',2,60,'1','1');
      } else {
  ...
  if ($O=='L') {
    ...
    if (count($RS)<=0) { 
      ...
      foreach ($RS as $row) {
        if (f($row,'st_mesma_lotacao')=='N') {
          ShowHTML('      <tr bgcolor="'.$conTrBgColorLightRed1.'">');
    ...
    if ($w_envia_arquivo=='S') {
      ...
      ShowHTML('      <tr><td colspan="3"  bgcolor="#f0f0f0" align=justify><font size="2"><b>DESTINO</b></font></td></tr>');
      ShowHTML('      <tr valign="top">');
      if ($p_tipo_despacho==f($RS_Parametro,'despacho_arqsetorial') || $w_gestor==false) {
    ...
    if ($w_desmembrar=='S') {
      ...
      ShowHTML('    <tr><td valign="top" colspan=3><b>Detalhamento do d<u>e</u>spacho:</b><br><textarea '.$w_Disabled.' accesskey="E" name="w_despacho" class="STI" ROWS=5 cols=75 title="Detalhe a ação a ser executada pelo destinatário.">'.nvl($w_despacho,(($p_tipo_despacho==f($RS_Parametro,'despacho_arqcentral')) ? 'Arquivar no Arquivo Central.' : '')).'</TEXTAREA></td>');
    }
...
function TramitCentral() {
  ...
  if ($O=='L') {
    $sql = new db_getProtocolo; $RS = $sql->getInstanceOf($dbms, f($RS_Menu,'sq_menu'), $w_usuario, $SG, $p_chave, $p_chave_aux, 
        ...
        $p_tipo_despacho, null, null, null, null, null, null, null);
    if (Nvl($p_ordena,'')>'') {
      $lista = explode(',',str_replace(' ',',',$p_ordena));
...
function Recebimento() {
  ...
  if ($O=='L') {
    ...
    $sql = new db_getProtocolo; $RS = $sql->getInstanceOf($dbms, $P2, $w_usuario, $SG, null, null, 
        $p_prefixo, $p_numero, $p_ano, $w_unid_autua, null, $w_nu_guia, $w_ano_guia, $p_ini, $p_fim, 
        null, null, $p_empenho, $p_solicitante, $p_unidade, $p_proponente, $p_sq_acao_ppa, $p_assunto, $p_processo);
    if (Nvl($p_ordena,'')>'') {
      $lista = explode(',',str_replace(' ',',',$p_ordena));
    ...
    $sql = new db_getProtocolo; $RS_Dados = $sql->getInstanceOf($dbms, $P2, $w_usuario, $SG, null, null, 
...
function BuscaProtocolo() {
  ...
  $sql = new db_getSolicList; $RS = $sql->getInstanceOf($dbms,f($RS_Menu,'sq_menu'),$w_usuario,f($RS_Menu,'sigla'),5,
  ...
  if (count($RS)>50 || $l_exibe>'') {
    ...
    Validate('l_assunto','Detalhamento do assunto','','','2','90','1','1');
  ...
  if (count($RS)>50 || $l_exibe>'') {
    ...
    ShowHTML('          <td><b>Detalhamento do <U>a</U>ssunto:<br><INPUT ACCESSKEY="A" '.$w_Disabled.' class="STI" type="text" name="l_assunto" size="40" maxlength="30" value="'.$l_assunto.'"></td>');
    ShowHTML('      <tr valign="top">');
...
function Grava() {
  ...
  if ($SG=='PADGERAL') {
    ...
    // Verifica se a Assinatura Eletrônica é válida
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],upper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
      $SQL = new dml_putDocumentoInter; $SQL->getInstanceOf($dbms,$O,$_REQUEST['w_chave'],$_REQUEST['w_chave_aux'],$_REQUEST['w_principal']);
      ScriptOpen('JavaScript');
    ...
    // Verifica se a Assinatura Eletrônica é válida
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],upper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
      $SQL = new dml_putDocumentoAssunto; $SQL->getInstanceOf($dbms,$O,null,$_REQUEST['w_chave'],$_REQUEST['w_chave_aux'],$_REQUEST['w_principal']);
      ScriptOpen('JavaScript');
      ShowHTML('  location.href=\''.montaURL_JS($w_dir,$R.'&w_chave='.$_REQUEST['w_chave'].'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET')).'\';');
      ScriptClose();
    } else {
    ...
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],upper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
      ...
      if (nvl($_REQUEST['w_pessoa_destino'],'')!='') {
        $sql = new db_getBenef; $RS_Destino = $sql->getInstanceOf($dbms,$w_cliente,$_REQUEST['w_pessoa_destino'],null,null,null,null,null,null,null,null,null,null,null,null);
        ...
        retornaFormulario('w_assinatura');
			  ...
			  // Se for informado um processo, verifica se ele existe
				if (nvl($_REQUEST['w_protocolo'],'')!='') {
					...
					$sql = new db_getProtocolo; $RS = $sql->getInstanceOf($dbms, f($RS_Menu,'sq_menu'), $w_usuario, 'EXISTE', $p_chave, $p_chave_aux, 
						$p_prefixo, $p_numero, $p_ano, $p_unid_autua, $p_unid_posse, $p_nu_guia, $p_ano_guia, $p_ini, $p_fim, 2, null, null, null, null, 
						null, null, null, null);
					$w_existe = 0;
					foreach($RS as $row) {
					  if (f($row,'processo')=='S') {
	      ...
	      $SQL = new dml_putDocumentoEnvio; $SQL->getInstanceOf($dbms,$_REQUEST['w_menu'],$_REQUEST['w_chave'],$w_usuario,$_REQUEST['w_tramite'],
    ...
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],upper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
      if (nvl($_REQUEST['w_arq_central'],'')=='S') {
        ...
        if (nvl($_REQUEST['w_protocolo'],'')!='') {
          ...
          $p_ano          = substr($_REQUEST['w_protocolo'],13,4);
          $sql = new db_getProtocolo; $RS = $sql->getInstanceOf($dbms, f($RS_Menu,'sq_menu'), $w_usuario, 'EXISTE', $p_chave, $p_chave_aux, 
              $p_prefixo, $p_numero, $p_ano, $p_unid_autua, $p_unid_posse, $p_nu_guia, $p_ano_guia, $p_ini, $p_fim, 2, null, null, null, 
              null, null, null, null, null);
          $w_existe = 0;
          foreach($RS as $row) {
            if (f($row,'processo')=='S') {
    ...
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],upper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
      ...
      if ($w_caixa==0) {
        $SQL = new dml_putCaixa; $SQL->getInstanceOf($dbms,'I',$w_cliente,null,$_SESSION['LOTACAO'],null,null,null,
    ...
    if (verificaAssinaturaEletronica($_SESSION['USERNAME'],upper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
      ...
      if (f($RS,'sq_siw_tramite')!=$_REQUEST['w_tramite'] || nvl(f($RS,'unidade_int_posse'),'')!=nvl($_REQUEST['w_unidade_posse'],'') || nvl(f($RS,'pessoa_ext_posse'),'')!=nvl($_REQUEST['w_pessoa_posse'],'')) {
        ...
        $SQL = new dml_putDocumentoConc; $SQL->getInstanceOf($dbms,$_REQUEST['w_menu'],$_REQUEST['w_chave'],$w_usuario,$_REQUEST['w_tramite'],$_REQUEST['w_inicio_real'],$_REQUEST['w_fim_real'],$_REQUEST['w_nota_conclusao'],$_REQUEST['w_custo_real']);
