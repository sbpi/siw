...
include_once($w_dir_volta.'classes/sp/dml_putConfCL.php');
...
function Vinculacao() {
  ...
  if ($O == 'L') {
    ...
    if ($O=='C')// Se for cópia
      ShowHTML('            <input class="STB" type="button" onClick="location.href=\''.montaURL_JS ($w_dir, $w_pagina.$par.'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.montaFiltro('GET')).'\';" name="Botao" value="Abandonar cópia">');
    else
      ShowHTML('            <input class="STB" type="button" onClick="location.href=\''.montaURL_JS ($w_dir, $w_pagina.$par.'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.montaFiltro('GET')).'\';" name="Botao" value="Remover filtro">');
    ShowHTML('          </td>');
    ShowHTML('      </tr>');
    ShowHTML('    </table>');
    ShowHTML('    </TD>');
...
// =========================================================================
// Rotina de configuração de viagens para um projeto
// -------------------------------------------------------------------------
function ConfiguraViagem() {
  extract($GLOBALS);
  global $w_Disabled;
  $w_chave     = $_REQUEST['w_chave'];
  ...
  if ($w_troca>'' && $O!='E') {
    ...
    $w_seguro         = $_REQUEST['w_seguro'];
    $w_bilhete        = $_REQUEST['w_bilhete'];
    $w_reembolso      = $_REQUEST['w_reembolso'];
    $w_ressarcimento  = $_REQUEST['w_ressarcimento'];
  } elseif ($O=='L') {
    $sql = new db_getPD_Financeiro; $RS_Fin = $sql->getInstanceOf($dbms,$w_cliente,$w_chave_aux,$w_chave,null,null,null,null,null,null,null,null,null,null);
    $RS_Fin = SortArray($RS_Fin,'cd_rubrica','asc','nm_rubrica','asc','nm_lancamento','asc');
  } elseif ($O!='I') {
    $sql = new db_getPD_Financeiro; $RS = $sql->getInstanceOf($dbms,$w_cliente,$w_chave_aux,$w_chave,null,null,null,null,null,null,null,null,null,null);
    ...
    $w_seguro        = f($RS,'seguro');
    $w_bilhete       = f($RS,'bilhete');
  ...
  ShowHTML('<TR><TD colspan=3><table border=1 width="100%"><tr><td bgcolor="#FAEBD7">');
  ShowHTML('    <TABLE WIDTH="100%" CELLSPACING="'.$conTableCellSpacing.'" CELLPADDING="'.$conTableCellPadding.'" BorderColorDark="'.$conTableBorderColorDark.'" BorderColorLight="'.$conTableBorderColorLight.'">');
  ShowHTML('        <tr><td colspan=3>Projeto:<br><b>'.$w_texto.'</b></td>');
  ShowHTML('        <tr><td colspan=3>Serviço:<br><b>'.upper(f($RS_Menu,'nome')).'</b></td>');
  ShowHTML('    </TABLE>');
  ShowHTML('</TABLE><BR>');

  ShowHTML('<tr><td colspan=3 bgcolor="'.$conTrBgColorLightBlue2.'"" style="border: 2px solid rgb(0,0,0);">Orientação:<ul><li>Inclua cada uma das rubricas e tipos de lançamento necessários para pagamento de diárias, hospedagens etc.<li>Os dados configurados nesta tela serão usados na integração com o módulo financeiro.</ul></b></font></td>');
  if ($O == 'L') {
    ShowHTML('<tr><td>');
    ShowHTML('<tr><td><a accesskey="I" class="SS" href="'.$w_dir.$w_pagina.$par.'&R='.$w_pagina.$par.'&O=I&w_chave='.$w_chave.'&w_menu='.$w_menu.'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'"><u>I</u>ncluir</a>&nbsp;');
    ShowHTML('        <a accesskey="F" class="ss" HREF="javascript:this.status.value;" onClick="window.close(); opener.focus();"><u>F</u>echar</a>&nbsp;');
    ShowHTML('    <td align="right">'.exportaOffice().'<b>Registros: '.count($RS_Fin));
    ShowHTML('<tr><td align="center" colspan=3>');
    ShowHTML('    <TABLE class="tudo" WIDTH="100%" bgcolor="'.$conTableBgColor.'" BORDER="'.$conTableBorder.'" CELLSPACING="'.$conTableCellSpacing.'" CELLPADDING="'.$conTableCellPadding.'" BorderColorDark="'.$conTableBorderColorDark.'" BorderColorLight="'.$conTableBorderColorLight.'">');
    ShowHTML('        <tr bgcolor="'.$conTrBgColor.'" align="center">');
    ...
    ShowHTML('         <td><b>'.LinkOrdena('Reemb.','reembolso').'</td>');
    ShowHTML('         <td><b>'.LinkOrdena('Devol.','ressarcimento').'</td>');
    ...
    if (count($RS_Fin)<=0) {
      ...
      foreach ($RS_Fin as $row) {
        ...
        ShowHTML('        <td>'.Nvl(f($row,'nm_lancamento'),'&nbsp;').'</td>');
        ShowHTML('        <td align="center">'.f($row,'nm_diaria').'</td>');
        ShowHTML('        <td align="center">'.f($row,'nm_hospedagem').'</td>');
        ShowHTML('        <td align="center">'.f($row,'nm_veiculo').'</td>');
        ShowHTML('        <td align="center">'.f($row,'nm_seguro').'</td>');
        ShowHTML('        <td align="center">'.f($row,'nm_bilhete').'</td>');
        ShowHTML('        <td align="center">'.f($row,'nm_reembolso').'</td>');
        ShowHTML('        <td align="center">'.f($row,'nm_ressarcimento').'</td>');
        ShowHTML('        <td class="remover" align="top" nowrap>');
        ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.$par.'&R='.$w_pagina.$par.'&O=A&w_chave='.$w_chave.'&w_menu='.$w_menu.'&w_chave_aux='.f($row,'chave').'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'" title="Altera as informações do registro">AL</A>&nbsp');
        ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.$par.'&R='.$w_pagina.$par.'&O=E&w_chave='.$w_chave.'&w_menu='.$w_menu.'&w_chave_aux='.f($row,'chave').'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'" title="Exclui o registro">EX</A>&nbsp');
        ShowHTML('      </td></tr>');
      } 
    } 
    ShowHTML('      </center>');
    ...
    MontaRadioSN("<b>Veículo?</b>",$w_veiculo,'w_veiculo');
    MontaRadioSN("<b>Seguro?</b>",$w_seguro,'w_seguro');
    MontaRadioSN("<b>Bilhete?</b>",$w_bilhete,'w_bilhete');
    MontaRadioSN("<b>Reembolso?</b>",$w_reembolso,'w_reembolso');
    MontaRadioSN("<b>Devolução de valores?</b>",$w_ressarcimento,'w_ressarcimento');
    ShowHTML('      </table>');
    ShowHTML('      <tr><td colspan=3><b>'.$_SESSION['LABEL_CAMPO'].':<BR> <INPUT ACCESSKEY="A" class="sti" type="PASSWORD" name="w_assinatura" size="30" maxlength="30" value=""></td></tr>');
    ShowHTML('      <tr><td align="center" colspan=5><hr>');
    if ($O=='E') {
    ...
    ShowHTML('            <input class="stb" type="button" onClick="location.href=\''.montaURL_JS($w_dir,$R.'&O=L&w_cliente='.$w_cliente.'&w_chave='.$w_chave.'&w_menu='.$w_menu.'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.montaFiltro('GET')).'\';" name="Botao" value="Cancelar">');
    ShowHTML('          </td>');
    ShowHTML('      </tr>');
    ShowHTML('    </table>');
    ShowHTML('    </TD>');
...
// =========================================================================
// Rotina de configuração de compras para um projeto
// -------------------------------------------------------------------------
function ConfiguraCompra() {
  extract($GLOBALS);
  global $w_Disabled;
  $w_chave     = $_REQUEST['w_chave'];
  $w_chave_aux = $_REQUEST['w_chave_aux'];
  ...
  if ($w_troca>'' && $O!='E') {
    ...
    $w_outros     = $_REQUEST['w_outros'];
  } elseif ($O=='L') {
    $sql = new db_getCLFinanceiro; $RS_Fin = $sql->getInstanceOf($dbms,$w_cliente,$w_menu, $w_chave,null,null,null,null,null,null,null,null);
    $RS_Fin = SortArray($RS_Fin,'cd_rubrica','asc','nm_rubrica','asc','nm_lancamento','asc');
  } elseif ($O!='I') {
    $sql = new db_getCLFinanceiro; $RS = $sql->getInstanceOf($dbms,$w_cliente,$w_menu, $w_chave, $w_chave_aux,null,null,null,null,null,null,null);
    foreach($RS as $row) { $RS = $row; break; }
    $w_rubrica    = f($RS,'sq_projeto_rubrica');
    $w_lancamento = f($RS,'sq_tipo_lancamento');
    $w_consumo    = f($RS,'consumo');
    $w_permanente = f($RS,'permanente');
    $w_servico       = f($RS,'servico');
    $w_outros     = f($RS,'outros');
  }
  ...
  if ($O == 'L') {
    ...
    if (count($RS_Fin)<=0) {
      ...
      foreach ($RS_Fin as $row) {
        ...
        ShowHTML('        <td align="center">'.f($row,'nm_servico').'</td>');
        ShowHTML('        <td align="center">'.f($row,'nm_outros').'</td>');
        ShowHTML('        <td class="remover" align="top" nowrap>');
        ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.$par.'&R='.$w_pagina.$par.'&O=A&w_chave='.$w_chave.'&w_menu='.$w_menu.'&w_chave_aux='.f($row,'chave').'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'" title="Altera as informações do registro">AL</A>&nbsp');
        ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.$par.'&R='.$w_pagina.$par.'&O=E&w_chave='.$w_chave.'&w_menu='.$w_menu.'&w_chave_aux='.f($row,'chave').'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'" title="Exclui o registro">EX</A>&nbsp');
        ShowHTML('      </td></tr>');
    ...
    MontaRadioSN("<b>Serviço?</b>",$w_servico,'w_servico');
    MontaRadioSN("<b>Outros?</b>",$w_outros,'w_outros');
    ShowHTML('      </table>');
    ShowHTML('      <tr><td colspan=3><b>'.$_SESSION['LABEL_CAMPO'].':<BR> <INPUT ACCESSKEY="A" class="sti" type="PASSWORD" name="w_assinatura" size="30" maxlength="30" value=""></td></tr>');
    ShowHTML('      <tr><td align="center" colspan=5><hr>');
    if ($O=='E') {
...
function CronPrestacao() {
  ...
  $sql = new db_getSolicData; $RS_Solic = $sql->getInstanceOf($dbms,$w_siw_solicitacao,'PJCAD');
  if ($w_troca>'' && $O!='E') {
    // Se for recarga da página
    $w_chave            = $_REQUEST['w_chave'];
    ...
    // Recupera todos os registros para a listagem
    $sql = new db_getContasCronograma; $RS = $sql->getInstanceOf($dbms,null,$w_siw_solicitacao,null,null,null,null,null,null);
    $RS = SortArray($RS,'tipo','desc','fim','desc');
  } elseif (!(strpos('AEV',$O)===false)) {
    // Recupera os dados do endereço informado
    $sql = new db_getContasCronograma; $RS = $sql->getInstanceOf($dbms,$w_chave,null,null,null,null,null,null,null);
    foreach ($RS as $row) {$RS = $row; break;}
    $w_chave            = f($RS,'chave');
  ...
  if ($O=='L') {
    ...
    ShowHTML('        <tr valign="top">');
    if($O=='I') {
      $sql = new db_getContasCronograma; $RS = $sql->getInstanceOf($dbms,null,$w_siw_solicitacao,null,null,null,null,'P',null);
...
function Grava() {
  ...
  switch ($SG) {
    ...
    case 'CONFCL':
      ...
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {
        if ($O!='E') {
          $sql = new db_getCLFinanceiro; $RS = $sql->getInstanceOf($dbms,$w_cliente,$w_menu,$_REQUEST['w_chave'],null,null,null,null,null,null,null,null);
          foreach ($RS as $row) {
            if (f($row,'sq_tipo_lancamento')==$_REQUEST['w_lancamento'] && f($row,'sq_projeto_rubrica')==$_REQUEST['w_rubrica'] && f($row,'chave')!=nvl($_REQUEST['w_chave_aux'],'0')) {
              ScriptOpen('JavaScript');
              ShowHTML('  alert(\'Combinação rubrica/tipo de lançamento já existe para o projeto!\');');
        ...
        $SQL = new dml_putConfCL; $SQL->getInstanceOf($dbms,$O,$w_cliente, $_REQUEST['w_menu'], $_REQUEST['w_chave'],
            Nvl($_REQUEST['w_chave_aux'],''),$_REQUEST['w_rubrica'],$_REQUEST['w_lancamento'],$_REQUEST['w_consumo'],
            $_REQUEST['w_permanente'],$_REQUEST['w_servico'],$_REQUEST['w_outros']);
        ScriptOpen('JavaScript');
    ...
    case 'CRONPREST':
      ...
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {
        if ($O!='E' && $_REQUEST['w_tipo']=='F') {
          $sql = new db_getContasCronograma; $RS = $sql->getInstanceOf($dbms,Nvl($_REQUEST['w_chave'],''),$_REQUEST['w_siw_solicitacao'],null,null,null,null,'F','EXISTE');
          if (count($RS)>0) {
            ScriptOpen('JavaScript');
        ...
        $SQL = new dml_putContasCronograma; $SQL->getInstanceOf($dbms,$O,Nvl($_REQUEST['w_chave'],''),$_REQUEST['w_siw_solicitacao'],$_REQUEST['w_prestacao_contas'],$_REQUEST['w_inicio'],$_REQUEST['w_fim'],$_REQUEST['w_limite']);
        ScriptOpen('JavaScript');
    ...
    case 'PRVINCITEM':
      ...
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],$w_assinatura) || $w_assinatura=='') {
        ...
        // Grava informação sobre relatório gerencial
        ...
        ScriptOpen('JavaScript');
        ShowHTML('  location.href=\''.montaURL_JS($w_dir,$R.'&O=L&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG=PERECURSO'.MontaFiltro('GET')).'\';');
        ScriptClose();
      } else {
        ScriptOpen('JavaScript');
        ShowHTML('  alert("'.$_SESSION['LABEL_ALERTA'].' inválida!");');
...
function Main() {
  ...
  global $w_Disabled;
  switch ($par) {
    case 'CONFIGURACOMPRA':    ConfiguraCompra();   break;
    case 'CONFIGURAVIAGEM':    ConfiguraViagem();   break;
