...
function Inicial() {
          ...
          if ($i==$w_ini_viagem) {
            ...
            if(Nvl($w_segundo_turno,'')!=''){
              if(Nvl($w_segundo_turno,'')=='tarde'){
                ...
                }elseif(Nvl($w_primeiro_turno,'')=='manhã'){
  ...
  // Exibe chamada para aprovação de folhas de ponto se o usuário for chefe de unidade
  if (count($RSUorg)) ShowHTML('  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a class="SS" href="'.$conRootSIW.'mod_rh/folha.php?par=lista&w_mes='.$w_mes.'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG=GPFPLISTA">ALTERNAR PARA MODO APROVAÇÃO</a>');
  ShowHTML('</tr>');
  ShowHTML('</FORM>');
  ...
  ShowHTML('          <td rowspan=2>SALDO<BR>DIÁRIO</td>');
  ShowHTML('        </tr>');
  ShowHTML('        <tr bgcolor="'.$conTrBgColor.'" align="center">');
  ShowHTML('          <td>ENTRADA</td>');
  ShowHTML('          <td>SAÍDA</td>');
  if(Nvl($w_segundo_turno,'')!=''){
    ShowHTML('          <td>ENTRADA</td>');
    ShowHTML('          <td>SAÍDA</td>');
  }
  ...
  for ($i=1; $i <= $w_dia_fim; $i++) {
    ...
    ShowHTML('        <tr bgcolor="'.$w_cor.'" align="center" '.(($w_imagem!='') ? 'title="Horários já autorizados pelo gestor!"' : '').'>');
  ...
  ShowHTML('      <tr valign="top"><td>Saldo mês (3)<td align="center"><input readonly type="text" name="w_banco1" class="stih" style="background-color:'.$conTableBgColor.'; border: 0; text-align:center;" SIZE="12" MAXLENGTH="12" VALUE="'.$w_banco.'">');
  ...
  if (nvl(f($RSMensal,'horas_autorizadas'),'')!='') ShowHTML('      <tr valign="top"><td>Horas já aprovadas<td align="center"><input readonly type="text" name="w_autorizadas" class="stih" style="background-color:'.$conTableBgColor.'; border: 0; text-align:center;" SIZE="12" MAXLENGTH="12" VALUE="'.f($RSMensal,'horas_autorizadas').'">');
  ...
  If(nvl(f($RSMensal,'ciencia_gestor'),'')!='') {
    ...
    ShowHTML('      <tr align="center" bgcolor="'.$conTrAlternateBgColor.'" valign="top"><td colspan="2"><b>CIÊNCIA DO GESTOR');    
...
// =========================================================================
// Rotina de aprovação em lote da folha de ponto
// -------------------------------------------------------------------------
function Lista() {
  extract($GLOBALS);
  $w_chave             = $_REQUEST['w_chave'];
  ...
  if (count($RSMensal)) {
	  ...
	  ShowHTML('  <tr bgcolor="'.$conTrBgColor.'"><td colspan="6"><div align="justify"><b><ul>Instruções</b>:');
	  ShowHTML('  <li>Use esta tela para aprovar folhas de ponto em lote.');
	  ShowHTML('  <li>Marque as folhas de ponto que deseja aprovar, informe sua assinatura eletrônica ao final desta página e clique sobre o botão gravar.');
	  ShowHTML('  <li>Serão aprovados todos os horários do mês corrente, anteriores à data atual.');
	  ShowHTML('  <li>Use a operação "Detalhar" para ver os horários da folha de ponto.');
	  ShowHTML('  </div></td></tr>');
	  $w_atual = '';
	  foreach($RSMensal as $row) {
	    if ($w_atual!=f($row,'sq_unidade')) {
	      ...
	      ShowHTML('      <tr><td colspan="6"><hr NOSHADE color=#000000 size=1></td></tr>');
			  ...
			  ShowHTML('        <td rowspan=2><b>Operações</b></td>');
			  ShowHTML('      </tr>');
			  ShowHTML('      <tr bgcolor="'.$conTrBgColor.'" align="center">');
...
// =========================================================================
// Rotina de visualização da folha de ponto mensal do colaborador
// -------------------------------------------------------------------------
function Visual() {
  extract($GLOBALS);
  Global $w_minutos_diarios;
  ...
  ShowHTML('<tr><td colspan="2" align="center"><br><font size="2"><b>FOLHA DE PONTO MÊS '.$w_mes.'</font></br></br></td></tr>');
  ...
  ShowHTML('      <tr><td>Movimentações mensais (2)<td align="center">&nbsp;'.$w_saldo_meses.'&nbsp;');
  ShowHTML('      <tr><td>Saldo mês (3)<td align="center">'.$w_banco);
  $w_tot_banco += horario2minutos('',$w_banco);
  ShowHTML('      <tr><td>Total (1+2+3)<td align="center">'.minutos2horario($w_tot_banco));
  if (nvl(f($RSMensal,'horas_autorizadas'),'')!='') ShowHTML('      <tr valign="top"><td>Horas já aprovadas<td align="center">'.f($RSMensal,'horas_autorizadas'));
  
  ShowHTML('      <tr align="center" bgcolor="'.$conTrAlternateBgColor.'"><td colspan="2"><b>JORNADA DIÁRIA');    
  If(nvl($w_entrada_manha,'')!='') ShowHTML('      <tr><td>Manhã<td nowrap align="center">&nbsp;'.$w_entrada_manha.'-'.$w_saida_manha.'&nbsp;');
  If(nvl($w_entrada_tarde,'')!='') ShowHTML('      <tr><td>Tarde<td nowrap align="center">&nbsp;'.$w_entrada_tarde.'-'.$w_saida_tarde.'&nbsp;');
  If(nvl($w_entrada_noite,'')!='') ShowHTML('      <tr><td>Noite<td nowrap align="center">&nbsp;'.$w_entrada_noite.'-'.$w_saida_noite.'&nbsp;');
  ShowHTML('      <tr><td>Carga horária<td align="center">&nbsp;'.$w_carga_diaria.'&nbsp;');
  ShowHTML('      <tr align="center" bgcolor="'.$conTrAlternateBgColor.'"><td colspan="2"><b>TOLERÂNCIA');
  ...
  switch ($SG) {
    ...
    case 'GPFPLISTA':
      ...
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],upper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
        ...
        ScriptOpen('JavaScript');
        ShowHTML('  alert("Autorizações registradas");');
        ScriptClose();
        retornaFormulario('w_assinatura');
      } else {
        ScriptOpen('JavaScript');
