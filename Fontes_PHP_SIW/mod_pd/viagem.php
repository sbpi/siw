...
function Inicial() {
  ...
  if ($O=='L') {
    if (!(strpos(strtoupper($R),'GR_')===false) || !(strpos(strtoupper($R),'PROJETO')===false)) {
      ...
      if ($p_projeto>'') {
        ...
        if (Nvl(f($RS,'cd_acao'),'')>'') {
          $w_filtro=$w_filtro.'<tr valign="top"><td align="right"><font size=1>Ação <td><font size=1>[<b><A class="HL" HREF="'.$w_dir.'acao.php?par=Visual&O=L&w_chave='.$p_projeto.'&w_tipo=Volta&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'" title="Exibe as informações da ação.">'.f($RS,'cd_unidade').'.'.f($RS,'cd_programa').'.'.f($RS,'cd_acao').' - '.f($RS,'nm_ppa').' ('.f($RS,'ds_unidade').')</a></b>]';
        } else {
          $w_filtro=$w_filtro.'<tr valign="top"><td align="right"><font size=1>Ação <td><font size=1>[<b><A class="HL" HREF="'.$w_dir.'acao.php?par=Visual&O=L&w_chave='.$p_projeto.'&w_tipo=Volta&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'" title="Exibe as informações da ação.">'.f($RS,'titulo').'</a></b>]';
        } 
      } 
      if ($p_atividade>'') {
        $RS = db_getSolicData_IS::getInstanceOf($dbms,$p_atividade,'ISTAGERAL');
        $w_filtro=$w_filtro.'<tr valign="top"><td align="right"><font size=1>Tarefa <td><font size=1>[<b><A class="HL" HREF="'.$w_dir.'tarefa.php?par=Visual&O=L&w_chave='.$p_atividade.'&w_tipo=Volta&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'" title="Exibe as informações da tarefa.">'.f($RS,'assunto').'('.f($RS,'sq_siw_solicitacao').')</a></b>]';
      } 
      if ($p_codigo>'') $w_filtro=$w_filtro.'<tr valign="top"><td align="right"><font size=1>PCD nº <td><font size=1>[<b>'.$p_codigo.'</b>]';
      ...
      if ($p_unidade>'') {
        ...
        $w_filtro=$w_filtro.'<tr valign="top"><td align="right"><font size=1>Unidade proponente <td><font size=1>[<b>'.f($RS,'nome').'</b>]';
      } 
      if ($p_proponente>'') $w_filtro=$w_filtro.'<tr valign="top"><td align="right"><font size=1>Proposto<td><font size=1>[<b>'.$p_proponente.'</b>]';
      if ($p_palavra>'') $w_filtro=$w_filtro.'<tr valign="top"><td align="right"><font size=1>CPF proposto <td><font size=1>[<b>'.$p_palavra.'</b>]';
      if ($p_pais>'') {
      ...
      if ($p_usu_resp>'') {
        ...
        $w_filtro=$w_filtro.'<tr valign="top"><td align="right"><font size=1>Companhia de viagem<td><font size=1>[<b>'.f($RS,'nome').'</b>]';
      } 
      if ($p_ativo>'') {
      ...
      if ($p_ini_i>'') $w_filtro=$w_filtro.'<tr valign="top"><td align="right"><font size=1>Mês <td><font size=1>[<b>'.$p_ini_i.'</b>]';
  ...
  if ($O=='L') {
    ...
    if ($RS->EOF) {
      ...
      while(!$RS->EOF && $RS->AbsolutePage==$P3) {
        ...
        if (f($RS,'concluida')=='N') {
          ...
          } elseif (f($RS,'aviso_prox_conc')=='S' && (f($RS,'aviso')<=time())) {
          ...
          if (f($RS,'fim')<Nvl(f($RS,'fim_real'),f($RS,'fim'))) {
        ...
        ShowHTML('        <A class="HL" HREF="'.$w_dir.$w_pagina.'Visual&R='.$w_pagina.$par.'&O=L&w_chave='.f($RS,'sq_siw_solicitacao').'&w_tipo=Volta&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'" title="Exibe as informações deste registro.">'.f($RS,'codigo_interno').'&nbsp;</a>');
        if (Nvl($_REQUEST['p_agrega'],'')=='GRPDACAO') {
          ShowHTML('        <td align="center"><font size="1">'.f($RS,'codigo_acao'));
        } 
        ShowHTML('        <td><font size="1">'.ExibePessoa('../',$w_cliente,f($RS,'sq_prop'),$TP,f($RS,'nm_prop')).'</td>');
        ShowHTML('        <td><font size="1">'.ExibeUnidade('../',$w_cliente,f($RS,'sg_unidade_resp'),f($RS,'sq_unidade_resp'),$TP).'</td>');
        if ($P1>2) {
          // Se for cadastramento ou mesa de trabalho, não exibe o executor, pois já é o usuário logado
          if (Nvl(f($RS,'nm_exec'),'---')>'---') {
            ShowHTML('        <td><font size="1">'.ExibePessoa('../',$w_cliente,f($RS,'executor'),$TP,f($RS,'nm_exec')).'</td>');
          } else {
            ShowHTML('        <td><font size="1">---</td>');
          } 
        } 
        if (f($RS,'sg_tramite')=='AT') {
          ShowHTML('        <td align="center"><font size="1">&nbsp;'.Nvl(FormataDataEdicao(f($RS,'inicio_real')),'-').'</td>');
          ShowHTML('        <td align="center"><font size="1">&nbsp;'.Nvl(FormataDataEdicao(f($RS,'fim_real')),'-').'</td>');
          if ($P1>1) {
            ShowHTML('        <td align="right"><font size="1">'.number_format(f($RS,'custo_real'),2,',','.').'&nbsp;</td>');
            $w_parcial += f($RS,'custo_real');
            ShowHTML('        <td nowrap><font size="1">'.f($RS,'nm_tramite').'</td>');
          } 
        } else {
          ShowHTML('        <td align="center"><font size="1">&nbsp;'.Nvl(FormataDataEdicao(f($RS,'inicio')),'-').'</td>');
          ShowHTML('        <td align="center"><font size="1">&nbsp;'.Nvl(FormataDataEdicao(f($RS,'fim')),'-').'</td>');
          if ($P1>1) {
            ShowHTML('        <td align="right"><font size="1">'.number_format(f($RS,'valor'),2,',','.').'&nbsp;</td>');
            $w_parcial += f($RS,'valor');
            ShowHTML('        <td nowrap><font size="1">'.f($RS,'nm_tramite').'</td>');
          } 
        } 
        ...
        if ($P1!=3 && $P1!=5) {
          ...
          if ($w_copia>'') {
            ...
            foreach($RS as $row) { $RS = $row; break; }
            ShowHTML('          <a accesskey="I" class="HL" href="'.$w_dir.$w_pagina.'Geral&R='.$w_pagina.$par.'&O=I&SG='.f($RS1,'sigla').'&w_menu='.$w_menu.'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&w_copia='.f($RS,'sq_siw_solicitacao').MontaFiltro('GET').'">Copiar</a>&nbsp;');
          } elseif ($P1==1) {
            // Se for cadastramento
            if ($w_submenu>'') {
              ShowHTML('          <A class="HL" HREF="menu.php?par=ExibeDocs&O=A&w_chave='.f($RS,'sq_siw_solicitacao').'&R='.$w_pagina.$par.'&SG='.$SG.'&TP='.$TP.'&w_documento='.f($RS,'codigo_interno').MontaFiltro('GET').'" title="Altera as informações cadastrais da PCD" TARGET="menu">Alterar</a>&nbsp;');
            } else {
              ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.$par.'&R='.$w_pagina.$par.'&O=A&w_chave='.f($RS,'sq_siw_solicitacao').'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'" title="Altera as informações cadastrais da PCD">Alterar</A>&nbsp');
            } 
            ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.'Excluir&R='.$w_pagina.$par.'&O=E&w_chave='.f($RS,'sq_siw_solicitacao').'&w_tipo=Volta&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'" title="Exclusão da PCD.">Excluir</A>&nbsp');
            ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.'Envio&R='.$w_pagina.$par.'&O=V&w_chave='.f($RS,'sq_siw_solicitacao').'&w_tipo=Volta&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'" title="Encaminhamento da PCD.">Enviar</A>&nbsp');
          } elseif ($P1==2) {
            // Se for execução
            if (f($RS,'sg_tramite')=='DF') {
              ShowHTML('          <A class="hl" HREF="javascript:location.href=this.location.href;" onClick="window.open(\''.$w_pagina.'DadosFinanceiros&R='.$w_pagina.$par.'&O=I&w_menu='.$w_menu.'&w_chave='.f($RS,'sq_siw_solicitacao').'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.' - Dados Financeiros&SG=DADFIN\',\'Financeiro\',\'toolbar=no,width=780,height=530,top=30,left=10,scrollbars=yes\');" title="Informar os dados financeiros da viagem.">Diárias</A>&nbsp');
            } elseif (f($RS,'sg_tramite')=='AE') {
              ShowHTML('          <A class="hl" HREF="javascript:location.href=this.location.href;" onClick="window.open(\''.$w_pagina.'Emissao&R='.$w_pagina.$par.'&O=L&w_menu='.$w_menu.'&w_chave='.f($RS,'sq_siw_solicitacao').'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'\',\'Financeiro\',\'toolbar=no,width=780,height=530,top=30,left=10,scrollbars=yes\');" title="Emitir autorização e proposta de concessão.">Emitir</A>&nbsp');
              ShowHTML('          <A class="hl" HREF="javascript:location.href=this.location.href;" onClick="window.open(\''.$w_pagina.'InformarPassagens&R='.$w_pagina.$par.'&O=I&w_menu='.$w_menu.'&w_chave='.f($RS,'sq_siw_solicitacao').'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.' - Informar dados das passagens&SG=INFPASS\',\'Passagens\',\'toolbar=no,width=780,height=530,top=30,left=10,scrollbars=yes\');" title="Informar os dados das passagens.">Informar</A>&nbsp');
            } elseif (f($RS,'sg_tramite')=='EE') {
              ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.'Anotacao&R='.$w_pagina.$par.'&O=V&w_chave='.f($RS,'sq_siw_solicitacao').'&w_tipo=Volta&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'" title="Registra anotações para a pcd, sem enviá-la.">Anotar</A>&nbsp');
            ...
            ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.'envio&R='.$w_pagina.$par.'&O=V&w_chave='.f($RS,'sq_siw_solicitacao').'&w_tipo=Volta&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'" title="Envia a PCD para outro responsável.">Enviar</A>&nbsp');
            if (f($RS,'sg_tramite')=='EE') {
              ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.'Concluir&R='.$w_pagina.$par.'&O=V&w_chave='.f($RS,'sq_siw_solicitacao').'&w_tipo=Volta&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'" title="Conclui a execução da pcd.">Concluir</A>&nbsp');
            } 
          } 
        } else {
          if (RetornaGestor(f($RS,'sq_siw_solicitacao'),$w_usuario)=='S') {
            ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.'envio&R='.$w_pagina.$par.'&O=V&w_chave='.f($RS,'sq_siw_solicitacao').'&w_tipo=Volta&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'" title="Envia a PCD para outro responsável.">Enviar</A>&nbsp');
          } else {
            ShowHTML('          ---&nbsp');
          } 
    ...
    ShowHTML('          <tr>');
    SelecaoProjeto('Pr<u>o</u>jeto:','O','Selecione o projeto da PCD na relação.',$p_projeto,$w_usuario,f($RS,'sq_menu'),'p_projeto','PJLIST',null);
    ShowHTML('          <tr>');
...
function Geral() {
  ...
  if ($w_troca>'') {
    ...
    if (Nvl($_REQUEST['w_cpf'],'')>'') {
      ...
      $RS = db_getBenef::getInstanceOf($dbms,$w_cliente,null,$_REQUEST['w_cpf'],null,null,1,null,null);
      if ($RS->RecordCount>0) {
        foreach($RS as $row) { $RS = $row; break; }
        $w_cpf          = f($RS,'cpf');
  ...
  if ($w_cadgeral=='N') {
    $RS = db_getBenef::getInstanceOf($dbms,$w_cliente,null,$_SESSION['USERNAME'],null,null,1,null,null);
    if ($RS->RecordCount>0) {
      foreach($RS as $row) { $RS = $row; break; }
      $w_cpf            = f($RS,'cpf');
      $w_sq_prop        = f($RS,'sq_pessoa');
      ...
      if ($w_nome>'') {
        ...
        if ($RS->EOF) {
          ...
          while(!$RS->EOF) {
            ...
            ShowHTML('        <td><font  size="1">'.f($RS,'nm_pessoa').'</td>');
            ShowHTML('        <td><font  size="1">'.f($RS,'nome_resumido').'</td>');
            ShowHTML('        <td align="center"><font  size="1">'.Nvl(f($RS,'cpf'),'---').'</td>');
            ShowHTML('        <td nowrap><font size="1">');
            ShowHTML('          <A class="hl" HREF="'.$w_dir.$w_pagina.$par.'&R='.$R.'&O=A&w_cpf='.f($RS,'cpf').'&w_sq_pessoa='.f($RS,'sq_pessoa').'&w_chave='.$w_chave.'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'&Botao=Selecionar">Selecionar</A>&nbsp');
            ShowHTML('        </td>');
            ShowHTML('      </tr>');
          } 
        } 
...
function Trechos() {
  ...
  if ($w_troca>'') {
    ...
    $w_data_saida   = FormataDataEdicao(f($RS,'saida'));
    $w_hora_saida   = FormataDataEdicao(f($RS,'saida'),2);
    $w_data_chegada = FormataDataEdicao(f($RS,'chegada'));
    $w_hora_chegada = FormataDataEdicao(f($RS,'chegada'),2);
  } 
  if ($O=='I') {
    if ($w_pais_orig=='') {
  ...
  if ($O=='L') {
    ...
    if ($RS->EOF) {
      ...
      while(!$RS->EOF) {
        $w_cor = ($w_cor==$conTrBgColor || $w_cor=='') ? $w_cor=$conTrAlternateBgColor : $w_cor=$conTrBgColor;
        ShowHTML('      <tr bgcolor="'.$w_cor.'" valign="top">');
        ShowHTML('        <td><font size="1">'.f($RS,'nm_origem').'</td>');
        ShowHTML('        <td><font size="1">'.f($RS,'nm_destino').'</td>');
        ShowHTML('        <td align="center"><font size="1">'.FormataDataEdicao(f($RS,'saida'),3).'</td>');
        ShowHTML('        <td align="center"><font size="1">'.FormataDataEdicao(f($RS,'chegada'),3).'</td>');
        ShowHTML('        <td align="top" nowrap><font size="1">');
        ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.$par.'&R='.$w_pagina.$par.'&O=A&w_chave_aux='.f($RS,'sq_deslocamento').'&w_chave='.f($RS,'sq_siw_solicitacao').'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'" title="Altera os dados do trecho.">Alterar</A>&nbsp');
        ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.'Grava&R='.$w_pagina.$par.'&O=E&w_chave_aux='.f($RS,'sq_deslocamento').'&w_chave='.f($RS,'sq_siw_solicitacao').'&w_tipo=Volta&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'" title="Exclusão do trecho." onClick="return(confirm(\'Confirma exclusão do trecho?\'));">Excluir</A>&nbsp');
        ShowHTML('        </td>');
        ShowHTML('      </tr>');
      } 
    } 
...
// =========================================================================
// Rotina de vinculação a tarefas e ações
// -------------------------------------------------------------------------
function Vinculacao() {
  extract($GLOBALS);
  global $w_Disabled;
  ...
  if ($O=='L') {
    ...
    $RS = db_getSolicList_IS::getInstanceOf($dbms,f($RS,'sq_menu'),$w_usuario,$SG,$P1,
  ...
  if ($O=='L') {
    ...
    if ($RS->EOF) {
      ...
      while(!$RS->EOF) {
        ...
        ShowHTML('        <A class="HL" HREF="'.$w_dir.'tarefas.php?par=visual&R='.$w_pagina.$par.'&O=L&w_chave='.f($RS,'sq_siw_solicitacao').'&w_tipo=Volta&P1=2&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'" title="Exibe as informações da tarefa.">'.f($RS,'sq_siw_solicitacao').'</a>');
        if (strlen(Nvl(f($RS,'titulo'),'-'))>50) $w_titulo=substr(Nvl(f($RS,'titulo'),'-'),0,50).'...'; else $w_titulo=Nvl(f($RS,'titulo'),'-');
        if (f($RS,'sg_tramite')=='CA') {
          ShowHTML('        <td title="'.str_replace('\\r\\n','\\n',str_replace('"','"',str_replace('\'','"',f($RS,'titulo')))).'"><font size="1"><strike>'.$w_titulo.'</strike></td>');
        } else {
          ShowHTML('        <td title="'.str_replace('\\r\\n','\\n',str_replace('"','"',str_replace('\'','"',f($RS,'titulo')))).'"><font size="1">'.$w_titulo.'</td>');
        } 
        if (f($RS,'concluida')=='N') {
          ShowHTML('        <td align="center"><font size="1">'.FormataDataEdicao(f($RS,'inicio')).'</td>');
          ShowHTML('        <td align="center"><font size="1">'.FormataDataEdicao(f($RS,'fim')).'</td>');
        } else {
          ShowHTML('        <td align="center"><font size="1">'.FormataDataEdicao(f($RS,'inicio_real')).'</td>');
          ShowHTML('        <td align="center"><font size="1">'.FormataDataEdicao(f($RS,'fim_real')).'</td>');
        } 
        ShowHTML('        <td><font size="1">'.f($RS,'nm_tramite').'</td>');
        ShowHTML('        <td align="top" nowrap><font size="1">');
        ShowHTML('          <A class="HL" HREF="'.$w_dir.$w_pagina.'Grava&R='.$w_pagina.$par.'&O=E&w_chave_aux='.f($RS,'sq_solic_missao').'&w_tarefa='.f($RS,'sq_siw_solicitacao').'&w_tipo=Volta&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'" title="Desvinculação da tarefa." onClick="return(confirm(\'Desvincula tarefa?\'));">Desvincular</A>&nbsp');
        ShowHTML('        </td>');
        ShowHTML('      </tr>');
      } 
    } 
    ...
    if (Nvl($p_assunto,'')>'') {
      $RS = db_getLinkData::getInstanceOf($dbms,$w_cliente,'ISTCAD');
      $RS = db_getSolicList_IS::getInstanceOf($dbms,f($RS,'sq_menu'),$w_usuario,'ISTCAD',4,
        null,null,null,null,null,null,null,null,null,null);
      $RS1->Sort='titulo';
      ShowHTML('<tr><td>');
      ShowHTML('    <TABLE WIDTH="100%" bgcolor="'.$conTableBgColor.'" BORDER="'.$conTableBorder.'" CELLSPACING="'.$conTableCellSpacing.'" CELLPADDING="'.$conTableCellPadding.'" BorderColorDark="'.$conTableBorderColorDark.'" BorderColorLight="'.$conTableBorderColorLight.'">');
      ...
      if ($RS->EOF) {
        ...
        while(!$RS->EOF) {
          $w_cor = ($w_cor==$conTrBgColor || $w_cor=='') ? $w_cor=$conTrAlternateBgColor : $w_cor=$conTrBgColor;
          ShowHTML('      <tr bgcolor="'.$w_cor.'" valign="top">');
          ShowHTML('          <td align="center"><input type="checkbox" name="w_tarefa" value="'.f($RS,'sq_siw_solicitacao').'">');
          ShowHTML('        <td nowrap><font size="1">');
          if (f($RS,'concluida')=='N') {
            if (f($RS,'fim')<time()) {
              ShowHTML('           <img src="'.$conImgAtraso.'" border=0 width=15 heigth=15 align="center">'); 
...
function DadosFinanceiros() {
  ...
  ShowHTML('            <tr><td><font size="1">Número:<b><br>'.f($RS,'codigo_interno').' ('.$w_chave.')</font></td>');
  $RS1 = db_getBenef::getInstanceOf($dbms,$w_cliente,Nvl(f($RS,'sq_prop'),0),null,null,null,1,null,null);
  foreach($RS1 as $row) { $RS1 = $row; break; }
  ShowHTML('                <td colspan="2"><font size="1">Proposto:<b><br>'.f($RS1,'nm_pessoa').'</font></td></tr>');
  $RS1->Close;
  ShowHTML('            <tr><td><font size="1">Tipo:<b><br>'.f($RS,'nm_tipo_missao').'</font></td>');
  ShowHTML('                <td><font size="1">Primeira saída:<br><b>'.FormataDataEdicao(f($RS,'inicio')).' </b></td>');
  ShowHTML('                <td><font size="1">Último retorno:<br><b>'.FormataDataEdicao(f($RS,'fim')).' </b></td></tr>');
  ShowHTML('          </TABLE></td></tr>');
  ShowHTML('      </table>');
  ShowHTML('  </table>');
  AbreForm('Form',$w_dir.$w_pagina.'Grava','POST','return(Validacao(this));',null,$P1,$P2,$P3,$P4,$TP,$SG,$R,$O);
  ...
  ShowHTML('            <td><font size="1"><b>Valor R$: </b><input type="text" name="w_vlr_alimentacao" class="sti" SIZE="10" MAXLENGTH="18" VALUE="'.number_format(Nvl(f($RS,'valor_alimentacao'),0),2,',','.').'" onKeyDown="FormataValor(this,18,2,event);" title="Informe o valor do auxílio-alimentação."></td>');
  ShowHTML('        </tr>');
  ShowHTML('        <tr valign="top">');
  ...
  ShowHTML('        <td><font size="1"><b>Valor R$: </b><input type="text" name="w_vlr_transporte" class="sti" SIZE="10" MAXLENGTH="18" VALUE="'.number_format(Nvl(f($RS,'valor_transporte'),0),2,',','.').'" onKeyDown="FormataValor(this,18,2,event);" title="Informe o valor do auxílio-transporte."></td>');
  ShowHTML('        </tr>');
  ...
  if (!$RS->EOF) {
    ...
    $i=1;
    while(!$RS->EOF) {
      $w_vetor_trechos[$i][1] = f($RS,'sq_diaria');
      $w_vetor_trechos[$i][2] = f($RS,'cidade_dest');
      $w_vetor_trechos[$i][3] = f($RS,'nm_destino');
      $w_vetor_trechos[$i][4] = FormataDataEdicao(f($RS,'saida'),3);
      $w_vetor_trechos[$i][5] = FormataDataEdicao(f($RS,'chegada'),3);
      $w_vetor_trechos[$i][6] = number_format(Nvl(f($RS,'quantidade'),0),1,',','.');
      $w_vetor_trechos[$i][7] = number_format(Nvl(f($RS,'valor'),0),2,',','.');
      $w_vetor_trechos[$i][8] = f($RS,'saida');
      $w_vetor_trechos[$i][9] = f($RS,'chegada');
      $i=$i+1;
    } 
...
function Encaminhamento() {
  ...
  if ($O=='V') {
    ...
    ValidateOpen('Validacao');
    if ($w_sg_tramite=='CI') {
      if (($w_inicio-$w_prazo)<time()) {
        Validate('w_justificativa','Justificativa','','1','1','2000','1','1');
      } 
    } else {
      if (Nvl(substr($w_erro,0,1),'')=='0' || $w_sg_tramite=='EE') {
        ...
        ShowHTML('     return false;');
        ShowHTML('  }');
        if (Nvl(substr($w_erro,0,1),'')=='1' || Nvl(substr($w_erro,0,1),'')=='2') {
          if (($w_inicio-$w_prazo)<time()) {
  ...
  ShowHTML(VisualViagem($w_chave,'V',$w_usuario,$P1,$P4));
  ShowHTML('<HR>');
  if (Nvl($w_erro,'')=='' || $w_sg_tramite=='EE' || (nvl(substr($w_erro,0,1),'0')=='2' && $w_sg_tramite=='CI') || (Nvl($w_erro,'')>'' && RetornaGestor($w_chave,$w_usuario)=='S')) {
    ...
    if ($P1!=1) {
      ...
      ShowHTML('      <input class="STB" type="button" onClick="location.href=\''.$R.'&O=L&w_chave='.$_REQUEST['w_chave'].'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'\';" name="Botao" value="Abandonar">');
    } 
    ShowHTML('      </td>');
    ShowHTML('    </tr>');
    ShowHTML('  </table>');
...
function Concluir() {
  ...
  if (!$RS->EOF) {
    ...
    while(!$RS->EOF) {
      $w_custo_real += Nvl(f($RS,'quantidade'),0)*Nvl(f($RS,'valor'),0);
      $w_fim_real   = f($RS,'chegada');
    } 
  } 
  ...
  ShowHTML('              <td align="right"><font size="1">'.number_format($w_custo_real,2,',','.').'</td>');
  ShowHTML('          </tr>');
  ShowHTML('          </table>');
...
function Emissao() {
  ...
  header('Content-Disposition'.': '.'attachment; filename=Emissão'.$w_chave.'.doc');
  header('Content-type: '.'application/msword');
  ...
  ShowHTML('\\par ');
  ShowHTML('\\par ');
  ShowHTML('\\par }\\pard \\ql \\li0\\ri0\\widctlpar\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0\\itap0\\pararsid9184405 {\\f1\\insrsid9184405 Autoriza\"e7\"e3o n\"ba '.substr(f($RS,'codigo_interno'),4));
  ShowHTML('\\par ');
  ShowHTML('\\par ');
  ShowHTML('\\par ');
  ShowHTML('\\par \\tab \\tab \\tab \\tab \\tab              Brasília-DF, '.FormataDataEdicao(time()).'.');
  ShowHTML('\\par ');
  ...
  ShowHTML('\\par }{\\f1\\insrsid9184405 ');
  ShowHTML('\\par ');
  if (($w_primeira_saida-time())<$w_prazo) {
  ...
  if (!$RS2->EOF) {
    ...
    while(!$RS2->EOF) {
      ...
      ShowHTML('\\ql \\li0\\ri0\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0\\pararsid12326642 {\\fs16\\insrsid16481867\\charrsid5664258 '.f($RS2,'nm_destino').'\\cell }\\pard \\qc \\li0\\ri0\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0\\pararsid12326642 {');
      ShowHTML('\\fs16\\insrsid16481867\\charrsid5664258 '.FormataDataEdicao(f($RS2,'saida')).'\\cell }\\pard \\qc \\li0\\ri0\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0\\pararsid6649499 {\\fs16\\insrsid16481867\\charrsid5664258 '.FormataDataEdicao(f($RS2,'chegada')).'\\cell }\\pard ');
      ShowHTML('\\qr \\li0\\ri0\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0\\pararsid12326642 {\\fs16\\insrsid16481867\\charrsid5664258 '.number_format(Nvl(f($RS2,'quantidade'),0),1,',','.').'\\cell '.number_format(Nvl(f($RS2,'valor'),0),2,',','.').'\\cell '.number_format((Nvl(f($RS2,'quantidade'),0)*Nvl(f($RS2,'valor'),0)),2,',','.').'\\cell }\\pard ');
      ...
      $w_total += Nvl(f($RS2,'quantidade'),0)*Nvl(f($RS2,'valor'),0);
    } 
  } 
...
function InformarPassagens() {
  ...
  if (!$RS->EOF) {
    ...
    $i = 1;
    while(!$RS->EOF) {
      $w_vetor_trechos[$i][1] = f($RS,'sq_deslocamento');
      $w_vetor_trechos[$i][2] = f($RS,'cidade_dest');
      $w_vetor_trechos[$i][10] = f($RS,'nm_origem');
      $w_vetor_trechos[$i][3] = f($RS,'nm_destino');
      $w_vetor_trechos[$i][4] = FormataDataEdicao(f($RS,'saida'),3);
      $w_vetor_trechos[$i][5] = FormataDataEdicao(f($RS,'chegada'),3);
      $w_vetor_trechos[$i][6] = f($RS,'sq_cia_transporte');
      $w_vetor_trechos[$i][7] = f($RS,'codigo_voo');
      $w_vetor_trechos[$i][8] = f($RS,'saida');
      $w_vetor_trechos[$i][9] = f($RS,'chegada');
      $i += 1;
    } 
...
function RelatorioViagem($w_chave) {
  ...
  if (!$RS1->EOF) {
    foreach($RS1 as $row) { $RS2 = $row; break; }
    $w_percurso = f($RS2,'nm_origem');
    $w_percurso .= '/'.f($RS2,'nm_destino');
    $w_diaria   += f($RS2,'quantidade');
    $w_valor    += (Nvl(f($RS2,'quantidade'),0)*Nvl(f($RS2,'valor'),0))+Nvl(f($RS,'valor_adicional'),0)-Nvl(f($RS,'desconto_alimentacao'),0)-Nvl(f($RS,'desconto_transporte'),0);
    $RS1->MoveNext;
    while(!$RS1->EOF) {
      $w_diaria     += f($RS1,'quantidade');
      $w_valor      += (Nvl(f($RS1,'quantidade'),0)*Nvl(f($RS1,'valor'),0));
      $w_percurso   .= '/'.f($RS1,'nm_destino');
    } 
  } 
  ...
  foreach($RS1 as $row) { $RS1 = $row; break; }
  $w_html='';
  $w_html .= '{\\rtf1\\ansi\\ansicpg1252\\uc1\\deff0\\stshfdbch0\\stshfloch0\\stshfhich0\\stshfbi0\\deflang1033\\deflangfe1033{\\fonttbl{\\f0\\froman\\fcharset0\\fprq2{\\*\\panose 02020603050405020304}Times New Roman;}{\\f1\\fswiss\\fcharset0\\fprq2{\\*\\panose 020b0604020202020204}Arial;}';
  $w_html .= '{\\f36\\froman\\fcharset238\\fprq2 Times New Roman CE;}{\\f37\\froman\\fcharset204\\fprq2 Times New Roman Cyr;}{\\f39\\froman\\fcharset161\\fprq2 Times New Roman Greek;}{\\f40\\froman\\fcharset162\\fprq2 Times New Roman Tur;}';
  $w_html .= '{\\f41\\froman\\fcharset177\\fprq2 Times New Roman (Hebrew);}{\\f42\\froman\\fcharset178\\fprq2 Times New Roman (Arabic);}{\\f43\\froman\\fcharset186\\fprq2 Times New Roman Baltic;}{\\f44\\froman\\fcharset163\\fprq2 Times New Roman (Vietnamese);}';
  $w_html .= '{\\f46\\fswiss\\fcharset238\\fprq2 Arial CE;}{\\f47\\fswiss\\fcharset204\\fprq2 Arial Cyr;}{\\f49\\fswiss\\fcharset161\\fprq2 Arial Greek;}{\\f50\\fswiss\\fcharset162\\fprq2 Arial Tur;}{\\f51\\fswiss\\fcharset177\\fprq2 Arial (Hebrew);}';
  $w_html .= '{\\f52\\fswiss\\fcharset178\\fprq2 Arial (Arabic);}{\\f53\\fswiss\\fcharset186\\fprq2 Arial Baltic;}{\\f54\\fswiss\\fcharset163\\fprq2 Arial (Vietnamese);}}{\\colortbl;\\red0\\green0\\blue0;\\red0\\green0\\blue255;\\red0\\green255\\blue255;\\red0\\green255\\blue0;';
  $w_html .= '\\red255\\green0\\blue255;\\red255\\green0\\blue0;\\red255\\green255\\blue0;\\red255\\green255\\blue255;\\red0\\green0\\blue128;\\red0\\green128\\blue128;\\red0\\green128\\blue0;\\red128\\green0\\blue128;\\red128\\green0\\blue0;\\red128\\green128\\blue0;\\red128\\green128\\blue128;';
  $w_html .= '\\red192\\green192\\blue192;}{\\stylesheet{\\ql \\li0\\ri0\\widctlpar\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0\\itap0 \\fs24\\lang1046\\langfe1046\\cgrid\\langnp1046\\langfenp1046 \\snext0 Normal;}{';
  $w_html .= '\\s1\\qc \\li0\\ri0\\keepn\\widctlpar\\aspalpha\\aspnum\\faauto\\outlinelevel0\\adjustright\\rin0\\lin0\\itap0 \\b\\i\\f1\\fs24\\lang1046\\langfe1046\\cgrid\\langnp1046\\langfenp1046 \\sbasedon0 \\snext0 heading 1;}{\\*\\cs10 \\additive \\ssemihidden Default Paragraph Font;}{\\*';
  $w_html .= '\\ts11\\tsrowd\\trftsWidthB3\\trpaddl108\\trpaddr108\\trpaddfl3\\trpaddft3\\trpaddfb3\\trpaddfr3\\tscellwidthfts0\\tsvertalt\\tsbrdrt\\tsbrdrl\\tsbrdrb\\tsbrdrr\\tsbrdrdgl\\tsbrdrdgr\\tsbrdrh\\tsbrdrv ';
  $w_html .= '\\ql \\li0\\ri0\\widctlpar\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0\\itap0 \\fs20\\lang1024\\langfe1024\\cgrid\\langnp1024\\langfenp1024 \\snext11 \\ssemihidden Normal Table;}}{\\*\\latentstyles\\lsdstimax156\\lsdlockeddef0}{\\*\\rsidtbl \\rsid1469200\\rsid6709522';
  $w_html .= '\\rsid13377954\\rsid13532367}{\\*\\generator Microsoft Word 11.0.5604;}{\\info{\\title RELAT\"d3RIO DE VIAGEM}{\\author TERESA SOARES}{\\operator Suporte T\"e9cnico}{\\creatim\\yr2006\\mo5\\dy12\\hr10\\min30}{\\revtim\\yr2006\\mo5\\dy12\\hr10\\min30}';
  $w_html .= '30302320070b0e0e0e282507212c0e120b212a2a2a2b0f04060401010101010115311401181c2f0400001801010115310401010101010102202e23242427272c';
  $w_html .= '0b0b0e280b26262623211a0e120b0b2d1318010f1c2e2e1b0c150f1c2e290b0b120e1a1a12070728272c0e120b2c2a2a2a2a27222d04010101010101012b2f18';
  $w_html .= '01041c140000010101012f0c010101010102080c2a2727272a272727120b0e280b122626262623221a1a12070905010f1c2e2e1b0c150118090b121a1a1a1207';
  $w_html .= '0707252727210e0b25272a2a2a2a2a2a2a1b080501010101010f311b0101182b0000010101182b040101050804080d272a27272a2727272420070e1a0b0b2926';
  $w_html .= '262623232c221a1a1207060f1c2e2e1b0c150607121a1a1a280b0707070b2c24271a0e0b1227272a2a2a2a2a2a2a2102040805010101142b0101010f00000101';
  $w_html .= '01040f080605020909021b2a2a272a2a2724272321070e1a250b0b212626232323232c22191912072d2b2e1b03071219191a280b0707070707212424271a0e07';
  $w_html .= '2824272a2a2a2a272a2a2a13090305020602041c0101010100000101010101030b060501080d24272a272a2a27272423210b121a120b0b252326232323232324';
  $w_html .= '21221920250b2d0b121919191a2507070707070728242424270e0e0b212424272a2a2a27272a27270c0105030706010401010101000001010101010808030308';
  $w_html .= '041b23242427272a27242326270b121a280b0b0b282323232323232323272c272a2a2a27272c2c280b070707070707252c2424242c1a120b21232427272a2a27';
  $w_html .= '242727242e0a090306020101010101010000010101010105050108030d261e171d232424242626262712251a280b0b0b0b292326232324272a2a2a2a2a272727';
  $w_html .= '272727272c1a250707070b2123242424211a120b2c242324272a2a2423242724241302010204010101010101000001010101010409030204111e1e16241e1e16';
  $w_html .= '1d1e1e242428071a1a0b0b0b0b0b2123242727272730302f2e3030302e2f303027272c212507292323242424211a0b122c24232424272a232626242426260803';
  $w_html .= '060101010101010100000101010101010406030d1e1717162417171d1d171727232907201a0b0b0b0b0b12242727272e30302e1c1b1b2b1b1b1c2e30302f242c';
  $w_html .= '2c29242323242424221a07282724232626262424242624241e1e0d080401010101010101000001010101010101040111171716161d17161d16161d241e1f0720';
  $w_html .= '19120b0b0b0b212c2730301b1c1c1c150f1b30140f142b1b1c1c30302423242323242427192007282724241e1e171d1d1e1e1e24171e1b040101010101010101';
  $w_html .= '000001010101010105090d1e1716171f1d161616171d1d1d171e092d19280b0b25212c30302f1b15151c300f140c2f151514301b15141c30303023232424242c';
  $w_html .= '1920091f24271e1e17161d1617171e24171d2603010101010101010100000101010101010408131f2c2c2a0f1e161d1e1317161d171e0925191a0b25212c301c';
  $w_html .= '2b1c2e0f0f1b302b2f2e2b2b2f2e2f0c0f142f1c2b2b30232324242c1912091f23272417171616171d16171d17171d0f01010101010101010000010101010101';
  $w_html .= '0208041422212a131e1d2c2a1317161d170c060b191a0b212330301b0c1b2f1c2e2e2b2e23272727232e2b2e2b1c2f0c0c1c3030232424211925060c26242324';
  $w_html .= '171616161d16161d17161d14050101010101010100000101010101050609090d22272a2a2a2c222a14101616170a040919221a2330302b1418142b2e2b23272a';
  $w_html .= '272727272727272e2b2f1c1518142e30302e2721190b050c26261e161d1716161616161d17161629060101010101010100000101010506080204020a2c2a2a2a';
  $w_html .= '2a2a272c0a11161613090307291f23232f2e2b30142e2b2e272730312f2f2f2b2f2f3027232b2e2b14302b2f2f2323221907092d1e1e1d16171d17161616161d';
  $w_html .= '1d1616110208060401010101000001040508060309090d2c2a2a2a2a2a2a2a1b020c161401020307202323301b1b1b2b302b23243031313131312f1b2b1c1b2f';
  $w_html .= '30232e2b302b1b0c1b302e1f2007060511171616172a271d1616161d2a2a1d100903060205040101000002090308050401182c2a2a2a2a27272a2a2a290d1305';
  $w_html .= '010101062d2e30300c0f152b2b2e30313131313131312f1b1b0c1c2f2f30232b2b1c0f0f1b30232e2d0601010f1716161d2a2a2a1d16161d2a2a2a2304050508';
  $w_html .= '03030501000005090909090d0d222a2a2a2a2a2727272a2a2c202d0d03060208292e302e1b1b0c2e2b23313131313131312b0c1414140c2f2f2f30231c2f0f1c';
  $w_html .= '1c30302e2806020a0d1e1d161d2a2a2a2a1d161d2a2a2a27220d09090903040100000105050405080a27272a272a2a2423272a27241402080a0309071f23302b';
  $w_html .= '2e302f1c2e3031313131313131312b141815141c2f2f2f302e1c2e302e2e3023220709030a0a1e161d2a272a272a1d1d2a2a2a27230f05040504010100000101';
  $w_html .= '050808021b24272a24272a2423242a2723110205040509292e301b0c0c1b2e2b30313131313131313131311c182f3131312f2f30231b2f0c0c0c1b302e2d0304';
  $w_html .= '04050c1d1d272727272a2a272a2a2a27240f080804010101000001010103090923232427262427242624241d1e1e0c0a0309201f2e301c180f2e1c2e32313131';
  $w_html .= '31313131313131311c313131312f2f2f302b1c1c180f2e302b1f2d09030a1324241e1e27242a2a24272a2a272420090601010101000001010101051826262424';
  $w_html .= '1e171d1e1e1e241617100c020920222e2e301c1b1b2b1b2e31312b2f3131313131312f313131313131312f2f302b1b1c1b0c2b302b1f222d0905141e1e161627';
  $w_html .= '23272723272a2a27241405010101010100000101010108091e1e1d1d17171d16171e1d1617172d0720221f2323302e2f2f2f1b2331312b1c1c1b313232312b1c';
  $w_html .= '2b1b313131312f2f2f2e1b2f2f2e2e302e1f222220090a1017161624241e1e1e242727242420020101010101000001010101040f1e171d1d17161d16171d1d16';
  $w_html .= '162d0720221f232e232f1c1b1b2e1b30312f1b1b0c1c323232311c1b0c1c31313131312f2f2e0c2f1b1b1c2f2e2928222220072d101617241e171716241e1e24';
  $w_html .= '2614010101010101000001010101040a1e17171d16161d16171d16162d0b20191f23262e2e1b15151b2e1b30311b150f0f143132321c150f0f142f313131312f';
  $w_html .= '2f2e0c300c0f142b2e2907122219200712101d1e171d17161d1616171e0a01010101010100000101010106091e17171d17161616171d162d0b20191f2626262e';
  $w_html .= '2e3014141b2e1b2f31312b18141c2f3232323118151c2b3131312f2f2f2e0c2e1b151b302b29070725221920072d1d17161d1d161d1616171709020101010101';
  $w_html .= '0000010101010104111617171d16161d1721250b1a191f262626262e2e302b2f2f2f0c2e31313114323232323232321b2f31311c2f2b1c2f302b1b2f302f2f30';
  $w_html .= '2b20070707251a19200b2d10161d17161d161d16110401010101010100000101040309091f1617171d16161d11090b1a191f2626262626262b302e1c1b2e1b1c';
  $w_html .= '3231323132323232323232313131311b1b1b2b2f301b1b2b1b1b2f301c2d07070707251a19200b2d101616171d17161620090903040101010000010102080805';
  $w_html .= '14171d1716161d21090b1a191f262626262626231c301b14141c1c1b2f313132323232313232323131311c0c0c0c1b2f2e0c2b1c14141c301b25070707070725';
  $w_html .= '1a1a200b2d10161d171d1610180508080501010100000102040402080c291d171d1d1025251a1a1f26262626262623231c231c0f141c2f0c1c3231323232322b';
  $w_html .= '1c2b1b31312f1c140f0f0c2b1b0c2f1c15152b2e11070707070707070b1a1a200b25101d161d16110608020405050101000002090925120d0d0a1b161d102525';
  $w_html .= '0e1a1f262626262626242c29222e301c2b2f2e1c0c2e31313232322b1b1b1b313131312f182b312b142b2e302b2b301c2e21282507070707070b1a1a0e0b2510';
  $w_html .= '1616110a030d250909090501000008090602050405080a101007250e1a1f26262626242128120b0b281b302f1b1c1c300c0c2f3131311c140c14142f31313131';
  $w_html .= '1c312e140c2f1c1b1c30231b2323232421282507070707200e0e0b2510100a02050405080309020100000104050603090903060809120e0e1f1e26262128120b';
  $w_html .= '0b0b0b0b0b112e301b15151c2f140c2f3131312b0f0f141c31313131312b14142f1b15151c301c2b232323232424242c28250707280e0e250b030a0309090306';
  $w_html .= '05040101000001010102060205080609120e0e1f242128120b0b0b0b0b0b0b0b0b201b2e0c0f0c302f2e14142b3131310f1c3131313131301b15142e2f2f0f14';
  $w_html .= '1b2f0c23232323232424242424242c2912280e0e1209060205020605010101010000010101010503090907120e0e212128120b0b0b0b0b0b0b0b0b0b0b0b131c';
  $w_html .= '302f2b2b1c2b2e0c0f141c2f2f313131322f1b150f1b2f2b1c2b1c2f301b2b23232323232424242424242427272c1a0e0e12070b070601010101010100000101';
  $w_html .= '010101020809120e0e0e0e0e1a1a1a1a1a28282812250b0b0b0b251b2b302f0c14142e2f2b140f0f0f15150f181818142b2f1c14140c2f301c1b232323232324';
  $w_html .= '24272c2c2121211a1a0e0e0e0e0e1209080501010101010100000101010101050307070b0b12120e0e0e1a1a1a1919192222211a1a28281a0c2e30151814302b';
  $w_html .= '2e2e2b1b14150f15141b2e2f2b2b300f180c302b0c23242c2c21211f221919191a1a1a0e0e0e12120b0b0707030101010101010100000101010101010409090d';
  $w_html .= '2d250b0707070b25121220201919222222221f1f1f0c2b2b2f1c1b0c141b302b2f2e2f2b2f2b2f0c140c1c1c2f2e1c141f1f2222222222191920201225250b07';
  $w_html .= '07070b25122d0903010101010101010100000101010101010502041527212c2c2c21292812250b0707070b252d202922221f0c1c30302b0f0f1b2f1b1b1c301b';
  $w_html .= '1b1b300c18152e30301b141f2229202d12250b090907070b2512281a212c2a2a271b040204010101010101010000010101010101050609031c2a2a272427272a';
  $w_html .= '1a19250d0f2b140d050d050a1f111f0c130c26260f02030202090801030705010101010101010101010101010000010101010101010101010101050805050605';
  $w_html .= '040608010303142424242624241e1e1e2402080a080d07201a2721281a200929110e22151513130f131813050213050903010608010206050208040101010101';
  $w_html .= '010101010101010100000101010101010101010101010101010101010101010209050d2324241e171d17171e1e1b0a080a0a06121a21211a1a25061515221c14';
  $w_html .= '0a0c13010d050a0a010a0a0209050101010101010101010101010101010101010101010100000101010101010101010101010101010101010101010102020d14';
  $w_html .= '1e17161d1d16171d1e1e20050d0a0507201a211a2007051513141414190c22151c14020d05050d05050101010101010101010101010101010101010101010101';
  $w_html .= '0000010101010101010101010101010101010101010101010403080113101617171d1e17171711040d02020b120e0e0e1209050a131f1a0c13131a1c1919180a';
  $w_html .= '0a010603010101010101010101010101010101010101010101010101000001010101010101010101010101010101010101010101020304020d0513161717180d';
  $w_html .= '0a0c13050d050803070e0e0e070305150f191a1b0c1b1c151a1914020d0504030401010101010101010101010101010101010101010101010000010101010101';
  $w_html .= '010101010101010101010101010101010105010306040d0f1011040d050606020d01080a09120e120603020f130c140c150d020d0c0c0d010306010501010101';
  $w_html .= '01010101010101010101010101010101010101010000010101010101010101010101010101010101010101010101020905080301080c0203010302080d010a06';
  $w_html .= '03070e07030308010d0c0c0a05090506020403020209020101010101010101010101010101010101010101010101010100000101010101010101010101010101';
  $w_html .= '01010101010101010108030601060501010101080409050a0a01030609070b070306060103080203010801010101020601030302010101010101010101010101';
  $w_html .= '01010101010101010101010100000101010101010101010101010101010101010101010101060706010401010101010102090103060103080906070309060301';
  $w_html .= '06060509050101010101010401060708010101010101010101010101010101010101010101010101000001010101010101010101010101010101010101010101';
  $w_html .= '01020604010101010101010102030409080109080605080506060301060301030501010101010101010506050101010101010101010101010101010101010101';
  $w_html .= '01010101000001010101010101010101010101010101010101010101010101010101010101010101010502090504030501010101010203010809050501010101';
  $w_html .= '01010101010101010101010101010101010101010101010101010101010101010000010101010101010101010101010101010101010101010101010101010101';
  $w_html .= '01010101010403070801050101010101010105010607060101010101010101010101010101010101010101010101010101010101010101010101010100000101';
  $w_html .= '01010101010101010101010101010101010101010101010101010101010101010101020304010101010101010101010105060501010101010101010101010101';
  $w_html .= '01010101010101010101010101010101010101010101010100000101010101010101010101010101010101010101010101010101010101010101010101010101';
  $w_html .= '010101010101010101010101010101010101010101010101010101010101010101010101010101010101010101010101010101010000040000002701ffff030000000000}\\par}}}{\\f1\\fs16\\insrsid1469200 Minist\"e9rio da Justi\"e7a';
  $w_html .= '\\par }\\pard \\qc \\li0\\ri0\\widctlpar\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0\\itap0 {\\f1\\fs16\\insrsid1469200 Secretaria Executiva';
  $w_html .= '\\par Subsecretaria de Planejamento, Or\"e7amento e Administra\"e7\"e3o';
  $w_html .= '\\par Coordena\"e7\"e3o-Geral de Log\"edstica';
  $w_html .= '\\par Divis\"e3o de Execu\"e7\"e3o Or\"e7ament\"e1ria e Financeira';
  $w_html .= '\\par ';
  $w_html .= '\\par }\\trowd \\irow0\\irowband0\\lastrow \\ts11\\trgaph70\\trleft-70\\trftsWidth1\\trpaddl70\\trpaddr70\\trpaddfl3\\trpaddfr3 \\clvertalt\\clbrdrt\\brdrtbl \\clbrdrl\\brdrtbl \\clbrdrb\\brdrtbl \\clbrdrr\\brdrtbl \\cltxlrtb\\clftsWidth3\\clwWidth9430\\clshdrawnil \\cellx9360';
  $w_html .= '\\pard\\plain \\s1\\qc \\li0\\ri0\\keepn\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\outlinelevel0\\adjustright\\rin0\\lin0 \\b\\i\\f1\\fs24\\lang1046\\langfe1046\\cgrid\\langnp1046\\langfenp1046 {\\insrsid1469200 RELAT\"d3RIO DE VIAGENS NACIONAIS/INTERNACIONAIS';
  $w_html .= '\\par }\\pard\\plain \\qc \\li0\\ri0\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0 \\fs24\\lang1046\\langfe1046\\cgrid\\langnp1046\\langfenp1046 {\\f1\\fs18\\insrsid1469200 Portaria N\"ba 47/MPO 29/04/2003 \\endash  DOU 30/04/2003\\cell }\\pard ';
  $w_html .= '\\ql \\li0\\ri0\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0 {\\b\\insrsid1469200 \\trowd \\irow0\\irowband0\\lastrow \\ts11\\trgaph70\\trleft-70\\trftsWidth1\\trpaddl70\\trpaddr70\\trpaddfl3\\trpaddfr3 \\clvertalt\\clbrdrt\\brdrtbl \\clbrdrl\\brdrtbl \\clbrdrb';
  $w_html .= '\\brdrtbl \\clbrdrr\\brdrtbl \\cltxlrtb\\clftsWidth3\\clwWidth9430\\clshdrawnil \\cellx9360\\row }\\pard \\ql \\li0\\ri0\\widctlpar\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0\\itap0 {\\b\\fs16\\insrsid1469200 ';
  $w_html .= '\\par }\\trowd \\irow0\\irowband0\\ts11\\trgaph70\\trleft-70\\trbrdrt\\brdrs\\brdrw10 \\trbrdrl\\brdrs\\brdrw10 \\trbrdrb\\brdrs\\brdrw10 \\trbrdrr\\brdrs\\brdrw10 \\trbrdrh\\brdrs\\brdrw10 \\trbrdrv\\brdrs\\brdrw10 \\trftsWidth1\\trpaddl70\\trpaddr70\\trpaddfl3\\trpaddfr3 \\clvertalt';
  $w_html .= '\\clbrdrt\\brdrs\\brdrw10 \\clbrdrl\\brdrs\\brdrw10 \\clbrdrb\\brdrs\\brdrw10 \\clbrdrr\\brdrs\\brdrw10 \\cltxlrtb\\clftsWidth3\\clwWidth9430\\clshdrawnil \\cellx9360\\pard\\plain ';
  $w_html .= '\\s1\\ql \\li0\\ri0\\keepn\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\outlinelevel0\\adjustright\\rin0\\lin0 \\b\\i\\f1\\fs24\\lang1046\\langfe1046\\cgrid\\langnp1046\\langfenp1046 {\\insrsid1469200 1 \\endash  IDENTIFICA\"c7\"c3O DO SERVIDOR\\cell }\\pard\\plain ';
  $w_html .= '\\ql \\li0\\ri0\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0 \\fs24\\lang1046\\langfe1046\\cgrid\\langnp1046\\langfenp1046 {\\insrsid1469200 \\trowd \\irow0\\irowband0\\ts11\\trgaph70\\trleft-70\\trbrdrt\\brdrs\\brdrw10 \\trbrdrl\\brdrs\\brdrw10 \\trbrdrb';
  $w_html .= '\\brdrs\\brdrw10 \\trbrdrr\\brdrs\\brdrw10 \\trbrdrh\\brdrs\\brdrw10 \\trbrdrv\\brdrs\\brdrw10 \\trftsWidth1\\trpaddl70\\trpaddr70\\trpaddfl3\\trpaddfr3 \\clvertalt\\clbrdrt\\brdrs\\brdrw10 \\clbrdrl\\brdrs\\brdrw10 \\clbrdrb\\brdrs\\brdrw10 \\clbrdrr\\brdrs\\brdrw10 ';
  $w_html .= '\\cltxlrtb\\clftsWidth3\\clwWidth9430\\clshdrawnil \\cellx9360\\row }\\pard \\ql \\li0\\ri0\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0 {\\insrsid1469200 ';
  $w_html .= '\\par }\\pard \\ql \\li0\\ri0\\widctlpar\\intbl\\tx5220\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0 {\\insrsid1469200 Nome:  }{\\insrsid6709522'.f($RS1,'nm_pessoa').']{\\insrsid1469200                }{\\insrsid6709522                 }{\\insrsid1469200 Matr\"ed';
  $w_html .= 'cula SIAPE: ';
  $w_html .= '\\par Fun\"e7\"e3o/Cargo : '.f($RS1,'nm_tipo_vinculo').'                             C\"f3digo: ';
  $RS2 = db_getCustomerData::getInstanceOf($dbms,$_SESSION['P_CLIENTE']);
  $w_html .= '\\par }\\pard \\ql \\li0\\ri0\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0 {\\insrsid1469200 \"d3rg\"e3o de Exerc\"edcio: '.f($RS2,'nome_resumido');
  $w_html .= '\\par \\cell }\\pard \\ql \\li0\\ri0\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0 {\\insrsid1469200 \\trowd \\irow1\\irowband1\\lastrow \\ts11\\trgaph70\\trleft-70\\trbrdrt\\brdrs\\brdrw10 \\trbrdrl\\brdrs\\brdrw10 \\trbrdrb\\brdrs\\brdrw10 \\trbrdrr\\brdrs\\brdrw10 ';
  $w_html .= '\\trbrdrh\\brdrs\\brdrw10 \\trbrdrv\\brdrs\\brdrw10 \\trftsWidth1\\trpaddl70\\trpaddr70\\trpaddfl3\\trpaddfr3 \\clvertalt\\clbrdrt\\brdrs\\brdrw10 \\clbrdrl\\brdrs\\brdrw10 \\clbrdrb\\brdrs\\brdrw10 \\clbrdrr\\brdrs\\brdrw10 \\cltxlrtb\\clftsWidth3\\clwWidth9430\\clshdrawnil ';
  $w_html .= '\\cellx9360\\row }\\pard \\ql \\li0\\ri0\\widctlpar\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0\\itap0 {\\fs16\\insrsid1469200 ';
  $w_html .= '\\par }\\pard\\plain \\s1\\ql \\li0\\ri0\\keepn\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\outlinelevel0\\adjustright\\rin0\\lin0 \\b\\i\\f1\\fs24\\lang1046\\langfe1046\\cgrid\\langnp1046\\langfenp1046 {\\insrsid1469200 2 \\endash  IDENTIFICA\"c7\"c3O DO AFASTAMENTO\\tab \\cell ';
  $w_html .= ']\\pard\\plain \\ql \\li0\\ri0\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0 \\fs24\\lang1046\\langfe1046\\cgrid\\langnp1046\\langfenp1046 {\\insrsid1469200 \\trowd \\irow0\\irowband0\\ts11\\trgaph70\\trleft-70\\trbrdrt\\brdrs\\brdrw10 \\trbrdrl\\brdrs\\brdrw10 ';
  $w_html .= '\\trbrdrb\\brdrs\\brdrw10 \\trbrdrr\\brdrs\\brdrw10 \\trbrdrh\\brdrs\\brdrw10 \\trbrdrv\\brdrs\\brdrw10 \\trftsWidth1\\trpaddl70\\trpaddr70\\trpaddfl3\\trpaddfr3 \\clvertalt\\clbrdrt\\brdrs\\brdrw10 \\clbrdrl\\brdrs\\brdrw10 \\clbrdrb\\brdrs\\brdrw10 \\clbrdrr\\brdrs\\brdrw10 ';
  $w_html .= '\\cltxlrtb\\clftsWidth3\\clwWidth9430\\clshdrawnil \\cellx9360\\row }\\pard \\ql \\li0\\ri0\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0 {\\insrsid1469200 Autoriza\"e7\"e3o do Afastamento: ';
  $w_html .= '\\par ';
  $w_html .= '\\par Percurso: '.strtolower($w_percurso).'     \\par Di\"e1rias recebidas: Qtd: '.number_format($w_diaria,1,',','.').' Valor: '.number_format($w_valor,2,',','.');
  $w_html .= '\\par ';
  $w_html .= '\\par }\\pard \\ql \\li0\\ri0\\widctlpar\\intbl\\tx5040\\tx5220\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0 {\\insrsid1469200 Sa\"edda: '.FormataDataEdicao(f($RS,'inicio')).'                                         Chegada:  '.FormataDataEdicao(f($RS,'fim')).'';
  $w_html .= '\\par }\\pard \\ql \\li0\\ri0\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0 {\\insrsid1469200 \\cell }\\pard \\ql \\li0\\ri0\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0 {\\insrsid1469200 \\trowd \\irow1\\irowband1\\lastrow ';
  $w_html .= '\\ts11\\trgaph70\\trleft-70\\trbrdrt\\brdrs\\brdrw10 \\trbrdrl\\brdrs\\brdrw10 \\trbrdrb\\brdrs\\brdrw10 \\trbrdrr\\brdrs\\brdrw10 \\trbrdrh\\brdrs\\brdrw10 \\trbrdrv\\brdrs\\brdrw10 \\trftsWidth1\\trpaddl70\\trpaddr70\\trpaddfl3\\trpaddfr3 \\clvertalt\\clbrdrt\\brdrs\\brdrw10 ';
  $w_html .= '\\clbrdrl\\brdrs\\brdrw10 \\clbrdrb\\brdrs\\brdrw10 \\clbrdrr\\brdrs\\brdrw10 \\cltxlrtb\\clftsWidth3\\clwWidth9430\\clshdrawnil \\cellx9360\\row }\\pard \\ql \\li0\\ri0\\widctlpar\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0\\itap0 {\\fs16\\insrsid1469200 ';
  $w_html .= '\\par }\\pard\\plain \\s1\\ql \\li0\\ri0\\keepn\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\outlinelevel0\\adjustright\\rin0\\lin0 \\b\\i\\f1\\fs24\\lang1046\\langfe1046\\cgrid\\langnp1046\\langfenp1046 {\\insrsid1469200 3 \\endash  DESCRI\"c7\"c3O SUCINTA DA VIAGEM\\cell }\\pard\\plain ';
  $w_html .= '\\ql \\li0\\ri0\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0 \\fs24\\lang1046\\langfe1046\\cgrid\\langnp1046\\langfenp1046 {\\insrsid1469200 \\trowd \\irow0\\irowband0\\ts11\\trgaph70\\trleft-70\\trbrdrt\\brdrs\\brdrw10 \\trbrdrl\\brdrs\\brdrw10 \\trbrdrb';
  $w_html .= '\\brdrs\\brdrw10 \\trbrdrr\\brdrs\\brdrw10 \\trbrdrh\\brdrs\\brdrw10 \\trbrdrv\\brdrs\\brdrw10 \\trftsWidth1\\trpaddl70\\trpaddr70\\trpaddfl3\\trpaddfr3 \\clvertalt\\clbrdrt\\brdrs\\brdrw10 \\clbrdrl\\brdrs\\brdrw10 \\clbrdrb\\brdrs\\brdrw10 \\clbrdrr\\brdrs\\brdrw10 ';
  $w_html .= '\\cltxlrtb\\clftsWidth3\\clwWidth9430\\clshdrawnil \\cellx9360\\row }\\trowd \\irow1\\irowband1\\ts11\\trgaph70\\trleft-70\\trbrdrt\\brdrs\\brdrw10 \\trbrdrl\\brdrs\\brdrw10 \\trbrdrb\\brdrs\\brdrw10 \\trbrdrr\\brdrs\\brdrw10 \\trbrdrh\\brdrs\\brdrw10 \\trbrdrv\\brdrs\\brdrw10 ';
  $w_html .= '\\trftsWidth1\\trpaddl70\\trpaddr70\\trpaddfl3\\trpaddfr3 \\clvertalt\\clbrdrt\\brdrs\\brdrw10 \\clbrdrl\\brdrs\\brdrw10 \\clbrdrb\\brdrs\\brdrw10 \\clbrdrr\\brdrs\\brdrw10 \\cltxlrtb\\clftsWidth3\\clwWidth1150\\clshdrawnil \\cellx1080\\clvertalt\\clbrdrt\\brdrs\\brdrw10 \\clbrdrl';
  $w_html .= '\\brdrs\\brdrw10 \\clbrdrb\\brdrs\\brdrw10 \\clbrdrr\\brdrs\\brdrw10 \\cltxlrtb\\clftsWidth3\\clwWidth8280\\clshdrawnil \\cellx9360\\pard\\plain \\s1\\qc \\li0\\ri0\\keepn\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\outlinelevel0\\adjustright\\rin0\\lin0 ';
  $w_html .= '\\b\\i\\f1\\fs24\\lang1046\\langfe1046\\cgrid\\langnp1046\\langfenp1046 {\\b0\\insrsid1469200 Data';
  $w_html .= '\\par }\\pard\\plain \\ql \\li0\\ri0\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0 \\fs24\\lang1046\\langfe1046\\cgrid\\langnp1046\\langfenp1046 {\\insrsid1469200 ';
  $w_html .= '\\par ';
  $w_html .= '\\par ';
  $w_html .= '\\par ';
  $w_html .= '\\par ';
  ...
  $w_html .= '\\par ';
  $w_html .= '\\par ';
  $w_html .= '\\par \\cell }\\pard\\plain \\s1\\qc \\li0\\ri0\\keepn\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\outlinelevel0\\adjustright\\rin0\\lin0 \\b\\i\\f1\\fs24\\lang1046\\langfe1046\\cgrid\\langnp1046\\langfenp1046 {\\b0\\insrsid1469200 Atividades';
  $w_html .= '\\par }\\pard\\plain \\ql \\li0\\ri0\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0 \\fs24\\lang1046\\langfe1046\\cgrid\\langnp1046\\langfenp1046 {\\insrsid1469200 ';
  $w_html .= '\\par ';
  $w_html .= '\\par ';
  $w_html .= '\\par \\cell }\\pard \\ql \\li0\\ri0\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0 {\\insrsid1469200 \\trowd \\irow1\\irowband1\\ts11\\trgaph70\\trleft-70\\trbrdrt\\brdrs\\brdrw10 \\trbrdrl\\brdrs\\brdrw10 \\trbrdrb\\brdrs\\brdrw10 \\trbrdrr\\brdrs\\brdrw10 \\trbrdrh';
  $w_html .= '\\brdrs\\brdrw10 \\trbrdrv\\brdrs\\brdrw10 \\trftsWidth1\\trpaddl70\\trpaddr70\\trpaddfl3\\trpaddfr3 \\clvertalt\\clbrdrt\\brdrs\\brdrw10 \\clbrdrl\\brdrs\\brdrw10 \\clbrdrb\\brdrs\\brdrw10 \\clbrdrr\\brdrs\\brdrw10 \\cltxlrtb\\clftsWidth3\\clwWidth1150\\clshdrawnil \\cellx1080';
  $w_html .= '\\clvertalt\\clbrdrt\\brdrs\\brdrw10 \\clbrdrl\\brdrs\\brdrw10 \\clbrdrb\\brdrs\\brdrw10 \\clbrdrr\\brdrs\\brdrw10 \\cltxlrtb\\clftsWidth3\\clwWidth8280\\clshdrawnil \\cellx9360\\row }\\trowd \\irow2\\irowband2\\lastrow \\ts11\\trgaph70\\trleft-70\\trbrdrt\\brdrs\\brdrw10 \\trbrdrl';
  $w_html .= '\\brdrs\\brdrw10 \\trbrdrb\\brdrs\\brdrw10 \\trbrdrr\\brdrs\\brdrw10 \\trbrdrh\\brdrs\\brdrw10 \\trbrdrv\\brdrs\\brdrw10 \\trftsWidth1\\trpaddl70\\trpaddr70\\trpaddfl3\\trpaddfr3 \\clvertalt\\clbrdrt\\brdrs\\brdrw10 \\clbrdrl\\brdrs\\brdrw10 \\clbrdrb\\brdrs\\brdrw10 \\clbrdrr';
  $w_html .= '\\brdrs\\brdrw10 \\cltxlrtb\\clftsWidth3\\clwWidth9430\\clshdrawnil \\cellx9360\\pard \\ql \\li0\\ri0\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0 {\\insrsid1469200 ';
  $w_html .= 'Data: _____/_____/_____                                __________________________________    ';
  $w_html .= '\\par                                                                                 Assinatura do Servidor/Colaborador';
  $w_html .= '\\par \\cell }\\pard \\ql \\li0\\ri0\\widctlpar\\intbl\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0 {\\insrsid1469200 \\trowd \\irow2\\irowband2\\lastrow \\ts11\\trgaph70\\trleft-70\\trbrdrt\\brdrs\\brdrw10 \\trbrdrl\\brdrs\\brdrw10 \\trbrdrb\\brdrs\\brdrw10 \\trbrdrr\\brdrs\\brdrw10 ';
  $w_html .= '\\trbrdrh\\brdrs\\brdrw10 \\trbrdrv\\brdrs\\brdrw10 \\trftsWidth1\\trpaddl70\\trpaddr70\\trpaddfl3\\trpaddfr3 \\clvertalt\\clbrdrt\\brdrs\\brdrw10 \\clbrdrl\\brdrs\\brdrw10 \\clbrdrb\\brdrs\\brdrw10 \\clbrdrr\\brdrs\\brdrw10 \\cltxlrtb\\clftsWidth3\\clwWidth9430\\clshdrawnil ';
  $w_html .= '\\cellx9360\\row }\\pard \\ql \\li0\\ri0\\widctlpar\\aspalpha\\aspnum\\faauto\\adjustright\\rin0\\lin0\\itap0 {\\insrsid1469200 ';
  $w_html .= '\\par }}';
...
function SolicMail($p_solic,$p_tipo) {
  ...
  $w_html='<HTML>'.'\r\n';
  $w_html .= BodyOpenMail(null).'\r\n';
  $w_html .= '<table border="0" cellpadding="0" cellspacing="0" width="100%">'.'\r\n';
  $w_html .= '<tr bgcolor="'.$conTrBgColor.'"><td align="center">'.'\r\n';
  $w_html .= '    <table width="97%" border="0">'.'\r\n';
  if ($p_tipo==1) {
    $w_html .= '      <tr valign="top"><td align="center"><font size=2><b>INCLUSÃO DE PCD</b></font><br><br><td></tr>'.'\r\n';
  } elseif ($w_sg_tramite=='EE') {
    $w_html .= '      <tr valign="top"><td align="center"><font size=2><b>PRESTAÇÃO DE CONTAS DE PCD</b></font><br><br><td></tr>'.'\r\n';
  } elseif ($p_tipo==2) {
    $w_html .= '      <tr valign="top"><td align="center"><font size=2><b>TRAMITAÇÃO DE PCD</b></font><br><br><td></tr>'.'\r\n';
  } elseif ($p_tipo==3) {
    $w_html .= '      <tr valign="top"><td align="center"><font size=2><b>CONCLUSÃO DE PCD</b></font><br><br><td></tr>'.'\r\n';
  } 
  if ($w_sg_tramite=='EE') {
    $w_html .= '      <tr valign="top"><td><font size=1><b><font color="#BC3131">ATENÇÃO</font>:<br>Conforme Portaria Nº 47/MPO 29/04/2003 – DOU 30/04/2003, é necessário elaborar o relatório de viagem e entregar os bilhetes de embarque.<br><br>Use o arquivo anexo para elaborar seu relatório de viagem e entregue-o assinado ao setor competente, juntamente com os bilhetes.</b></font><br><br><td></tr>'.'\r\n';
  } else {
    $w_html .= '      <tr valign="top"><td><font size=2><b><font color="#BC3131">ATENÇÃO</font>: Esta é uma mensagem de envio automático. Não responda esta mensagem.</b></font><br><br><td></tr>'.'\r\n';
  } 
  $w_html .= '\r\n'.'<tr bgcolor="'.$conTrBgColor.'"><td align="center">';
  $w_html .= '\r\n'.'    <table width="99%" border="0">';
  // Identificação da PCD
  $w_html .= '\r\n'.'      <tr><td valign="top" colspan="2" align="center" bgcolor="#D0D0D0" style="border: 2px solid rgb(0,0,0);"><font size="1"><b>EXTRATO DA PCD</td>';
  $w_html .= '\r\n'.'      <tr><td valign="top" colspan="2"><table border=0 width="100%" cellspacing=0>';
  $w_html .= '\r\n'.'          <tr valign="top">';
  $w_html .= '\r\n'.'          <td><font size="1">Proposto:<br><b>'.f($RSM,'nm_prop').'</b></td>';
  $w_html .= '\r\n'.'          <td><font size="1">Unidade proponente:<br><b>'.f($RSM,'nm_unidade_resp').'</b></td>';
  $w_html .= '\r\n'.'          <tr valign="top">';
  $w_html .= '\r\n'.'          <td><font size="1">Primeira saída:<br><b>'.FormataDataEdicao(f($RSM,'inicio')).' </b></td>';
  $w_html .= '\r\n'.'          <td><font size="1">Último retorno:<br><b>'.FormataDataEdicao(f($RSM,'fim')).' </b></td>';
  $w_html .= '\r\n'.'          </table>';
  // Informações adicionais
  if (Nvl(f($RSM,'descricao'),'')>'') {
    if (Nvl(f($RSM,'descricao'),'')>'') $w_html .= '\r\n'.'      <tr><td valign="top"><font size="1">Descrição da PCD:<br><b>'.CRLF2BR(f($RSM,'descricao')).' </b></td>';
  } 
  $w_html .= '\r\n'.'    </table>';
  $w_html .= '\r\n'.'</tr>';
  if ($p_tipo==2) {
    ...
    $w_html .= '\r\n'.'      <tr><td valign="top" colspan="2"><table border=0 width="100%" cellspacing=0>';
    $w_html .= '\r\n'.'          <tr valign="top">';
    $w_html .= '\r\n'.'          <td><font size="1">De:<br><b>'.f($RS,'responsavel').'</b></td>';
    $w_html .= '\r\n'.'          <td><font size="1">Despacho:<br><b>'.CRLF2BR(Nvl(f($RS,'despacho'),'---')).' </b></td>';
    $w_html .= '\r\n'.'          </table>';
  } 
  ...
  $w_html .= '      <tr valign="top"><td><font size=2>'.'\r\n';
  $w_html .= '         Para acessar o sistema use o endereço: <b><a class="SS" href="'.f($RS,'logradouro').'" target="_blank">'.f($RS,'Logradouro').'</a></b></li>'.'\r\n';
  $w_html .= '      </font></td></tr>'.'\r\n';
  $w_html .= '      <tr valign="top"><td><font size=2>'.'\r\n';
  $w_html .= '         Dados da ocorrência:<br>'.'\r\n';
  $w_html .= '         <ul>'.'\r\n';
  $w_html .= '         <li>Responsável: <b>'.$_SESSION['NOME'].'</b></li>'.'\r\n';
  $w_html .= '         <li>Data do servidor: <b>'.$FormatDateTime[time()][1].', '.strftime('%H:%M:%S %p').'</b></li>'.'\r\n';
  $w_html .= '         <li>IP de origem: <b>'.$_SERVER['REMOTE_HOST'].'</b></li>'.'\r\n';
  $w_html .= '         </ul>'.'\r\n';
  $w_html .= '      </font></td></tr>'.'\r\n';
  $w_html .= '    </table>'.'\r\n';
  $w_html .= '</td></tr>'.'\r\n';
  ...
  if ($w_sg_tramite=='EE') {
    ...
    $w_file = 'relatorio_'.str_replace('/','-',$w_nome).'.doc';
    $F1=$FS->CreateTextFile($conFilePhysical.$w_cliente.'\\'.$w_file,$true);
  ...
  switch ($SG) {
    case 'PDIDENT':
      ...
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
        ...
        dml_putViagemGeral($O,$w_cliente,
            $_REQUEST['w_chave'],$_REQUEST['w_menu'],$_SESSION['LOTACAO'],$_REQUEST['w_sq_unidade_resp'],
            $_REQUEST['w_sq_prop'],$_REQUEST['sq_pessoa'.'_session'],$_REQUEST['w_tipo_missao'],$_REQUEST['w_descricao'],
        ...
        if ($O=='I') {
          ...
          ShowHTML('  parent.menu.location=\'../menu.php?par=ExibeDocs&O=A&w_chave='.$w_chave_nova.'&w_documento='.$w_codigo.'&R='.$R.'&SG='.f($RS1,'sigla').'&TP='.RemoveTP($TP).'\';');
          ScriptClose();
        } elseif ($O=='E') {
          ScriptOpen('JavaScript');
          ShowHTML('  location.href=\''.$R.'&O=L&R='.$R.'&SG='.substr($SG,0,2).'INICIAL&w_menu='.$w_menu.'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.MontaFiltro('GET').'\';');
          ScriptClose();
        } else {
          // Aqui deve ser usada a variável de sessão para evitar erro na recuperação do link
          $RS1 = db_getLinkData::getInstanceOf($dbms,$_SESSION['P_CLIENTE'],$SG);
          ScriptOpen('JavaScript');
          ShowHTML('  location.href=\''.str_replace($w_dir,'',f($RS1,'link')).'&O='.$O.'&w_chave='.$_REQUEST['w_Chave'].'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'\';');
          ScriptClose();
        } 
      } else {
        ScriptOpen('JavaScript');
    ...
    case 'PDOUTRA':
      ...
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
        dml_putViagemOutra($O,$SG,
            ...
            $_REQUEST['w_ddd']);
        ScriptOpen('JavaScript');
        ShowHTML('  location.href=\''.$R.'&O='.$O.'&w_chave='.$_REQUEST['w_Chave'].'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'\';');
        ScriptClose();
      } else {
        ScriptOpen('JavaScript');
        ShowHTML('  alert(\'Assinatura Eletrônica inválida!\');');
    ...
    case 'PDTRECHO':
      ...
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
        ...
        ScriptOpen('JavaScript');
        // Aqui deve ser usada a variável de sessão para evitar erro na recuperação do link
        $RS1 = db_getLinkData::getInstanceOf($dbms,$_REQUEST['p_cliente'],$SG);
        ShowHTML('  location.href=\''.str_replace($w_dir,'',f($RS1,'link')).'&O=L&w_chave='.$_REQUEST['w_Chave'].'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'\';');
        ScriptClose();
      } else {
        ScriptOpen('JavaScript');
        ShowHTML('  alert(\'Assinatura Eletrônica inválida!\');');
    ...
    case 'PDVINC':
      ...
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
        if ($O=='I') {
          for ($w_cont=1; $w_cont<=$_POST['w_tarefa'].$Count; $w_cont=$w_cont+1) {
            if (Nvl($_POST['w_tarefa']($w_cont),'')>'') {
              dml_putPdTarefa($O,$_REQUEST['w_chave'],$_POST['w_tarefa']($w_cont));
            } 
          } 
        } elseif ($O=='E') {
          dml_putPdTarefa($O,$_REQUEST['w_chave_aux'],$_REQUEST['w_tarefa']);
        } 
        ScriptOpen('JavaScript');
        ShowHTML('  location.href=\''.$R.'&O=L&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.'&w_chave='.$_REQUEST['w_chave'].'\';');
        ScriptClose();
      } else {
        ScriptOpen('JavaScript');
        ShowHTML('  alert(\'Assinatura Eletrônica inválida!\');');
    ...
    case 'DADFIN':
      // Verifica se a Assinatura Eletrônica é válida
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
        dml_putPDMissao(null,$_REQUEST['w_chave'],Nvl($_REQUEST['w_vlr_alimentacao'],0),Nvl($_REQUEST['w_vlr_transporte'],0),Nvl($_REQUEST['w_adicional'],0),
            Nvl($_REQUEST['w_desc_alimentacao'],0),Nvl($_REQUEST['w_desc_trasnporte'],0),null,null,null,null);
        for ($i=1; $i<=$_POST['w_sq_diaria'].$Count; $i=$i+1) {
          if ($_POST['w_sq_diaria']($i)>'') {
            dml_putPDDiaria('A',$_REQUEST['w_chave'],$_POST['w_sq_diaria']($i),$_POST['w_sq_cidade']($i),
                Nvl($_POST['w_qtd_diarias']($i),0),Nvl($_POST['w_vlr_diarias']($i),0));
          } else {
            dml_putPDDiaria('I',$_REQUEST['w_chave'],null,$_POST['w_sq_cidade']($i),
                Nvl($_POST['w_qtd_diarias']($i),0),Nvl($_POST['w_vlr_diarias']($i),0));
          } 
        } 
        ScriptOpen('JavaScript');
        ShowHTML('  location.href=\''.$w_pagina.'DadosFinanceiros&O='.$O.'&w_chave='.$_REQUEST['w_Chave'].'&w_menu='.$_REQUEST['w_menu'].'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'\';');
        ScriptClose();
      } else {
        ScriptOpen('JavaScript');
        ShowHTML('  alert(\'Assinatura Eletrônica inválida!\');');
    ...
    case 'INFPASS':
      // Verifica se a Assinatura Eletrônica é válida
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
        dml_putPDMissao(null,$_REQUEST['w_chave'],null,null,null,
            null,null,$_REQUEST['w_pta'],$_REQUEST['w_emissao_bilhete'],$_REQUEST['w_valor_passagem'],$SG);
        for ($i=1; $i<=$_POST['w_sq_deslocamento'].$Count; $i=$i+1) {
          dml_putPD_Deslocamento('P', $_REQUEST['w_chave'],$_POST['w_sq_deslocamento']($i), null,null,null,null,null,null,
            $_POST['w_sq_cia_transporte']($i),Nvl($_POST['w_codigo_voo']($i),0));
        } 
        ScriptOpen('JavaScript');
        ShowHTML('  location.href=\''.$w_pagina.'Informarpassagens&O='.$O.'&w_chave='.$_REQUEST['w_Chave'].'&w_menu='.$_REQUEST['w_menu'].'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.$SG.MontaFiltro('GET').'\';');
        ScriptClose();
      } else {
        ScriptOpen('JavaScript');
        ShowHTML('  alert(\'Assinatura Eletrônica inválida!\');');
      ...
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
        if ((strpos(strtoupper($_SERVER['http_content_type']),'MULTIPART/FORM-DATA') ? strpos(strtoupper($_SERVER['http_content_type']),'MULTIPART/FORM-DATA')+1 : 0)>0) {
          ...
          if (f($RS,'sq_siw_tramite')!=$_REQUEST['w_tramite']) {
            ...
            if ($ul->State==0) {
              ...
              foreach ($ul->Files as $Field) {
                if ($Field->Length>0) {
                  ...
                  $FS=$CreateObject['Scripting.FileSystemObject'];
                  $w_file=nvl($_REQUEST['w_atual'],str_replace('.tmp',substr($Field->FileName,(strpos($Field->FileName,'.') ? strpos($Field->FileName,'.')+1 : 0)-1,30),$FS->GetTempName()));
              ...
              dml_putDemandaEnvio($w_menu,$_REQUEST['w_chave'],$w_usuario,$_REQUEST['w_tramite'],
            ...
            // Volta para a listagem 
            $RS = db_getMenuData::getInstanceOf($dbms,$w_menu);
            ShowHTML('  location.href=\''.str_replace($w_dir,'',f($RS,'link')).'&O=L&w_chave='.$_REQUEST['w_chave'].'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.RemoveTP($TP).'&SG='.f($RS,'sigla').MontaFiltro('UPLOAD').'\';');
            ScriptClose();
          } 
        } else {
          ...
          if (f($RS,'sq_siw_tramite')!=$_REQUEST['w_tramite']) {
            ...
            if ($_REQUEST['w_envio']=='N') {
              dml_putViagemEnvio($_REQUEST['w_menu'],$_REQUEST['w_chave'],$w_usuario,$_REQUEST['w_tramite'],null,
                $_REQUEST['w_envio'],$_REQUEST['w_despacho'],$_REQUEST['w_justificativa']);
            } else {
              dml_putViagemEnvio($_REQUEST['w_menu'],$_REQUEST['w_chave'],$w_usuario,$_REQUEST['w_tramite'],$_REQUEST['w_novo_tramite'],
                $_REQUEST['w_envio'],$_REQUEST['w_despacho'],$_REQUEST['w_justificativa']);
            } 
    ...
    case 'PDCONC':
      ...
      if (verificaAssinaturaEletronica($_SESSION['USERNAME'],strtoupper($_REQUEST['w_assinatura'])) || $w_assinatura=='') {
        ...
        if (f($RS,'sq_siw_tramite')!=$_REQUEST['w_tramite']) {
          ...
          dml_putDemandaConc($w_menu,$_REQUEST['w_chave'],$w_usuario,$_REQUEST['w_tramite'],$_REQUEST['w_inicio_real'],$_REQUEST['w_fim_real'],$_REQUEST['w_nota_conclusao'],$_REQUEST['w_custo_real'],
            $w_file,$w_tamanho,$w_tipo,$w_nome);
          ...
          // Volta para a listagem
          $RS = db_getMenuData::getInstanceOf($dbms,$w_menu);
          ShowHTML('  location.href=\''.str_replace($w_dir,'',f($RS,'link')).'&O=L&w_chave='.$_REQUEST['w_chave'].'&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4='.$P4.'&TP='.$TP.'&SG='.f($RS,'sigla').MontaFiltro('UPLOAD').'\';');
          ScriptClose();
        } 
      } else {
        ScriptOpen('JavaScript');
