...
<!-- #INCLUDE FILE="../mod_pd/Funcoes.php" -->
...
$ul->UploadID=$UploadID;
  ...
  $p_codigo=strtoupper($ul->Texts.$Item["p_codigo"]);
  ...
  else
{
  ...
  if ((strpos("PDTRECHO,PDVINC",$SG) ? strpos("PDTRECHO,PDVINC",$SG)+1 : 0)>0)
  {
...
if ($O=="L")
{
  ...
  if ((strpos(strtoupper($R),"GR_") ? strpos(strtoupper($R),"GR_")+1 : 0)>0 || (strpos(strtoupper($R),"PROJETO") ? strpos(strtoupper($R),"PROJETO")+1 : 0)>0)
  {
    ...
    if ($p_codigo>"")
    {
      $w_filtro=$w_filtro."<tr valign=\"top\"><td align=\"right\"><font size=1>PCD nº <td><font size=1>[<b>".$p_codigo."</b>]";
    }
;
    } 
    if ($p_assunto>"")
    {
      $w_filtro=$w_filtro."<tr valign=\"top\"><td align=\"right\"><font size=1>Descrição <td><font size=1>[<b>".$p_assunto."</b>]";
    }
;
    } 
    if ($p_solicitante>"")
...
// Se for cópia, aplica o filtro sobre todas as PCDs visíveis pelo usuário
    ...
    else
  {
    ...
    if (Nvl(${"p_agrega"},"")=="GRPDACAO")
    {

      $DB_GetSolicList_IS$RS      $RS["sq_menu"]      $w_usuario      Nvl(${"p_agrega"},$SG);      $P1      $p_ini_i      $p_ini_f      $null      $null      $p_atraso      $p_solicitante      $p_unidade      $null      $p_ativo      $p_proponente      $p_chave      $p_assunto      $p_pais      $p_regiao      $p_uf      $p_cidade      $p_usu_resp      $p_uorg_resp      $p_palavra      $p_prazo      $p_fase      $p_projeto      $p_atividade      $null      $p_codigo      $null      $null      $w_ano;
    }
      else
    if (Nvl(${"p_agrega"},"")=="GRPDCIAVIAGEM" || Nvl(${"p_agrega"},"")=="GRPDCIDADE" || Nvl(${"p_agrega"},"")=="GRPDDATA")
    {

      DB_GetSolicViagem($rs,$RS["sq_menu"],$w_usuario,Nvl(${"p_agrega"},$SG),3,
      $p_ini_i,$p_ini_f,$p_fim_i,$p_fim_f,$p_atraso,$p_solicitante,
      $p_unidade,$p_prioridade,$p_ativo,$p_proponente);
    }
      else
    {
      ...
      DB_GetSolicList($rs,$RS["sq_menu"],$w_usuario,Nvl(${"p_agrega"},$SG),$P1,
      $p_ini_i,$p_ini_f,$p_fim_i,$p_fim_f,$p_atraso,$p_solicitante,
      $p_unidade,$p_prioridade,$p_ativo,$p_proponente);
    } 
...
("          <td><font size=\"1\"><b>".LinkOrdena("Usuário atual","nm_exec")."</font></td>");} 
...
// Se for execução
    ...
    else
  {
    ...
    if (RetornaGestor($RS["sq_siw_solicitacao"],$w_usuario)=="S")
    {
...
DB_GetBenef($RS,$w_cliente,null,${"w_cpf"},null,null,1,null,null);
if ($RS->RecordCount>0)
{
...
$w_justif_dia_util=${"w_justif_dia_util"};
$w_inicio=${"w_inicio"};
$w_fim=${"w_fim"};
...
DB_GetBenef($RS,$w_cliente,null,$USERNAME_session,null,null,1,null,null);
if ($RS->RecordCount>0)
{
...
// & w_dir & w_pagina & par & "'; document.Form.O.value='" & O & "'; document.Form.w_troca.value='w_tarefa'; document.Form.target=''; document.Form.submit();""", w_projeto//ShowHTML "      </tr>"
...
DB_GetUorgList($RS,$w_cliente,$LOTACAO_session,"VIAGEMUNID",null,null,$w_ano);
if (!$RS->EOF)
{
...
$w_pessoa_atual=${"w_pessoa_atual"};

DB_GetSolicData($RS,$w_chave,$SG);
if ($w_sq_pessoa=="" && (strpos(${"botao"},"Selecionar") ? strpos(${"botao"},"Selecionar")+1 : 0)==0)
{
...
if ((strpos(${"botao"},"Selecionar") ? strpos(${"botao"},"Selecionar")+1 : 0)==0)
{
...
$w_chave_aux=${"w_chave_aux"};
$w_nome=${"w_nome"};
$w_nome_resumido=${"w_nome_resumido"};
$w_sq_pessoa_pai=${"w_sq_pessoa_pai"};
$w_nm_tipo_pessoa=${"w_nm_tipo_pessoa"};
$w_sq_tipo_vinculo=${"w_sq_tipo_vinculo"};
$w_nm_tipo_vinculo=${"w_nm_tipo_vinculo"};
$w_sq_banco=${"w_sq_banco"};
$w_sq_agencia=${"w_sq_agencia"};
$w_operacao=${"w_operacao"};
$w_nr_conta=${"w_nr_conta"};
$w_sq_pais_estrang=${"w_sq_pais_estrang"};
$w_aba_code=${"w_aba_code"};
$w_swift_code=${"w_swift_code"};
$w_endereco_estrang=${"w_endereco_estrang"};
$w_banco_estrang=${"w_banco_estrang"};
$w_agencia_estrang=${"w_agencia_estrang"};
$w_cidade_estrang=${"w_cidade_estrang"};
$w_informacoes=${"w_informacoes"};
$w_codigo_deposito=${"w_codigo_deposito"};
$w_interno=${"w_interno"};
$w_vinculo_ativo=${"w_vinculo_ativo"};
$w_sq_pessoa_telefone=${"w_sq_pessoa_telefone"};
$w_ddd=${"w_ddd"};
$w_nr_telefone=${"w_nr_telefone"};
$w_sq_pessoa_celular=${"w_sq_pessoa_celular"};
$w_nr_celular=${"w_nr_celular"};
$w_sq_pessoa_fax=${"w_sq_pessoa_fax"};
$w_nr_fax=${"w_nr_fax"};
$w_email=${"w_email"};
$w_sq_pessoa_endereco=${"w_sq_pessoa_endereco"};
$w_logradouro=${"w_logradouro"};
$w_complemento=${"w_complemento"};
$w_bairro=${"w_bairro"};
$w_cep=${"w_cep"};
$w_sq_cidade=${"w_sq_cidade"};
$w_co_uf=${"w_co_uf"};
$w_sq_pais=${"w_sq_pais"};
$w_pd_pais=${"w_pd_pais"};
$w_cpf=${"w_cpf"};
$w_nascimento=${"w_nascimento"};
$w_rg_numero=${"w_rg_numero"};
$w_rg_emissor=${"w_rg_emissor"};
$w_rg_emissao=${"w_rg_emissao"};
$w_matricula=${"w_matricula"};
$w_sq_pais_passaporte=${"w_sq_pais_passaporte"};
$w_sexo=${"w_sexo"};
$w_cnpj=${"w_cnpj"};
$w_inscricao_estadual=${"w_inscricao_estadual"};
}
  else
{

if ((strpos(${"botao"},"Alterar") ? strpos(${"botao"},"Alterar")+1 : 0)==0 && (strpos(${"botao"},"Procurar") ? strpos(${"botao"},"Procurar")+1 : 0)==0 && ($O=="A" || $w_sq_pessoa>"" || $w_cpf>"" || $w_cnpj>""))
{
...
FormataData();
ValidateOpen("Validacao");
if (($w_cpf=="" && $w_cnpj=="") || (strpos(${"botao"},"Procurar") ? strpos(${"botao"},"Procurar")+1 : 0)>0 || (strpos(${"botao"},"Alterar") ? strpos(${"botao"},"Alterar")+1 : 0)>0)
{
...
if ((strpos("CREDITO,DEPOSITO",$w_forma_pagamento) ? strpos("CREDITO,DEPOSITO",$w_forma_pagamento)+1 : 0)>0)
{
...
ShowHTML("</HEAD>");
ShowHTML("<BASE HREF=\"".$conRootSIW."\">");
if (($w_cpf=="" && $w_cnpj=="") || (strpos(${"botao"},"Alterar") ? strpos(${"botao"},"Alterar")+1 : 0)>0 || (strpos(${"botao"},"Procurar") ? strpos(${"botao"},"Procurar")+1 : 0)>0)
{
// Se o beneficiário ainda não foi selecionado

if ((strpos(${"botao"},"Procurar") ? strpos(${"botao"},"Procurar")+1 : 0)>0)
{
...
$w_nome=${"w_nome"};
if ((strpos(${"botao"},"Alterar") ? strpos(${"botao"},"Alterar")+1 : 0)>0)
{
...
$w_nome_resumido=null;

$w_sq_pessoa_pai=null;

$w_sq_tipo_pessoa=null;

$w_nm_tipo_pessoa=null;

$w_sq_tipo_vinculo=null;

$w_nm_tipo_vinculo=null;

$w_interno=null;

$w_vinculo_ativo=null;

$w_forma_pagamento=null;
...
$w_sq_pais_estrang=null;

$w_aba_code=null;

$w_swift_code=null;

$w_endereco_estrang=null;

$w_banco_estrang=null;

$w_agencia_estrang=null;

$w_cidade_estrang=null;

$w_informacoes=null;

$w_codigo_deposito=null;

$w_sq_pessoa_telefone=null;
...
$w_sq_pais_passaporte=null;

$w_sexo=null;

$w_cnpj=null;

$w_inscricao_estadual=null;
...
$w_pais_orig=${"w_pais_orig"};
$w_uf_orig=${"w_uf_orig"};
$w_cidade_orig=${"w_cidade_orig"};
$w_pais_dest=${"w_pais_dest"};
$w_uf_dest=${"w_uf_dest"};
$w_cidade_dest=${"w_cidade_dest"};
$w_data_saida=${"w_data_saida"};
$w_hora_saida=${"w_hora_saida"};
$w_data_chegada=${"w_data_chegada"};
$w_hora_chegada=${"w_hora_chegada"};
}
  else
if ($O=="L")
{
...
if ((strpos("AE",$O) ? strpos("AE",$O)+1 : 0)>0)
{
...
$w_hora_saida=substr($FormatDateTime[$RS["saida"]][3],0,5);
$w_data_chegada=FormataDataEdicao($FormatDateTime[$RS["chegada"]][2]);
$w_hora_chegada=substr($FormatDateTime[$RS["chegada"]][3],0,5);
DesconectaBD();
} 
...
ShowHTML("        <td align=\"center\"><font size=\"1\">".$FormatDateTime[$RS["saida"]][2].", ".substr($FormatDateTime[$RS["saida"]][3],0,5)."</td>");
ShowHTML("        <td align=\"center\"><font size=\"1\">".$FormatDateTime[$RS["chegada"]][2].", ".substr($FormatDateTime[$RS["chegada"]][3],0,5)."</td>");
ShowHTML("        <td align=\"top\" nowrap><font size=\"1\">");
...
$DB_GetSolicList_IS$RS$RS["sq_menu"]$w_usuario$SG$P1$null$null$null$null$null$null$null$null$null$null$w_chave$null$null$null$null$null$null$null$null$null$null$null$null$null$null$null$null$w_ano;
$RS->Sort="titulo";
} 
...
$DB_GetLinkData$RS$w_cliente//ISTCAD"$DB_GetSolicList_IS$RS$RS["sq_menu"]$w_usuario//ISTCAD", 4, _$null$null$null$null$null$null$null$null$null$null$null$p_assunto$null$null$null$null$null$null$null$null$null$null$null$null$null$null$null$w_ano;
$RS1->Sort="titulo";
ShowHTML("<tr><td>");
ShowHTML("    <TABLE WIDTH=\"100%\" bgcolor=\"".$conTableBgColor."\" BORDER=\"".$conTableBorder."\" CELLSPACING=\"".$conTableCellSpacing."\" CELLPADDING=\"".$conTableCellPadding."\" BorderColorDark=\"".$conTableBorderColorDark."\" BorderColorLight=\"".$conTableBorderColorLight."\">");
ShowHTML("        <tr bgcolor=\"".$conTrBgColor."\" align=\"center\">");
...
$w_vetor_trechos[$i][4]=FormataDataEdicao($FormatDateTime[$RS["saida"]][2]).", ".substr($FormatDateTime[$RS["saida"]][3],0,5);
$w_vetor_trechos[$i][5]=FormataDataEdicao($FormatDateTime[$RS["chegada"]][2]).", ".substr($FormatDateTime[$RS["chegada"]][3],0,5);
...
$w_aux_alimentacao=null;

$w_aux_transporte=null;

$w_vlr_alimentacao=null;

$w_vlr_transporte=null;
...
$w_desc_alimentacao=null;

$w_desc_transporte=null;
...
$w_tramite=${"w_tramite"};
$w_sg_tramite=${"w_sg_tramite"};
$w_sg_novo_tramite=${"w_tramite"};
...
if ($O=="V")
{
$w_erro=$ValidaViagem[$w_cliente][$w_chave][$SG]["PDGERAL"][null];
}
;
} 
...
ShowHTML("    <tr><td><font size=\"1\"><b><u>J</u>ustificativa para não cumprimento do prazo regulamentar de ".$w_prazo." dias:</b><br><textarea ".$w_Disabled." accesskey=\"J\" name=\"w_justificativa\" class=\"STI\" ROWS=5 cols=75 title=\"Se o início da viagem for anterior a ".FormataDataEdicao($FormatDateTime[time()()+$w_prazo][2]).", justifique o motivo do não cumprimento do prazo regulamentar para o pedido.\">".$w_justificativa."</TEXTAREA></td>");
} 
...
if ($P1==1 && $w_tipo=="Volta")
{

ShowHTML("      <input class=\"STB\" type=\"button\" onClick=\"location.href='".$R."&O=L&w_chave=".${"w_chave"}."&P1=".$P1."&P2=".$P2."&P3=".$P3."&P4=".$P4."&TP=".$TP."&SG=".$SG.MontaFiltro("GET")."';\" name=\"Botao\" value=\"Abandonar\">");
} 
...
ShowHTML("\par ");
ShowHTML("\par ");
ShowHTML("\par }\pard \ql \li0\ri0\widctlpar\aspalpha\aspnum\faauto\adjustright\rin0\lin0\itap0\pararsid9184405 {\f1\insrsid9184405 Autoriza\'e7\'e3o n\'ba ".substr($RS["codigo_interno"],4));
ShowHTML("\par ");
ShowHTML("\par ");
ShowHTML("\par ");
ShowHTML("\par \tab \tab \tab \tab \tab              Bras\'edlia-DF, ".substr($FormatDateTime[time()][1],(strpos($FormatDateTime[time()()][1]," ") ? strpos($FormatDateTime[time()()][1]," ")+1 : 0)-1).".");
ShowHTML("\par ");
...
if (strtoupper(trim($RS1["nm_tipo_vinculo"]))=="COLABORADOR EVENTUAL")
{
...
if (strtoupper(trim($RS1["nm_tipo_vinculo"]))=="QUADRO PERMANENTE")
{
...
if (strtoupper(trim($RS1["nm_tipo_vinculo"]))=="DIRIGENTE" || (strpos($RS1["nm_tipo_vinculo"],"Função") ? strpos($RS1["nm_tipo_vinculo"],"Função")+1 : 0)>0)
{
...
ShowHTML("\fs14\insrsid16481867\charrsid13388689 DATA:".FormataDataEdicao($FormatDateTime[time()()][2])."\cell }{\fs16\insrsid16481867\charrsid5664258 __________________________________________\cell }{\f1\fs16\insrsid16481867\charrsid5664258 \cell }\pard ");
...
$w_sq_cia_transporte=null;

$w_codigo_voo=null;

$w_valor_passagem=null;

$w_pta=null;

$w_emissao_bilhete=null;

$w_sq_deslocamento=null;
...
DB_GetCustomerData($RS2,${"p_cliente"."_session"});
...
$w_html=$w_html."\par ";
$w_html=$w_html."\par Percurso: ".strtolower($w_percurso)."     \par Di\'e1rias recebidas: Qtd: ".$FormatNumber[$w_diaria][1]." Valor: ".$FormatNumber[$w_valor][2];
$w_html=$w_html."\par ";
...
$w_html=$w_html."      <tr valign=\"top\"><td align=\"center\"><font size=2><b>INCLUSÃO DE PCD</b></font><br><br><td></tr>"."\r\n";
}
  else
if ($w_sg_tramite=="EE")
{

$w_html=$w_html."      <tr valign=\"top\"><td align=\"center\"><font size=2><b>PRESTAÇÃO DE CONTAS DE PCD</b></font><br><br><td></tr>"."\r\n";
}
  else
if ($p_tipo==2)
{

$w_html=$w_html."      <tr valign=\"top\"><td align=\"center\"><font size=2><b>TRAMITAÇÃO DE PCD</b></font><br><br><td></tr>"."\r\n";
}
  else
if ($p_tipo==3)
{

$w_html=$w_html."      <tr valign=\"top\"><td align=\"center\"><font size=2><b>CONCLUSÃO DE PCD</b></font><br><br><td></tr>"."\r\n";
} 
...
$w_html=$w_html."\r\n"."          <tr valign=\"top\">";
$w_html=$w_html."\r\n"."          <td><font size=\"1\">Proposto:<br><b>".$RSM["nm_prop"]."</b></td>";
$w_html=$w_html."\r\n"."          <td><font size=\"1\">Unidade proponente:<br><b>".$RSM["nm_unidade_resp"]."</b></td>";
$w_html=$w_html."\r\n"."          <tr valign=\"top\">";
$w_html=$w_html."\r\n"."          <td><font size=\"1\">Primeira saída:<br><b>".FormataDataEdicao($RSM["inicio"])." </b></td>";
$w_html=$w_html."\r\n"."          <td><font size=\"1\">Último retorno:<br><b>".FormataDataEdicao($RSM["fim"])." </b></td>";
$w_html=$w_html."\r\n"."          </table>";
...
if (Nvl($RSM["descricao"],"")>"")
{
$w_html=$w_html."\r\n"."      <tr><td valign=\"top\"><font size=\"1\">Descrição da PCD:<br><b>".CRLF2BR($RSM["descricao"])." </b></td>";
}
;
} 
} 
...
$w_html=$w_html."\r\n"."          <td><font size=\"1\">Despacho:<br><b>".CRLF2BR(Nvl($RS["despacho"],"---"))." </b></td>";
$w_html=$w_html."\r\n"."          </table>";
...
$dbms->BeginTrans;
if (Nvl(${"w_justif_dia_util"},"")=="" && (RetornaExpediente(${"w_inicio"},$w_cliente,null,null,null)=="N" || RetornaExpediente(${"w_fim"},$w_cliente,null,null,null)=="N"))
{
...
$DML_PutViagemGeral$O$w_cliente${"w_chave"}${"w_menu"}session_register("lotacao");
${"lotacao"."_session"}=${"w_sq_unidade_resp"};
${"w_sq_prop"}session_register("sq_pessoa");
${"sq_pessoa"."_session"}=${"w_tipo_missao"};
${"w_descricao"}${"w_justif_dia_util"}${"w_inicio"}${"w_fim"}${"w_data_hora"}${"w_aviso"}${"w_dias"}${"w_projeto"}${"w_tarefa"}${"w_cpf"}${"w_nm_prop"}${"w_nm_prop_res"}${"w_sexo"}${"w_vinculo"}${"w_inicio_atual"}$w_chave_nova$w_copia$w_codigo;
...
$DML_PutViagemOutra$O$SG${"w_chave"}${"w_chave_aux"}${"w_sq_pessoa"}${"w_cpf"}${"w_nome"}${"w_nome_resumido"}${"w_sexo"}${"w_sq_tipo_vinculo"}${"w_matricula"}${"w_rg_numero"}${"w_rg_emissao"}${"w_rg_emissor"}${"w_ddd"}${"w_nr_telefone"}${"w_nr_fax"}${"w_nr_celular"}${"w_sq_agencia"}${"w_operacao"}${"w_nr_conta"}${"w_sq_pais_estrang"}${"w_aba_code"}${"w_swift_code"}${"w_endereco_estrang"}${"w_banco_estrang"}${"w_agencia_estrang"}${"w_cidade_estrang"}${"w_informacoes"}${"w_codigo_deposito"}

ScriptOpen("JavaScript");
ShowHTML("  location.href='".$R."&O=".$O."&w_chave=".${"w_Chave"}."&P1=".$P1."&P2=".$P2."&P3=".$P3."&P4=".$P4."&TP=".$TP."&SG=".$SG.MontaFiltro("GET")."';");
$ScriptClose;
$dbms->CommitTrans;
}
  else
...
$DML_PutPD_Deslocamento$O${"w_chave"}${"w_chave_aux"}${"w_cidade_orig"}${"w_data_saida"}${"w_hora_saida"}${"w_cidade_dest"}${"w_data_chegada"}${"w_hora_chegada"}$null$null;
...
$DB_GetLinkData$RS1session_register("p_cliente");
${"p_cliente"."_session"}=$SG;
ShowHTML("  location.href='".str_replace($w_dir,"",$RS1["link"])."&O=L&w_chave=".${"w_Chave"}."&P1=".$P1."&P2=".$P2."&P3=".$P3."&P4=".$P4."&TP=".$TP."&SG=".$SG.MontaFiltro("GET")."';");
$ScriptClose;
$dbms->CommitTrans;
}
  else
...
$DML_PutPdTarefa$O${"w_chave"}$_POST["w_tarefa"]$w_cont);
} 
...
$DML_PutPdTarefa$O${"w_chave_aux"}${"w_tarefa"}
} 

ScriptOpen("JavaScript");
ShowHTML("  location.href='".$R."&O=L&P1=".$P1."&P2=".$P2."&P3=".$P3."&P4=".$P4."&TP=".$TP."&SG=".$SG."&w_chave=".${"w_chave"}."';");
$ScriptClose;
$dbms->CommitTrans;
}
  else
...
$DML_PutPDMissaonull${"w_chave"}Nvl(${"w_vlr_alimentacao"},0);Nvl(${"w_vlr_transporte"},0);Nvl(${"w_adicional"},0);Nvl(${"w_desc_alimentacao"},0);Nvl(${"w_desc_trasnporte"},0);$null$null$null$null;
...
$DML_PutPDDiaria"A"${"w_chave"}$_POST["w_sq_diaria"]$i)$_POST["w_sq_cidade"]$i)Nvl($_POST["w_qtd_diarias"]($i),0);Nvl($_POST["w_vlr_diarias"]($i),0);
}
  else
{

$DML_PutPDDiaria"I"${"w_chave"}$null$_POST["w_sq_cidade"]$i)Nvl($_POST["w_qtd_diarias"]($i),0);Nvl($_POST["w_vlr_diarias"]($i),0);
} 
...
ScriptOpen("JavaScript");
ShowHTML("  location.href='".$w_pagina."DadosFinanceiros&O=".$O."&w_chave=".${"w_Chave"}."&w_menu=".${"w_menu"}."&P1=".$P1."&P2=".$P2."&P3=".$P3."&P4=".$P4."&TP=".$TP."&SG=".$SG.MontaFiltro("GET")."';");
$ScriptClose;
$dbms->CommitTrans;
}
  else
...
$DML_PutPDMissaonull${"w_chave"}$null$null$null$null$null${"w_pta"}${"w_emissao_bilhete"}${"w_valor_passagem"}$SG;

for ($i=1; $i<=$_POST["w_sq_deslocamento"].$Count; $i=$i+1)
{

$DML_PutPD_Deslocamento"P"${"w_chave"}$_POST["w_sq_deslocamento"]$i)$null$null$null$null$null$null$_POST["w_sq_cia_transporte"]$i)Nvl($_POST["w_codigo_voo"]($i),0);

} 


ScriptOpen("JavaScript");
ShowHTML("  location.href='".$w_pagina."Informarpassagens&O=".$O."&w_chave=".${"w_Chave"}."&w_menu=".${"w_menu"}."&P1=".$P1."&P2=".$P2."&P3=".$P3."&P4=".$P4."&TP=".$TP."&SG=".$SG.MontaFiltro("GET")."';");
$ScriptClose;
$dbms->CommitTrans;
}
  else
...
} $DML_PutDemandaEnvio$w_menu$ul->Texts.$Item["w_chave"]$w_usuario$ul->Texts.$Item["w_tramite"]$ul->Texts.$Item["w_novo_tramite"]//N", ul.Texts.Item("w_observacao"), ul.Texts.Item("w_destinatario"), ul.Texts.Item("w_despacho"), _ $w_file$w_tamanho$w_tipo$w_nome;
}
  else
{
...
$DB_GetTramiteList$RS${"w_tramite"}//PROXIMO", null}
  else
{

$DB_GetTramiteList$RS${"w_tramite"}//ANTERIOR", null} 

DB_GetTramiteSolic($RS1,${"w_chave"},$RS["sq_siw_tramite"],null,null);
...
$DML_PutViagemEnvio${"w_menu"}${"w_chave"}$w_usuario${"w_tramite"}$null${"w_envio"}${"w_despacho"}${"w_justificativa"}
}
  else
{

$DML_PutViagemEnvio${"w_menu"}${"w_chave"}$w_usuario${"w_tramite"}${"w_novo_tramite"}${"w_envio"}${"w_despacho"}${"w_justificativa"}
} 
...
DML_PutDemandaConc($w_menu,${"w_chave"},$w_usuario,${"w_tramite"},${"w_inicio_real"},${"w_fim_real"},${"w_nota_conclusao"},${"w_custo_real"},
$w_file,$w_tamanho,$w_tipo,$w_nome);
...
$DB_GetMenuData$RS$w_menu;
ShowHTML("  location.href='".str_replace($w_dir,"",$RS["link"])."&O=L&w_chave=".${"w_chave"}."&P1=".$P1."&P2=".$P2."&P3=".$P3."&P4=".$P4."&TP=".$TP."&SG=".$rs["sigla"].MontaFiltro("UPLOAD")."';");
$DesconectaBD;
$ScriptClose;
} 
