...
// -------------------------------------------------------------------------
function Rel_Perm_Usuarios() {
  extract($GLOBALS);
  $w_chave      = $_REQUEST['w_chave'];
  $w_tipo_rel   = strtoupper(trim($_REQUEST['w_tipo_rel']));
  $p_servico     = strtoupper(trim($_REQUEST['p_servico']));
  if ($_REQUEST['p_modulo']>'' && $p_servico=='') $p_modulo = $_REQUEST['p_modulo'];
  else                                                $p_modulo = substr($p_servico,0,4);
  $p_cd_acao            = substr($p_servico,4,4);
  ...
  if ($w_tipo_rel=='WORD') {
    ...
    if (!(strpos('P',$O)===false)) {
      ...
      Validate('p_modulo','Programa','SELECT','','1','18','','1');
      Validate('p_servico','Ação','SELECT','','1','18','1','1');
    ...
    if ($O=='L') {
      ...
      BodyOpen('onLoad=\'document.Form.p_modulo.focus()\';');
        ...
        if (!(strpos($p_campos,'aprovado')===false) || !(strpos($p_campos,'saldo')===false) || !(strpos($p_campos,'autorizado')===false) || !(strpos($p_campos,'realizado')===false) || !(strpos($p_campos,'liquidar')===false))
          ...
          if ($p_metas>'') {
            ...
            $RS2 = db_getSolicList_IS::getInstanceOf($dbms,f($RS1,'sq_menu'),$w_usuario,f($RS1,'sigla'),4,
                      ...
                      f($RS1,'cd_acao'),null,substr(f($row,'chave'),8,4),$w_ano);
            ...
            if (count($RS2)<=0) {
              ...
              $RS3 = db_getSolicMeta_IS::getInstanceOf($dbms,f($RS2,'sq_siw_solicitacao'),null,'LISTA',null,null,null,null,null,null,null,null,null);
    ...
    if (count($RS)<=0) {
      ...
      foreach($RS as $row) {
        if ($w_linha>20 && $w_tipo_rel=='WORD') {
          ...
          if (!(strpos($p_campos,'telefone')===false))      ShowHTML('     <td><b>Telefone</td>');
          if (!(strpos($p_campos,'aprovado')===false))      ShowHTML('          <td><b>Aprovado</td>');  
          if (!(strpos($p_campos,'empenhado')===false))     ShowHTML('          <td><b>Empenhado</td>'); 
          if (!(strpos($p_campos,'saldo')===false))         ShowHTML('          <td><b>Saldo</td>');   
          if (!(strpos($p_campos,'liquidado')===false))     ShowHTML('          <td><b>Liquidado</td>');  
          if (!(strpos($p_campos,'liquidar')===false))      ShowHTML('          <td><b>A liquidar</td>');  
          ShowHTML('      </tr>');
          $w_linha+=1;
        } 
        ...
        if ($w_atual!=f($row,'chave')) {
          if (!(strpos($p_campos,'aprovado')===false) || !(strpos($p_campos,'saldo')===false) || !(strpos($p_campos,'empenhado')===false) || !(strpos($p_campos,'liquidado')===false) || !(strpos($p_campos,'liquidar')===false) && $p_sq_isprojeto=='' && $w_sq_siw_solicitacao>'') {
            ShowHTML('   <tr bgcolor="'.$conTrBgColor.'" valign="top">');
            ShowHTML('     <td colspan='.$w_col.' align="right"><b>Totais do programa interno<d>');
            if (!(strpos($p_campos,'aprovado')===false))  ShowHTML('        <td align="right">'.number_format($w_acao_aprovado,2,',','.').'</td>');
        ...
        if ($w_sq_siw_solicitacao>'') {
          if ($p_metas>'') {
            ...
            if (count($RS2)<=0) {
              ...
              foreach($RS2 as $row2){$RS2=$row2; break;}
              $RS3 = db_getSolicMeta_IS::getInstanceOf($dbms,f($RS2,'sq_siw_solicitacao'),null,'LSTNULL',null,null,null,null,null,null,null,null,null);
              $RS3 = SortArray($RS3,'ordem','asc');
              if (count($RS3)<=0) {
                // Se não foram selecionados registros, exibe mensagem
                ShowHTML(' <tr bgcolor="'.$conTrBgColor.'"><td colspan='.$w_col_word.' align="center"><b>Não foram encontrados registros(metas).</b></td></tr>');
      ...
      if (!(strpos($p_campos,'aprovado')===false) || !(strpos($p_campos,'saldo')===false) || !(strpos($p_campos,'empenhado')===false) || !(strpos($p_campos,'liquidado')===false) || !(strpos($p_campos,'liquidar')===false)) {
        if ($p_sq_isprojeto>' ') {
          ShowHTML('       <tr bgcolor="'.$conTrBgColor.'" valign="top">');
          ShowHTML('         <td colspan='.$w_col.' align="right"><b>Totais do programa interno </b></td>');
          if (!(strpos($p_campos,'aprovado')===false))  ShowHTML('        <td align="right">'.number_format($w_acao_aprovado,2,',','.').'</td>');
          if (!(strpos($p_campos,'empenhado')===false)) ShowHTML('        <td align="right">'.number_format($w_acao_empenhado,2,',','.').'</td>');
          if (!(strpos($p_campos,'saldo')===false))     ShowHTML('        <td align="right">'.number_format($w_acao_saldo,2,',','.').'</td>');
          if (!(strpos($p_campos,'liquidado')===false)) ShowHTML('        <td align="right">'.number_format($w_acao_liquidado,2,',','.').'</td>');
        ...
        if ($p_indicador>'') {
          ...
          if (count($RS2)<=0) {
            ...
            if (count($RS3)<=0) {
              ...
              ShowHTML('          <td><b>Indice<br>referência</td>');
              ShowHTML('          <td><b>Indice<br>programado</td>');
              ShowHTML('          <td><b>Indice<br>apurado</td>');
              ShowHTML('          <td><b>Data<br>apuracao</td>');
              ShowHTML('          <td><b>Unidade<br>medida</td>');
              ShowHTML('        </tr>');
              $w_linha += 1;
              foreach($RS3 as $row3) {
    ...
    if (count($RS)<=0) {
      ...
      foreach($RS as $row) {
        ...
        if (count($RS2)>0) {
          ...
          $w_visao=0;
          if ($w_visao<2) {
            $RS3 = db_getSolicMeta_IS::getInstanceOf($dbms,f($RS2,'sq_siw_solicitacao'),null,'LSTNULL',null,null,null,null,null,null,$p_exequivel,$p_programada,$p_fim_previsto);
            $RS3 = SortArray($RS3,'ordem','asc');
            if (count($RS3)>0) {
              $w_teste_metas=1;
...
function Rel_Sintetico_PPA() {
  ...
  $w_tipo_rel   = strtoupper(trim($_REQUEST['w_tipo_rel']));
  $p_servico     = strtoupper(trim($_REQUEST['p_servico']));
  if (strtoupper(trim($_REQUEST['p_modulo']))>'' && $p_servico=='') $p_modulo = strtoupper(trim($_REQUEST['p_modulo']));
  else                                                                  $p_modulo = strtoupper(trim(substr($p_servico,0,4)));
  $p_cd_acao        = strtoupper(trim(substr($p_servico,4,4)));
  $p_sq_unidade_resp= strtoupper(trim($_REQUEST['p_sq_unidade_resp']));
  $p_prioridade     = strtoupper(trim($_REQUEST['p_prioridade']));
  $p_selecao_mp     = strtoupper(trim($_REQUEST['p_selecao_mp']));
  $p_selecao_se     = strtoupper(trim($_REQUEST['p_selecao_se']));
  ...
  if ($w_tipo_rel=='WORD') {
    ...
    if (!(strpos('P',$O)===false)) {
      ScriptOpen('JavaScript');
      ValidateOpen('Validacao');
      Validate('p_modulo','Programa','SELECT','','1','18','1','1');
      Validate('p_servico','Ação','SELECT','','1','18','1','1');
      ValidateClose();
      ScriptClose();
    } 
    ...
    if (count($RS)<=0) {
      ...
      foreach($RS as $row) {
        ...
        if (count($RS2)>0) {
          ...
          $w_visao=0;
          if ($w_visao<2) {
            $RS3 = db_getSolicMeta_IS::getInstanceOf($dbms,f($RS2,'sq_siw_solicitacao'),null,'LSTNULL',null,null,null,null,null,$p_programada,$p_exequivel,null,$p_fim_previsto);
            $RS3 = SortArray($RS3,'ordem','asc');
            if (count($RS3)>0) {
              $w_teste_metas=1;
            } elseif ($p_programada=='' && $p_exequivel=='' && $p_fim_previsto=='') {
...
function Rel_Sintetico_Prog() {
  ...
  $w_troca          = $_REQUEST['w_troca'];
  $w_tipo_rel       = strtoupper(trim($_REQUEST['w_tipo_rel']));
  $p_modulo    = strtoupper(trim($_REQUEST['p_modulo']));
  $p_sq_unidade_resp= strtoupper(trim($_REQUEST['p_sq_unidade_resp']));
  $p_prioridade     = strtoupper(trim($_REQUEST['p_prioridade']));
  $p_selecao_mp     = strtoupper(trim($_REQUEST['p_selecao_mp']));
  $p_selecao_se     = strtoupper(trim($_REQUEST['p_selecao_se']));
...
function Rel_Gerencial_Acao() {
  ...
  if ($O=='L') {
    ...
    if ($w_tipo!='WORD') {
      ShowHTML('&nbsp;&nbsp;<IMG ALIGN="CENTER" TITLE="Imprimir" SRC="images/impressora.jpg" onClick="window.print();">');
      ShowHTML('&nbsp;&nbsp;<a target="MetaWord" href="'.$w_dir.$w_pagina.'Rel_Gerencial_Acao&R='.$w_pagina.$par.'&O=L&w_tipo=word&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4=1&TP='.$TP.'&SG='.$SG.'&w_chave='.$w_chave.MontaFiltro('GET').'"><IMG border=0 ALIGN="CENTER" TITLE="Gerar word" SRC="images/word.gif"></a>');
    } 
    ShowHTML('</TD></TR>');
    ShowHTML('</FONT></B></TD></TR></TABLE>');
    ...
    ScriptOpen('JavaScript');
    ValidateOpen('Validacao');
    Validate('p_servico','Ação PPA','SELECT','1','1','18','1','1');
    ValidateClose();
    ScriptClose();
    ShowHTML('</HEAD>');
    ShowHTML('<BASE HREF="'.$conRootSIW.'">');
    BodyOpen('onLoad=\'document.Form.p_servico.focus()\';');
    ShowHTML('<B><FONT COLOR="#000000">'.$w_TP.'</FONT></B>');
    ShowHTML('<HR>');
    ShowHTML('<div align=center><center>');
    ShowHTML('<table border="0" cellpadding="0" cellspacing="0" width="100%">');
...
function Rel_Gerencial_Prog() {
  ...
  if ($O=='L') {
    ...
    if ($w_tipo!='WORD') {
      ShowHTML('&nbsp;&nbsp;<IMG ALIGN="CENTER" TITLE="Imprimir" SRC="images/impressora.jpg" onClick="window.print();">');
      ShowHTML('&nbsp;&nbsp;<a target="MetaWord" href="'.$w_dir.$w_pagina.'Rel_Gerencial_Prog&R='.$w_pagina.$par.'&O=L&w_tipo=word&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4=1&TP='.$TP.'&SG='.$SG.'&w_chave='.$w_chave.MontaFiltro('GET').'"><IMG border=0 ALIGN="CENTER" TITLE="Gerar word" SRC="images/word.gif"></a>');
    } 
    ShowHTML('</TD></TR>');
    ShowHTML('</FONT></B></TD></TR></TABLE>');
    $RS = db_getProgramaPPA_IS::getInstanceOf($dbms,$p_modulo,$w_cliente,$w_ano,null,null,null,null);
    ...
    ScriptOpen('JavaScript');
    ValidateOpen('Validacao');
    Validate('p_modulo','Programa','SELECT','1','1','18','1','1');
    ValidateClose();
    ScriptClose();
    ShowHTML('</HEAD>');
    ShowHTML('<BASE HREF="'.$conRootSIW.'">');
...
function Rel_Gerencial_Tarefa() {
  ...
  if ($O=='L') {
    ...
    if ($w_tipo!='WORD') {
      ShowHTML('&nbsp;&nbsp;<IMG ALIGN="CENTER" TITLE="Imprimir" SRC="images/impressora.jpg" onClick="window.print();">');
      ShowHTML('&nbsp;&nbsp;<a target="MetaWord" href="'.$w_dir.$w_pagina.'Rel_Gerencial_Tarefa&R='.$w_pagina.$par.'&O=L&w_tipo=word&P1='.$P1.'&P2='.$P2.'&P3='.$P3.'&P4=1&TP='.$TP.'&SG='.$SG.'&w_chave='.$w_chave.MontaFiltro('GET').'"><IMG border=0 ALIGN="CENTER" TITLE="Gerar word" SRC="images/word.gif"></a>');
    } 
    ShowHTML('</TD></TR>');
    ShowHTML('</FONT></B></TD></TR></TABLE>');
    ShowHTML('<HR>');
...
function Rel_Metas() {
  ...
  $RS = db_getSolicMeta_IS::getInstanceOf($dbms,null,null,'LSTNULL',$w_ano,$p_sq_unidade,$p_modulo,$p_cd_acao,$p_preenchida,$p_meta_ppa,$p_exequivel,null,null);
  ...
  if ($w_tipo_rel=='WORD') {
    ...
    if (!(strpos('P',$O)===false)) {
      ...
      Validate('p_modulo','Programa','HIDDEN','','1','18','1','1');
...
function Rel_Det_Tarefa() {
  ...
  $RS = db_getSolicList_IS::getInstanceOf($dbms,f($RS1,'sq_menu'),$w_usuario,'ISTCAD',4,
          ...
          null,null,null,null,null,null,$p_modulo,substr($p_cd_acao,4,4),null,null,$w_ano);
    ...
    if (count($RS)<=0) {
      ...
      foreach($RS as $row) {
        if ($w_linha>19 && $w_tipo_rel=='WORD') {
          ...
          if ($p_modulo>'')  ShowHTML('    <td width="20%"><b>Programa:</b></td><td>'.f($row,'descricao_programa').'</td></tr>');
...
function Rel_Det_Prog() {
  ...
  $RS4 = db_getSolicList_IS::getInstanceOf($dbms,f($RS1,'sq_menu'),$w_usuario,'ISPCAD',4,
            ...
            $p_sq_unidade,null,null,null,
            null,null,null,null,null,null,null,
            null,null,null,null,null,null,null,$p_modulo,null,null,$w_ano);
...
// -------------------------------------------------------------------------
function MetaLinha($l_chave,$l_chave_aux,$l_titulo,$l_word,$l_programada,$l_unidade_medida,$l_quantidade,$l_fim,$l_perc,$l_oper,$l_tipo,$l_loa) {
  extract($GLOBALS);
  $w_cor = ($w_cor==$conTrBgColor || $w_cor=='') ? $w_cor=$conTrAlternateBgColor : $w_cor=$conTrBgColor;
...
// -------------------------------------------------------------------------
function Indicadorlinha($l_chave,$l_chave_aux,$l_titulo,$l_valor_ref,$l_valor_prog,$l_valor_apurado,$l_apuracao_ind,$l_unidade_medida,$l_word,$l_destaque,$l_oper,$l_tipo,$l_loa) {
  extract($GLOBALS);
  if ($l_loa>'')    $l_loa='Sim';
  else              $l_loa='Não';
  $l_row='';
...
function Main() {
  extract($GLOBALS);
  switch ($par) {
    case 'RELPERMIS':               Rel_Perm_Usuarios();    break;
