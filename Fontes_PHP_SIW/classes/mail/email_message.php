 ...
 * email_message.php
 *
 * @(#) $Header: /home/mlemos/cvsroot/mimemessage/email_message.php,v 1.73 2007/04/17 22:10:07 mlemos Exp $
 *
 *
 */
...
<class>
  ...
  <version>@(#) $Id: email_message.php,v 1.73 2007/04/17 22:10:07 mlemos Exp $</version>
...
{metadocument}
  <variable>
    <name>mailer</name>
    <type>STRING</type>
    <value>http://www.phpclasses.org/mimemessage $Revision: 1.73 $</value>
    <documentation>
...
{/metadocument}
*/
  var $mailer='http://www.phpclasses.org/mimemessage $Revision: 1.73 $';
...
*/
  ...
  Function SendMail($to,$subject,&$body,&$headers,$return_path)
  {
    if(!function_exists("mail"))
      return($this->OutputError("a função mail() não está disponível na instalação do PHP"));
    if(strlen($return_path))
    {
      if(!defined("PHP_OS"))
        return($this->OutputError("o header Return-Path não pode ser configurado na sua versão do PHP"));
      if(!strcmp(substr(PHP_OS,0,3),"WIN"))
        return($this->OutputError("o header Return-Path não pode ser configurado diretamente pelo script PHP no Windows"));
      if($this->GetPHPVersion()<4000005)
        return($this->OutputError("o header Return-Path não pode ser configurado por versões anteriores do PHP 4.0.5"));
      if(function_exists("ini_get")
      && ini_get("safe_mode"))
        return($this->OutputError("o header Return-Path não pode ser configurado devido a restrições do modo seguro do PHP (safe mode)"));
  ...
  Function StartSendingMessage()
  {
    if(strcmp($this->delivery["State"],""))
      return($this->OutputError("o envio da mensagem já foi iniciado"));
  ...
  Function SendMessageHeaders($headers)
  {
    ...
    {
      if(!strcmp($this->delivery["State"],""))
        return($this->OutputError("o envio da mensagem ainda não foi iniciado"));
      else
        return($this->OutputError("os headers da mensagem já foram enviados"));
    }
  ...
  Function SendMessageBody(&$data)
  {
    if(strcmp($this->delivery["State"],"SendingBody"))
      return($this->OutputError("os headers da mensagem ainda não foram enviados"));
  ...
  {
    if(strcmp($this->delivery["State"],"SendingBody"))
      return($this->OutputError("o corpo da mensagem ainda não foi envado"));
    if(!IsSet($this->delivery["Headers"])
    || count($this->delivery["Headers"])==0)
      return($this->OutputError("não há headers configurados para a mensagem"));
    ...
    if(!IsSet($has["to"]))
      return($this->OutputError("não foi especificado um destinatário válido (To: header)"));
    if(!IsSet($has["subject"]))
      return($this->OutputError("não foi especificado um assunto válido (Subject: header)"));
  ...
  Function GetPartHeaders(&$headers,$part)
  {
    if(!IsSet($this->parts[$part]["Content-Type"]))
      return($this->OutputError("foi adicionada uma parte sem Content-Type: definido"));
      ...
      case "multipart":
        switch($sub_type)
        {
          ...
          case "parallel":
            ...
            break;
          default:
            return($this->OutputError("multipart Content-Type sub_type $sub_type não suportado"));
        }
        break;
      default:
        return($this->OutputError("Content-Type: $full_type não suportado"));
    }
    ...
    switch($type)
    {
      ...
      case "message":
        if(IsSet($this->parts[$part]["FILENAME"]))
        {
          ...
          if(GetType($size)=="integer"
          && strlen($body)!=$size)
            return($this->OutputError("o tamanho do arquivo lido não equivale ao tamanho do arquivo da parte ".$this->parts[$part]["FILENAME"]." devido a dados corrompidos"));
        }
        else
        {
          if(!IsSet($this->parts[$part]["DATA"]))
            return($this->OutputError("foi adicionada uma parte sem um body PART"));
...
*/
  ...
  Function GetFileDefinition(&$file,&$definition,$require_name=1)
  {
    ...
    {
      if(!IsSet($file["Data"]))
        return($this->OutputError("não foi indicado o nome do arquivo da parte"));
    }
    if(IsSet($file["Name"]))
      $name=$file["Name"];
    if($require_name
    && strlen($name)==0)
      return($this->OutputError("não foi indicado o nome da parte do arquivo"));
    $encoding="base64";
      ...
      switch($type)
      {
        ...
        case "automatic":
          switch($sub_type)
          {
            case "name":
              if(strlen($name)==0)
                return($this->OutputError("não é possível determinar o tipo do arquivo"));
...
*/
  Function GetDataURL($file)
  {
    ...
    if(IsSet($definition["FILENAME"]))
    {
      ...
      && strlen($body)!=$size)
      {
        $this->OutputError("o tamanho do arquivo lido não equivale ao tamanho da parte ".$definition["FILENAME"]." devido a dados corrompidos");
        return("");
      }
      if(function_exists("ini_get")
    ...
    else
    {
      ...
      {
        $this->OutputError("não foi indicado o arquivo ou bloco de dados");
        return("");
      }
...
*/
  ...
  Function GetHeadersAndBody(&$headers, &$body)
  {
    ...
    if($this->body_parts==0)
      return($this->OutputError("não há partes definidas para o corpo da mensagem"));
...
*/
  ...
  Function OpenMailing(&$mailing,&$mailing_properties)
  {
    ...
    || strlen($mailing_properties["Name"])==0)
      return($this->OutputError("não foi especificado um nome válido"));
    if(!IsSet($mailing_properties["Return-Path"])
    || strlen($mailing_properties["Return-Path"])==0)
      return($this->OutputError("valor inválido de Return-Path"));
    $separator="";
  ...
  Function AddMailingRecipient($mailing,&$recipient_properties)
  {
    ...
    if(!IsSet($this->mailings[$mailing]))
      return($this->OutputError("valor inválido de mailing"));
    if(!IsSet($recipient_properties["Address"])
    || strlen($recipient_properties["Address"])==0)
      return($this->OutputError("valor inválido do endereço de e-mail"));
    if(!@fwrite($this->mailings[$mailing]["Envelope"],"T".$recipient_properties["Address"].chr(0)))
      return($this->OutputPHPError("não foi possível escrever o endereço do destinatário no mailing envelope file", $php_errormsg));
    return("");
  }
