  ...
  cursor c_unidade (p_unidade in number) is
     select pt.sq_unidade, a.sq_unidade_pai, Nvl(pt.nome, -1) sq_pessoa_titular,
      ...
      from siw.eo_unidade  a
           left outer join (select b.sq_unidade, a.nome_resumido nome
                              from siw.co_pessoa                  a
                                   inner join siw.eo_unidade_resp b on (a.sq_pessoa       = b.sq_pessoa and 
                                                                        b.tipo_respons    = 'T' and 
                                                                        b.fim             is null and b.sq_unidade      = p_unidade
                                                                       ) 
                           ) pt on (a.sq_unidade  = pt.sq_unidade)
           left outer join (select b.sq_unidade, nome_resumido nome
                              from siw.co_pessoa          a 
                                   inner join siw.eo_unidade_resp    b on (a.sq_pessoa      = b.sq_pessoa and 
...
begin
 ...
 select a.acesso_geral, a.sq_menu, a.sq_modulo, a.sigla,
   ...
   from siw.sg_autenticacao                       b,
        siw.siw_solicitacao                       d
           inner      join siw.siw_menu           a on (a.sq_menu                = d.sq_menu)
 ...
 Else 
    ...
    select a.solicitante, b.sq_unidade
      ...
      from siw.siw_solicitacao a inner join siw.sg_autenticacao b on (a.solicitante = b.sq_pessoa)
     where a.sq_siw_solicitacao = p_solicitacao;
 End If;
    ...
    Else
       ...
       select count(*) 
         ...
         from siw.sg_tramite_pessoa        a
              inner   join siw.siw_tramite c on (a.sq_siw_tramite      = c.sq_siw_tramite)
                inner join siw.siw_menu    b on (b.sq_menu             = c.sq_menu)
        where Nvl(c.sigla,'---')    <> 'CI'
       ...
       Else
          ...
          Else
             ...
             Else
                ...
                select count(*) into w_existe
                  from siw.eo_unidade_resp       a
                       inner join siw.gd_demanda b on (a.sq_unidade = b.sq_unidade_resp)
                ...
                Else
                   ...
                   select count(*) into w_existe
                     from siw.eo_unidade_resp       a
                          inner join siw.pj_projeto b on (a.sq_unidade = b.sq_unidade_resp)
                    where b.sq_siw_solicitacao = p_solicitacao
                      and a.sq_pessoa          = p_usuario
                      and a.fim                is null;
                   If w_existe > 0 Then 
