   ...
   If p_restricao is null Then
      open p_result for
         select a.sq_isprojeto chave, a.cliente, a.codigo, a.nome, a.responsavel, a.telefone, 
           ...
           from is_projeto                            a
                left outer join   is_acao             b  on (a.sq_isprojeto       = b.sq_isprojeto and
                                                             a.cliente            = b.cliente)
                  left outer join siw.siw_solicitacao b1 on (b.sq_siw_solicitacao = b1.sq_siw_solicitacao)
                  left outer join siw.siw_tramite     b2 on (b1.sq_siw_tramite    = b2.sq_siw_tramite and
                                                             'CA'                 <> Nvl(b2.sigla,'-'))                                     
                left outer join siw.pj_projeto c on (b.sq_siw_solicitacao = c.sq_siw_solicitacao)
