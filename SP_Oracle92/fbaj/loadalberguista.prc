...
select '009'||substr(a.crdcardid, 7) carteira, substr(initcap(b.cstfirstname),1,60) nome,
       case when b.cstdayofbirth is null then null else to_char(to_date(substr(replace(replace(replace(replace(replace(replace(replace(b.cstdayofbirth,'-FEB-','-FEV-'),'-APR-','-ABR-'),'-MAY-','-MAI-'),'-AUG-','-AGO-'),'-SEP-','-SET-'),'-OCT-','-OUT-'),'-DEC-','-DEZ-'),1,7)||19||substr(b.cstdayofbirth,8,2),'dd-mon-yyyy'),'dd-mm-yyyy') end nascimento,
       ...
       case when b.cstgender is null then null else substr(b.cstgender,1,1) end sexo,
       to_date(a.crdissuedate, 'dd/mm/yy') carteira_emissao,
       to_date(a.crdexpirydate, 'dd/mm/yy') carteira_validade
  from cards_temp a inner join customers_temp b on (a.cstcustomerid = b.cstcustomerid)
 where a.crdcardid is not null
