create or replace procedure SP_PUTDICIONARIO
  ...
  cursor c_tabela is
    select f.sq_tabela chave, e.sq_tabela_tipo, c.sq_usuario, d.sq_sistema, a.table_name nome,
      ...
      from all_tables                       a
           ...
           inner      join siw_des.dc_usuario   c on (a.owner          = c.nome)
           inner      join siw_des.dc_sistema   d on (c.sq_sistema     = d.sq_sistema and
                                                  d.cliente        = p_cliente and
                                                  d.sigla          = upper(p_sistema)
                                                 )
           left outer join siw_des.dc_tabela    f on (a.table_name     = f.nome and
                                                  f.sq_usuario     = c.sq_usuario and
                                                  f.sq_sistema     = d.sq_sistema
                                                 ),
           siw_des.dc_tabela_tipo                e
    ...
    select e.sq_coluna chave, a.sq_tabela, b.sq_dado_tipo, t2.column_name nome, t2.column_id ordem, 
      ...
      from all_tables                         t1
           inner        join all_tab_columns  t2 on (t2.OWNER       = t1.owner and
             ...
             inner      join siw_des.dc_dado_tipo b  on (b.nome         = replace(replace(replace(replace(replace(replace(replace(replace(replace(t2.data_type,'VARCHAR2','VarChar'),'CHAR','Char'), 'NUMBER', 'Numeric'), 'DATE','Date'), 'BLOB','Blob'), 'BFILE', 'Blob'), 'RAW', 'Blob'), 'FLOAT', 'Numeric'), 'LONG', 'VarChar'))
           inner        join all_col_comments t3 on (t3.owner       = t1.owner and
                                                     t3.table_name  = t2.TABLE_NAME and
                                                     t3.column_name = t2.COLUMN_NAME
                                                    )
           inner        join siw_des.dc_tabela    a  on (t1.table_name  = a.nome)
               inner    join siw_des.dc_usuario   c  on (a.sq_usuario   = c.sq_usuario and
                                                     t1.owner       = c.nome and
           ...
           left outer   join siw_des.dc_coluna    e  on (a.sq_tabela    = e.sq_tabela and
    ...
    select f.sq_relacionamento chave, t1.constraint_name nome, 
      ...
      from all_constraints                        t1
           ...
           inner       join all_constraints       t3 on (t1.r_constraint_name = t3.constraint_name and
             ...
             inner     join siw_des.dc_tabela         b  on (t3.table_name        = b.nome)
               inner   join siw_des.dc_usuario        g  on (b.sq_usuario         = g.sq_usuario and
                                                         t3.owner             = g.nome
                                                        )
                 inner join siw_des.dc_sistema        h  on (g.sq_sistema         = h.sq_sistema and
                                                         d.sq_sistema         = h.sq_sistema and
                                                         h.cliente            = p_cliente and
                                                         h.sigla              = upper(p_sistema)
                                                        )
           left outer  join siw_des.dc_relacionamento f  on (t1.constraint_name   = f.nome and
  ...
  cursor c_trigger is
    select d.sq_trigger chave, a.sq_tabela, b.sq_usuario, c.sq_sistema, t.trigger_name nome, 
      ...
      from dba_triggers                     t
           inner        join siw_des.dc_tabela  a on (t.table_name   = a.nome)
             inner      join siw_des.dc_usuario b on (a.sq_usuario   = b.sq_usuario and
                                                   b.nome        = upper(p_owner)
                                                  )
               inner    join siw_des.dc_sistema c on (b.sq_sistema   = c.sq_sistema and
                                                  c.cliente      = p_cliente and
                                                  c.sigla        = upper(p_sistema)
                                                 )
             left outer join siw_des.dc_trigger d on (t.trigger_name = d.nome and
  ...
  cursor c_indice is
    select e.sq_indice chave, b.sq_indice_tipo, c.sq_usuario, c.sq_sistema, t1.index_name nome, 
      ...
      from all_indexes                        t1
           ...
           inner      join siw_des.DC_INDICE_TIPO b  on (b.nome        = case t1.uniqueness when 'NONUNIQUE' then 'Normal' else 'Único' end)
           left outer join siw_des.dc_indice      e  on (t1.index_name = e.nome and
  ...
  cursor c_procedure is
    select d.sq_stored_proc chave, a.sq_sp_tipo, b.sq_usuario, c.sq_sistema, object_name nome, 
      ...
      from all_objects                        t
           inner      join siw_des.dc_sp_tipo     a on (t.object_type = a.nome)
           inner      join siw_des.dc_sistema     c on (c.cliente      = p_cliente and
                                                    c.sigla        = upper(p_sistema)
                                                   )
             inner    join siw_des.dc_usuario     b on (b.sq_sistema   = c.sq_sistema and
                                                    b.nome         = t.owner
                                                   )
           left outer join siw_des.dc_stored_proc d on (t.object_name  = d.nome and
  ...
  cursor c_procedure_param is
    select f.sq_sp_param chave, a.sq_stored_proc, b.sq_dado_tipo, t2.position ordem, 
      ...
      from sys.all_objects                    t1
           ...
           inner      join siw_des.dc_stored_proc a  on (t1.object_name   = a.nome)
             inner    join siw_des.dc_usuario     d  on (a.sq_usuario     = d.sq_usuario and
                                                     t1.owner         = d.nome
                                                    )
               inner  join siw_des.dc_sistema     e  on (d.sq_sistema     = e.sq_sistema and
                                                     e.cliente        = p_cliente and
                                                     e.sigla          = upper(p_sistema)
                                                    )
           left outer join siw_des.dc_sp_param    f  on (a.sq_stored_proc = f.sq_stored_proc and
                                                     f.nome           = case when t2.argument_name is null then 'RESULT' else t2.argument_name end
                                                    ),
           siw_des.dc_dado_tipo                   b
     where t1.owner       = upper(p_owner)
...
begin
  ...
  for crec in c_tabela loop
     If crec.chave is null Then
        insert into siw_des.dc_tabela 
               (sq_tabela, sq_tabela_tipo, sq_usuario, sq_sistema, nome, descricao)
        values (siw_des.sq_tabela.nextval, 
                crec.sq_tabela_tipo, 
                crec.sq_usuario, 
                crec.sq_sistema, 
                crec.nome, 
     ...
     Else
        update siw_des.dc_tabela set descricao = crec.descricao where sq_tabela = crec.chave;
     End If;
  end loop;
  ...
  for crec in c_relacionamento loop
     If crec.chave is null Then
        insert into siw_des.dc_relacionamento 
               (sq_relacionamento, nome, descricao, tabela_pai, tabela_filha, sq_sistema)
        values (siw_des.sq_relacionamento.nextval, 
                crec.nome, 
                crec.descricao, 
                crec.tabela_pai, 
                crec.tabela_filha, 
     ...
     Else
        update siw_des.dc_relacionamento set
  ...
  delete from 
  (select a.*
     from siw_des.dc_relac_cols                      a
          inner       join siw_des.dc_relacionamento b on (a.sq_relacionamento = b.sq_relacionamento)
            inner     join siw_des.dc_tabela         c on (b.tabela_filha      = c.sq_tabela)
              inner   join siw_des.dc_usuario        d on (c.sq_usuario        = d.sq_usuario and
  ...
  (select a.*
     from siw_des.dc_relac_cols                      a
          inner       join siw_des.dc_relacionamento b on (a.sq_relacionamento = b.sq_relacionamento)
            inner     join siw_des.dc_tabela         c on (b.tabela_pai        = c.sq_tabela)
              inner   join siw_des.dc_usuario        d on (c.sq_usuario        = d.sq_usuario and
                                                       d.nome              = upper(p_owner)
                                                      )
                inner join siw_des.dc_sistema        e on (d.sq_sistema        = e.sq_sistema and
  ...
  insert into siw_des.dc_relac_cols (sq_relacionamento, coluna_pai, coluna_filha)
  (select e.sq_relacionamento, d.sq_coluna coluna_pai, b.sq_coluna coluna_filha
     from all_constraints                          t1
          inner         join all_cons_columns      t2 on (t2.owner             = t1.owner and 
            ...
            inner       join siw_des.dc_tabela         a  on (t2.table_name        = a.nome)
              inner     join siw_des.dc_coluna         b  on (a.sq_tabela          = b.sq_tabela and
                                                          t2.column_name       = b.nome
                                                         )
              inner     join siw_des.dc_usuario        f  on (a.sq_usuario         = f.sq_usuario)
                inner   join siw_des.dc_sistema        g  on (f.sq_sistema         = g.sq_sistema and
                                                          g.cliente            = p_cliente
                                                         )
          inner         join all_constraints       t3 on (t1.r_owner           = t3.owner and
            ...
            inner       join all_cons_columns      t4 on (t3.owner             = t4.owner and
              ...
              inner     join siw_des.dc_tabela         c on (t4.table_name         = c.nome)
                inner   join siw_des.dc_coluna         d on (c.sq_tabela           = d.sq_tabela and
                                                         t4.column_name        = d.nome
                                                        )
                inner   join siw_des.dc_usuario        h  on (c.sq_usuario         = h.sq_usuario)
                  inner join siw_des.dc_sistema        i  on (h.sq_sistema         = i.sq_sistema and
                                                          g.sq_sistema         = i.sq_sistema and
                                                          i.cliente            = p_cliente
                                                         )
          inner         join siw_des.dc_relacionamento e on (t1.constraint_name    = e.nome)
            inner       join siw_des.dc_sistema        j on (e.sq_sistema          = j.sq_sistema and
                                                         j.cliente             = p_cliente
                                                        )
    where t1.owner                   = upper(p_owner)
      and t1.constraint_type         = 'R'
  ...
  for crec in c_trigger loop
     If crec.chave is null Then
        insert into siw_des.dc_trigger 
               (sq_trigger, sq_tabela, sq_usuario, sq_sistema, nome, descricao)
        values (siw_des.sq_trigger.nextval, 
                crec.sq_tabela,
                crec.sq_usuario,
                crec.sq_sistema,
                crec.nome, 
  ...
  insert into siw_des.dc_trigger_evento (sq_trigger, sq_evento)
  (select a.sq_trigger, b.sq_evento
     from dba_triggers                    t 
          inner      join siw_des.dc_trigger  a on (t.trigger_name     = a.nome)
            inner    join siw_des.dc_usuario  c on (a.sq_usuario       = c.sq_usuario and
                                                t.owner            = c.nome and
                                                c.nome             = upper(p_owner)
                                               )
          ...
          inner      join siw_des.dc_evento   b on (t.triggering_event = b.nome)
   where t.owner = upper(p_owner)
  );
  ...
  for crec in c_indice loop
     If crec.chave is null Then
        insert into siw_des.dc_indice 
               (sq_indice, sq_indice_tipo, sq_usuario, sq_sistema, nome, descricao)
        values (siw_des.sq_indice.nextval, 
                crec.sq_indice_tipo,
                crec.sq_usuario,
                crec.sq_sistema,
                crec.nome, 
     ...
     Else
        update siw_des.dc_indice set descricao = crec.descricao where sq_indice = crec.chave;
     End If;
  end loop;
  ...
  delete siw_des.dc_indice_cols a 
   where a.sq_coluna in 
         (select sq_coluna 
            from siw_des.dc_coluna 
           where sq_tabela in (select sq_tabela 
                                 from siw_des.dc_tabela 
                                where sq_usuario = (select sq_usuario 
                                                      from siw_des.dc_usuario 
                                                     where nome       = upper(p_owner)
                                                       and sq_sistema = (select sq_sistema 
  ...
  insert into siw_des.dc_indice_cols (sq_indice, sq_coluna, ordem, ordenacao)
  (select b.sq_indice, c.sq_coluna, T2.COLUMN_POSITION, case T2.DESCEND when 'ASC' then 'A' else 'D' end ordenacao
     from all_indexes                    t1
          ...
          inner     join siw_des.dc_indice   b  on (t1.index_name  = b.nome and
                                                d.sq_usuario   = b.sq_usuario
                                               )
          inner     join all_ind_columns t2 on (t1.owner       = t2.index_owner and
                                                t2.index_name  = t1.index_name
                                               )
            inner   join siw_des.dc_coluna   c  on (t2.column_name = c.nome and
  ...
  for crec in c_procedure loop
     If crec.chave is null Then
        insert into siw_des.dc_stored_proc 
               (sq_stored_proc, sq_sp_tipo, sq_usuario, sq_sistema, nome, descricao)
        values (siw_des.sq_stored_proc.nextval, 
                crec.sq_sp_tipo,
                crec.sq_usuario,
                crec.sq_sistema,
                crec.nome, 
     ...
     Else
        update siw_des.dc_stored_proc set descricao = crec.descricao where sq_stored_proc = crec.chave;
     End If;
  end loop;
  ...
  for crec in c_procedure_param loop
     ...
     Else
        update siw_des.dc_sp_param set
           sq_dado_tipo = crec.sq_dado_tipo,
           descricao    = crec.descricao,
           tipo         = crec.tipo,
           ordem        = crec.ordem
  ...
  insert into siw_des.dc_sp_tabs (sq_stored_proc, sq_tabela)
  (select a.sq_stored_proc, b.sq_tabela
     from sys.all_dependencies           t
          inner join siw_des.dc_stored_proc  a on (t.name             = a.nome)
            inner   join siw_des.dc_usuario  d on (a.sq_usuario       = d.sq_usuario and
                                               t.owner            = d.nome
                                               )
              inner join siw_des.dc_sistema  e on (d.sq_sistema       = e.sq_sistema and
          ...
          inner join siw_des.dc_tabela       b on (t.referenced_name  = b.nome)
            inner   join siw_des.dc_usuario  f on (b.sq_usuario       = f.sq_usuario and
  ...
  delete siw_des.dc_sp_sp a 
   where a.sp_pai in 
         (select sq_stored_proc 
            from siw_des.dc_stored_proc
           where sq_usuario = (select sq_usuario 
                                 from siw_des.dc_usuario 
                                where nome       = upper(p_owner)
                                  and sq_sistema = (select sq_sistema 
  ...
  insert into siw_des.dc_sp_sp (sp_pai, sp_filha)
  (select b.sq_stored_proc, a.sq_stored_proc
     from sys.all_dependencies t
          ...
          inner join siw_des.dc_stored_proc b on (t.referenced_name  = b.nome)
            inner   join siw_des.dc_usuario f on (b.sq_usuario       = f.sq_usuario and
                                              t.referenced_owner = f.nome
                                              )
              inner join siw_des.dc_sistema g on (f.sq_sistema       = g.sq_sistema and
  ...
  delete siw_des.dc_relac_cols
   where sq_relacionamento in 
        (select x.sq_relacionamento
          from siw_des.dc_relacionamento          x
               inner      join siw_des.dc_tabela  m on (x.tabela_filha = m.sq_tabela)
               inner      join siw_des.dc_usuario n on (m.sq_usuario   = n.sq_usuario and
                                                    n.nome         = upper(p_owner)
                                                   )
               inner      join siw_des.dc_sistema o on (n.sq_sistema   = o.sq_sistema and
               ...
               left outer join (select f.sq_relacionamento
                                  from all_constraints                        t1
                                       ...
                                       inner       join siw_des.dc_relacionamento f  on (t1.constraint_name   = f.nome)
                                 where t1.owner                   = upper(p_owner)
                                   and t1.constraint_type         ='R'
                                   and (instr(t1.table_name, '$') = 0 
                                   and upper(t1.table_name)       not in ('PLAN_TABLE', 'SG_PESSOA_MENU_TMP', 'LIGORI', 'LIGNATE', 'LIGREC', 'SQLEXPERT_PLAN1'))
  ...
  delete siw_des.dc_coluna 
  where sq_coluna in (select x.sq_coluna
                        from siw_des.dc_coluna x
                             inner      join siw_des.dc_tabela  z on (x.sq_tabela = z.sq_tabela)
                             inner      join siw_des.dc_usuario w on (z.sq_usuario   = w.sq_usuario and
                                                                  w.nome         = upper(p_owner)
                                                                 )
                             inner      join siw_des.dc_sistema v on (w.sq_sistema   = v.sq_sistema and
                                                                  v.sigla        = upper(p_sistema) and
                                                                  v.cliente      = p_cliente
                                                                 )
                             left outer join (select a.sq_coluna, e.owner, e.TABLE_NAME, e.COLUMN_NAME
                                                from siw_des.dc_coluna                   a
                                                     inner   join siw_des.dc_tabela   b  on (a.sq_tabela    = b.sq_tabela)
                                                       inner join siw_des.dc_usuario  c  on (b.sq_usuario   = c.sq_usuario and
                                                                                         c.nome         = upper(p_owner)
                                                                                        )
                                                       inner join siw_des.dc_sistema  d  on (c.sq_sistema   = d.sq_sistema and
                                                                                         d.sigla        = upper(p_sistema) and
  ...
  delete siw_des.dc_relacionamento 
   where sq_relacionamento in 
        (select x.sq_relacionamento
          from siw_des.dc_relacionamento          x
               inner      join siw_des.dc_tabela  m on (x.tabela_filha = m.sq_tabela)
  ...
  delete siw_des.dc_trigger 
   where sq_trigger in 
        (select x.sq_trigger
          from siw_des.dc_trigger x
               inner      join siw_des.dc_tabela  z on (x.sq_tabela  = z.sq_tabela)
               inner      join siw_des.dc_usuario w on (z.sq_usuario = w.sq_usuario and
                                                    w.nome       = upper(p_owner)
                                                   )
               ...
               left outer join (select d.sq_trigger
                                  from dba_triggers                  t
                                       inner     join siw_des.dc_tabela  a on (t.table_name   = a.nome)
                                         ...
                                         inner   join siw_des.dc_trigger d on (t.trigger_name = d.nome and
                                                                           d.sq_tabela    = a.sq_tabela
                                                                          )
                                 where t.owner = upper(p_owner)
  ...
  delete siw_des.dc_indice
   where sq_indice in 
        (select x.sq_indice
          from siw_des.dc_indice x
               inner      join siw_des.dc_usuario w on (x.sq_usuario = w.sq_usuario and
                                                    w.nome       = upper(p_owner)
                                                   )
               inner      join siw_des.dc_sistema v on (x.sq_sistema = v.sq_sistema and
                                                    v.sigla      = upper(p_sistema) and
                                                    v.cliente    = p_cliente
                                                   )
               left outer join (select e.sq_indice
                                  from all_indexes                       t1
                                       ...
                                       inner     join siw_des.dc_indice      e  on (t1.index_name = e.nome and
                                                                                e.sq_usuario  = c.sq_usuario and
                                                                                e.sq_sistema  = d.sq_sistema
                                                                               )
                                 where t1.owner                   = upper(p_owner)
  ...
  delete siw_des.dc_sp_param
   where sq_sp_param in 
      (select x.sq_sp_param
        from siw_des.dc_sp_param x
             inner      join siw_des.dc_stored_proc u on (x.sq_stored_proc = u.sq_stored_proc)
             inner      join siw_des.dc_usuario     w on (u.sq_usuario = w.sq_usuario and
                                                      w.nome       = upper(p_owner)
                                                     )
             inner      join siw_des.dc_sistema     v on (w.sq_sistema = v.sq_sistema and
                                                      v.sigla      = upper(p_sistema) and
             ...
             left outer join (select f.sq_sp_param
                                from sys.all_objects                   t1
                                     ...
                                     inner     join siw_des.dc_sp_param    f  on (a.sq_stored_proc = f.sq_stored_proc and
                                                                              f.nome           = case when t2.argument_name is null then 'RESULT' else t2.argument_name end
                                                                             )
                               where t1.owner       = upper(p_owner)
                                 and t1.object_type in ('FUNCTION', 'PROCEDURE')
          ...
          from siw_des.dc_stored_proc             x
               ...
               left outer join (select d.sq_stored_proc
                                  from all_objects                     t
                                       ...
                                       inner   join siw_des.dc_stored_proc d on (t.object_name  = d.nome and
                                                                             b.sq_usuario   = d.sq_usuario
                                                                            )
                                 where t.owner = upper(p_owner)
                                   and t.object_type in ('FUNCTION', 'PROCEDURE')
  ...
  delete siw_des.dc_tabela
   where sq_tabela in 
        (select x.sq_tabela
          from siw_des.dc_tabela                  x
               inner      join siw_des.dc_usuario w on (x.sq_usuario = w.sq_usuario and
                                                    w.nome       = upper(p_owner)
                                                   )
               inner      join siw_des.dc_sistema v on (w.sq_sistema = v.sq_sistema and
               ...
               left outer join (select f.sq_tabela
                                  from all_tables                       a
                                       ...
                                       inner join siw_des.dc_tabela    f on (a.table_name     = f.nome and
                                                                              f.sq_usuario     = c.sq_usuario and
                                                                              f.sq_sistema     = d.sq_sistema
                                                                             ),
                                       siw_des.dc_tabela_tipo                e
...
/
GRANT EXECUTE ON SP_PUTDICIONARIO TO SIW_DES
