create or replace procedure SP_PutDocumentoArquivo
...
begin
   If p_operacao = 'I' Then -- Inclusão
      ...
      -- Insere registro em fn_documento_arq
      insert into fn_documento_arq
        (sq_lancamento_doc, sq_siw_arquivo, ordem)
      values
        (p_chave, w_chave, p_ordem);
   Elsif p_operacao = 'A' Then -- Alteração
      -- Atualiza a tabela de arquivos
      update fn_documento_arq 
        set ordem = p_ordem 
      where sq_lancamento_doc = p_chave and sq_siw_arquivo = p_chave_aux;
   ...
   Elsif p_operacao = 'E' Then -- Exclusão
      ...
      delete fn_documento_arq where sq_lancamento_doc = p_chave and sq_siw_arquivo = p_chave_aux;
      
      -- Remove da tabela de arquivos
      delete siw_arquivo where sq_siw_arquivo = p_chave_aux;
   End If;
end SP_PutDocumentoArquivo;
/
