create or replace procedure SP_PutCLEnvio
   (p_menu                in number,
    p_chave               in number,
    p_pessoa              in number,
    p_tramite             in number,
...
begin
   ...
   If p_destinatario is  not null Then
      ...
      -- Atualiza o responsável atual pela solicitacao
   ...
   -- Recupera a nova chave da tabela de encaminhamentos da solicitaao
   select sq_cl_log.nextval into w_chave_dem from dual;
   
   -- Insere registro na tabela de encaminhamentos da solicitacao
   Insert into cl_solicitacao_log 
      (sq_cl_log,                 sq_siw_solicitacao, cadastrador, 
   ...
   If p_caminho is not null Then
      ...
      -- Decide se o vínculo do arquivo será com o log da solicitação.
      If p_tramite <> p_novo_tramite Then
      ...
      Else
         -- Insere registro em CL_SOLICITACAO_ARQ_LOG
         insert into cl_solicitacao_log_arq (sq_cl_log, sq_siw_arquivo)
