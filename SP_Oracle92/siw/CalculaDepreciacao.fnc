...
begin
  ...
  loop
     ...
     If p_inicio is not null and p_fim is not null and w_fim_vida_util < p_inicio Then
        ...
        Result := 0;
     Elsif p_inicio is not null and p_fim is not null and crec.data_tombamento < p_inicio and w_fim_vida_util between p_inicio and p_fim Then
        -- Se tombamento ocorreu antes do início do período e vida útil termina até o fim do período, 
        -- depreciação é o que falta para zerar o valor do bem
        select crec.valor_atual - calculaDepreciacao(p_chave, p_moeda, null, p_inicio-1)
          into Result
          from dual;
     ...
     Else
        ...
        If p_inicio is null or p_inicio <= crec.data_tombamento Then
              -- A depreciação começa somente no dia seguinte ao de tombamento.
