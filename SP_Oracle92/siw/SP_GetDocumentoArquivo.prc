create or replace procedure SP_GetDocumentoArquivo
   (p_chave        in  number,
...
begin
   -- Recupera os documentos ligados a um documento
   open p_result for
      select a.sq_lancamento_doc as chave, a.ordem,
        ...
        from fn_documento_arq              a
             inner   join siw_arquivo      b on (a.sq_siw_arquivo  = b.sq_siw_arquivo)
               left  join siw_tipo_arquivo c on (b.sq_tipo_arquivo = c.sq_tipo_arquivo)
