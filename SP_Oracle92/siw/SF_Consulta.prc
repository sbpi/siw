create or replace procedure SF_Consulta(
       ...
       p_documento    in varchar2 default null,
       p_inicio       in date     default null,
       p_fim          in date     default null,
       p_comprovante  in varchar2 default null,
       p_inicio_nf    in date     default null,
       p_fim_nf       in date     default null,
       p_result       out sys_refcursor
      ) is
begin
  If upper(p_restricao) = 'DOLAR' Then
     open p_result for
        select distinct(to_char(data,'yyyy/mm')) mes,valor
          from corporativo.gn_moedacotacoes@sicof a
         where data between sysdate-360 and sysdate+10
         order by 1 desc;
