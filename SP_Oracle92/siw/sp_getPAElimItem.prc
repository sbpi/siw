create or replace procedure sp_getPAElimItem
...
begin
   If p_restricao is null Then
      ...
      open p_result for 
         select a.protocolo as chave,    a.sq_siw_solicitacao,          a.eliminacao,
                b.inicio,                b.fim,
                ...
                inner       join pa_documento             d  on (a.protocolo                = d.sq_siw_solicitacao)
                  ...
                  inner     join pa_documento_assunto     d4 on (d.sq_siw_solicitacao       = d4.sq_siw_solicitacao and
                    ...
                    inner   join pa_assunto               d5 on (d4.sq_assunto              = d5.sq_assunto)
                      inner join pa_tipo_guarda           d8 on (d5.destinacao_final        = d8.sq_tipo_guarda)
                  left      join pa_caixa                 d6 on (d.sq_caixa                 = d6.sq_caixa)
                    left    join eo_unidade               d7 on (d6.sq_unidade              = d7.sq_unidade)
                  inner     join siw_solicitacao          d9 on (d.sq_siw_solicitacao       = d9.sq_siw_solicitacao)
          where (p_chave         is null or (p_chave         is not null and a.protocolo           = p_chave))
            ...
            and (p_atraso        is null or (p_atraso        is not null and a.eliminacao          is null and b.fim < trunc(sysdate)))
            and (p_ini           is null or (p_ini           is not null and a.eliminacao           between p_ini and p_fim));
   End If;
end sp_getPAElimItem;
/
