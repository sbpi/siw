...
select chave, cliente, to_char(data_formatada,'yyyy') as ano, data_formatada, nome, expediente, nm_expediente
  from (select a.sq_data_especial as chave, a.cliente, 
         ...
         from eo_data_especial  a,
              (select to_char(sysdate,'yyyy')-3 as ano from dual
               UNION
               select to_char(sysdate,'yyyy')-2 as ano from dual
               UNION
               select to_char(sysdate,'yyyy')-1 as ano from dual
               UNION
               select to_char(sysdate,'yyyy')+0 as ano from dual
               UNION
               select to_char(sysdate,'yyyy')+1 as ano from dual
               UNION
               select to_char(sysdate,'yyyy')+2 as ano from dual
         ...
         where a.ativo = 'S'
           and (a.tipo <> 'E' or (a.tipo = 'E' and substr(a.data_especial, 7, 4) = b.ano))
       );
