...
begin
   If p_restricao is null Then
      ...
      open p_result for 
         select a.sq_valores as chave, a.nome, a.tipo, a.codigo_externo, a.ativo,
                case a.tipo  when 'S' then 'Supressão' else 'Acréscimo' end as nm_tipo,
                case a.ativo when 'S' then 'Sim'       else 'Não'       end as nm_ativo,
                acentos(nome) as ordena
           from fn_valores                a
                left join fn_valores_vinc b on (a.sq_valores = b.sq_valores and
                                                b.sq_menu    = coalesce(p_menu,0)
                                                )
