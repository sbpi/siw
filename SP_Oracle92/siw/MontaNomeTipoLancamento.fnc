create or replace function MontaNomeTipoLancamento(p_chave in number, p_retorno in varchar2 default null) return varchar2 is
  cursor c_ordem is
     select sq_tipo_lancamento, sq_tipo_lancamento_pai, nome
       from fn_tipo_lancamento
     start with sq_tipo_lancamento = p_chave
     connect by prior sq_tipo_lancamento_pai = sq_tipo_lancamento;
