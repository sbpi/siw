create or replace procedure SP_GetSolicCL
   (p_menu          in number,
    ...
    p_seq_protocolo in number   default null,
    p_situacao      in varchar2 default null,
    p_ano_protocolo in number   default null,
    ...
    p_classif       in varchar2 default null,
      ...
      open p_result for
         select a.sq_menu,            a.sq_modulo,                   a.nome,
           ...
           from siw_menu                                        a
                ...
                inner             join siw_solicitacao          b  on (a.sq_menu                  = b.sq_menu)
                   ...
                   inner          join co_cidade                f  on (b.sq_cidade_origem         = f.sq_cidade)
                     ...
                     inner        join co_regiao                f2 on (f.sq_regiao                = f2.sq_regiao)
                     inner        join co_uf                    f3 on (f.sq_pais                  = f3.sq_pais and
                                                                       f.co_uf                    = f3.co_uf
                                                                      )
          ...
          where (p_menu           is null or (p_menu          is not null and a.sq_menu              = p_menu))
            ...
            and (p_seq_protocolo  is null or (p_seq_protocolo is not null and d.processo             like '%'||p_seq_protocolo||'%'))
            and (p_ano_protocolo  is null or (p_ano_protocolo is not null and d.processo             like '%'||p_ano_protocolo||'%'))
            ...
            and (p_pais           is null or (p_pais          is not null and f1.sq_pais             = p_pais))
            and (p_regiao         is null or (p_regiao        is not null and f2.sq_regiao           = p_regiao))
            and (p_uf             is null or (p_uf            is not null and f3.co_uf               = p_uf))
            ...
            and (p_classif        is null or (p_classif       = 'CLASSIF' and b.sq_cc is not null) or (p_classif <> 'CLASSIF' and m.sq_menu = to_number(p_classif)))
            and (p_situacao       is null or (p_situacao      is not null and d6.sq_lcsituacao       = to_number(p_situacao)))
