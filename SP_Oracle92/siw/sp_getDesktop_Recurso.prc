...
begin
   ...
   open p_result for
     select a.qtd as qt_visao, b.qtd as qt_gestao,
       ...
       from (select count(x.sq_recurso) as qtd from eo_recurso x where x.cliente = p_cliente and 0 < acesso_recurso(x.sq_recurso, p_usuario)) a,
            (select count(x.sq_recurso) as qtd from eo_recurso x where x.cliente = p_cliente and 4 = acesso_recurso(x.sq_recurso, p_usuario)) b,
