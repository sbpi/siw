...
begin
   ...
   open p_result for
     select a.qtd as qt_visao, b.qtd as qt_gestao,
       ...
       from (select /*+ ordered */ count(x.sq_recurso) as qtd from eo_recurso x, (select sq_recurso, acesso_recurso(sq_recurso, p_usuario) acesso from eo_recurso group by sq_recurso) y where x.sq_recurso = y.sq_recurso and x.cliente = p_cliente and x.ativo = 'S' and x.exibe_mesa = 'S' and 0 < y.acesso) a,
            (select /*+ ordered */ count(x.sq_recurso) as qtd from eo_recurso x , (select sq_recurso, acesso_recurso(sq_recurso, p_usuario) acesso from eo_recurso group by sq_recurso) y where x.sq_recurso = y.sq_recurso and x.cliente = p_cliente and x.ativo = 'S' and x.exibe_mesa = 'S' and 4 = y.acesso) b,
