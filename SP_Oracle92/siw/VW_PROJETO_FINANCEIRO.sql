...
select 'I' TIPO, b.sq_siw_solicitacao sq_projeto, b.codigo_interno cd_projeto, a1.sigla sg_tramite,
       ...
       case when f.sq_siw_solicitacao is null then e.valor_total else trunc(e.valor_total*f.fator,2) end valor,  -- Valor convertido na moeda do projeto
       ...
       c2.data brl_taxa_data, c2.taxa_compra brl_taxa_compra, c2.taxa_venda brl_taxa_venda,
       ...
       case when a2.sigla          = 'BRL' then e.valor_total
            when nvl(b2.sigla,'-') = 'BRL' and f.sq_siw_solicitacao is not null then trunc(e.valor_total*f.fator,2)
  ...
  from siw_solicitacao                      a
       ...
       inner         join fn_lancamento     c  on (a.sq_siw_solicitacao  = c.sq_siw_solicitacao)
         left        join co_moeda_cotacao  c2 on (c.cliente             = c2.cliente and
