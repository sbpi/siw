create or replace function CalculaIDCC(p_chave in number, p_data in date default null, p_inicio in date default null, p_aditivo in number default null) return float is
  Result        float      := 0;
  w_existe      number(18);
  w_cliente     number(18);
       ...
       from siw_solicitacao w
            inner join (select a.sq_siw_solicitacao, sum(a.valor) as valor
                         ...
                         where a.sq_siw_solicitacao = p_chave
                           and a.vencimento between coalesce(p_inicio, z.inicio) and coalesce(p_data,sysdate)
                           and (y.prazo_indeterm    = 'S' or
                                (w_aditivo          is     null and a.vencimento between z.inicio and z.fim) or
                                (w_aditivo          is not null and a.sq_acordo_aditivo = w_aditivo)
                               )
            ...
            left  join (select a.sq_siw_solicitacao, sum(d.valor) as valor
                          from ac_acordo_parcela              a
                               ...
                               inner     join fn_lancamento   c on (c.cliente            = p_cliente and
                                 ...
                                 inner   join ac_acordo       x on (b.sq_siw_solicitacao = x.sq_siw_solicitacao)
                                   inner join ac_tipo_acordo  y on (x.sq_tipo_acordo     = y.sq_tipo_acordo)
                                   inner join siw_solicitacao z on (x.sq_siw_solicitacao = z.sq_siw_solicitacao)
                         where a.sq_siw_solicitacao = p_chave
                           and a.vencimento between coalesce(p_inicio, b.inicio) and coalesce(p_data,sysdate)
                           and (y.prazo_indeterm    = 'S' or
                                (w_aditivo          is     null and a.vencimento between z.inicio and z.fim) or
                                (w_aditivo          is not null and a.sq_acordo_aditivo = w_aditivo)
                               )
