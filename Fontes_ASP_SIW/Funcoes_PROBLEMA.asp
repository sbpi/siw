...
Function OpcaoMenu(p_sq_menu)
  ...
  DB_GetMenuUpper l_rs, p_sq_menu
  l_texto = ""
  l_cont  = 0
  ...
  While NOT RS.EOF
     l_Cont = l_Cont + 1
     If l_Cont = 1 Then
        l_texto = "<font color=""#FF0000"">" & l_rs("nome") & "</font> -> " & l_texto
     Else
        l_texto = l_rs("nome") & " -> " & l_texto
     End If
     RS.MoveNext
  Wend
...
Sub SelecaoPessoa1 (label, accesskey, hint, chave, chaveAux, campo, restricao)
  ...
  If cDbl(nvl(chave,0)) > 0 Then
     DB_GetPersonList l_rs, w_cliente, chave, restricao
     l_rs.Sort = "nome_resumido"
     l_rs.Filter = "sq_pessoa = " & chave
     w_nm_usuario = l_rs("nome_resumido") & " (" & l_rs("sg_unidade") & ")"
  End If
  If IsNull(hint) Then
...
Sub SelecaoPessoa (label, accesskey, hint, chave, chaveAux, campo, restricao)
  Dim l_rs
  Set l_rs = Server.CreateObject("ADODB.RecordSet")
  DB_GetPersonList l_rs, w_cliente, ChaveAux, restricao
  l_rs.Sort = "nome_resumido_ind"
  ...
  ShowHTML "          <option value="""">---"
  While Not l_rs.EOF
     If cDbl(nvl(l_rs("sq_pessoa"),0)) = cDbl(nvl(chave,0)) Then
        ShowHTML "          <option value=""" & l_rs("sq_pessoa") & """ SELECTED>" & l_rs("NOME_RESUMIDO") & " (" & l_rs("SG_UNIDADE") & ")"
     Else
        ShowHTML "          <option value=""" & l_rs("sq_pessoa") & """>" & l_rs("NOME_RESUMIDO") & " (" & l_rs("SG_UNIDADE") & ")"
     End If
     l_rs.MoveNext
  Wend
  ShowHTML "          </select>"
...
Sub SelecaoSolicResp (label, accesskey, hint, chave, chaveAux, tramite, chaveAux2, campo, restricao)
  Dim l_rs
  Set l_rs = Server.CreateObject("ADODB.RecordSet")
  DB_GetSolicResp l_rs, chaveAux, tramite, chaveAux2, restricao
  l_rs.Sort = "nome_resumido"
  If IsNull(hint) Then
     ShowHTML "          <td valign=""top""><font size=""1""><b>" & Label & "</b><br><SELECT ACCESSKEY=""" & accesskey & """ CLASS=""sts"" NAME=""" & campo & """ " & w_Disabled & ">"
  Else
  ...
  ShowHTML "          <option value="""">---"
  While Not l_rs.EOF
     If cDbl(nvl(l_rs("sq_pessoa"),0)) = cDbl(nvl(chave,0)) or (l_rs.RecordCount = 1) Then
        ShowHTML "          <option value=""" & l_rs("sq_pessoa") & """ SELECTED>" & l_rs("NOME_RESUMIDO") & " (" & l_rs("SG_UNIDADE") & ")"
     Else
        ShowHTML "          <option value=""" & l_rs("sq_pessoa") & """>" & l_rs("NOME_RESUMIDO") & " (" & l_rs("SG_UNIDADE") & ")"
     End If
     If l_rs.RecordCount = 1 Then
        chave = l_rs("sq_pessoa")
     End If
     l_rs.MoveNext
  Wend
  ShowHTML "          </select>"
...
Sub SelecaoUsuUnid (label, accesskey, hint, chave, chaveAux, campo, restricao)
  Dim l_rs
  Set l_rs = Server.CreateObject("ADODB.RecordSet")
  DB_GetUserList l_rs, w_cliente, null, null, null, null, null, null, "S"
  l_rs.Sort = "nome_indice"
  l_rs.Filter = "contratado = 'S'"
  If IsNull(hint) Then
     ShowHTML "          <td valign=""top""><font size=""1""><b>" & Label & "</b><br><SELECT ACCESSKEY=""" & accesskey & """ CLASS=""sts"" NAME=""" & campo & """ " & w_Disabled & ">"
  Else
     ShowHTML "          <td valign=""top"" title=""" & hint & """><font size=""1""><b>" & Label & "</b><br><SELECT ACCESSKEY=""" & accesskey & """ CLASS=""sts"" NAME=""" & campo & """ " & w_Disabled & ">"
  ...
  While Not l_rs.EOF
     If cDbl(nvl(l_rs("sq_pessoa"),0)) = cDbl(nvl(chave,0)) Then
        ShowHTML "          <option value=""" & l_rs("sq_pessoa") & """ SELECTED>" & l_rs("Nome")
     Else
        ShowHTML "          <option value=""" & l_rs("sq_pessoa") & """>" & l_rs("Nome")
     End If
     l_rs.MoveNext
  Wend
  ShowHTML "          </select>"
...
Sub SelecaoVinculo (label, accesskey, hint, chave, chaveAux, campo, restricao)
  Dim l_rs
  Set l_rs = Server.CreateObject("ADODB.RecordSet")
  DB_GetVincKindList l_rs, w_cliente
  If Nvl(restricao,"") > "" Then l_rs.Filter = restricao End If
  If IsNull(hint) Then
     ShowHTML "          <td valign=""top""><font size=""1""><b>" & Label & "</b><br><SELECT ACCESSKEY=""" & accesskey & """ CLASS=""sts"" NAME=""" & campo & """ " & w_Disabled & ">"
  Else
     ShowHTML "          <td valign=""top"" title=""" & hint & """><font size=""1""><b>" & Label & "</b><br><SELECT ACCESSKEY=""" & accesskey & """ CLASS=""sts"" NAME=""" & campo & """ " & w_Disabled & ">"
  ...
  ShowHTML "          <option value="""">---"
  While Not l_rs.EOF
     If cDbl(nvl(l_rs("sq_tipo_vinculo"),0)) = cDbl(nvl(chave,0)) Then
        ShowHTML "          <option value=""" & l_rs("sq_tipo_vinculo") & """ SELECTED>" & l_rs("nome")
     Else
        ShowHTML "          <option value=""" & l_rs("sq_tipo_vinculo") & """>" & l_rs("nome")
     End If
     l_rs.MoveNext
  Wend
  ShowHTML "          </select>"
...
Sub SelecaoTipoPosto (label, accesskey, hint, chave, chaveAux, campo, restricao)
  Dim l_rs
  Set l_rs = Server.CreateObject("ADODB.RecordSet")
  DB_GetTipoPostoList l_rs, w_cliente, null
  If Nvl(restricao,"") > "" Then l_rs.Filter = restricao End If
  If IsNull(hint) Then
     ShowHTML "          <td valign=""top""><font size=""1""><b>" & Label & "</b><br>"
  Else
  ...
  End If
  While Not l_rs.EOF
     If cDbl(nvl(l_rs("sq_eo_tipo_posto"),0)) = cDbl(nvl(chave,0)) Then
        ShowHTML "              <input " & w_Disabled & " type=""radio"" name=""" & campo & """ value=""" & l_rs("sq_eo_tipo_posto") & """ checked>"  & l_rs("descricao") & "<br>"
     Else
        ShowHTML "              <input " & w_Disabled & " type=""radio"" name=""" & campo & """ value=""" & l_rs("sq_eo_tipo_posto") & """>"  & l_rs("descricao") & "<br>"
     End If
     l_rs.MoveNext
  Wend
  ShowHTML "          </select>"
...
Sub SelecaoGrupoDef (label, accesskey, hint, chave, chaveAux, campo, restricao)
  ...
  ShowHTML "          <option value="""">---"
  While Not l_rs.EOF
     If cDbl(nvl(l_rs("SQ_GRUPO_DEFIC"),0)) = cDbl(nvl(chave,0)) Then
        ShowHTML "          <option value=""" & l_rs("SQ_GRUPO_DEFIC") & """ SELECTED>" & l_rs("Nome")
     Else
        ShowHTML "          <option value=""" & l_rs("SQ_GRUPO_DEFIC") & """>" & l_rs("Nome")
     End If
     l_rs.MoveNext
  Wend
  ShowHTML "          </select>"
...
Sub SelecaoTipoPessoa (label, accesskey, hint, chave, chaveAux, campo, restricao, atributo)
  Dim l_rs
  Set l_rs = Server.CreateObject("ADODB.RecordSet")
  DB_GetKindPersonList l_rs
  If restricao > "" Then
     l_rs.Filter = restricao
  End If
  If IsNull(hint) Then
     ShowHTML "          <td valign=""top""><font size=""1""><b>" & Label & "</b><br><SELECT ACCESSKEY=""" & accesskey & """ CLASS=""sts"" NAME=""" & campo & """ " & w_Disabled & " " & atributo & ">"
  Else
  ...
  ShowHTML "          <option value="""">---"
  While Not l_rs.EOF
     If cDbl(nvl(l_rs("sq_tipo_pessoa"),0)) = cDbl(nvl(chave,0)) Then
        ShowHTML "          <option value=""" & l_rs("sq_tipo_pessoa") & """ SELECTED>" & l_rs("Nome")
     Else
        ShowHTML "          <option value=""" & l_rs("sq_tipo_pessoa") & """>" & l_rs("Nome")
     End If
     l_rs.MoveNext
  Wend
  ShowHTML "          </select>"
...
Sub SelecaoFormaPagamento (label, accesskey, hint, chave, chave_aux, campo, restricao)
  Dim l_rs
  Set l_rs = Server.CreateObject("ADODB.RecordSet")
  DB_GetFormaPagamento l_rs, w_cliente, null, chave_aux, restricao
  l_rs.Sort = "nome"
  l_rs.Filter = "ativo = 'S'"
  If IsNull(hint) Then
     ShowHTML "          <td valign=""top""><font size=""1""><b>" & Label & "</b><br><SELECT ACCESSKEY=""" & accesskey & """ CLASS=""sts"" NAME=""" & campo & """ " & w_Disabled & ">"
  ...
  ShowHTML "          <option value="""">---"
  While Not l_rs.EOF
     If cDbl(nvl(l_rs("sq_forma_pagamento"),0)) = cDbl(nvl(chave,0)) Then
        ShowHTML "          <option value=""" & l_rs("sq_forma_pagamento") & """ SELECTED>" & l_rs("Nome")
     Else
        ShowHTML "          <option value=""" & l_rs("sq_forma_pagamento") & """>" & l_rs("Nome")
     End If
     l_rs.MoveNext
  Wend
  ShowHTML "          </select>"
...
Sub SelecaoPais (label, accesskey, hint, chave, chaveAux, campo, restricao, atributo)
  Dim l_rs
  Set l_rs = Server.CreateObject("ADODB.RecordSet")
  DB_GetCountryList l_rs
  l_rs.Sort = "padrao desc, Nome"
  If restricao > "" Then
     l_rs.Filter = restricao
  End If
  If IsNull(hint) Then
  ...
  ShowHTML "          <option value="""">---"
  While Not l_rs.EOF
     If cDbl(nvl(l_rs("sq_pais"),0)) = cDbl(nvl(chave,0)) or (l_rs.RecordCount = 1) Then
        ShowHTML "          <option value=""" & l_rs("sq_pais") & """ SELECTED>" & l_rs("Nome")
     Else
        ShowHTML "          <option value=""" & l_rs("sq_pais") & """>" & l_rs("Nome")
     End If
     If l_rs.RecordCount = 1 Then
        chave = l_rs("sq_pais")
     End If
...
Sub SelecaoRegiao (label, accesskey, hint, chave, chaveAux, campo, restricao, atributo)
  Dim l_rs
  Set l_rs = Server.CreateObject("ADODB.RecordSet")
  DB_GetRegionList l_rs, chaveAux, null
  l_rs.Sort = "Ordem"
  If restricao > "" Then
     l_rs.Filter = restricao
  End If
  ...
  ShowHTML "          <option value="""">---"
  While Not l_rs.EOF
     If cDbl(nvl(l_rs("sq_regiao"),0)) = cDbl(nvl(chave,0)) Then
        ShowHTML "          <option value=""" & l_rs("sq_regiao") & """ SELECTED>" & l_rs("Nome")
     Else
        ShowHTML "          <option value=""" & l_rs("sq_regiao") & """>" & l_rs("Nome")
     End If
     l_rs.MoveNext
  Wend
  ShowHTML "          </select>"
...
Sub SelecaoEstado (label, accesskey, hint, chave, chaveAux, chaveAux2, campo, restricao, atributo)
    Dim l_rs
    Set l_rs = Server.CreateObject("ADODB.RecordSet")
    DB_GetStateList l_rs, cDbl(Nvl(chaveAux,0))
    'l_rs.Sort = "padrao desc, nome"
    If restricao > "" Then
       l_rs.Filter = restricao
    End If
    ...
    ShowHTML "          <option value="""">---"
    While Not l_rs.EOF
       If nvl(l_rs("co_uf"),0) = nvl(chave,0) or l_rs.RecordCount = 1 Then
          ShowHTML "          <option value=""" & l_rs("co_uf") & """ SELECTED>" & l_rs("nome")
       Else
          ShowHTML "          <option value=""" & l_rs("co_uf") & """>" & l_rs("nome")
       End If
       If l_rs.RecordCount = 1 Then
          chave = l_rs("co_uf")
       End If
...
Sub SelecaoCidade (label, accesskey, hint, chave, chaveAux, chaveAux2, campo, restricao, atributo)
    Dim l_rs
    Set l_rs = Server.CreateObject("ADODB.RecordSet")
    DB_GetCityList l_rs, cDbl(Nvl(chaveAux,0)), chaveAux2
    l_rs.Sort = "capital desc, nome"
    If restricao > "" Then
       l_rs.Filter = restricao
    End If
    If IsNull(hint) Then
    ...
    ShowHTML "          <option value="""">---"
    While Not l_rs.EOF
       If cDbl(nvl(l_rs("sq_cidade"),0)) = cDbl(nvl(chave,0)) Then
          ShowHTML "          <option value=""" & l_rs("sq_cidade") & """ SELECTED>" & l_rs("nome")
       Else
          ShowHTML "          <option value=""" & l_rs("sq_cidade") & """>" & l_rs("nome")
       End If
       l_rs.MoveNext
    Wend
    ShowHTML "          </select>"
...
Sub SelecaoEndereco (label, accesskey, hint, chave, chaveAux, campo, restricao)
    Dim l_rs
    Set l_rs = Server.CreateObject("ADODB.RecordSet")
    DB_GetaddressList l_rs, w_cliente, ChaveAux, restricao
    If IsNull(hint) Then
       ShowHTML "          <td valign=""top""><font size=""1""><b>" & Label & "</b><br><SELECT ACCESSKEY=""" & accesskey & """ CLASS=""sts"" NAME=""" & campo & """ " & w_Disabled & ">"
    Else
       ShowHTML "          <td valign=""top"" title=""" & hint & """><font size=""1""><b>" & Label & "</b><br><SELECT ACCESSKEY=""" & accesskey & """ CLASS=""sts"" NAME=""" & campo & """ " & w_Disabled & ">"
    ...
    ShowHTML "          <option value="""">---"
    While Not l_rs.EOF
       If cDbl(nvl(l_rs("sq_pessoa_endereco"),0)) = cDbl(nvl(chave,0)) Then
          ShowHTML "          <option value=""" & l_rs("sq_pessoa_endereco") & """ SELECTED>" & l_rs("endereco")
       Else
          ShowHTML "          <option value=""" & l_rs("sq_pessoa_endereco") & """>" & l_rs("endereco")
       End If
       l_rs.MoveNext
    Wend
    ShowHTML "          </select>"
...
Sub SelecaoTelefone (label, accesskey, hint, chave, chaveAux, campo, restricao)
    Dim l_rs
    Set l_rs = Server.CreateObject("ADODB.RecordSet")
    DB_GetFoneList l_rs, w_cliente, ChaveAux, restricao
    If IsNull(hint) Then
       ShowHTML "          <td valign=""top""><font size=""1""><b>" & Label & "</b><br><SELECT ACCESSKEY=""" & accesskey & """ CLASS=""sts"" NAME=""" & campo & """ " & w_Disabled & ">"
    Else
       ShowHTML "          <td valign=""top"" title=""" & hint & """><font size=""1""><b>" & Label & "</b><br><SELECT ACCESSKEY=""" & accesskey & """ CLASS=""sts"" NAME=""" & campo & """ " & w_Disabled & ">"
    ...
    While Not l_rs.EOF
       If cDbl(nvl(l_rs("sq_pessoa_telefone"),0)) = cDbl(nvl(chave,0)) Then
          
          ShowHTML "          <option value=""" & l_rs("sq_pessoa_telefone") & """ SELECTED>" & l_rs("numero")
       Else
          ShowHTML "          <option value=""" & l_rs("sq_pessoa_telefone") & """>" & l_rs("numero")
       End If
       l_rs.MoveNext
    Wend
    ShowHTML "          </select>"
...
Sub SelecaoModulo (label, accesskey, hint, chave, chaveAux, campo, restricao, atributo)
    Dim l_rs
    Set l_rs = Server.CreateObject("ADODB.RecordSet")
    DB_GetSiwCliModLis l_rs, chaveAux, restricao
    l_rs.Sort = "nome"
    If IsNull(hint) Then
       ShowHTML "          <td valign=""top""><font size=""1""><b>" & Label & "</b><br><SELECT ACCESSKEY=""" & accesskey & """ CLASS=""sts"" NAME=""" & campo & """ " & w_Disabled & " " & atributo & ">"
    Else
    ...
    ShowHTML "          <option value="""">---"
    While Not l_rs.EOF
       If cDbl(nvl(l_rs("sq_modulo"),0)) = cDbl(nvl(chave,0)) Then
          ShowHTML "          <option value=""" & l_rs("sq_modulo") & """ SELECTED>" & l_rs("nome")
       Else
          ShowHTML "          <option value=""" & l_rs("sq_modulo") & """>" & l_rs("nome")
       End If
       l_rs.MoveNext
    Wend
    ShowHTML "          </select>"
...
Sub SelecaoServico (label, accesskey, hint, chave, chaveAux, campo, restricao, atributo)
    Dim l_rs
    Set l_rs = Server.CreateObject("ADODB.RecordSet")
    DB_GetMenuList l_rs, w_cliente, "I", null
    l_rs.Filter = "tramite='S'"
    If IsNull(hint) Then
       ShowHTML "          <td valign=""top""><font size=""1""><b>" & Label & "</b><br><SELECT ACCESSKEY=""" & accesskey & """ CLASS=""sts"" NAME=""" & campo & """ " & w_Disabled & " " & atributo & ">"
    Else
       ShowHTML "          <td valign=""top"" title=""" & hint & """><font size=""1""><b>" & Label & "</b><br><SELECT ACCESSKEY=""" & accesskey & """ CLASS=""sts"" NAME=""" & campo & """ " & w_Disabled & " " & atributo & ">"
    ...
    ShowHTML "          <option value="""">---"
    While Not l_rs.EOF
       If cDbl(nvl(l_rs("sq_menu"),0)) = cDbl(nvl(chave,0)) Then
          ShowHTML "          <option value=""" & l_rs("sq_menu") & """ SELECTED>" & l_rs("nome")
       Else
          ShowHTML "          <option value=""" & l_rs("sq_menu") & """>" & l_rs("nome")
       End If
       l_rs.MoveNext
    Wend
    ShowHTML "          </select>"
...
Function RetornaCliente()
  ...
  If Request("w_cgccpf") > "" and Len(Request("w_cgccpf")) > 11 Then
     DB_GetCompanyData l_rs, Session("p_cliente"), Request("w_cgccpf")
     If Not RS.EOF Then
        RetornaCliente = l_rs("sq_pessoa")
     Else
...
REM =========================================================================
REM Rotina de Fechamento do RecordSet especificado
REM -------------------------------------------------------------------------
Sub FechaRS(p_rs)
   p_rs.Sort = ""
   p_rs.Filter = ""
