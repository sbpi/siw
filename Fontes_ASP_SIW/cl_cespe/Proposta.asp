  ...
  If O = "L" Then
    ...
    Else
      ...
      While Not RS.EOF and RS.AbsolutePage = P3
        ' Recupera os dados complementares da solicitação
        ...
        If cDbl(Nvl(RS1("limite_passagem"),0)) <> 0 Then
           ShowHTML "          <A class=""hl"" HREF=""" & w_dir & w_Pagina & "Passagens&R=" & w_Pagina & par & "&O=L&w_chave=" & RS("sq_siw_solicitacao") & "&P1=" & P1 & "&P2=" & P2 & "&P3=" & P3 & "&P4=" & P4 & "&TP=Passagens&SG=" & SG & MontaFiltro("GET") & """ title=""Altera as informações cadastrais do projeto"">Passagens</A>&nbsp"
        End If
        If cDbl(Nvl(RS("or_tramite"),0)) = 1 Then 
           ShowHTML "          <A class=""hl"" HREF=""" & w_dir & w_Pagina & "Envio&R=" & w_Pagina & par & "&O=V&w_chave=" & RS("sq_siw_solicitacao") & "&P1=" & P1 & "&P2=" & P2 & "&P3=" & P3 & "&P4=" & P4 & "&TP=Enviar&SG=" & SG & MontaFiltro("GET") & """ title=""Envia o projeto para análise. Só pode ser feito após informar os dados do proponente e do preposto."">Enviar</A>&nbsp"
        End If
        ShowHTML "        </td>"
        ShowHTML "      </tr>"
        RS.MoveNext
 ...
 '    ShowHTML "}"
  ...
  If Instr("IA",O) > 0 Then
    ...
    Else
       ...
       ShowHTML "      <tr><TD colspan=""2"" align=""center"" bgcolor=""#D0D0D0""><b>Identificação do proponente</td></td></tr>"
       ShowHTML "      <tr><TD colspan=""2"" align=""center"" height=""1"" bgcolor=""#000000""></td></tr>"
...
Sub Preposto
  ...
  If Instr("IA",O) > 0 Then
    ...
    If w_cpf = "" or InStr(Request("botao"), "Alterar") > 0 or Instr(Request("botao"),"Procurar") > 0 Then
       ...
       ShowHTML "<tr bgcolor=""" & conTrBgColor & """><TD>"
       ShowHTML "    <table border=""0"">"
       ShowHTML "        <tr><TD colspan=4>Informe o CPF do preposto e clique no botão ""Selecionar"" para continuar.</TD>"
       ShowHTML "        <tr><TD colspan=4><b><u>C</u>PF:<br><INPUT ACCESSKEY=""C"" TYPE=""text"" class=""sti"" NAME=""w_cpf"" VALUE=""" & w_cpf & """ SIZE=""14"" MaxLength=""14"" onKeyDown=""FormataCPF(this, event);"">"
       ShowHTML "            <INPUT class=""stb"" TYPE=""submit"" NAME=""Botao"" VALUE=""Selecionar"">"
       ShowHTML "        <tr><TD colspan=4><p>&nbsp</p>"
       ShowHTML "        <tr><TD colspan=4 heigth=1 bgcolor=""#000000"">"
...
Sub Passagens
  ...
  If O <> "L" Then
     ...
     ElseIf O = "I" or O = "A" Then
        ...
        Else
           ...
           ' Para pessoas que tem o CPF/CNPJ gerado pelo sistema, não é possível alterar
           ' o passaporte nem o país emissor
           If O = "I" Then
              Validate "w_passaporte_numero", "Passaporte", "1", "1", 3, 15, "1", "1"
              Validate "w_sq_pais_passaporte", "País emissor do Passaporte", "SELECT", 1, 1, "999", "1", "1"
           End If
        End If
        Validate "w_ddd", "DDD", "1", "1", 3, 4, "", "0123456789"
        Validate "w_nr_telefone", "Telefone", "1", 1, 7, 25, "1", "1"
  ...
  ElseIf Instr("IA",O) > 0 Then
    ...
    If w_cpf = "" or InStr(w_botao, "Alterar") > 0 or Instr(w_botao,"Procurar") > 0 Then
       ...
       ShowHTML "        <tr><td colspan=4><INPUT class=""stb"" TYPE=""submit"" NAME=""Botao"" VALUE=""Pessoa sem CPF nem código gerado pelo sistema"" onClick=""Botao.value=this.value;"">"
       ShowHTML "        <tr><td colspan=4><p>&nbsp</p>"
       ShowHTML "        <tr><td colspan=4 heigth=1 bgcolor=""#000000"">"
       ShowHTML "        <tr><td colspan=4>"
    ...
    Else
       ...
       Else
          ...
          ShowHTML "          <td><font size=""1"">País emissor do passaporte<b><br>" & w_nm_pais_passaporte & "</td>"
...
Sub Encaminhamento
  ...
  If Nvl(w_erro,"") = "" Then
     ...
     ShowHTML "<INPUT type=""hidden"" name=""w_despacho"" value=""Envio do projeto pelo proponente, para análise pelo gerente."">"
        ...
        If (VerificaAssinaturaEletronica(Session("Username"),w_assinatura) and w_assinatura > "") or _
           ...
           If w_cpf = "GERAR" Then
              ...
              If Not RS.EOF Then
                 ...
                 ScriptOpen "JavaScript"
                 ShowHTML "  alert('Os dados do passaporte informado já estão associados ao código " & w_cpf & "  - " & RS("nome_resumido") & "');"
                 ShowHTML "  history.back(1);"
                 ScriptClose
                 Exit Sub
              Else
