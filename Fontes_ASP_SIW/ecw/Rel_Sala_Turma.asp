...
w_Pagina     = "Rel_Sala_Turma.asp?par="
w_Dir        = "ecw/"
w_Disabled   = "ENABLED"
...
Sub Inicial
  ...
  Dim p_unidade
  Dim p_modalidade, p_turno, p_serie, p_turma, p_ambiente, p_tipo_sala
  ...
  p_ambiente         = trim(uCase(Request("p_ambiente")))
  p_tipo_sala        = trim(uCase(Request("p_tipo_sala"))) 
  ...
  If O = "L" or O = "W" Then
     If Session("regional") = "" or Session("regional") = "00" Then
        DB_GetRoomList RS1, Session("periodo"), null, p_unidade, p_modalidade, p_turno, p_serie, p_turma, p_ambiente, p_tipo_sala
     Else
        DB_GetRoomList RS1, Session("periodo"), Session("regional"), p_unidade, p_modalidade, p_turno, p_serie, p_turma, p_ambiente, p_tipo_sala
     End If
     'If p_unidade & p_modalidade & p_turno & p_serie & p_turma & p_ambiente & p_tipo_sala > "" Then
     '   w_filter = ""
     ...
     '   If p_ambiente   > ""       Then w_filter = w_filter & " and co_seq_ambiente = '" & p_ambiente   & "' "    End If
     '   If p_tipo_sala  > ""       Then w_filter = w_filter & " and co_tipo_sala    = '" & p_tipo_sala  & "' "    End If
     '   RS1.Filter = Mid(w_filter,6,255)
     'End If
     If p_ordena > "" Then 
        RS1.sort = "ds_gre, co_unidade, " & p_ordena
     Else
        RS1.Sort = "ds_gre, co_unidade, ds_sala, co_sala"
     End If
  End If
  ...
  If O = "W" Then
     ...
     w_linha = 6
     If p_Tipo = "S" Then
        CabecalhoWord w_cliente, "Quantitativo de salas e turmas", w_pag
     Else
        CabecalhoWord w_cliente, "Lista de salas e turmas", w_pag
     End If
  ...
  Else
     ...
     If O = "L" Then
        If p_Tipo = "S" Then
           CabecalhoRelatorio w_cliente, "Quantitativo de salas e turmas"
        Else
           CabecalhoRelatorio w_cliente, "Lista de salas e turmas"
        End If
        ExibeParametrosRel w_cliente
        ShowHTML "<BR>"
     Else
  ...
  If O = "L" or O = "W" Then
      ...
      While Not RS1.EOF and (RS1.AbsolutePage = P3 or O = "W" or p_tipo = "S")
        If w_regional <> RS1("regional") or w_atual <> RS1("co_unidade") Then
           ...
           If p_tipo = "N" Then
                 ...
                 ShowHTML "          <td><font size=""1""><b>Sala</font></td>"
                 ...
                 ShowHTML "          <td><font size=""1""><b>Ambiente</font></td>"
                 ShowHTML "          <td width=""13%""><font size=""1""><b>Tipo de sala</font></td>"
                 ShowHTML "          <td><font size=""1""><b>Área da sala</font></td>"
                 ShowHTML "          <td><font size=""1""><b>Capacidade</font></td>"              
                 ShowHTML "          <td><font size=""1""><b>Matriculados</font></td>"              
                 ShowHTML "          <td><font size=""1""><b>Ativos</font></td>"      
                 ShowHTML "          <td><font size=""1""><b>ANEE</font></td>" 
                 ShowHTML "        </tr>"
           End If
       End If
           ...
           If p_tipo = "N" Then
              ...
              ShowHTML "        <td align=""center""><font size=""1"">" & Nvl(RS1("co_sala"),"---") & "</td>"
              ShowHTML "        <td align=""center""><font size=""1"">" & Nvl(RS1("ds_sala"),"---") & "</td>"
              ...
              ShowHTML "        <td align=""center""><font size=""1"">" & Nvl(RS1("ds_ambiente"),"---") & "</td>"
              ShowHTML "        <td align=""center""><font size=""1"">" & Nvl(RS1("ds_tipo_sala"),"---") & "</td>"
              ShowHTML "        <td align=""center""><font size=""1"">" & Nvl(RS1("nu_metragem"),"---") & "</td>"
              ...
              ShowHTML "        <td align=""center""><font size=""1"">" & Nvl(RS1("aluno_ativo"),"---") & "</td>"
              ShowHTML "        <td align=""center""><font size=""1"">" & Nvl(RS1("tp_anee"),"---") & "</td>"
              ShowHTML "      </tr>"
              w_linha = w_linha + 1
           End If
           RS1.MoveNext
    ...
    If (w_atual > "" and p_tipo = "S") or (w_atual > "" and O = "W")  Then
       ...
       ShowHTML "      <tr bgcolor=""" & conTrTotalBgcolor & """><td colspan=12><font size=""2""><b>TOTAL DA REGIONAL: " & FormatNumber(w_tot2,0) & "</b></td></tr>"
       If Session("regional") = "" or Session("regional") = "00" Then
          ShowHTML "      <tr bgcolor=""" & conTrBgcolor & """><td colspan=12><font size=""2""><b>TOTAL DE SALAS E TURMAS: " & FormatNumber(RS1.RecordCount,0) & "</b></td></tr>"
       End If
    End If
    ShowHTML "      </center>"
    ShowHTML "    </table>"
  ...
  ElseIf Instr("P",O) > 0 Then
    ...
    SelecaoAmbiente "<u>A</u>mbiente:", "A", null, p_ambiente, null, "p_ambiente", null, null
    SelecaoTipoSala "Tipo <u>S</u>ala:", "S", null, p_tipo_sala, null, "p_tipo_sala", null, null
    ShowHTML "          </table>"
    ShowHTML "      </tr>"
    ShowHTML "      <tr><td><table border=0 cellpadding=0 cellspacing=0 width=""100%""><tr valign=""top"">"
    ...
    If p_Ordena="DS_AMBIENTE" Then
       ShowHTML "          <option value="""">Sala<option value=""DS_AMBIENTE"" SELECTED>Ambiente<option value=""ds_tipo_sala"">Tipo de sala"
    ElseIf p_Ordena="DS_TIPO_SALA" Then
       ShowHTML "           <option value="""">Sala<option value=""DS_AMBIENTE"">Ambiente<option value=""ds_tipo_sala"" SELECTED>Tipo de sala"
    Else
       ShowHTML "           <option value="""" SELECTED>Sala<option value=""DS_AMBIENTE"">Ambiente<option value=""ds_tipo_sala"">Tipo de sala"
    End If
    ShowHTML "          </select></td>"
  ...
  Set p_serie            = Nothing 
  Set p_turma            = Nothing 
  Set p_ambiente         = Nothing 
  Set p_tipo_sala        = Nothing 
