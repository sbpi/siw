...
w_Pagina     = "Rel_Professor.asp?par="
w_Dir        = "ecw/"
w_Disabled   = "ENABLED"
...
Sub Inicial
  ...
  p_4                = uCase(Request("p_4"))
  p_5                = uCase(Request("p_5"))
  p_6                = uCase(Request("p_6"))
  p_ordena           = uCase(Request("p_ordena"))
  
  If O = "L" or O = "W" Then
     If Session("regional") = "" or Session("regional") = "00" Then
        DB_GetProfRel RS1, Session("periodo"), null, p_1, p_2, p_3, p_5, p_4, p_local, p_6, p_unidade, p_escolaridade, p_cargo, p_sexo, p_mat_ini, p_mat_fim
     Else
        DB_GetProfRel RS1, Session("periodo"), Session("regional"), p_1, p_2, p_3, p_5, p_4, p_local, p_6, p_unidade, p_escolaridade, p_cargo, p_sexo, p_mat_ini, p_mat_fim
     End If
     'If p_unidade & p_cargo & p_sexo & p_mat_ini & p_mat_fim & p_escolaridade & p_local > "" Then
     '   w_filter = ""
     '   If p_unidade      > ""       Then w_filter = w_filter & " and unidade            = '" & p_unidade & "' "        End If
  ...
  If O = "W" Then
     ...
     w_linha = 6
     If p_Tipo = "S" Then
        CabecalhoWord w_cliente, "Quantitativo de Professores", w_pag
     Else
        CabecalhoWord w_cliente, "Lista de Professores", w_pag
     End If
  ...
  Else
     ...
     If InStr("P",O) > 0 Then
        ...
        If O="P" Then
           ...
           ShowHTML "     alert('Para toda a rede de ensino não é possível listar os professores. Escolha uma regional ou marque \""Apenas totais\"" no campo \""Exibir\""');"
     ...
     If O = "L" Then
        If p_Tipo = "S" Then
           CabecalhoRelatorio w_cliente, "Quantitativo de Professores"
        Else
           CabecalhoRelatorio w_cliente, "Lista de Professores"
        End If
        ExibeParametrosRel w_cliente
        ShowHTML "<BR>"
  ...
  If O = "L" or O = "W" Then
    ...
    End If
    If p_2 > ""       Then w_filter = w_filter & " [Série: <b>" & p_2  & "</b>]&nbsp;"  End If
    If p_3 > "" Then 
    ...
    End If
    If p_5 > "" Then 
       DB_GetDisciplineTypeData RS, p_5
       w_filter = w_filter & " [Comp.Cur.: <b>" & RS("ds_tipo_disciplina") & "</b>]&nbsp;"
    End If

    If p_4    > ""     Then w_filter = w_filter & " [Turno: <b>" & p_4 & "</b>]&nbsp;"                                   End If
    ...
    If p_6     = "S"     Then w_filter = w_filter & " [Exibir: <b>Somente professores em turma</b>]&nbsp;" 
      ...
      While Not RS1.EOF and (RS1.AbsolutePage = P3 or O = "W" or p_tipo = "S")
         If w_regional <> RS1("regional") or w_atual <> RS1("co_unidade") Then
            ...
            If p_tipo = "N" Then
               ...
               ShowHTML "          <td align=""center""><font size=""1""><b>Nome do professor</font></td>"
               ...
               If p_mat_ini > "" Then ShowHTML "<td align=""center""><font size=""1""><b>Dt.Admissão</font></td>"
               ShowHTML "        </tr>"
            End If
         End If
         If w_linha > 30 and O = "W" Then
            If p_tipo = "N" Then
               ShowHTML "<tr bgcolor=""" & conTrAlternateBgColor & """><td colspan=7><font size=""1""><b>Res/Trab:</b> Local de residência em relação ao local de trabalho</font></td>"
            End If
            ShowHTML "    </table>"
            ShowHTML "  </td>"
            ShowHTML "</tr>"
         ...
         If p_tipo = "N" Then
            ...
            If p_mat_ini > "" Then ShowHTML "        <td><font size=""1"">" & FormataDataEdicao(RS1("dt_admissao")) & "</td>"
            w_linha = w_linha + 1
         End If
         RS1.MoveNext
         w_tot1  = w_tot1 + 1
    ...
    If (w_atual > "" and p_tipo = "S") or O = "W" Then
       ...
       ShowHTML "      <tr bgcolor=""" & conTrTotalBgcolor & """><td colspan=7><font size=""2""><b>TOTAL DA REGIONAL: " & FormatNumber(w_tot2,0) & "</b></td></tr>"
       If Session("regional") = "" or Session("regional") = "00" Then
          ShowHTML "      <tr bgcolor=""" & conTrBgcolor & """><td colspan=7><font size=""2""><b>TOTAL DE PROFESSORES: " & FormatNumber(RS1.RecordCount,0) & "</b></td></tr>"
       End If
    End If
  ...
  ElseIf Instr("P",O) > 0 Then
    ...
    ShowHTML "      </tr>"
    ShowHTML "      <tr>"
    SelecaoDisciplina "<u>C</u>omponente curricular:", "C", null, p_5, null, "p_5", null, null
    ShowHTML "      </tr>"
    ShowHTML "      <tr><td><table border=0 cellpadding=0 cellspacing=0 width=""100%""><tr valign=""top"">"
    ...
    ShowHTML "      <tr><td><table border=0 cellpadding=0 cellspacing=0 width=""100%""><tr valign=""top"">"
    SelecaoCargo "Car<u>g</u>o:", "T", null, p_cargo, null, "p_cargo", "ds_cargo like '%PROFE%'"
    SelecaoSexo "Se<u>x</u>o:", "X", null, p_sexo, null, "p_sexo", null, null
    ShowHTML "          </table>"
    ShowHTML "      </tr>"
    ShowHTML "      <tr><td colspan=2><table border=0 cellpadding=0 cellspacing=0 width=""100%""><tr valign=""top"">"
    ...
    ShowHTML "          </table>"
    ShowHTML "      </tr>"
    ShowHTML "      <tr><td valign=""top"" colspan=3><font size=""1""><b>Data de admissão: &nbsp;&nbsp;&nbsp;&nbsp;<input type=""text"" class=""sti"" size=10 maxlength=10 name=""p_mat_ini"" value=""" & p_mat_ini & """ onKeyDown=""FormataData(this,event);""> a <input type=""text"" class=""sti"" size=10 maxlength=10 name=""p_mat_fim"" value=""" & p_mat_fim & """ onKeyDown=""FormataData(this,event);"">"
    ...
    If p_Ordena="NU_MATRICULA_MEC" Then
       ShowHTML "          <option value=""NU_MATRICULA_MEC"" SELECTED>Matrícula<option value=""DT_NASCIMENTO"">Data Nascimento<option value=""TP_SEXO"">Sexo<option value="""">Nome"
    ElseIf p_Ordena="DS_FUNCIONARIO" Then
       ShowHTML "          <option value=""NU_MATRICULA_MEC"">Matrícula<option value=""DT_NASCIMENTO"">Data Nascimento<option value=""TP_SEXO"">Sexo<option value="""">Nome"
    ElseIf p_Ordena="DT_NASCIMENTO" Then
       ShowHTML "          <option value=""NU_MATRICULA_MEC"">Matrícula<option value=""DT_NASCIMENTO"" SELECTED>Data Nascimento<option value=""TP_SEXO"">Sexo<option value="""">Nome"
    ElseIf p_Ordena="TP_SEXO" Then
       ShowHTML "          <option value=""NU_MATRICULA_MEC"">Matrícula<option value=""DT_NASCIMENTO"">Data Nascimento<option value=""TP_SEXO"" SELECTED>Sexo<option value="""">Nome"
    Else
       ShowHTML "          <option value=""NU_MATRICULA_MEC"">Matrícula<option value=""DT_NASCIMENTO"">Data Nascimento<option value=""TP_SEXO"">Sexo<option value="""" SELECTED>Nome"
    End If
    ShowHTML "          </select></td>"
    ShowHTML "      </table>"
    ShowHTML "      </tr>"
    ShowHTML "      <tr>"
    MontaRadioNS "<b>Somente professores em turma?</b>", p_6, "p_6"
    ShowHTML "      </tr>" 
    ShowHTML "      <tr><td colspan=3><table border=0 cellpadding=0 cellspacing=0 width=""100%""><tr valign=""top"">"
