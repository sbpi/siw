...
Sub Identificacao
  ...
  Else
     ...
     If RS.RecordCount > 0 Then 
        ...
        w_sq_etnia             = RS("sq_etnia") 
        w_sq_deficiencia       = RS("sq_deficiencia")
        w_sexo                 = RS("sexo")
  ...
  If Instr("IAEV",O) > 0 Then
    ...
    ShowHTML "          <td title=""Informe seu nome completo, sem abreviações.""><font size=""1""><b><u>N</u>ome:</b><br><input " & w_Disabled & " accesskey=""N"" type=""text"" name=""w_nome"" class=""sti"" SIZE=""40"" MAXLENGTH=""60"" VALUE=""" & w_nome & """></td>"
    ShowHTML "          <td title=""Informe o nome pelo qual você prefere ser chamado ou pelo qual é mais conhecido.""><font size=""1""><b>Nome <u>r</u>esumido:</b><br><input " & w_Disabled & " accesskey=""R"" type=""text"" name=""w_nome_resumido"" class=""sti"" SIZE=""15"" MAXLENGTH=""15"" VALUE=""" & w_nome_resumido & """></td>"
...
Sub Historico
  ...
  Dim w_residencia_outro_pais, w_mudanca_nacionalidade, w_mudanca_nacionalidade_medida
  Dim w_emprego_seis_meses, w_impedimento_viagem_aerea, w_objecao_informacoes, w_prisao_envolv_justica
  Dim w_motivo_prisao, w_fato_relevante_vida, w_servidor_publico, w_servico_publico_inicio, w_servico_publico_fim
  Dim w_atividades_civicas, w_familiar
  ...
  ' da própria tela (se for recarga da tela) ou do banco de dados (se não for inclusão)
  If w_troca > "" Then ' Se for recarga da página
     w_residencia_outro_pais        = Request("w_residencia_outro_pais") 
     w_mudanca_nacionalidade        = Request("w_mudanca_nacionalidade") 
     w_mudanca_nacionalidade_medida = Request("w_mudanca_nacionalidade_medida") 
     w_emprego_seis_meses           = Request("w_emprego_seis_meses") 
     w_impedimento_viagem_aerea     = Request("w_impedimento_viagem_aerea")
     w_objecao_informacoes          = Request("w_objecao_informacoes") 
     w_prisao_envolv_justica        = Request("w_prisao_envolv_justica") 
     w_motivo_prisao                = Request("w_motivo_prisao") 
     w_fato_relevante_vida          = Request("w_fato_relevante_vida") 
     w_servidor_publico             = Request("w_servidor_publico") 
     w_servico_publico_inicio       = Request("w_servico_publico_inicio") 
     w_servico_publico_fim          = Request("w_servico_publico_fim")
     w_atividades_civicas           = Request("w_atividades_civicas") 
     w_familiar                     = Request("w_familiar")
  Else
     ' Recupera os dados do currículo a partir da chave
     DB_GetCV RS, w_cliente, w_chave, SG, "DADOS"
     If RS.RecordCount > 0 Then 
        w_residencia_outro_pais        = RS("residencia_outro_pais") 
        w_mudanca_nacionalidade        = RS("mudanca_nacionalidade") 
        w_mudanca_nacionalidade_medida = RS("mudanca_nacionalidade_medida") 
        w_emprego_seis_meses           = RS("emprego_seis_meses") 
        w_impedimento_viagem_aerea     = RS("impedimento_viagem_aerea")
        w_objecao_informacoes          = RS("objecao_informacoes") 
        w_prisao_envolv_justica        = RS("prisao_envolv_justica") 
        w_motivo_prisao                = RS("motivo_prisao") 
        w_fato_relevante_vida          = RS("fato_relevante_vida") 
        w_servidor_publico             = RS("servidor_publico") 
        w_servico_publico_inicio       = FormataDataEdicao(RS("servico_publico_inicio"))
        w_servico_publico_fim          = FormataDataEdicao(RS("servico_publico_fim"))
        w_atividades_civicas           = RS("atividades_civicas") 
        w_familiar                     = RS("familiar")
        O                      = "A"
        DesconectaBD
     Else
        w_nome                 = null
  ...
  If Instr("IAEV",O) > 0 Then
    ...
    ShowHTML "      <tr><td valign=""top""><font size=""1""><b>Exponha algum outro fato relevante. Inclua informações relacionadas a qualquer residência fora do país de origem:</b><br>"
    ...
    ShowHTML "      <tr><td valign=""top""><font size=""1""><b>Informe alguma sociedade profissional ou atividades ligadas a assuntos cívicos, públicos ou internacionais das quais você faz parte:</b><br>"
  ...
  Set w_residencia_outro_pais        = Nothing
  Set w_mudanca_nacionalidade        = Nothing
  Set w_mudanca_nacionalidade_medida = Nothing
  Set w_emprego_seis_meses           = Nothing
  Set w_impedimento_viagem_aerea     = Nothing
  Set w_objecao_informacoes          = Nothing
  Set w_prisao_envolv_justica        = Nothing
  Set w_motivo_prisao                = Nothing
  Set w_fato_relevante_vida          = Nothing
  Set w_servidor_publico             = Nothing
  Set w_servico_publico_inicio       = Nothing
  Set w_servico_publico_fim          = Nothing
  Set w_atividades_civicas           = Nothing
  Set w_familiar                     = Nothing
...
Sub Experiencia
  ...
  Dim w_duracao_ano, w_motivo_saida, w_ultimo_salario, w_nm_area
  ...
  If w_troca > "" then
     ...
     w_ultimo_salario           = Request("w_ultimo_salario")
  end if   
  ...
  ElseIf InStr("AEV",O) > 0 and w_Troca = "" Then
     ...
     w_ultimo_salario           = FormatNumber(Nvl(RS("ultimo_salario"),0),2)
     DesconectaBD
  End If
  ...
  ElseIf Instr("IAEV",O) > 0 Then
    ...
    ShowHTML "              <td valign=""top""><font size=""1""><b><u>U</u>ltimo salário mensal:</b><br><input " & w_Disabled & " accesskey=""U"" type=""text"" name=""w_ultimo_salario"" class=""sti"" SIZE=""15"" MAXLENGTH=""15"" VALUE=""" & w_ultimo_salario & """ onKeyDown=""FormataValor(this,14,2, event)""></td>"
    ShowHTML "          </table>"
  ...
  Select Case SG
    ...
    Case "CVHIST"
       ...
       If (VerificaAssinaturaEletronica(Session("Username"),w_assinatura) and w_assinatura > "") or _
          ...
          DML_PutCVHist O, _
              Request("w_chave"), Request("w_residencia_outro_pais"), Request("w_mudanca_nacionalidade"), _
              Request("w_mudanca_nacionalidade_medida"), Request("w_emprego_seis_meses"), Request("w_impedimento_viagem_aerea"), _
              Request("w_objecao_informacoes"), Request("w_prisao_envolv_justica"), Request("w_motivo_prisao"), _
              Request("w_fato_relevante_vida"), Request("w_servidor_publico"), Request("w_servico_publico_inicio") , _
     ...
     Case "CVEXPPER"
       ...
       If (VerificaAssinaturaEletronica(Session("Username"),w_assinatura) and w_assinatura > "") or _
          ...
          DML_PutCVExperiencia O, w_usuario, _
              ...
              Request("w_duracao_mes"), Request("w_duracao_ano"), Request("w_motivo_saida"), Request("w_ultimo_salario"), _
