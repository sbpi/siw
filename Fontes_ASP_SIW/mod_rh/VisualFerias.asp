<%
REM =========================================================================
REM Rotina de visualização do pedido de férias
REM -------------------------------------------------------------------------
Function VisualFerias(w_chave, O, w_usuario, P1, P4)
  ...
  If O = "L" or O = "V" Then ' Se for listagem dos dados
     ...
     w_html = w_html & VbCrLf & "      <tr><td align=""center"" bgcolor=""#D0D0D0"" style=""border: 2px solid rgb(0,0,0);""><font size=""1""><b>Identificação</td>"
      ' Identificação do lançamento
     If Nvl(RS("cd_acordo"),"") > "" Then
        If Not (P1 = 4 or P4 = 1) Then
           w_html = w_html & VbCrLf & "      <tr><td><font size=""1"">Contrato: <b><A class=""hl"" HREF=""" & "mod_ac/Contratos.asp?par=Visual&O=L&w_chave=" & RS("sq_solic_pai") & "&w_tipo=&P1=2&P2=" & P2 & "&P3=" & P3 & "&P4=" & P4 & "&TP=" & TP & "&SG=GC" & Mid(SG,3,1) & "CAD"" title=""Exibe as informações do contrato."" target=""Contrato"">" & RS("cd_acordo") & " (" & RS("sq_solic_pai") & ")</a> </b></td>"
        Else
     ...
     If Not IsNull(RS("nm_projeto")) Then
        If Not (P1 = 4 or P4 = 1) Then
           w_html = w_html & VbCrLf & "      <tr><td><font size=""1"">Projeto: <b><A class=""hl"" HREF=""" & "Projeto.asp?par=Visual&O=L&w_chave=" & RS("sq_solic_pai") & "&w_tipo=Volta&P1=2&P2=" & P2 & "&P3=" & P3 & "&P4=" & P4 & "&TP=" & TP & "&SG=" & SG & """ title=""Exibe as informações do projeto."" target=""Projeto"">" & RS("nm_projeto") & "</a></b></td>"
        Else
     ...
     If Not IsNull(RS("sq_cc")) Then
        w_html = w_html & VbCrLf & "      <tr><td><font size=""1"">Classificação: <b>" & RS("nm_cc") & " </b>"
     End If
     w_html = w_html & VbCrLf & "      <tr><td><font size=""1"">Tipo de lançamento: <b>" & RS("nm_tipo_lancamento") & " </b></td>"
     w_html = w_html & VbCrLf & "      <tr><td><font size=1>Finalidade: <b>" & CRLF2BR(RS("descricao")) & "</b></font></td></tr>"
     If Not (P1 = 4 or P4 = 1) Then
        w_html = w_html & VbCrLf & "      <tr><td><font size=""1"">Unidade responsável: <b>" & ExibeUnidade(w_dir_volta, w_cliente, RS("nm_unidade_resp"), RS("sq_unidade"), TP) & "</b></td>"
     Else
        w_html = w_html & VbCrLf & "      <tr><td><font size=""1"">Unidade responsável: <b>" & RS("nm_unidade_resp") & "</b></td>"
     End If
     ...
     If Nvl(RS("conclusao"),"") > "" and Nvl(RS("quitacao"),"") > "" Then
        w_html = w_html & VbCrLf & "      <tr><td colspan=""2"" align=""center"" bgcolor=""#D0D0D0"" style=""border: 2px solid rgb(0,0,0);""><font size=""1""><b>Dados da liquidação</td>"
        ...
        If Nvl(RS("codigo_deposito"),"") > "" Then
           w_html = w_html & VbCrLf & "          <td><font size=""1"">Código do depósito:<br><b>" & RS("codigo_deposito") & " </b></td>"
        End If
        w_html = w_html & VbCrLf & "          </table>"
        w_html = w_html & VbCrLf & "      <tr><td><font size=""1"">Observação:<br><b>" & CRLF2BR(Nvl(RS("observacao"),"---")) & " </b></td>"
     End If
     ...
     Else
        ...
        End If
        If cDbl(RS("sq_tipo_pessoa")) = 1 Then
           w_html = w_html & VbCrLf & "      <tr><td align=""center"" style=""border: 1px solid rgb(0,0,0);""><font size=""1""><b>Endereço comercial, Telefones e e-Mail</td>"
        Else
           w_html = w_html & VbCrLf & "      <tr><td align=""center"" style=""border: 1px solid rgb(0,0,0);""><font size=""1""><b>Endereço principal, Telefones e e-Mail</td>"
        End If
        w_html = w_html & VbCrLf & "      <tr><td><table border=0 width=""100%"" cellspacing=0>"
        w_html = w_html & VbCrLf & "          <tr valign=""top"">"
  ...
  If Not Rs.EOF Then
     ...
     If cDbl(w_vl_retencao) <> 0 or cDbl(w_vl_normal) <> 0 Then
        ...
        w_html = w_html & VbCrLf & "              <tr><td colspan=4><font size=1><b>Resumo da tributação sobre os documentos</b></font></td></tr>"
        w_html = w_html & VbCrLf & "              <tr valign=""top"">"
        w_html = w_html & VbCrLf & "              <td width=""25%""><font size=""1"">Valor Bruto:<br><b>" & FormatNumber(w_vl_total,2) & " </b></td>"
        w_html = w_html & VbCrLf & "              <td width=""25%""><font size=""1"">Retenção:<br><b>" & FormatNumber(w_vl_retencao,2) & " </b></td>"
        w_html = w_html & VbCrLf & "              <td width=""25%""><font size=""1"">Impostos:<br><b>" & FormatNumber(w_vl_normal,2) & " </b></td>"
        w_html = w_html & VbCrLf & "              <td width=""25%""><font size=""1"">Valor líquido:<br><b>" & FormatNumber(Nvl(w_valor,0),2) & " </b></td>"
        w_html = w_html & VbCrLf & "            </table>"
     End If
     ...
     w_html = w_html & VbCrLf & "          <td><font size=""1""><b>Série</font></td>"
     w_html = w_html & VbCrLf & "          <td><font size=""1""><b>Valor</font></td>"
     w_html = w_html & VbCrLf & "          <td><font size=""1""><b>Patrimônio</font></td>"
     ...
     While Not Rs.EOF
       ...
       Else
          ...
          w_html = w_html & VbCrLf & "          <td colspan=2><font size=""1""><b>Retenção</td>"
          ...
          w_html = w_html & VbCrLf & "          <td><font size=""1""><b>Alíquota</td>"
          w_html = w_html & VbCrLf & "          <td><font size=""1""><b>Valor</td>"
          w_html = w_html & VbCrLf & "          <td><font size=""1""><b>Alíquota</td>"
          w_html = w_html & VbCrLf & "          <td><font size=""1""><b>Valor</td>"
          w_html = w_html & VbCrLf & "          <td><font size=""1""><b>Alíquota</td>"
          ...
          w_html = w_html & VbCrLf & "          <tr bgcolor=""" & w_cor & """ valign=""top"">"
          w_html = w_html & VbCrLf & "          <td align=""center""><font size=""1""><b>Líquido</td>"
  ...
  If w_erro > "" Then
     ...
     If Mid(w_erro,1,1) = "0" Then
        w_html = w_html & VbCrLf &  "  <font color=""#BC3131""><b>ATENÇÃO:</b></font> Foram identificados os erros listados abaixo, não sendo possível seu encaminhamento para fases posteriores à atual, nem sua liquidação."
  ...
  w_html = w_html & VbCrLf & "      <tr><td align=""center"" bgcolor=""#D0D0D0"" style=""border: 2px solid rgb(0,0,0);""><font size=""1""><b>Ocorrências e Anotações</td>"
  w_html = w_html & VbCrLf & "      <tr><td align=""center"">"
  w_html = w_html & VbCrLf & "        <TABLE WIDTH=""100%"" bgcolor=""" & conTableBgColor & """ BORDER=""" & conTableBorder & """ CELLSPACING=""" & conTableCellSpacing & """ CELLPADDING=""" & conTableCellPadding & """ BorderColorDark=""" & conTableBorderColorDark & """ BorderColorLight=""" & conTableBorderColorLight & """>"
  w_html = w_html & VbCrLf & "          <tr bgcolor=""" & w_TrBgColor & """ align=""center"">"
  w_html = w_html & VbCrLf & "            <td><font size=""1""><b>Data</font></td>"
