...
If InStr(uCase(Request.ServerVariables("http_content_type")),"MULTIPART/FORM-DATA") > 0 Then  
   ...
   p_restricao      = uCase(ul.Form("p_restricao"))  
   p_endereco       = uCase(ul.Form("p_endereco"))  
   p_unidade        = uCase(ul.Form("p_unidade"))  
   p_modalidade     = uCase(ul.Form("p_modalidade"))  
   p_fundamentacao  = uCase(ul.Form("p_fundamentacao"))  
   p_finalidade     = uCase(ul.Form("p_finalidade"))  
   p_criterio       = uCase(ul.Form("p_criterio"))  
   p_situacao       = uCase(ul.Form("p_situacao"))  
   p_aber_i         = uCase(ul.Form("p_aber_i"))
   p_aber_f         = uCase(ul.Form("p_aber_f"))    
   p_objeto         = uCase(ul.Form("p_objeto"))  
   p_processo       = uCase(ul.Form("p_processo"))  
   p_empenho        = uCase(ul.Form("p_empenho"))  
   p_publicar       = uCase(ul.Form("p_publicar"))  
   p_pais           = uCase(ul.Form("p_pais"))  
   p_regiao         = uCase(ul.Form("p_regiao"))  
   p_uf             = uCase(ul.Form("p_uf"))  
   p_cidade         = uCase(ul.Form("p_cidade"))  
   p_fonte          = uCase(ul.Form("p_fonte"))
  ...
  ElseIf Instr("IAEV",O) > 0 Then
    ...
    If (w_cpf = "" and w_cnpj = "" and w_sq_pessoa = "") or InStr(Request("botao"), "Troca") > 0 or Instr(Request("botao"),"Procurar") > 0 Then
       ...
       If Request("w_nome") > "" Then
          DB_GetPersonList RS, w_cliente, null, "TODOS"
